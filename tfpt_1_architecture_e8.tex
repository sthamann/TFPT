...
odd primes covered; upgraded to \emph{all} odd shells in round
36 --- see the global shell theorem below) --- the per-prime
measurement was the prime
...
\emph{The global Gaussian shell theorem (2026-08-08, round 36):}
the $-1/15$ census law above is now a \emph{theorem for all
shells} (\veri{v875\_shell\_global\_lean.py}, $15/15$,
\textsc{shell-global-theorem}): on $V = L/(1{+}i)L$, for every
$n \ge 1$, $\Theta_v(n) = (\Theta_L(n) - \Theta_0(n))/15$ for all
$v \ne 0$ (the division exact), $\hat\Theta_u(n) =
16\,(16\cdot\mathbf 1_{2\mid n}\,\sigma_3(n/2) - \sigma_3(n))$,
and at every \emph{odd} $n$: $\hat\Theta_u(n)/\Theta_L(n) =
-1/15$ \emph{exactly} --- no census cap.  The three inputs, each
warded: the $G_{31}$ class action is the $60$ symplectic
transvections generating $\mathrm{Sp}(4,2)$ (order $720$),
\emph{transitive} on the $15$ nonzero classes and identical to
the shell-module action; norm doubling by $(1{+}i)$
($\Theta_0(2m) = \Theta_L(m)$, $\Theta_0(\text{odd}) = 0$, exact
from $2 = -i(1{+}i)^2$); and $\Theta_L = 240\,\sigma_3$
(classical Eisenstein $E_4$; the $n \le 16500$ census retained
as the exact ward).  A non-transitive control subgroup admits an
invariant \emph{non}-uniform census (the law breaks ---
transitivity is load-bearing), and the even-shell formula
differs correctly ($7/135$ at $n = 2$).  The consequence chain
is kernel-checked from the named hypothesis bundle in
\texttt{TfptCarrier/GaussianShells.lean} ($7$ theorems, exact
integer identities, standard axioms, no \texttt{sorry}) \mE.
\emph{The winding decoder of the clock alphabet (2026-08-08,
round 36):} the Paper-2 winding quadratic $q_{\rm wind}(t) = t^2
- \gcar t + |Z_2|$ \emph{decodes} the clock alphabet
(\veri{v874\_feedback\_decoder\_lean.py}, $11/11$,
\textsc{winding-decoder-exact}): $q(\{2,3,5,6\}) =
(-|\mu_4|, -|\mu_4|, |Z_2|, h(D_5)) = (-4,-4,2,8)$, with $q(2) =
q(3)$ \emph{forced} by the palindrome axis $\gcar/2$ and $2 + 3
= 5 = \gcar$; the decoder polynomial $D(y) = (y{+}4)^2(y{-}2)
(y{-}8) = y^4 - 2y^3 - 48y^2 - 32y + 256$ carries the budget
coefficients $(|Z_2|,\ \Oadm = 48,\ 2^{\gcar} = 32,\
2^{\mathrm{rank}(E_8)} = 256)$; and the Fourier bridge lands:
$|\mu_4|\,q(|Z_2|)/|R(E_8)| = 4\times(-4)/240 = -1/15$ is
\emph{exactly} the nonzero-character Walsh eigenvalue of the
census above ($-16 = |\mu_4|\,q(|Z_2|) = \hat r(u \ne 0)$).
Sibling deformation directions collapse the bridge and
alternative alphabets change the decoder; honest lines typed:
the $-32$ coefficient alone is not selective (the selective
object is the full vector), and the $6$-vs-$7$ tension carries
over verbatim.  Type: exact \emph{audit} theorem, not a physical
functor --- an explicit intertwiner would be needed to make it
causal \mE.
\emph{The divisor lattice of $210$, with its canonicity guard
(2026-08-08):} the label register $\mathbb{F}_2^4$ \emph{is} the
divisor lattice of $210 = 2\cdot3\cdot5\cdot7$ --- the
Walsh--Hadamard transform carries the lattice M\"obius function
($4\,\mathrm{WH}[1111, v] = \mu(d(v)) = (-1)^{\mathrm{wt}(v)}$ on
all $16$ labels), and the vacuum column is the rank-one Redheffer
closure on the register's own lattice, with the
matrix-determinant lemma giving the Euler products \emph{exactly}
(half-weighted $\det = \prod_p(1 - 1/p) = 8/35$; unweighted
$= \prod_p(1 - p^{-1/2})$, exact radical maps)
(\veri{v863\_redheffer\_colligation.py}, sympy-exact).  The hard
canonicity guard then quantifies what selects the primes
(\veri{v868\_divisor210\_audits.py}, $36/36$,
\textsc{divisor210-gauge-family(2)}): the Boolean/Walsh/$\mu$
layer is \emph{measured generic} ($210/210$ quadruples of primes
$< 30$ pass it --- layer one carries no selection content, exactly
as warned), the two deployed Euler determinants pin
$\{2,3,5,7\}$ \emph{uniquely} in the predeclared field ($1$ match
each; non-matches separated by $\ge 5.2\times10^{-3}$ at $50$
digits; the tautological $\prod p = 210$ test excluded from the
verdict), the anchor prime $2$ is \emph{forced} by ramification
(the unique $\Z[i]$-ramified prime onto the unique
$\sigma$-fixed $q^{*}{=}1$ class --- the very prime at which $V =
L/(1{+}i)L$ is cut), the $\mu_4$ weld grade cuts the gauge $C_6
\to C_3$, and after the ramification crossref exactly \emph{two}
gauge classes remain --- the family-cycle chirality $(3,5,7)$ vs
$(3,7,5)$, which is then \emph{proven gauge} by two exact no-go
wards ($15/15$, \textsc{chirality-degenerate}: the register
orientation functional vanishes identically --- the M\"obius
complement $d \mapsto 210/d$ pairs the orbits with reversed
orientation --- and quadratic readouts are provably
chirality-blind by transposition).  The Gram-only baseline is a
simply transitive $720$-torsor cut to $6$ ordered bases by the
deployed pins --- the semantics, not the lattice shape, carry the
selection; the $6$-vs-$7$ honesty note stands untouched (the
clock alphabet $\{2,3,5,6\}$ and the quadruple $\{2,3,5,7\}$ are
different objects: $6 = 2\cdot3$ sits \emph{inside} the lattice
as the unique weight-2 divisor from two alphabet primes, $7$ is
prime and not in the alphabet); and the $17 = 12{+}3{+}1{+}1$
moving-sector identification is honestly \emph{buried at space
level} ($12/12$, \textsc{moving12-dimension-only}: the count is
exact, the principal angles against the only deployed bridge are
${\sim}90^\circ$) \mE.
\emph{The Redheffer/Mertens Smith echo (2026-08-08):} the
register's vacuum-completion pattern has an exact arithmetic
instantiation (\veri{v861\_krein\_normalform\_lean.py}, $37/37$,
\textsc{redheffer-echo-exact}): $\det R_n = M(n)$ (Bareiss
integer determinants on $62$ sizes; the matrix-determinant lemma
extends the identity along all $n \le 2\times10^6$), the witness
minor is $1$ exactly, so $\mathrm{SNF}(R_n) = (1, \dots, 1,
|M(n)|)$ --- vacuum completion is a rank-one origin update whose
\emph{entire} Smith deviation is one new invariant factor; the
mod-$2$ shadow is the squarefree-count parity; and the code-side
instantiation is the vacuum transformation $[[15,5,3]] \to
[[16,4,4]]$ above, typed at parameter grade (pattern-grade
cross-domain identification --- no functor claimed).  The same
module types the register wiring: the $\mu$-sign and the deck
are \emph{two separate} $C_2$s (the deployed reading character
separates them; the identification is one faithful-$\mu_4$
character away, three candidate constructions machine-verified
--- diagonal characters, the Galois bookkeeping $\mu_K((p)) =
\chi_4(p)$ on $609 + 619$ primes, the Hall chain parity), and
the 2-torsion weld law explains the refusal structurally
(\veri{v862\_defect\_polar\_weld.py}, $22/22$,
\textsc{weld-without-contractor}: the deck's anticommutant is
entirely faithful-$\mu_4$ --- the deck enters a Clifford weld
only as the quarter turn $J = MD$, never as a second involution;
all four weld candidates close exactly on their forced domains,
the unwelded control refuses exactly) \mE.
\end{keybox}
...
\begin{keybox}[The four finite anchor theorems (2026-08-09) --- the census-free $q^{*}$, the exclusive anchor classification, the doily as a multiplication table, and the $g=5$ exhaustion \mE\ (\veri{v880\_finite\_anchor\_theorems.py})]
Four theorems close the finite-anchor layer of this section at
identity grade ($26/26$, \textsc{finite-anchor-closed}; the
discovery probe embedded byte-exact).  \textbf{(1) The
census-free normal form.}  The selector-picked refinement has the
closed weight-only form
\[
q^{*}(x) \;=\; \sum_{i<j} x_i x_j + \sum_i x_i
\;=\; \binom{|x|+1}{2} \bmod 2,
\]
value table $(0,1,1,0,0)$ on weights $0$--$4$ --- and the
\texttt{v845} selector \emph{forces} the linear part $(1,1,1,1)$:
the $2^{16}$ brute-force search of the normal-form keybox above is
replaced by a theorem.  \textbf{(2) The exclusive anchor
classification.}  Every line of $PG(3,2)$ is classified by its
$q^{*}$-weight multiset, and the classification is
\emph{exclusive}:

\begin{center}
\begin{tabular}{lccc}
lines & count & weight multiset & anchor\\
\hline
doily (isotropic) & $15$ & $\{2,1,1\}$ & $a = (1,1,2)$\\
secants & $10$ & $\{2,2,1\}$ & the \emph{wrong} anchor $(1,2,2)$\\
external & $10$ & $\{1,1,1\}$ & ---\\
\end{tabular}
\end{center}

\noindent All $15$ doily lines carry the anchor $a = (1,1,2)$
with per-line characteristic polynomial $(t-2)(t-1)^2 = t^3 -
4t^2 + 5t - 2$ and Vieta data $(e_1, e_2, e_3) = (4, 5, 2)$ ---
$e_2 = \gcar$ and $e_3 = |Z_2|$ read off one line; the $20$
non-isotropic lines split \emph{exactly} as $10$ secants carrying
the wrong anchor $(1,2,2)$ (Vieta $(5,8,4)$ --- the
\texttt{v844} control multiset) plus $10$ external lines
$\{1,1,1\}$.  The incidence budget lands as counting: $15 = M_0$
and $30 = M_1 = h(E_8)$ (counting only, no new structure
claimed).  The anchor--Vieta report closes the loop: $p_2(a) =
6$, $\chi_L - \chi_R = -p_2(a)\,(t^2 - e_2 t + e_3)$, and $M_n =
15\,(1, e_3, e_1, e_1 e_3, e_1^2)$.  \textbf{(3) The doily is a
multiplication table.}  The doily \emph{is} the multiplication
table $\Lambda^2 E \times \Lambda^2 E \to \Lambda^4 E$ of the
five-slot carrier: each doily line has support partition $\{i\}
\cup \{j,k\} \cup \{l,m\}$, the wedge $b_{jk} \wedge b_{lm} =
\pm f_i$ holds exactly, the assignment is a bijection onto all
$5\times3 = 15$ such configurations, and the wedge is nonzero
\emph{exactly} on the doily lines.  \textbf{(4) The $g = 5$
exhaustion.}  The quadratic one-step exhaustion $\Lambda^2 \wedge
\Lambda^2 = \Lambda^{g-1}$ \emph{forces} $\gcar = 5$ uniquely
among odd $g \in \{3, \dots, 11\}$.  The honest split is explicit
and load-bearing: the \emph{theorem} (exhaustion $\Rightarrow$
$g = 5$) is \mE; the \emph{physical premise} (why the carrier
must satisfy the quadratic one-step exhaustion --- the Calder\'on
reading) stays \mO\ --- P2 is \emph{narrowed}, not eliminated; no
marker moves.
\end{keybox}

\begin{keybox}[The finite Pfaffian/$K_6$ signature (2026-08-09, rounds 44--49) --- the $Gr(2,5)$ Pl\"ucker reading, the canonical sign character, the self-hosting counting theorem, the flavor hexagon as the automorphism orbit, and the vacuum selection \mE\ (\veri{v888\_finite\_pfaffian\_signature.py})]
Rounds 44--49 close the finite Pfaffian/$K_6$ layer of the
compiler at identity grade ($27/27 + 19/19 + 42/42$; verdicts
\textsc{s6plucker-closed}, \textsc{c6flavor-measured} $+$
\textsc{intertwined}, \textsc{k6pfaffian-closed}; three
discovery probes embedded byte-exact; exact integer / Fraction /
sympy arithmetic in every decision --- no floats, no RNG, no
fits).  \textbf{(1) The $Gr(2,5)$ reading.}  The $15$ doily
lines \emph{are} the $15$ monomials of the five Pl\"ucker
quadrics of $Gr(2,5)$: the bijection is $15/15$ with
\emph{perfect} sign match --- every quadric reads $(+,-,+)$,
the \texttt{v880} Hodge table (converse warded:
secants/externals produce no monomial).  The $S_6$ duad
representation decomposes as $15 = 1 + 5 + 9$ (Specht
$S^{(6)} + S^{(5,1)} + S^{(4,2)}$, characters from scratch by
Murnaghan--Nakayama) and the normalized doily operator acts on
the isotypics by the exact scalars $(1, 0, 2/3)$, with the
anchor identities $2/3 = e_3/(e_2 - e_3)$ and $15 = 1 + e_2 +
(e_2 - e_3)^2$ at $(e_1, e_2, e_3) = (4, 5, 2)$; and the
$q^{*}$ zero set is a $(16, 6, 2)$ Hadamard difference set with
the anchor signature $(v, k, \lambda, v - k) = (2^{e_1},
p_2(a), e_3, \binom{e_2}{2}) = (16, 6, 2, 10)$.  \textbf{(2)
The $K_6$ motif theorem.}  Under the duad labeling the $35$
lines of $PG(3,2)$ are \emph{exactly} the $15$ perfect
matchings of $K_6$ (the doily) plus the $10$ triangles through
the vacuum vertex (the secants --- the wrong-anchor class) plus
the $10$ triangles avoiding it (the external lines, $20$
triangles exhausting $\binom{6}{3}$), and the edge law is
$q^{*}(e) = 0$ iff $e$ touches the vacuum vertex --- $q^{*}$
\emph{is} vacuum incidence.  \textbf{(3) The canonical sign
character.}  The fermionic Pfaffian signs of the symbolic
$6\times6$ expansion ($15$ monomials, coefficients $\pm1$) are
gauge-matched to the Hodge table via the canonical global
character $\chi(i) = (-1)^{i+1}$ on the five vacuum edges ---
triple-identified as the vacuum-row Laplace sign of
$\operatorname{Pf}(A) = \sum_i (-1)^{i+1} a_{0i}
\operatorname{Pf}(A_{\hat{0}\hat{\imath}})$, the
$\Lambda^4$-vs-$\Lambda^5$ volume-form conversion (verified on
all $15$), and the $C_6$ crossing character
($\operatorname{Pf}(PAP^{\mathsf T}) =
\det(P)\operatorname{Pf}(A)$ symbolically) --- a global gauge,
not a line-dependent fudge (the flipped-$\chi$ control produces
exactly the $3$ expected mismatches); each $4\times4$ minor
Pfaffian \emph{equals} the corresponding $Gr(2,5)$ Pl\"ucker
quadric.  \textbf{(4) The self-hosting counting theorem.}  The
pairing count $\binom{g+1}{2} = g!!$ has exactly the solutions
$\{1, 5\}$ over odd $g$ ($g = 1$ trivial, stated; growth
separation $g!! > \binom{g+1}{2}$ for all odd $g \ge 7$ by
symbolic induction), so $g = 5$ is the \emph{unique} nontrivial
fixed point, with $(g-2)!! = 3 = \Nfam$.  The honest split is
explicit and load-bearing: the counting \emph{theorem} is \mE;
the \emph{physical premise} (the boundary grammar is a
self-hosting Wick pair compiler) stays \mO\ --- P2 is narrowed
to \emph{one} premise, not eliminated; no marker moves.
\textbf{(5) The flavor hexagon is the automorphism orbit.}
$\mathrm{Aut}(C_{\rm fin}) \cong C_6$ is rebuilt exactly; the
pinned generator $g$ satisfies $g^2 = \sigma$ (the deployed
family $3$-cycle) \emph{exactly} and $g^3 =$ the $W$-edge sheet
swap; its action on the ten pair-messages has cycle type $1 + 3
+ 6$ with orbits equal to the charge shells, the $g$-orbit of
$\{f_1, w_1\}$ traverses the deployed flavor hexagon edge by
edge ($D_6$-equivalent to the transport cycle), and the
recovery chain reads $(|Z_2|/\Nfam)^{|\mathrm{Aut}(C_{\rm
fin})|} = (2/3)^6 = 64/729$ --- the frozen deployed transfer
eigenvalue as an exact Fraction (typed honestly as a consistent
\emph{new reading}: the corpus derivation of the exponent via
$p_2 = |R_+(A_3)|$ stands; $0$ of $600$ symplectic
non-automorphisms factor, scrambled transport fails).
\textbf{(6) The vacuum-selection theorem.}  $Sp(4,2) \cong S_6$
acts transitively on the six Arf-$1$ refinements;
$\mathrm{Stab}(q^{*})$ has order $120$ and acts faithfully
\emph{and} surjectively as $S_5$ on the remaining five, with
orbits on the $16$ messages exactly the $\iota$-weight classes
$1 + 5 + 10 = (\Lambda^0, \Lambda^4, \Lambda^2)$ --- the
five-slot carrier is the residual symmetry after choosing one
of six equivalent vacua.  Two side reports: the anchor
power-sum clock $p_n = 2 + 2^n$ has mod-$30$ orbit $4 \to 6 \to
10 \to 18$ of period exactly $4 = |\mu_4|$, typed
\textsc{clock-distinguished} (the purely period-$4$ moduli are
exactly $\{5, 10, 15, 30\}$, all dividing $30$); the
operative-Coxeter-$30$ reading is separately dead (sandbox,
both sides).  All controls fired (Arf-$0$ refinements, secant
substitution, wrong scalar, flipped sign character, $g = 3/7$
counterexamples).  No marker moves \mE.
\end{keybox}

\begin{keybox}[The Wick functor arc (2026-08-09, rounds 50--55) --- the six compiler roles in the deployed seam, the proved scalar obstruction, the block resolution, the seam-diagonal demand, and the constructed channel-mixing candidate \mE/\mO\ (\veri{v896\_wick\_block\_functor.py}, \veri{v898\_kms\_schur\_mixing.py})]
Rounds 50--53 take the one remaining P2 premise of the keybox
above --- the physical Wick-compiler reading, [O] --- and close
its \emph{structure} level ($27/27 + 33/33 + 26/26$; verdicts
\textsc{wickfunctor-partial}, \textsc{wickmix-measured
(wickmix-obstructed)}, \textsc{wickblock-measured
(blockfunctor-constructed / seam-diagonal)}; three discovery
probes embedded byte-exact; exact integer / rational /
symbolic checks in every decision typed \textsc{proved} or
\textsc{symbolic}).  \textbf{(1) The six roles exist.}  The
compiler roles are present in the deployed seam one-particle
space --- five $D_5$ carrier slot pairs plus the $A_3$
boundary block (dims $2,2,2,2,2,6$ from the \texttt{v155/v156}
split $16 = 10 + 6$); the $\mathfrak{so}(16)$ sector law holds
(current-sector dims $12$ on vacuum edges / $4$ on pair edges
$=$ the $q^{*}$ edge grading, ratio $3 = \Nfam$); and the
$C_6$ intertwining is \emph{exact}: the $\mathrm{Aut}(C_{\rm
fin})$ generator $g$ ($g^2 = \sigma$) induces the channel
permutation of cycle type $(1)(2)(3)$ fixing channel $0$.  But
the deployed \emph{vacuum kernel} is channel-diagonal, so all
$15$ duad two-point functions vanish
(\textsc{duadmap-degenerate-on-vacuum}): the structure level
of the functor exists, the value level needs a channel-mixing
covariance --- \textsc{wickfunctor-partial}, the honest
typing (the candidate is constructed in round 55, item (5)
below).  \textbf{(2) The scalar obstruction is proved.}  On
the $6$-dimensional scalar channel space \emph{every}
$C_6$-covariant antisymmetric covariance vanishes on the
transposed duad --- the $C_6$ $2$-cycle $\{4, 5\}$ forces a
duad zero, so the scalar route reaches at most $14/15$ nonzero
duads, never $15/15$: a \emph{theorem of the deployed action},
exhaustively verified over the isotypic decomposition (the
multiplicity list $[3,0,1,1,1,0]$ re-derived by hand and by
exact root-of-unity sums; the frozen expected list carried a
transcription error, caught fail-first); no equivariant
scalarization rescues the scalar space, and both relaxation
witnesses are constructed.  \textbf{(3) The block
resolution.}  The full block-valued functor on the $16$-dim
one-particle space is \textsc{blockfunctor-constructed} ---
block commutant dimension $33$; all $15/15$ duad cross-blocks
nonzero; all $15/15$ block Wick monomials obey the canonical
sign law $\chi(i) = (-1)^{i+1}$ (the round-49 gauge of the
keybox above); the interleaving character $+1$
\emph{symbolically}; the review's five conditions hold at
block level.  \textbf{(4) The seam-diagonal demand.}  The
decisive fact: the \emph{deployed} seam covariance (the
\texttt{v113} family rebuilt exactly) has \emph{zero}
cross-blocks everywhere (\textsc{seam-diagonal}) --- the value
level of the reality bridge needs a \emph{physical
channel-mixing mechanism} (a boundary/transport-twisted state,
not the vacuum); with it, the $K_6$/Pfaffian grammar of
\texttt{v888} would be the measured correlation grammar of the
seam.  \textbf{(5) The constructed mixing candidate (round 55,
\veri{v898\_kms\_schur\_mixing.py}, $30/30$; verdict
\textsc{kmsmix-measured [kmsmix-found, schurmix-generates,
rp-theta-open]}).}  The demanded state now \emph{exists as a
candidate}, by two routes.  \emph{The KMS route:} the
Majorana-compatible $C_6$-invariant Hamiltonians $K = ih$ are
parametrized exactly (the hermitian $O_{16}$-commutant has
dimension $112 = 62 + 50$; the antisymmetric slice splits as
$33$ cross --- exactly the block space of (3) --- plus $17$
diagonal, with the two forced zeros of the transposed $\{4,5\}$
duad), and already the \emph{first} point of the frozen grid,
$h = -(A_{16}^{\rm dep} + \tfrac18 A_{\rm int})$ at $\beta = 1$,
passes \emph{all} gates: strict CAR positivity, exact $C_6$
block covariance, all $15$ duad cross-blocks at the frozen
floor, the $q^{*}$/sheet grading carried exactly ($5/5$ vacuum
$+$ $10/10$ carrier), and all $15$ block Wick monomials nonzero
with the \emph{canonical} Pf$_4$ sign pattern of (3); robust at
$\beta = 2$.  \emph{The Schur route (exact, sympy):} eliminating
the boundary of the coupled diagonal state $A_{\rm full} =
[[\kappa A_{CC}, tV], [-tV^{\mathsf T}, mJ_3]]$ gives $A_{\rm
eff} = \kappa A_{CC} + \tfrac{t^{2}m}{1 - m^{2}}\, V J_3
V^{\mathsf T}$ \emph{exactly}, and $S = V J_3 V^{\mathsf T}$
deposits nonzero correlation on \emph{all ten} carrier duads
(uniform $3J$ block, canonical Pf$_4$ signs) from the bare
diagonal state --- the same Schur/Feshbach elimination that
creates the RH port creates the carrier mixing: on both fronts
the real correlation lives in the \emph{eliminated boundary}.
The frozen $15$-signature table of the winner is the registered
prediction any physical seam state must reproduce; the
OS-positivity under a \emph{spatial} reflection is the named
open check (\textsc{rp-theta-open} --- no spatial $\theta$ is
deployed on the $16$-dim space; the Majorana real structure
satisfies the \texttt{v426}/\texttt{v440} dictionary
automatically but is particle-hole, not spatial).  The honest
split: the obstruction theorem, the block construction and the
Schur generation identity \mE; the constructed states are
\emph{candidates} --- whether the \emph{actual} seam realizes
the signature pattern is untouched, so the physical
Wick-compiler premise (\texttt{SEAM.CFIN.WICKFUNCTOR.01},
\texttt{SEAM.CFIN.KMSMIX.01}) stays \mO, unmoved --- the
physical realization is the named demand.  \emph{Dated addendum
(2026-08-10, rounds 57--59):} the open check
\textsc{rp-theta-open} is now \emph{measured} at family level and
the answer is an exclusion --- strict reflection positivity under
both deployable spatial reflections forces $t = 0$ (OS positivity
$\iff$ the mixing floor falls: mutually exclusive on this
family), the complete $C_6$-twisted census is exclusionary
($0/6$), and the parent-dilation route yields only a marginal
cone-boundary witness (with the exact floor identity
$t^2 \cdot 3m/(1-m^2) = 1/200$) while the strict collar route is
obstructed by an exact linear $2t$ law
(\veri{v903\_seam\_rp\_exclusion.py}; a measurement on the
complete finite candidate set, not a universal no-go); the
\mO\ premise stays unmoved.  \emph{Dated addendum (2026-08-10/11,
rounds 60--63):} the exclusion is now \emph{anatomized} --- strict
2-cycle RP is impossible on the \emph{whole} $C_6$-covariant
class (the seat law reduces exactly to $\operatorname{diag}(a_J,
-a_J)$; the only escape is covariance-breaking at exact linear
price $2\varepsilon$, with the floor exchange
$\operatorname{Pf}_4(\varepsilon) = (\varepsilon -
\tfrac1{200})(\varepsilon + \tfrac1{200})$), while the
strict-collar obstruction is a two-seat linear law whose
