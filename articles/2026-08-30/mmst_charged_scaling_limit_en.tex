...
C2a inputs, not all of TEL-b; the exact current CF/DG residue is stated below.
\end{remark}
...
\mG &
TV, C1 and C2a are proved by native interval/analytic certificates
(v1022/v1025); the analytic scalar C2b contribution is $<0.400$.
The full-CAR collar bound remains open exactly at CF and DG below.\\
...
\smallskip\noindent
\textbf{(TEL-B-EXTERNAL) Exact remaining C2b estimates.}
For every even $N\ge16$, prove
\[
 \sqrt N\,\norm{(K'_N-K_N)_{\rm offdiag}}_{\HS}\le0.139,
 \qquad
 \sqrt N\,\norm{(\mathrm{Res}_N-K_N)_{\rm diag}}_{\HS}\le0.060.
\]
The two components are Hilbert--Schmidt orthogonal.  Combined with the
certified scalar contribution, these bounds close the stated
$c_E\le0.542702$ budget.  Finite observations are smaller, but neither
all-$N$ estimate is presently proved.  The old scalar dyadic-increment
route is retired and is not an alternative premise.
...
For the original microscopic-collar formulation, CF and DG are additional
analytic inputs to that exhaustion.
...
algebra.  For the original microscopic collar, TEL-b has been reduced
past TV/C1/C2a and the scalar C2b budget to the two all-$N$ estimates CF
and DG; neither is replaced by a finite plateau.  Relative locality and
...
qualification; the original collar additionally needs CF and DG.
