...
\mG &
Typed \texttt{TEL-B-EXTERNAL}.  The third embedding-tail attempt has
defect $\simeq2.07$, not $O(N^{-2})$; only an unproved scalar $O(N^{-1})$
increment remains.\\
...
\textbf{Single remaining gap box:
  microscopic exhaustion (with one external analytic input).}

\smallskip
The Green-form part and the edge-mode graph part are no longer
assumptions: Theorems~\ref{thm:UGF} and \ref{thm:TELa} prove them with
$K_G=\pi^2/4$ and dyadic rate $N^{-2}$.  What remains is to identify
that edge-mode system with the sharply localized scaling algebra of the
sixteen-Majorana $M=1$, $N_y=8$ microscopic collar.

\smallskip\noindent
\textbf{(TEL-B-EXTERNAL) Matrix-valued two-edge estimate.}
For the cofinal dyadic scaling sequence, an external specialist must
certify $\norm{R_{96}}_{\HS}$ by rigorous interval arithmetic and prove
\[
 \left|\norm{R_{2N}}_{\HS}-\norm{R_N}_{\HS}\right|
 \le\frac{A_R}{N}\qquad(N=96\cdot2^k,\ k\ge0)
\]
with one explicit $A_R$.  Then
\[
 C_R^{\rm dyad}
 :=\sup_{k\ge0}\norm{R_{96\cdot2^k}}_{\HS}
 \le R_{96}^{+}+\frac{2A_R}{96}.
\]
The target $C_R^{\rm dyad}<3$ requires only
$A_R<48(3-R_{96}^{+})\simeq42.956$; the measured products
$N|\norm{R_{2N}}-\norm{R_N}|$ are $0.120$--$0.123$ but are not a proof.
A supremum over every integer $N$ is stronger than this dyadic scaling
theorem needs.  Separately, \textup{(ALG-EXH)} must control the
microscopic charged implementer relative to the edge-mode implementer.
...
\ref{thm:outerness}; they are not part of this residue.

\smallskip
For the edge-projected theorem, \textup{(ALG-EXH)} is the sole remaining
hypothesis.  For the original microscopic CAR formulation,
\textup{(TEL-B-EXTERNAL)} is a named analytic input to that exhaustion.
...
algebra.  In particular, the all-$N$ TEL-b remainder bound is still
typed \texttt{TEL-B-EXTERNAL}; replacing it by the $N\le96$ HS plateau
would be mathematically invalid.  Relative locality and outerness are
proved, leaving only the Buchholz--Verch upper inclusion in house.
