...
\smallskip\\
\noindent\textbf{The parabolic self-code and the $81$ as a lattice index
(\veri{v566\_parabolic\_anchor\_selfcode.py}).} The cross carries more than curvature: $U$ and $V$
generate the \emph{full} $2|1$ parabolic algebra $\mathcal A$ (dim $7$, word determinant $-81$,
multiplicatively closed) \mE; the anchor returns as the spectrum of $H=I+\tfrac12V(V-I)$ with
$\chi_H=x^3-4x^2+5x-2$ double-coded by $(4,5,2)=(\text{flag},\text{Levi},\text{radical})$, and $m=2$
is the unique all-integer member of the parabolic family \mE\ (details and fences in Paper 1).  Two
findings live on \emph{this} page: (i) the \textbf{staircase sum $81$ is simultaneously a lattice
index} --- the saturated integral order $\Z\langle U,V\rangle$ has elementary divisors
$(1,1,1,3,3,3,3)$ in the parabolic lattice, so $[P_{\Z}:\Z\langle U,V\rangle]=3^4=81$ with cokernel
$(\Z/3)^4$: a global determinant charge and an integral saturation index coincide \mE\ (the
mark-local mechanism --- one $\Z/3$ per $\mu_4$ mark --- stays \mO\ with its kill condition
recorded); (ii) the \textbf{basic algebra of $\mathcal A$ is the $A_2$ path algebra}
($\dim\operatorname{Ext}^1=1$), so dimension-vector-$(1,1)$ modules fall into exactly \emph{two}
isomorphism classes --- split and one nonsplit: the split/nonsplit alignment bit gains a canonical
algebra \emph{carrier} \mE, while its \emph{selection} stays with geometry and reflection positivity
(\vref{v528}/\vref{v534}) \mC.  The anchor power compiler is the trace code $p_n(a)=1+
\operatorname{tr}(V^n)$ ($240$ and $\operatorname{rank}E_8$ as trace readouts) --- a re-encoding of
the Lean AnchorLadder, typed as hardening, not as new content.
\smallskip\\
\noindent\textbf{The Gaussian code bridge, and the quartic half (2026-08-03).}
Two exact results extend this code page to the lattice itself.
\emph{The bridge} (\veri{v689\_gaussian\_code\_bridge.py}, $26$ checks,
exact arithmetic): the $\mu_4$ complex structure makes $E_8$ a rank-4
$\Z[i]$-lattice whose $(1+i)$-quotient is $\mathbb F_2^4$ --- the
information space of $\mathrm{RM}(1,3)$ (SNF exact,
presentation-independent); the $240$ roots fall $15\times16$ over the
nontrivial classes (zero class provably empty, each class $=4$ of the
$60$ G31 lines), the clock $\sigma=c^4$ acts as the RM(1,3)
\emph{family permutation} (3-cycle $+$ fixed anchor, the v638
info-bit action; residual gauge of order $18$), and the coordinate
block is the sum-of-families label $F_1{+}F_2{+}F_3$; three
must-fail controls fire \mE\ (\texttt{E8.GAUSSCODE.01}).  \emph{The
quartic half} (\veri{v690\_quartic\_half.py}, $22$ checks, no
floats): the degree-$\{2,14,18,30\}$ vanishing under holomorphic
$\mu_4$ restriction is proven but \emph{trivial} ($\mu_4$-orbit
factor, honest typing); the substance: $F_8|,F_{12}|,F_{20}|,F_{24}|$
are algebraically independent G31 \emph{basic} invariants (Molien-unique
degrees, product $46080=|G31|$, Chevalley cited) --- the holomorphic
restriction of the $W(E_8)$ ring generates the full G31 invariant
ring, and the Jacobian carries the $60$ mirror lines $60/60$
(\texttt{E8.QUARTICHALF.01}) \mE.  No marker moves.
\emph{Follow-up (2026-08-03):} the $\Z[i]$-module structure of this
code page carries a Hecke tower whose primitive degrees are exactly
the Gaussian prime norms and whose $\sigma$-descended traces
reproduce the atom layer of the Weil measure at deviation $0.00$
(\veri{v714\_moonshot\_hecke\_groupoid.py}, \textsc{moonshot-go};
fenced honestly: the finite counting facts are E8-unspecific on that
slice --- the stage-2 specificity question has since been
\emph{measured}: the arch sector of the same tower glues with one
normalization and E8 becomes forced \emph{at the gluing}
(\veri{v716\_moonshot\_arch\_glue.py}, $18/18$,
\textsc{moonshot-stage2-glued}; the $\{\mathbb{Z}^8,E_8\}$ census of
\veri{v719\_moonshot\_traceformula.py}: only the Gaussian E8 glues);
the uniqueness theorem stays open under
\texttt{PRIME.Z1.MOONSHOT.01}, no marker moves).
\emph{The orbit packet lemma (2026-08-04,
\veri{v736\_orbit\_packet.py}, $26/26$,
\textsc{orbit-packet-exact}; Lean companion
\texttt{OrbitPacket.lean}, $34$ theorems):} the code page's compact
number family in one normal form --- Burnside $7=(15+3+3)/3$ on the
$15$ nonzero classes; the canonical packet split $240=5\times48$
($1$ fixed block $+$ $4$ moved blocks of $12$ Gaussian lines each,
label-free canonical; packets $=1+4=5=\gcar$); the anchor $(1,1,2)$
from the fixed-syndrome leader weights $(0,1,1,2)$ (leader pair
$e_6/e_7$ $=$ the v638 anchor pair); and $240=16\cdot15$,
$248=240+8$.  \emph{Circularity fence:} P1/P2 come back as
\emph{exact internal reconstruction}, not derivation, and the $1+4$
law is $\sigma$-class-specific (free order-$3$ elements give
Burnside $5$) \mE.
\emph{The symplectic code lemma (2026-08-04,
\veri{v752\_projective\_hamming\_incidence.py}, $36/36$,
\textsc{proj-hamming-exact}; Lean companion
\texttt{ProjectiveHamming.lean}, $32$ theorems):} the code page's
quotient is a \emph{geometry} --- $h=H/4$ is $\Z[i]$-valued with
hermitian Gram determinant $1$ (the unimodular hermitian E8 lattice
over $\Z[i]$), the reduction $\bar h$ makes $V=L/(1+i)L$ a
symplectic $\mathbb{F}_2^4$, and the $15$ nonzero classes carry the
exact incidence identity $BB^{\mathsf T}=4I+3J$ (design parameters
$2/3/4/7/15/16$; eigenvalues $7^1(+2)^9(-2)^5$ --- the
multiplicities $(1,9,5)$ are the symplectic signature;
$K^2=(4/49)I+(45/49)\Pi_0$ for $K=B/7$).  The NS/R parity of the
$120+128$ split is the \emph{point} $y_0=\mathrm{FSUM}$
($\chi_{\mathrm{NSR}}=\bar h(\cdot,y_0)$, $\sigma$-fixed;
$112+128=240$, $+8$ Cartan: $248=120+128$), and the v737 seam-48
distribution is derived exactly with the $5\times48$
coordinatization existing via exactly one $\bar u$-invariant spread
(kill explained, not revised) \mE.
\emph{The Arf/spinor compiler (2026-08-05,
\veri{v774\_arf\_spinor\_compiler.py}, $46/46$,
\textsc{arf-spinor-exact}; Lean companion
\texttt{ArfSpinorCompiler.lean}, $23$ theorems):} the quotient's
geometry has a canonical quadratic refinement --- the frozen
selector picks a unique $q^{*}$ among the exactly $16$ refinements
of $\bar h$, the parity lift $\iota\colon V\cong C_{\rm even}(5)$
carries $\beta=\bar h$ exactly, $\mathrm{Stab}(q^{*})\cong S_5$
makes $16=1+\bar 5+10$ an Arf geometry ($K_6$ duads;
$B=I+A_{KG(6,2)}$ with exact ovoid eigenvectors), the hypercharge
table $X=(-2,-2,-2,3,3)$ reproduces the \texttt{v310}/\texttt{v14}
dictionary with the master-moment chain $120/40/41/240/248$, the
Pati--Salam split $16=8+8$ carries $B{-}L=1-2n_c/3$ exactly, and
$\chi_{\rm NSR}$ is the anchor bit; $A_3^{\rm Fam}$ (family cycle
on message bits) and $A_3^{\rm PS}$ ($3{+}2$ slot split on carrier
legs) are the \emph{same} $\sigma$ in two registers --- naming
discipline \mE.  \emph{The fired matter fence (2026-08-05,
\veri{v775\_gaussian\_class\_d5\_purity.py}, $18/18$,
\textsc{rootclass-mixed}):} the preregistered code$\to$matter kill
\emph{fired} --- no $D_5{\oplus}A_3$/$SU(5)$ convention makes even
one full set of $15$ classes pure ($\ge7$ adjoint-side classes for
every embedding; $0/56$ splits); the code$\to$matter table,
including the ``empty mailbox $=$ right-handed neutrino'' reading,
must not enter any main claim \mE.  \emph{Boundary code uniqueness
(2026-08-05, \veri{v776\_boundary\_hamming\_uniqueness.py},
$31/31$, \textsc{boundary-code-unique}):} the non-circular chain
boundary $\to$ code $\to E_8\to\mathbb F_2^4\to$ five slots closes
with $\gcar=5$ as \emph{output} (census $135\to30\to14\to2\to1$;
the flag datum is one deck-invariant $\Z_2$); honest residuals
R1--R3 typed then (universe premise, semantic deck placement,
anchor naming); no P2-removal claim.  \emph{Type II from the seam
axioms (2026-08-06, \veri{v799\_seam\_code\_typeii.py}, $19/19$,
\textsc{typeii-forced}):} residual R1 is replaced by three physical
axioms --- exhaustively over all $308{,}993$ subspaces of
$\mathbb F_2^8$ (dim $\le4$), mutual locality of the
Construction-A boundary vertex words $\iff$ self-orthogonal,
integer conformal spin $\iff$ doubly even, holomorphy/index one
$\iff$ self-dual, with the survivor set exactly the $30$
$\hat e_8$ copies --- so the universe premise reduces to A1 (bose
statistics), A2 (modular $T$-alignment), A3 (holomorphic index
one); R2 is placed and R3 named by the metaplectic $S$-lift
(\veri{v798\_seam\_clifford\_modular\_s.py}); the sharpened
residual is \texttt{SEAM.CODE.TYPEII.01} \mC\ (the seam satisfies
A1--A3); still no P2-removal claim.
\emph{The Reed--Muller cascade (2026-08-06,
\veri{v819\_prime\_packet\_rm14.py}, $21/21$, \textsc{rm14-exact};
Lean companion \texttt{PacketRM14.lean}, $30$ theorems):} the
$15$-label quotient channel is itself a code --- the row hull of
$B$ is the punctured $\mathrm{RM}(1,4)^*=[15,5,7]$ with enumerator
$1+15z^7+15z^8+z^{15}$ exact, $C_0$ the $[15,4,8]$ simplex,
$\chi_{\rm NSR}$ \emph{one} codeword (the $7{+}8$ split its two
sides, the $C_2$ register the affine bit), and the
self-reproduction cycle $\mathrm{RM}(1,3)\to E_8\to V\to
\mathrm{RM}(1,4)^*\to\mathrm{RM}(1,3)$ closes with exactly $1344$
equivalences; the induced CSS code is $[[15,1,3]]$ with dual
$[15,10,4]$ and triorthogonality $8/4/2$ \mE.
\emph{The plane-count Hecke match (2026-08-06,
\veri{v820\_prime\_kraus\_rm24.py}, $16/16+14/14$,
\textsc{kraus-rm24-rule-canonical}$+$\textsc{planeframes-exact}):}
the $C_2$ packet pair $(A_n,B_n)$ of the positive lift is carried
by the two affine plane frames of $(V,\omega)$ --- iso ($60$, Gram
$12I{+}3J$) and non-iso ($80$, $16I{+}4J$) --- with $A_3=12$ /
$B_3=16$ \emph{equal to} the per-point iso/non-iso plane
completion counts, so $\sigma_3(3)=28$ and $a_3=-4$ become
plane-count statements, and the graded frame operator realizes
$V^*V=\sigma_3(n)I$, $V^*\Gamma V=a_nI$ exactly on $\mathbf1^\perp$
for $n=3,5,7$; the rule-A Kraus protocol set is canonically the
$105$ minimal parity checks of $[15,10,4]$ (the literal leg
bijection is dead twice --- weights and $Sp(4,2)$ orbits) \mE.
\smallskip\\
\noindent\textbf{The No-Carrier-Reconstruction audit, Stage A.}
\veri{v567\_carrier\_free\_q\_reconstruction.py}\ The self-code's circularity question is
executed as an enumeration with the carrier certificates \emph{forbidden} (the \vref{v11} budgets:
rows $(4,5,6)$, columns $(9,5,1)$, $\chi_Q$ with its $5$; monotone rows).  Under the corpus' own
carrier-free data --- $\Sigma$, the winding normalisation $Q(I{+}\Sigma)=2\Nfam W$ (\vref{v10}),
$\operatorname{Spec}Q_+=\{1,2,3\}$, $Q_-^2=\Nfam P$, nonneg integrality, the seam-line demand ---
exactly $52\to28$ (parabolic) reconstructions survive, and \textbf{all $28$ code the same anchor}
($\chi_{H'}=x^3-4x^2+5x-2$, spectrum $(1,1,2)$): \emph{the anchor self-code is carrier-free} \mE.
The single family invariant $81=\Nfam^4$ (the saturation index above) pins $Q$ to a
\emph{two-element} residual family --- the compiler $Q$ ($q_{32}=2=|\Z_2|$) and its uncoupled twin
($q_{32}=0$), difference $2E_{32}$ --- on which the anticommutator spectrum $\{6,-4,-2\}$, the
commutator characteristic polynomial $t(t^2{+}12)$, the determinant and the saturation index all
\emph{coincide}; the one certificate that separates them is the carrier column budget $(9,5,1)$ ---
exactly the forbidden input \mE.  So $Q$ is carrier-free up to \emph{one binary sheet-coupling
choice}; the carrier-free selector of that bit is open (its resonance with the split/nonsplit
alignment bit above is a typed observation, not a claim) \mO, and whether the geometric inputs
themselves derive from $\mu_4$/$D_4$/$H^1$ parabolic geometry is exactly \texttt{GATE.QGEO} ---
the audit is \emph{relocated}, not closed.  Ablation honesty: dropping the winding input widens
the bound-$3$ set $28\to1376$ (real selection load); the census is bound-stable ($B=6\to9$).
\smallskip\\
\noindent\textbf{The bit located, and its canonical witness.}
\veri{v568\_bit\_selector\_ladder.py}\ The residual choice is now dissected.  (i) The two
$81$-twins generate \emph{different integral orders} (equal index, different lattices): the bit
is integrally visible where every classical invariant coincides \mE.  (ii) \textbf{The
selector:} the dominant mode of the sheet direction is $v(c)=(1,2,2c)$ \emph{identically} in the
coupling, and demanding the \emph{ladder self-consistency} --- the dominant mode reproduces its
own eigenvalue as the geometric ladder $(2^0,2^1,2^2)=(1,2,4)$ --- selects $c=2$, the compiler
$Q$, \emph{uniquely on all $28$ members} of the carrier-free class, with no $81$-pin and no
column budget \mE; the witness is prior structure (the binary AnchorLadder steps $2^n$,
$2=|\Z_2|$, the trace code), not a found number \mC.  (iii) The \emph{split} geometry sits at
$q_{32}=1$ --- a module complement exists exactly there --- and is \emph{excluded} by the
demands already imposed (its algebra collapses to dimension $5$): the $A_2$ Ext arrow is
nonzero on the whole reconstruction class, so the bit does not toggle split/nonsplit; it
toggles \emph{which} nonsplit integral order is realised \mE.  (iv) Priority-2 candidates,
honest both ways: the $\mu_4$-grading fails (the order is not graded --- the $81$ stays a
fingerprint), while left $V$-multiplication induces a canonical $\mathbb F_3$-action on the
cokernel $(\Z/3)^4$ --- the mark-local question is sharpened to a concrete module computation
\mO.  \emph{Why} the dominant mode should satisfy the ladder self-consistency is the relocated
open question --- the same structural slot as the RP-selection of the alignment bit
(\vref{v528}/\vref{v534}) \mO.
\smallskip\\
\noindent\textbf{The RP form on the twins: spectrally blind, integrally anchor-visible.}
\veri{v572\_rp\_bit\_form.py}\ The kinematic half of that RP-slot question is executed.  With the
corpus reflection $\theta(X)=\Sigma X^{\mathsf T}\Sigma$ (the sheet involution) and the declared
states (trace; anchor $(1,1,2)$; ladder $(1,2,4)$; unit), the RP kernel
$K(x,y)=\omega(\theta(x)y)$ on each twin's integral order has \emph{identical inertia} on both
twins for every state and is never PSD --- no kinematic v534-style positivity selection exists
(honest negative; v534's selection was nonperturbative there too) \mE.  But the \emph{integral}
Smith towers of the anchor-state Gram \emph{separate} the twins --- $(2,2,18,18,18,18,0)$ vs
$(2,2,2,18,18,18,0)$: one slot, $18$ vs $2$, a factor $9=\Nfam^2$ --- and the ladder state
separates identically while the anonymous states (trace, unit) are blind: \textbf{the bit is
visible to the RP form only through anchor-coupled states}, with the trace discriminant
$-81^2$ on both twins (the saturation index squared) \mE.  The blindness is robust under the
compiler's own flow (the $V$-semigroup-deformed kernels keep identical inertia on both twins
for every state): a dynamical selection, if any, needs genuine interaction.  And the
\emph{Gibbs pencil} of the anchor Hamiltonian delivers exactly that, at the rank level: with
$\omega_g\propto\operatorname{tr}(e^{-gH}\,\cdot)$, $H=I+\Pi_{\mathrm{dom}}$ (the v566
anchor operator), the pencil determinant $\det(K_0+sK_\Pi)$ factors
$\tfrac{6561}{4096}(s^2{+}16s{+}16)^2(15s^2{-}16s{-}16)$ on the compiler twin against
$-\tfrac{6561}{4096}(s^2{+}16s{+}16)^3$ on the alternative ($6561=81^2$, the saturation index
squared) --- and the compiler's extra factor has its root $s^*=(8-4\sqrt{19})/15$ \emph{inside}
the physical Gibbs range: the compiler twin's kernel degenerates at a finite critical
temperature, the alternative twin's never does.  The first one-parameter \emph{dynamical}
distinction of the bit.  And the critical mode is \emph{identified}: the null direction at
$s^*$ is rank one, supported entirely on the seam row (the radical / $A_2$-Ext-carrier row),
fixes the seam line, is a fixed point of the sheet flow ($VX=X$) and is annihilated by the
dominant projector --- the Gibbs criticality lives exactly in the Ext-carrier slot, so the
typed resonance with the alignment bit now has a \emph{shared carrier}.  The half-sided
restrictions (Levi and radical sublattices) stay blind across the Gibbs range: the positivity
selection resists every kinematic and half-sided design tried --- the genuinely
\emph{interacting} construction (the v534 FK-quartet analogue) is the one remaining route.
Honest typing: a rank-drop marker; the positivity selection itself stays the open RP-slot
\mE/\mO.
\smallskip\\
\noindent\textbf{The interacting route, executed: Ext-source response separates the twins in
the unit box.} \veri{v584\_ext\_source\_response.py}\ The remaining route is run.  Coupling a
rank-one Ext source to the Gibbs pencil, $K(s,J)=K_0+sK_\Pi+J\,vv^{\mathsf T}$ with
$v_i=\operatorname{tr}(\theta(x_i)N)$ and $N$ a shared radical arrow $E_{31}/E_{32}$, the flip
thresholds are \emph{exact rational curves} $J_c(s)=-1/(v^{\mathsf T}K(s)^{-1}v)$ whose
numerators are exactly the twins' pencil factors --- compiler
$(15s^2{-}16s{-}16)/(16(s^2{-}s{-}1))$ and $(15s^2{-}16s{-}16)/16$, alternative
$(s^2{+}16s{+}16)/(16(s{+}1))$ and $-(s^2{+}16s{+}16)/16$.  In the crossed phase
$s\in(-1,s^*]$ the compiler twin is \emph{healed} (inertia restored to the $(4,0,3)$ pattern
form) by positive coupling $J_c\le 15/16<1$ through either arrow, while the alternative twin
cannot be flipped by \emph{any} positive coupling in the open unit box (its $E_{31}$ threshold
stays $\ge1$ on the whole physical range, saturating exactly at $s=0$; its $E_{32}$ threshold
is negative throughout): \textbf{positive Ext coupling inside $|J|<1$ selects exactly the
compiler twin}, and negative coupling never heals --- the direction matches the v534 toy's RP
selection of positive seam coupling \mE.  Exact anchors: $J_c(-1)=15/16$; the mechanism is the
Ext-visibility of the critical mode ($J_c(s^*)=0$ exactly); and the compiler response pole
sits on the golden polynomial $s^2-s-1$ ($s=-1/\varphi=-0.618$, next to $s^*=-0.629$) ---
observed structure, no claim built on it.  Honest typing: a \emph{response-level} selection
(inertia restoration in a declared coupling box), not an OS/RP positivity theorem --- no
kernel is ever PSD and the alignment-bit contract stays open \mE/\mC.
\smallskip\\
\noindent\textbf{The sheet involution exists unconditionally.}
\veri{v590\_involution\_existence.py}\ The involution-existence input of
\texttt{GATE.QGEO.01} is derived.  The integral involutions commuting
with the sheet flow $V$ are \emph{exactly} the spectral sign group
$(\mathbb{Z}/2)^3$ --- all eight sign choices on $V$'s eigenspaces
$\{0,1,2\}$ are integer matrices stabilizing the order, on both twins
(a rigidity fact; generic integral operators do not have integral
spectral projectors) \mE.  The narrowing chain: the v574 rank theorem
kills the trace $+1$ half ($6\to3$, all of exact $\Sigma$-signature
$(1,-1,-1)$); seam reversal $Se_3=-e_3$ narrows $3\to2$; fixing the
zero mode ($+1$ on $\ker V$, flipping exactly the dynamical sheet
pair) singles out $S_0=[[1,-1,0],[0,-1,0],[0,0,-1]]$ uniquely ---
existence is \emph{unconditional} \mE.  Honest scope: the corpus RP
reflection $\Sigma=\mathrm{diag}(1,-1,-1)$ (an anti-automorphism) does
not commute with $V$ --- a different structure; and the alt twin admits
the same classification, so the involution is not a bit selector.
Deriving the two demands from the continuum geometry
($\mu_4$/$D_4$/$H^1$) is the remaining open of the gate \mE/\mC.
A geometric candidate has since emerged
(\veri{v597\_mu4\_cover\_model.py}): the $\mu_3$-cover $y^3=x^4-1$
(branched at $\mu_4$ with order-3 cusp monodromies) hits four compiler
fingerprints --- rank-3 Eisenstein $H_1$, projective $D_4$ via the
deck, cusp-class group $(\mathbb{Z}/3)^4$ of order $81$, and the
unique invariant hermitian form of Lorentz signature $(1,2)$; the
dictionary since stands at three exact entries
(\veri{v598\_cover\_dictionary.py}: $U\sim G_2$, $Q_-\sim G_1-G_3$
--- the sheet coupling as the diamond-diagonal Gram difference ---
and the binary ladder $\sim1+G_1+G_3$), with the grading half absent
from $7296$ canonical combinations; the real structure is the named
suspect for the missing generator \mE/\mC.  The suspect is now
\emph{constructed} (\veri{v599\_real\_structure.py}: $R$ unique with
$R^2=+1$; the grading functor closes the gaps at char level; the
invariant form has $\det J=8$, Lorentz $(1,2)$), and the joint
embedding has landed (\veri{v600\_joint\_embedding.py}): a
$\Gamma$-even degree-2 pair realizes $(U,V)$ exactly ---
$\mathbb{Z}[\omega]$-integral, conjugate to the compiler pair,
integer on the saturated fixed lattice, saturation index $81=3^4$
with the v566 divisor pattern; canonicity stays open, the gate does
not move \mE/\mC.  Canonicity has since been answered
(\veri{v601\_equivariant\_dual.py}): the J-adapted projector
presentation is $D_4$-equivariant by construction (mark choice $=$
winding choice), realizes the \emph{dual} parabolic exactly, and the
v590 Klein group lives integrally on the curve \mE/\mC.
\smallskip\\
\noindent\textbf{Stage B, first narrowing: the winding normalisation is forced.}
\veri{v574\_stageb\_winding.py}\ Two of the five declared geometric inputs of the Stage-A audit
relocate to \emph{consistency-forced}.  The involution \emph{type} falls to a rank theorem: any
involution with a two-dimensional positive eigenspace gives a rank-$\le1$ sheet direction
(char.\ poly.\ $x^2(x-t)$ identically in $Q$), so the binary trace code
$\operatorname{Spec}V=\{0,1,2\}$ is unreachable --- only the corpus type can carry the self-code
\mE.  The winding normalisation falls to a \emph{double self-consistency}: $(E)$ bounds
$N\le3$ ($q_{11}=N$ must itself be an $A_3$ exponent); the ladder self-consistency selects
exactly \emph{one} member for every $N\in\{1,2,3\}$ ($28/16/14\to1/1/1$ --- the mechanism is
robust); and the saturation self-consistency $\mathrm{sat}=N^4$ holds \emph{only} at $N=3$
($81=3^4$ against $32\neq1$ and $512\neq16$): the pair (ladder, saturation) pins
$(N,Q)=(3,\text{compiler }Q)$ uniquely across the whole $N$-ablated family --- equivalently,
only $N=3$ carries the binary trace code $p_n=2+2^n$ \mE.  The honest decomposition (the
spectrum ablation, full $35$-spectrum scan): freeing the exponent spectrum \emph{itself}, the
one surviving double-selfconsistent family is exactly $\{1,2,N\}$ with the compiler shape
$Q(N)=[[N,1,0],[N,2,0],[N,2,1]]$ --- so the self-consistency forces the \emph{sheet} spectrum
$\{1,2\}$ (the binary trace code) and the shape, while the $A_3$ exponent demand $(E)$
contributes exactly \emph{one} datum, $N=3$: both factors are load-bearing, neither alone
suffices \mE.  The redundancy ablation goes further: relative to the self-code demands
(parabolic algebra $+$ ladder $+$ saturation $81$), the monodromy budget $(M)$ and the
seam-line demand $(L)$ are \emph{redundant} at the declared bound --- dropping both widens the
admissible class to $1274$ members and the self-code still picks exactly the compiler $Q$: the
declared geometric input list of the reconstruction shrinks to $(S)$-existence, $(W)$-form and
$(E)$ --- and the \emph{maximal} ablation drops $(E)$ too: the winding column $3(1,1,1)$ plus
integrality plus the self-code package alone pin the compiler uniquely ($29$ ladder candidates,
one hit) \mE.  And the winding form's content is located exactly: dropped entirely, the
self-code leaves a \emph{four}-column family $\{(0,1,1),(0,3,3),(3,3,3),(5,1,5)\}$ of which
exactly one is uniform --- the compiler: the winding demand \emph{is} the uniformity choice
($1$ of $4$) \mE.  What stays declared is the involution \emph{existence}, the column
\emph{uniformity} and $\Nfam=3$ (independently anchored in the $E_8$ glue, \vref{v1}):
\texttt{GATE.QGEO.01} is narrowed by five items, \emph{not} closed; no $P2$ reduction,
markers untouched \mC.
\smallskip\\
...
\emph{The winding quadratic (2026-08-08):} the whole line above is \emph{one} rank-one determinant lemma
(\veri{v857\_simplex\_fourier\_winding.py}, $20/20$, \textsc{winding-quadratic-exact}):
$\chi_{R_s} - \chi_R = -s\,q_{\rm wind}$ with
$q_{\rm wind} = e_1^{\!\top}\mathrm{adj}(tI - R)\,\mathbf 1 = t^2 - \gcar\,t + |\Z_2|$
exact and irreducible over $\Q$ ($\operatorname{disc} 17$) --- the keybox coefficients
$(9{+}s,\, 10{+}5s,\, 8{+}2s)$, the per-winding det increment $2 = |\Z_2|$ and the minor law
$(5,\, 3(s{+}1),\, 2(s{+}1))$ (the carrier minor $5$ $s$-invariant) are all readouts of this single
quadratic; the triple lock re-verified (trace/det/Coxeter each solve to $s = 6$ alone, intersection
$\{6\} = \{|R^+(A_3)|\}$); the locus typed exactly: $\Delta(6) = 39200 = 2\cdot140^2 > 0$ is \emph{not} a
collision point and the sole $2{\times}$square hit in the integer window, and the reality threshold is
the unique $\Delta$-root in $(2,3)$, $s^* \approx 2.8250$ (Sturm-exact --- the deployed $\approx 2.83$
above); every sibling direction breaks the lock \emph{and} changes the quadratic --- $(1, -5, 2)$ is
direction-specific \mE.
\emph{The feedback normal form (2026-08-08, round 36):} the winding line has an exact
\emph{gain-plus-companion} normal form (\veri{v874\_feedback\_decoder\_lean.py}, $14/14$,
\textsc{feedback-normalform-exact}): with the integral basis $P = [\,2\cdot\mathbf 1 \mid e_3 \mid
R e_3\,]$ ($\det P = -4$, $\mathrm{SNF} = \mathrm{diag}(1,2,2)$), the \emph{whole} line conjugates
symbolically in $s$ to $P^{-1} R_s P = \bigl[\begin{smallmatrix} s+4 & 0 & 3\\ 0 & 0 & -2\\ 4 & 1 & 5
\end{smallmatrix}\bigr]$ --- a gain channel $s + |\mu_4|$, the \emph{companion block} of $q_{\rm wind}$,
and the coupling pair $(3,4)$ with loop product $\Nfam \times |\mu_4| = 12 = \dim g_{\rm SM}$ --- so
$\chi_{R_s} = (t - s - |\mu_4|)\,q_{\rm wind} - 12\,t$ identically (the remainder identity
$\chi_R - (t{-}4)\,q_{\rm wind} = -12\,t$: a pure $t$-tap at gain offset $|\mu_4|$), and at $s = 6$ the
$(15, 40, 20)$ coefficients are the three gain-$10$ readouts $10{+}5$, $50{+}2{-}12$, $10\times2$.  The
direction $\mathbf 1 e_1^{\!\top}$ is the \emph{only} one (against $\mathbf 1 e_2^{\!\top}$,
$\mathbf 1 e_3^{\!\top}$, $e_1\mathbf 1^{\!\top}$ --- all three controls fire) whose conjugated
deformation is the pure gain $E_{11}$ and whose remainder is a pure $t$-tap.  \emph{The honest typing
(load-bearing):} the quotient $\Z^3/P\Z^3 = \Z_2\times\Z_2$ has \emph{cardinality} $4 = |\mu_4|$ but
exponent $2$ --- not the group $\mu_4$; no identification is claimed.  Kernel-checked in
\texttt{TfptCarrier/FlavorFeedback.lean} ($11$ theorems: the action identities, the inverse-free
conjugation $P F_s = R_s P$ over $\Z$ symbolic in $s$, $\chi_{R_s}$ in $\Z[X]$, and the SNF
determinantal divisors --- standard axioms, no \texttt{sorry}) \mE.
\smallskip\\
...
one irreducible input as $v_{\mathrm{geo}}$ (the No-Unit theorem, \vref{v153}; the whole $v_{\mathrm{geo}}$
interface is now structurally closed as an $\mathbb R_+$ scale torsor in calibration form ---
export table, dimension-matrix rank $1$, $\lambda$-homogeneity, two-anchor consistency $0.11\%$ ---
\veri{v725\_phys\_vgeo\_torsor\_audit.py}, $21/21$, with the one remaining dimensionless ratio
\emph{named}: $H_{\mathrm{EW}}=\ln(\Mbar/v_{\mathrm{EW}})=37.1776$; no scale derivation, typing
unchanged); the dimensionless angles and ordering
...
\par\smallskip\noindent\textbf{The scalaron-trace rung, a structured point inside the window.}
\veri{v986\_\allowbreak nu\_\allowbreak scalaron\_\allowbreak rung.py}
The external master-object review (wave 2, 2026-08-28)
proposes a \emph{different} mechanism class for a structured $M_R$: the Majorana operator rides
the scalaron (the common trace mode, $M_{\mathrm{scal}}=c_3^{7/2}\Mbar$, \vref{v253}/\vref{v272})
times a generation-space insertion whose third-generation slot is $\Nfam=3$.
\emph{Honest retype (2026-08-28, review wave~3):} the original trace argument
$\operatorname{Tr}_{A_3}I=\Nfam=3$ is \emph{loose} --- $\operatorname{Spec}(I)=\{1,1,1\}$, and a
trace of $3$ does not make $3$ an eigenvalue.  The operator reading is the $Q_+$ eigenvalue
route $\operatorname{Spec}\{1,2,3\}$ (\vref{v10}/\vref{v50}/\vref{v69}); the mixed insertion
texture $M_R=M_{\mathrm{scal}}\,\mathrm{diag}(\varepsilon,2\varepsilon,3)$ is typed
\textsc{data-constrains-texture} in the $\nu$-scalaron v2 project (untextured $Y\propto I$ is
killed $\times10^4$; an aligned diagonal $Y$ with the forced ratios is incompatible with the
\vref{v270} PMNS).  The \emph{number} below is unchanged: $M_3=3M_{\mathrm{scal}}$ remains the
candidate rung, consistent with the generation-blindness argument above, and \emph{not} an
$SO(10)$ Clebsch, so the \vref{v488} negative does not apply to it. \mC\ the number: $M_R=\Nfam
M_{\mathrm{scal}}=3c_3^{7/2}\Mbar=9.18{\times}10^{13}$ GeV sits $-1.8\%$ from the
carrier-normalised $1$-loop requirement $9.35{\times}10^{13}$ GeV (\vref{v481}, same frozen
inputs, premise $y_\nu{=}y_t$); $m_3$ at the candidate is $0.0512$ eV ($+1.9\%$, inside the
$>50\%$ RG envelope of \vref{v482}), ${\sim}20\times$ sharper than the declined integer rung.
\mE\ exact ladder position: $3c_3^{7/2}=(3\sqrt{c_3})\,c_3^3$ with $3\sqrt{c_3}=3/\sqrt{8\pi}
=0.598$, i.e.\ $0.26\%$ from the $3/5$ of the closed Clebsch door --- the adjacency is typed,
not consumed. The same $M_{\mathrm{scal}}$ already carries $f_a=M_{\mathrm{scal}}/128$
(\vref{v185}): no new scale atom. \mO\ the mechanism itself (why the Majorana term is the
scalaron times the family trace) is \emph{not} derived --- the row registers a structured,
falsifiable point (\texttt{FLAV.NUSCALE.05}), it does not close the seesaw. \mX\ the
$\Sigma m_\nu=0.059$ eV floor and a $\gtrsim5\%$ shift of the required $M_R$ are the kill
tests; the window candidate and the frozen record are untouched.
\par\smallskip\noindent\textbf{Canonical note (2026-08-28, completeness wave,
\texttt{NU\_TEXTURE\_CENSUS\_NULL}; not a module).}
A frozen $3\times 3$ inventory search over the corpus address matrices, $Q_+$ projectors and
$\varphi_0$-ladder grammar returns $0/1607$ candidates and $0/200$ random-control hits
(best miss a factor $12.6$ on the reactor angle; provenance
\texttt{experiments/tfpt-discovery/nu\_texture\_census\_probe.py}).  The $Q_+$ union needs
structure beyond the frozen $3\times 3$ inventory --- the mechanism \mO\ is sharpened, not
closed.  No v3 freeze; $M_2=1.73{\times}10^{10}$~GeV remains the unused prediction.
\par\smallskip\noindent\textbf{Candidate (2026-08-29, review wave~4,
\texttt{FLAV.NUSCALE.06} \mC; no closure; \texttt{FLAV.NUSCALE.05} unmoved).}
(\veri{v1001\_\allowbreak nu\_\allowbreak pentagon\_\allowbreak chain.py}.)
$U_e=I$ inventory theorem; misalignment $U=U_{v9}R_{13}(\theta,\phi)$; pentagon double hit
$\phi=288^\circ=4(2\pi/5)$ frozen, all three measured angles $\le0.56\sigma$ (honest max
pull $0.557$) at $\theta=2\pi/35$, the unique LEE survivor of nine pre-declared candidates
(LEE $2.7\%$); v3 chain SHA-16 \texttt{a4c28732fa687620} with $\Sigma=0.0599$~eV,
$m_\beta=9.0$~meV, $m_{\beta\beta}\in[1.5,3.8]$~meV, $\delta_{\mathrm{CP}}=287.66^\circ$;
v270--$\theta_{23}$ tension $1.85\sigma$ TYPED; census null $0/1607$.
Mechanism $=$ the $Q_+$-to-flavor operator \mO.
Kills: DESI floor on $\Sigma$; DUNE $\delta_{\mathrm{CP}}$ discriminates $287.7$ vs $240$;
JUNO.  Candidate class, not a seesaw closure.
\par\smallskip\noindent\textbf{Binding constraints (2026-08-29, review wave~5,
\texttt{FLAV.NU.TEXTURE.MECHANISM.01} \mO; \texttt{FLAV.NUSCALE.05}/\texttt{.06} unmoved).}
(\veri{v1003\_\allowbreak nu\_\allowbreak structural\_\allowbreak nulls.py}.)
Charged-sector data $h=(1,1,1)$, $d=(64,60,64)$ \mE.  Schur-texture NULL \mX: $8/8$ D4-even
$(K,B)$ fail; $\lambda_1=\lambda_3$ forced; required $K\sim[3536.998,1768.499,1/3]$ matches none
of $9$ comparators.  Orientation doubling lifts the degeneracy generically \mE.  Scale NULL
\mX: $12/12$ fail; $1-\lvert c/a\rvert=1.885{\times}10^{-4}$ not supplied.  The mechanism must
be D4-odd \emph{and} supply a $\sim2{\times}10^{-4}$ even-odd cancellation; pure seam data
excluded \mX-typed.  No seesaw closure.
\par\smallskip\noindent\textbf{Round-3 parent-internal intertwiner
(2026-09-05; \texttt{FLAV.NU.PARENT.INTERTWINER.01};
\veri{v1024\_\allowbreak parent\_\allowbreak internal\_\allowbreak boundaries.py}).}
The existing seam clock and six $\nu^c$ modes support a finite positive,
gauge-neutral, parity-even and non-additive local coupling.  It retains the
checked parent gap $0.25$ conditional on the inherited full-parent bound and has
mixed resolvent $0.104712041885$.  Ordinary $D_4$ acts on the selected pair
operators only; the full fermionic lift is projective,
$C_f^8=1$, $C_f^4=(-1)^{N_1+N_3}\ne(-1)^F$ and
$J_fC_fJ_f=(-1)^F C_f^{-1}$.  The block therefore supplies neither a neutrino
mass matrix nor the required $2{\times}10^{-4}$ cancellation and is not a
$4$D functional.  \texttt{FLAV.NU.TEXTURE.MECHANISM.01} remains \mO.
...
\noindent\textbf{The local net reading ($G_{\mathrm{net}}$): the
algebraic-QFT counterpart, finite axioms exact
(\texttt{GNET.LOCALNET.01}).}  The OS package above has a
Haag--Kastler counterpart on the quotient circle, and its
\emph{finite} layer is now a machine-checked theorem
(\veri{v746\_phys\_gnet\_local\_functor.py}, $16/16$,
\texttt{GNET-LOCAL-FUNCTOR-ALIVE}): the interval assignment
$I\mapsto\mathcal A(I)$ satisfies isotony, graded locality and
clock covariance \emph{exactly}, and the Watatani index of the
local fixed-point inclusion is exactly $4$ for \emph{every}
interval $\ell\ge2$ --- the $[D,A]$ index statement is
interval-local, not merely global.  The Ramond sector is typed as a
\emph{half-line/bond defect} (solitonic, local footprint with
leakage $\sqrt{2/N}$) --- exactly the DHR/solitonic structure the
seam--Calder\'on identification needs; four must-fail controls fire,
so the axioms measure something.  The missing continuum theorems
are named, not claimed: the GNS limit net (tightness), index
continuity along the inductive limit (Pimsner--Popa/Longo
heredity), the identification of the limit fixed-point net with the
Longo Q-system extension, the solitonic sector construction
(twisted DHR/Longo--Roberts), and Haag duality/split for the
quotient-circle net.  No marker moves. \mE\ (finite net axioms $+$
local index) $+$ \mC\ (the named continuum theorems)
\emph{Follow-up (2026-08-05, the continuum strand,
\veri{v779\_gnet\_gns\_arf.py}, $27/27+27/27$,
\textsc{gns-precursors-coherent}/\textsc{gnet-arf-no-correspondence}):}
the finite-level precursors of four of the five named theorems are
now measured coherent --- the Haar scaling isometry tower
intertwines exactly ($\le2.2\times10^{-16}$), the index-4
expectation squares commute exactly (the finite half of the
Q-system identification), the canonical chiral-sea state is Cauchy
with geometric contraction ${\sim}1/4$ per doubling, the index
census is machine-exact at every level with zero degradation, the
split witness stabilizes, and the finite Haag-duality census gives
twisted-duality defect $0$ with $\dim B'/\dim A'=4$; the exact dim
law $d_c(\ell)=4^{\ell-1}+2^{\ell-1}\cos(\pi c/2)$ makes the
$\mathbb C[\Z_4]$ quasi-basis obstruction exactly $2^{-\ell}$
(explaining the $\ell=1$ anomaly).  The honest negative: the
$16=1{+}5{+}10$ counting echo with the Arf geometry of the code
page is exact but non-geometric (rank $3$ vs $4$; $GL(4,2)$ census
$0/20160$), and $K_{\rm matter}$ does not govern the tower
transport.  The five continuum theorems stay named, not claimed;
no marker moves.
\emph{Follow-up (2026-08-06, the martingale route,
\veri{v802\_gnet\_martingale.py}, $15/15$,
\textsc{martingale-hypotheses-met}):} the limit-existence step of
the continuum strand is reduced to measured hypotheses, all met ---
the Haar ladder is an exact filtration with \emph{one} index-4
Watatani expectation for the whole tower (quasi-basis explicit,
$185/3041$ elements at $\ell=2/3$), the local-state defects are
summable at measured rate ${\sim}0.25$ per doubling (one power
better than the $2^{-\ell}$ dim-law envelope; reported, not
forced), the unique limit state follows by elementary telescoping
and is verified on every rung, and the split/normality precursors
are uniform (determinant witness $\ge0.985$, stabilizing).  The
controls carry the structure: decimation converges to the
\emph{wrong} degenerate limit --- the coherent Haar bond is
load-bearing.  Steps 5--7 (GNS reconstruction, local normality,
split $+$ $\mu_4$ simple-current extension) stay typed; the open
QGEO majorant residues are bypassed for limit existence and
relocated into the normality step --- a reallocation, not a proof;
no marker moves.
\emph{Follow-up (2026-08-05, the stabilizer identification,
\veri{v783\_two\_qubit\_clifford.py}, $32/32$,
\textsc{clifford-partial(h4)}):} the finite compiler carrier now
has an exact quantum-information normal form --- under one
predeclared chart the $60$ Gaussian lines \emph{are} the $60$ pure
two-qubit stabilizer rays entry-exactly over $\Z[i]$, $G_{31}$ is
an index-2 subgroup of the two-qubit Clifford matrix group
($46080=\mu_4\times\mathrm{Pauli}_{16}\times Sp(4,2)$; the $15$
classes are the $15$ Pauli contexts, the $6$ spreads the MUB
pentads), and the index-2 bit is identified as the Galois
$\sqrt2$/Hadamard bit (the honest H4 partial: not
$J\leftrightarrow-J$, which is the separate antiunitary outer
$\Z_2$).  \emph{Round-21 completion
(\veri{v798\_seam\_clifford\_modular\_s.py}, $27/27$,
\textsc{modular-s-partial}):} the missing coset $\zeta_8G_{31}$ is
identified --- the per-binary-factor Hadamard is the
metaplectic/Weil lift of modular $S$ at its fixed point $\tau=i$
(exact relations; Weil constant $\zeta_8$), the total Fourier lies
inside $G_{31}$ with anomaly $(T_3K_3)^3=iI=J$ (the deck as the
anomaly of the total modular lift), and the strict $q^{*}$-fixing
census over the coset is empty (the Arf defect unavoidable; the
$\sigma$-orbit defects XOR to the anchor).  Stabilizer-formalism
mathematics about the carrier; the fired \textsc{rootclass-mixed}
matter fence is unaffected; no marker moves.
...
\par\smallskip\noindent\emph{Round-22 geometry note (2026-08-06):} the hexagon itself acquires a
carrier-geometric home --- it \emph{is}, edge by edge, the distance-2 shell of the Petersen graph
on the $K_5$ edges (charges $(-4,+1,+6)=$ the certified $SU(5)$ decuple; distance shells $(1,3,6)$
charge-pure), and the radial dynamics of that machine has
$\mathrm{spec}(P^6)=\{1,64/729,1/729\}$ --- exactly the deployed transport spectrum
$\{1,(2/3)^6,(1/3)^6\}$; the PSD sixth root stays spectral-only (the corpus freezes the spectrum,
not the basis) (\veri{v808\_petersen\_sixthroot.py}, $19/19+16/16$,
\textsc{petersen-radial-exact}$+$\textsc{sixthroot-spectral-only}) \mE.
\par\smallskip\noindent\emph{Round-23 follow-up (2026-08-06):} the six-step question is decided
--- the rate $(2/3)^6$ arises canonically on three spatial routes but each carries a typed
obstruction (no unit mode; the bipartite sign; the wrong space), and sixth-power blindness makes
per-step data mandatory: the deployed six is the \emph{clock} exponent; and the sixth-root
census is corrected bit-exactly --- $T_{v221}$ (the exact spectral sum of \texttt{v221}'s
transfer $(1-w)^6$) $=B^6$ \emph{bit-exactly}, upgrading \textsc{sixthroot-spectral-only} to
\textsc{bitexact}-vs-v221; the canonical $10$-dim proposal $T_{10}=P_{10}^6$ in the $\Lambda^2$
basis passes all gates (equitable quotient $=(Q_{\rm Pet}/3)^6$ bit-exact) --- a proposal, no
deploy claim (\veri{v814\_k5\_sixstep\_transport.py}, $23/23$,
\textsc{sixstep-clock-only}$+$\textsc{transport10-proposed}) \mE.
\end{winbox}
...
\emph{Follow-up (2026-08-03):} the ``external clock'' typing of these transfer targets is now
\emph{confirmed from the inside} --- both thermal-time routes for internalizing the four clocks are
machine-killed (\veri{v723\_phys\_modular\_clock.py}, $11/11$, \textsc{stt-killed}: the QCD $1$-loop
clock is parabolic and unreachable by any constant-rescaled modular flow, while the pole/Boltzmann
pair is confirmed internal with multiplier exactly $(2/3)^6$; and
\veri{v724\_phys\_t3b\_modular\_flows.py}, $12/12$, \textsc{t3b-dead}: conditioned modular flows of
one internal mother state give loxodromic classes, not the frozen solver classes).  The external
anchor census sharpens to $\{$one unit ($v_{\mathrm{geo}}$-class), one angle $\theta_i$, one lattice
$O(1)$ $C_p\}$; markers unchanged.
\end{gapbox}
