...
bridge lemma exhibited at the finite level over the $U(1)$-twist moduli, \vref{v472}; both named faces now
carrying \emph{computed} witnesses --- the BFK diagonal-zeta face executed on the seam circle, the inflow
reading backed by an exact Laughlin-pump count, the finite Quillen identity two-sided
(\veri{v974\_alpha\_faces\_computed.py}); the channel-swap face since resolved at the finite level by
$\mu_4$-character grading (\veri{v985\_quillen\_channel\_swap.py}) --- the remaining open content typed
as rigidity premise $+$ continuum identification, still \mO) and this
