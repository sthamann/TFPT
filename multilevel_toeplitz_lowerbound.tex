...
we study the prediction defect $\varepsilon=1-b^{\T}A^{-1}b$, which is the Galerkin
error of a piecewise-constant approximation to a fixed function, and we produce
\emph{lower} bounds for it on a nested ladder of resolutions. The mechanism is a
change of direction: written as a saturation telescope, the quantity to be supplied
at each level is a residual measured in the \emph{inverse} energy norm, hence a
\emph{maximum} over test vectors, so a single explicit test vector already yields a
valid lower bound and the coarse-level block enters only through its sign --- never
through its smallest eigenvalue, its condition number, or a strengthened
Cauchy--Schwarz constant. Combining this certified rung with a margin-free bordering
criterion, we obtain a chain that transports semidefiniteness along a nested family
of windows, and we exhibit a verified instance in which every load-bearing step is a
completed Cholesky factorisation above an explicitly declared floating-point floor:
$430$ such certificates over $52$ windows and $88$ rungs. Two features of the result
are stated up front rather than in a remark. First, an accounting convention: the
numerator of the certified rung is a function of the coarse solution carried along
the ladder, so the bound is a bound for the carried-solution rung. Second, a measured
negative result: the solution-free variant of the same rung fails, not by a small
margin but by cancellation --- the coupling term annihilates the explicit high-pass
part of the data instead of perturbing it, and a norm-only Cauchy--Schwarz estimate of
it returns exactly zero of the rung on every rung of the tested ladder. The example
class that motivates the construction has a symbol built from a logarithmic profile
minus an explicit cosine comb at the frequencies $\log n$, $n$ a prime power; the
arithmetic enters only as the source of the comb positions and only through two
classical gap inequalities.
\end{abstract}
...
\item \textbf{A direction reversal that makes a certified rung possible}
  ...
  for the majorant is available in closed form.
\item \textbf{A margin-free composition of the certified rungs into a chain}
  (Corollary~\ref{cor:chain}). Two exact identities (Lemmas~\ref{lem:zeroblock} and
  \ref{lem:handover}) show that on a nested family of windows at one common
  resolution the trailing block of the next window's form \emph{is} the current
  window's form. Albert's criterion \cite{Albert1969} with Douglas' range condition
  \cite{Douglas1966} then transports semidefiniteness from one window to the next
  using only the \emph{sign} of that block --- never the size of $\varepsilon$ ---
  and the residual condition is a Schur complement of the small dimension of the
  window increment.
\item \textbf{An honest boundary, in both directions.} The certified rung is stated
  for the carried-solution numerator, and this accounting convention is part of the
  hypothesis list, not a remark (Convention~\ref{conv:carried}). The solution-free
  variant is then shown, by measurement, to fail through cancellation
  (Proposition~\ref{prop:negative}); we report this as a negative result about a
  specific route on a specific ladder, since it is what tells a reader that the
  convention was not an oversight. Section~\ref{sec:limits} states what the
  construction does not give, the principal item being uniformity in the window index.
\end{enumerate}

Everything load-bearing is verified numerically in Section~\ref{sec:numerics}: a
single script, runtime about ten seconds, produces $430$ completed Cholesky
certificates and the identity residuals, each stated relative to a declared
floating-point floor in the sense of Wilkinson and Higham
\cite{Wilkinson1963,Higham2002}.
...
\begin{assumption}[exact nesting]
\label{ass:nesting}
For every $l<L$,
\begin{equation}
\label{eq:nesting}
...
Assumption~\ref{ass:nesting} is not restrictive; it is exactly the statement that the
ladder is one bilinear form on nested spaces. Its second half is unconditional.
...
The first half of \eqref{eq:nesting} is a property of the sequence $c$: it holds
whenever $c_{m}$ at level $l$ is the exact lag sequence of a fixed kernel against the
cell basis at width $D_{l}$, so that the coarse form is the fine form restricted to
the coarse subspace. For the example class of Section~\ref{sec:example} this is a
closed-form identity in $D$; in the verification of Section~\ref{sec:numerics} it is
checked to relative accuracy $3.0\cdot10^{-14}$ on every rung and reported as such.
...
Assumption~\ref{ass:nesting} identifies $A_{c}=A^{(l)}$: \emph{the coarse block of a
...
Three comments, all of them limitations of scope rather than claims.
...
\label{lem:telescope}
Assume $A^{(l)}\PD$ for all $l$ and Assumption~\ref{ass:nesting}. Put
\begin{equation}
\label{eq:delta}
...
\item\label{H:nest} Assumption~\ref{ass:nesting} at level $l$, so that
      $A_{c}=A^{(l)}$;
...
Theorem~\ref{thm:rung} bounds one rung. Two further ingredients turn the rungs into
a statement that travels: the telescope of Lemma~\ref{lem:telescope}, which needs
only semidefiniteness at the finest level, and a bordering step that grows the
window.
...
\emph{Identities.} The nesting of Assumption~\ref{ass:nesting} holds to relative
$3.0\cdot10^{-14}$ for the form and $6.2\cdot10^{-16}$ for the pole vector on all
$88$ rungs; the saturation identity, the dual-residual identity \eqref{eq:dual} and
...
$P^{\T}A^{(l+1)}P=A^{(l)}$; $P^{\T}b^{(l+1)}=b^{(l)}$
  & identity; residuals $3.0\cdot10^{-14}$ / $6.2\cdot10^{-16}$ on $88/88$ rungs \\
