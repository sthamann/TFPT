...
$116$ preregistered probes with $2909$ machine checks, which localized the residual object. That
...
Szeg\H{o}--Levinson prediction-error bound for one symbol). The last part executes the
...
$\varepsilon(\alpha,D)\ge c_0D^{\theta}\alpha^{\varphi}$ across the prime-power jumps.
...
$n=11\ldots116$). The probe file, the contract name, the verdict and the check count of every part are
...
% =====================================================================
\section{The certification arc (T102--T116)}\label{sec:certification}
% =====================================================================
The last fifteen parts stopped improving measurements and started converting them into certificates ---
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110 $\to$ T111 $\to$ T112 $\to$ T113 $\to$ T114 $\to$ T115 $\to$ T116\\[2pt]
  ...
  dissolve the wall --- then split the transport, break the compute cap, and run the
  induction as a pure boundary process};
\draw[ar] (a) -- (o);
\draw[ar] (c) -- (o);
\draw[ar] (o) -- (n);
\end{tikzpicture}
...
from the start. The continuum form is \emph{exactly critical} ---
...
\noindent T117 (\texttt{epsilon\_theorem\_probe.py}, contract \texttt{EPSILON.THEOREM} --- the
exact Galerkin identity, an Aubin--Nitsche \emph{lower}-bound chain with explicit constants,
exact jump formulas via Sherman--Morrison, a theorem candidate for the target inequality) was
...
state --- the deep march itself certifies the band model's cost geometry, declared as
such).\\[3pt]
...
criticality $\tilde t^{\mathsf T}T^{-1}\tilde t\to0.999975745$ (fits/measurements); every scaling
...
Szeg\H{o}--Kolmogorov, Aubin--Nitsche duality, Wilkinson/Higham backward error.
\end{keybox}
...
truncation budget grows like $n^{2.17}$). Since T116 the mountain is one two-variable
inequality with a named classical proof technique ---
$\varepsilon(\alpha,D)\ge c_0D^{\theta}\alpha^{\varphi}$, a piecewise-constant Galerkin error
under Aubin--Nitsche duality, to be proved across the prime-power jumps --- plus the comb as
the honest support obstruction. T117 (\texttt{epsilon\_theorem\_probe.py}) was running at the
time of writing.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T116}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $106$ parts T11--T116 that the current diary file records
($107$ runs --- T104 was a double run with two independent arms --- and $2717$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T116: \textbf{$116$ probes, $2909/2909$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T117
(\texttt{epsilon\_theorem\_probe.py}, contract \texttt{EPSILON.THEOREM}) was still running
