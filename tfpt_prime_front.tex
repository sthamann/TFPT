...
the one structure that refuses compression. It is a
...
$117$ preregistered probes with $2932$ machine checks, which localized the residual object. That
...
symbol, two of them constants rather than rates.
...
$n=11\ldots117$). The probe file, the contract name, the verdict and the check count of every part are
...
% =====================================================================
\section{The certification arc (T102--T117)}\label{sec:certification}
% =====================================================================
The last sixteen parts stopped improving measurements and started converting them into certificates ---
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110 $\to$ T111 $\to$ T112 $\to$ T113 $\to$ T114 $\to$ T115 $\to$ T116 $\to$ T117\\[2pt]
  ...
  induction as a pure boundary process, and make the one inequality theorem-shaped};
\draw[ar] (a) -- (o);
\draw[ar] (c) -- (o);
\draw[ar] (o) -- (n);
\end{tikzpicture}
...
compression}, not about $\varepsilon$.
\end{keybox}

\noindent T118 (\texttt{symbol\_lemmas\_probe.py}, contract \texttt{SYMBOL.LEMMAS} --- the three
lemmas against their classical addresses: the $S$-symbol at the Nyquist frequency, the
Kac--Murdock--Szeg\H{o}/Widom corner asymptotics, the Bank--Smith saturation constant) was
...
T116 factor-$120$ reading, a sweep artefact), and the saturation ratio in $[0.675,0.744]$;
...
Kac--Murdock--Szeg\H{o}/Widom, the Bank--Smith/D\"orfler--Nochetto saturation assumption,
...
(\texttt{symbol\_lemmas\_probe.py}) was running at the time of writing.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T117}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $107$ parts T11--T117 that the current diary file records
($108$ runs --- T104 was a double run with two independent arms --- and $2740$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T117: \textbf{$117$ probes, $2932/2932$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T118
(\texttt{symbol\_lemmas\_probe.py}, contract \texttt{SYMBOL.LEMMAS}) was still running
