...
$119$ preregistered probes with $2995$ machine checks, which localized the residual object. That
...
% =====================================================================
\section{The certification arc (T102--T119)}\label{sec:certification}
% =====================================================================
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110 $\to$ T111 $\to$ T112 $\to$ T113 $\to$ T114 $\to$ T115 $\to$ T116 $\to$ T117 $\to$ T118 $\to$ T119\\[2pt]
...
\noindent T120 (\texttt{harnack\_probe.py}, contract \texttt{HARNACK} --- the $\alpha$-audit
(does it defuse (D2)?), the sign structure and maximum principle, the $R$ bound (Levinson
recursion, parity symmetry, window certificate), the $\gamma$ formula, and a theorem candidate
V4) was running at the time of writing and is \emph{not} reported here.
...
$\lambda_{\min}(T_h(\sigma*K))\ge\lambda_{\min}(T_h(\sigma))$ by unitary averaging).\\[3pt]
...
(extrapolations of the certified growth law); every scaling exponent quoted in this
document.\\[3pt]
...
Harnack inequality through the exact identity $\kappa_{\mathrm{end}}=1/(1+R)$. Since T119 the
mountain is \emph{one} discrete Harnack inequality with a classical address --- plus two
constants ((D2), (H1)) and the \textbf{[SUPPORT]} obstruction; the chain contains no zeta
input anywhere. T120 (\texttt{harnack\_probe.py}) was running at the time of writing.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T119}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $109$ parts T11--T119 that the current diary file records
($110$ runs --- T104 was a double run with two independent arms --- and $2803$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T119: \textbf{$119$ probes, $2995/2995$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T120
(\texttt{harnack\_probe.py}, contract \texttt{HARNACK}) was still running
