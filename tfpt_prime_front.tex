...
$124$ preregistered probes with $3105$ machine checks, which localized the residual object. That
...
constant decays with the width.
...
$n=11\ldots122$). The probe file, the contract name, the verdict and the check count of every part are
...
% =====================================================================
\section{The certification arc (T102--T124)}\label{sec:certification}
% =====================================================================
The last twenty-three parts stopped improving measurements and started converting them into certificates ---
...
\noindent T125 (\texttt{grand\_assembly\_probe.py}, contract \texttt{GRAND.ASSEMBLY} ---
the series finale: the complete chain end to end, a theorem V-final with full attribution,
the series balance (kill list, cascade, certification degree, the honest RH distance), and
the extraction reassessment) was running at the time of writing and is \emph{not} reported
here.
...
residual split of the certified balance, closing to $2.8\times10^{-17}$).\\[3pt]
...
$\varepsilon_f/\varepsilon_c=0.177$--$0.546\sim\alpha^{+0.030}$ (a fit); every scaling
...
chain entirely. Since T124 the mountain is the unchanged Harnack pair (F1)/(F2) plus one
bookkeeping convention ((F5): the certified rung carries the coarse solution along; the
solution-free version fails honestly) --- plus the \textbf{[SUPPORT]} obstruction. T125
(\texttt{grand\_assembly\_probe.py}, the series finale) was running at the time of writing.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T124}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $114$ parts T11--T124 that the current diary file records
($115$ runs --- T104 was a double run with two independent arms --- and $2913$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T124: \textbf{$124$ probes, $3105/3105$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T125
(\texttt{grand\_assembly\_probe.py}, contract \texttt{GRAND.ASSEMBLY} --- the series finale) was still
running when this document was written and is deliberately absent.
