...
$112$ preregistered probes with $2801$ machine checks, which localized the residual object. That
...
not the zone index. The last part acts on that reinterpretation and rebuilds the construction in a
...
hidden. \textbf{No claim of progress on the Riemann Hypothesis is
...
% =====================================================================
\section{The certification arc (T102--T112)}\label{sec:certification}
% =====================================================================
The last ten parts stopped improving measurements and started converting them into certificates ---
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110 $\to$ T111 $\to$ T112\\[2pt]
  ...
  measure where the propagation fails, and rescale the frame};
\draw[ar] (a) -- (o);
\draw[ar] (c) -- (o);
\draw[ar] (o) -- (n);
\end{tikzpicture}
...
\noindent T113 (\texttt{limit\_operator\_probe.py}, contract \texttt{LIMIT.OPERATOR} --- the currency
question: is the margin fall in the scaled frame a normalisation artefact, $\lambda_2/\lambda_1$
constant against $m\sim n^{-0.95}$? --- plus the limit operator made explicit, the atom perturbation
as a gap functional, and regrid against reserve) was running at the time of writing and is \emph{not}
reported here.
...
in the gap-coupled frame, but the margin wall is frame-invariant at exponent $-0.974$ --- since T112
the mountain is two analytic demands, the positivity of one limit operator and a convergence rate
below the step reserve, with the prime-gap dependence disclosed (Bertrand--Chebyshev suffices
upward; Zhang/Maynard force $\theta$-uniformity downward). T113
(\texttt{limit\_operator\_probe.py}, the limit operator made explicit) was running at the time of
writing.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T112}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $102$ parts T11--T112 that the current diary file records
($103$ runs --- T104 was a double run with two independent arms --- and $2609$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T112: \textbf{$112$ probes, $2801/2801$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T113
(\texttt{limit\_operator\_probe.py}, contract \texttt{LIMIT.OPERATOR}) was still running when
