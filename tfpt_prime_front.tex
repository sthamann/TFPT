...
$118$ preregistered probes with $2968$ machine checks, which localized the residual object. That
...
symbol, two of them constants rather than rates. The closing part measures those three lemmas
...
% =====================================================================
\section{The certification arc (T102--T118)}\label{sec:certification}
% =====================================================================
The last seventeen parts stopped improving measurements and started converting them into certificates ---
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110 $\to$ T111 $\to$ T112 $\to$ T113 $\to$ T114 $\to$ T115 $\to$ T116 $\to$ T117 $\to$ T118\\[2pt]
  ...
  and measure its three missing lemmas against their classical addresses};
\draw[ar] (a) -- (o);
\draw[ar] (c) -- (o);
\draw[ar] (o) -- (n);
\end{tikzpicture}
...
log-singular symbols with atoms.
\end{keybox}

\noindent T119 (\texttt{oscillation\_mass\_probe.py}, contract \texttt{OSCILLATION.MASS} ---
the arithmetic half of L-B (the $D_0(\alpha)$ formula, deep verification), the (H2) section,
the (H3$'$) mean-square form, and a theorem candidate V3) was running at the time of writing
and is \emph{not} reported here.
...
the DTFT factor $|\sin(\theta/2)|$).\\[3pt]
...
$32$--$76\%$ on $14/14$ windows, and the zero crossings of $\inf\sigma_z$ on $3/5$ zones;
every scaling exponent quoted in this document.\\[3pt]
...
Bornemann--Erdmann--Kornhuber as the honest caveat that saturation can fail),
...
T119 (\texttt{oscillation\_mass\_probe.py}) was running at the time of writing.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T118}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $108$ parts T11--T118 that the current diary file records
($109$ runs --- T104 was a double run with two independent arms --- and $2776$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T118: \textbf{$118$ probes, $2968/2968$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T119
(\texttt{oscillation\_mass\_probe.py}, contract \texttt{OSCILLATION.MASS}) was still running
