...
$111$ preregistered probes with $2781$ machine checks, which localized the residual object. That
...
The last part stops extrapolating and measures: on a deep ladder of $199$ prime-power zones with
...
not the zone index. \textbf{No claim of progress on the Riemann Hypothesis is
...
% =====================================================================
\section{The certification arc (T102--T111)}\label{sec:certification}
% =====================================================================
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110 $\to$ T111\\[2pt]
  ...
  and measure where the propagation fails};
\draw[ar] (a) -- (o);
\draw[ar] (c) -- (o);
\draw[ar] (o) -- (n);
\end{tikzpicture}
...
\noindent T112 (\texttt{adaptive\_scaling\_probe.py}, contract \texttt{ADAPTIVE.SCALING} --- the
construction in an $n$-adaptively scaled frame with $D$ tied to the local prime gap, testing whether
all three walls fall simultaneously, plus a fixed-point / limit-object search) was running at the
time of writing and is \emph{not} reported here.
...
$\delta_0$ to the local prime gap. T112 (\texttt{adaptive\_scaling\_probe.py}, the $n$-adaptively
scaled frame with $D$ tied to the local prime gap) was running at the time of writing.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T111}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $101$ parts T11--T111 that the current diary file records
($102$ runs --- T104 was a double run with two independent arms --- and $2589$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T111: \textbf{$111$ probes, $2781/2781$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T112
(\texttt{adaptive\_scaling\_probe.py}, contract \texttt{ADAPTIVE.SCALING}) was still running when
