...
$120$ preregistered probes with $3016$ machine checks, which localized the residual object. That
...
$n=11\ldots118$). The probe file, the contract name, the verdict and the check count of every part are
...
% =====================================================================
\section{The certification arc (T102--T120)}\label{sec:certification}
% =====================================================================
...
count went three $\to$ four, and that is the honest direction.
\end{keybox}

\noindent T121 (\texttt{wide\_window\_probe.py}, contract \texttt{WIDE.WINDOW} --- section
positivity instead of symbol positivity (the Christoffel-function address), the net
$\alpha$-balance of the chain (the demand falls like $\alpha^{-6}$ --- does the chain close
despite the decaying links?), the Fisher--Hartwig corner comparison, and a theorem candidate
V5) was running at the time of writing and is \emph{not} reported here.
...
$D_0$ criterion on all $1492$ zones, $\alpha^*=0.693$).\\[3pt]
...
$1-\gamma^2=0.079$--$0.283$ with fitted slope $-0.297\pm0.013$ in $\alpha$; every scaling
...
anywhere. T121 (\texttt{wide\_window\_probe.py}) was running at the time of writing.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T120}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $110$ parts T11--T120 that the current diary file records
($111$ runs --- T104 was a double run with two independent arms --- and $2824$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T120: \textbf{$120$ probes, $3016/3016$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T121
(\texttt{wide\_window\_probe.py}, contract \texttt{WIDE.WINDOW}) was still running
