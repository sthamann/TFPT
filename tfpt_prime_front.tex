...
%  honest residual.  This is a RESEARCH DOCUMENTATION, not a results
...
question was compressed, over a hundred exploratory probes, down to one scalar ratio. It is a
...
out to be \emph{Hecke} data. This companion documents that line end to end. Seven modules are
...
$108$ preregistered probes with $2701$ machine checks, which localized the residual object. That
...
$r=\kappa/\varepsilon\le1$ (measured $0.0053$--$0.1814$ on $16$ zones) and then, once $\varepsilon$ is
identified as an exact energy, to two scalars whose open part is a boundary-decay statement about one
explicit vector. \textbf{No claim of progress on the Riemann Hypothesis is
...
channel, three certificate candidates for the avoidance norm, and the naive margin route --- because in
...
% =====================================================================
\begin{gapboxnb}[Honesty statement --- read this before anything else]
...
the primes at all; Section~\ref{sec:verified} is the verified layer; Sections~\ref{sec:i5},
\ref{sec:handover} and \ref{sec:certification} are the sandbox program in three acts;
Section~\ref{sec:kills} is the kill list; Section~\ref{sec:status} is the honest status.
...
\begin{tikzpicture}[
  font=\small,
  node distance=6mm,
  box/.style={draw=tfblue!70!black,fill=tfblue!5,rounded corners=2pt,inner sep=4pt,align=center,minimum height=8mm},
  gbox/.style={draw=tfgreen!65!black,fill=tfgreen!6,rounded corners=2pt,inner sep=4pt,align=center,minimum height=8mm},
  ar/.style={-{Latex[length=2mm]},draw=tfgray!80,thick}]
\node[box] (ax) {two axioms\\[-1pt]\footnotesize $\cthree=\tfrac1{8\pi}$, $\gcar=5$};
\node[box,right=8mm of ax] (e8) {$D_5\oplus A_3+\mu_4$\\[-1pt]$\Rightarrow E_8$};
\node[box,right=8mm of e8] (glue) {glue group $\mu_4$\\[-1pt]\footnotesize quartic character $\chi_4$};
\node[box,right=8mm of glue] (zi) {$\Z[i]$ \,/\, $\theta_3^2=r_2$\\[-1pt]\footnotesize signed census};
\node[gbox,below=9mm of glue] (hecke) {Hecke data on the census\\[-1pt]\footnotesize $\nu_p=a\,\mathrm{Id}+b\,T_p$, \ $a_p=-c(p)/8$};
\node[box,below=9mm of zi] (lfun) {Dirichlet $L(s,\chi_{-4})$\\[-1pt]\footnotesize split law $p=a^2+b^2$};
\node[gbox,below=9mm of e8] (weight4) {weight-4 newform $f_8$\\[-1pt]\footnotesize $\eta(2\tau)^4\eta(4\tau)^4$};
\draw[ar] (ax) -- (e8);
\draw[ar] (e8) -- (glue);
\draw[ar] (glue) -- (zi);
\draw[ar] (glue) -- (hecke);
...
\noindent The honest first question of the series was therefore not ``does this prove anything about
...
form \emph{is} Bombieri's classical object, and answered the sharpness question honestly: the boundary
...
\begin{gapboxnb}[Data integrity: the T92/T93 correction --- reported because it happened]
T92 (\texttt{zone\_extension\_proof\_probe.py}, $36/36$) failed to prove the lattice zone extension and
said so (\textsc{t-skeleton}): what it certified is small and real ($Q\ge6.7\times10^{-12}>0$ on an
$8$-dimensional sine-window subspace across the whole band, with an error certificate and a
$1057$-point covering), and it named two independent walls --- $\lambda_{\min}$ collapses geometrically
(factor $11.4$ per mode), and the high-mode complement would need $N^*\gtrsim5069$ modes. Verdict:
\emph{the finite-block route is structurally the wrong instrument, not under-resourced}. In the same
run T92 raised a calibration discrepancy against T89/T91, and the entire band map was put under
reservation. T93 (\texttt{band\_reconciliation\_probe.py}, $41/41$) resolved it with a third
independent implementation: one diary prose value ($6.98$) had been stale, one discrepancy was a
factor-2 convention plus a $1\%$ grid interpolation artefact, and exactly one real bug sat in T92
itself. Net effect: the T89/T91 map survives, with $a_{\mathrm{neg}}$ recalibrated $0.7410\to0.7486$
and T92's own band scoping withdrawn. The corrected band map:
classical edge $0.34657$ $\cdot$ prime-free crossing $0.3723$--$0.3763$ $\cdot$ band top
$\alpha^*=0.46265$ $\cdot$ second atom $\log3/2=0.54931$.
\end{gapboxnb}
...
\texttt{isprime}, no sieve, no \texttt{\%}, \texttt{//} or \texttt{/}). The cost honesty is part of the
...
\draw[tfred!60!black,dashed] (0,0.0) -- (17.4,0.0)
   node[right=-1mm,above,font=\scriptsize,tfred!60!black,align=left,xshift=-30mm,yshift=0.5mm]{spectrum floor (schematic)};
% T101 instrument: slope -0.1622 per zone in ln  ->  -0.07044 in log10
\draw[tfred!75!black,very thick] (1,0.970) -- (16,-0.087);
\node[tfred!75!black,font=\scriptsize,align=left,anchor=west] at (9.4,0.52)
  {T101 instrument\\[-1pt]slope $-0.1622\pm0.0562$};
% T103 instrument: 9.33 -> 2.70
\draw[tfgreen!60!black,very thick] (1,0.970) -- (16,0.431);
\node[tfgreen!45!black,font=\scriptsize,align=left,anchor=west] at (10.2,0.92)
  {T103 instrument\\[-1pt]slope $-0.0748\pm0.0116$};
\filldraw[tfgreen!60!black] (1,0.970) circle (1.4pt) node[above left=-1pt,font=\scriptsize,black]{$9.33$};
\filldraw[tfgreen!60!black] (16,0.431) circle (1.4pt) node[right=1pt,font=\scriptsize,black]{$2.70$};
...
% =====================================================================
\section{The certification arc (T102--T108)}\label{sec:certification}
% =====================================================================
The last seven parts stopped improving measurements and started converting them into certificates ---
...
\node[tfgreen!45!black,anchor=west,font=\scriptsize,align=left] at (5.6,3.25)
...
\node[anchor=west,font=\scriptsize,tfred!70!black,align=left] at (0.25,3.55)
...
(Figure~\ref{fig:race}). The honest price is explicit: the number of explicit modes grows $2\to232$.
...
\begin{tikzpicture}[font=\small,
  box/.style={draw=tfblue!70!black,fill=tfblue!5,rounded corners=2pt,inner sep=5pt,align=center,text width=54mm},
  tgt/.style={draw=tfgold!70!black,fill=tfgold!8,rounded corners=2pt,inner sep=5pt,align=center,text width=62mm},
  nxt/.style={draw=tfgreen!65!black,fill=tfgreen!6,rounded corners=2pt,inner sep=5pt,align=center,text width=62mm},
  ar/.style={-{Latex[length=2mm]},draw=tfgray!80,thick}]
\node[box] (a) at (-3.6,0) {\textbf{Door A} --- T102 (arithmetic bound)\\[1pt]
...
\node[box] (c) at (3.6,0) {\textbf{Door C} --- T103 (instrument)\\[1pt]
  \footnotesize the remainder is not the bulk but the wing slack $S=1-\rho$,
  falling $0.2091\to0.0392$};
\node[tgt] (o) at (0,-2.2) {\textbf{One object:} the wing near-null direction\\[1pt]
...
\noindent The rank-2 Weil pole splits \emph{exactly} into a positive rank-one lift in the $J$-even
...
T109 (\texttt{boundary\_decay\_probe.py}, contract \texttt{BOUNDARY.DECAY} --- Combes--Thomas/Green
decay, a Szeg\H{o} recursion bound, $\omega$ via PSD matrix caps) was running at the time of writing and
...
\node[op] (eps) at (0,-10.85) {$\iff$ two scalars: $\omega<1$ \ and one statement at
  $x=T_{\mathrm{odd}}^{-1}\tilde t$ \ \footnotesize(T108; $\varepsilon$ identity; open object $=$ one
  boundary value $x_0$)};
\foreach \a/\b in {rh/hand,hand/schur,schur/bare,bare/par,par/loew,loew/ratio,ratio/eps}
  \draw[ar] (\a) -- (\b);
...
architecture of a would-be sufficient route, verified on finitely many windows only. The dashed box is
the reason this figure is not a proof sketch.}\label{fig:cascade}
\end{figure}
...
% =====================================================================
\section{Honest status and outlook}\label{sec:status}
% =====================================================================
...
accumulation bounds, the Sherman--Morrison/Woodbury reduction of (R) to $r\le1$, and the three T108
identities ($\varepsilon$ as a $Q|_{\mathrm{odd}}$ energy, the rank-one overlap $hh^{\mathsf T}/
\varepsilon$, and $\varepsilon$ as the square of the last Cholesky pivot).\\[3pt]
...
check''. Two candidates are named in the diary and are repeated here as the honest outlook:
...
regime --- with the honest note that Robin's \emph{unconditional} 1983 bound suffices at the measured
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T108}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $98$ parts T11--T108 that the current diary file records
($99$ runs --- T104 was a double run with two independent arms --- and $2509$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T108: \textbf{$108$ probes, $2701/2701$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T109
(\texttt{boundary\_decay\_probe.py}, contract \texttt{BOUNDARY.DECAY}) was still running when this
document was written and is deliberately absent.
...
T93 & \texttt{\scriptsize band\_\allowbreak reconciliation} & data-integrity reconciliation of the T89/T91 map & MIXED (map recalibrated) & 41/41\\
