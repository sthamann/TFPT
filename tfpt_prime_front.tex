...
of a window remains; and (R) reduces first to one scalar ratio and then to two scalars, whose open part
is a boundary-decay statement about one explicit vector (Section~\ref{sec:certification}). The full
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108\\[2pt]
  \footnotesize certify \emph{bare}, prove the avoidance law, split the parity channels,
  reduce to one scalar ratio, then to two scalars};
\draw[ar] (a) -- (o);
\draw[ar] (c) -- (o);
\draw[ar] (o) -- (n);
\end{tikzpicture}
