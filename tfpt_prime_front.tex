...
$110$ preregistered probes with $2758$ machine checks, which localized the residual object. That
...
margin input, itself two to six orders of magnitude weaker than the conclusion it buys. The last part
then closes the induction circle on the measured zones end to end --- a certified base case, fifteen
certified handover steps through a graded Loewner minorant, and the structural finding that the atom
entry costs nothing --- while leaving three sharp, named gaps: no reserve, no scalar step law, and no
uniformity in $k$, with an extrapolated crossing warning at $n\approx170$. \textbf{No claim of progress on the Riemann Hypothesis is
...
$n=11\ldots110$). The probe file, the contract name, the verdict and the check count of every part are
...
% =====================================================================
\section{The certification arc (T102--T110)}\label{sec:certification}
% =====================================================================
The last nine parts stopped improving measurements and started converting them into certificates ---
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110\\[2pt]
...
the measured ladder (certified base case, graded Loewner minorant), leaving the three gaps of the next
subsection. The dashed box is the reason this figure is not a
...
crossing at $n\approx170$. Propagation gets \emph{harder} with $k$, not easier.
\end{enumerate}
\end{keybox}

\noindent T111 (\texttt{deep\_zone\_stress\_probe.py}, contract \texttt{DEEP.ZONE.STRESS} --- the deep
ladder to $n\sim200$, testing the $n\approx170$ crossing directly and measuring the uniformity object
at depth) was running at the time of writing and is \emph{not} reported here.
...
budget), and the three T110 pieces of the propagation circle (the exact splitting of the growth step,
...
$\lambda_{\min}\ge6.93\times10^{-2}$ at three resolutions).\\[3pt]
...
margin map $m_k\ge\mathrm{need109}$ on $16/16$ zones (ratio $2.09$--$179.6$) with its trend
$m_k\sim n^{-1.93}$ against $\mathrm{need109}\sim n^{-0.98}$ and extrapolated crossing $n\approx170$
(fit); the closure map $44/64$; the handover margins; the two race slopes; every scaling
...
Combes--Thomas/Jaffard, Prager--Synge, first-order Kato perturbation.
\end{keybox}
...
stands, and why the document does not present it as one. Since T110 the mountain also has a
\emph{precise} formulation --- the three gaps of Section~\ref{sec:certification}: no reserve (the
propagation chain lives on a retention of $1-2\times10^{-7}$; a factor-$2$ loss would break it in two
steps), no scalar step law (structurally excluded by the boundary layer), and no uniformity in $k$
(every certificate is one finite Cholesky, and the measured ratio trend $\sim n^{-0.96}$ extrapolates
to a crossing at $n\approx170$). T111 (\texttt{deep\_zone\_stress\_probe.py}, the deep ladder to
$n\sim200$) was running at the time of writing to test that crossing directly.
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T110}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $100$ parts T11--T110 that the current diary file records
($101$ runs --- T104 was a double run with two independent arms --- and $2566$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T110: \textbf{$110$ probes, $2758/2758$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T111
(\texttt{deep\_zone\_stress\_probe.py}, contract \texttt{DEEP.ZONE.STRESS}) was still running when
