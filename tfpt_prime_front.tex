...
$113$ preregistered probes with $2828$ machine checks, which localized the residual object. That
...
hidden. The last part settles the currency question, with a fundamental reinterpretation: the wall
...
four parts, led by a margin-free step certificate now under test. \textbf{No claim of progress on the Riemann Hypothesis is
...
% =====================================================================
\section{The certification arc (T102--T113)}\label{sec:certification}
% =====================================================================
The last twelve parts stopped improving measurements and started converting them into certificates ---
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110 $\to$ T111 $\to$ T112 $\to$ T113\\[2pt]
  ...
  measure where the propagation fails, rescale the frame, and settle the currency question};
\draw[ar] (a) -- (o);
\draw[ar] (c) -- (o);
\draw[ar] (o) -- (n);
\end{tikzpicture}
...
\noindent T114 (\texttt{margin\_free\_step\_probe.py}, contract \texttt{MARGIN.FREE.STEP} ---
rebuild the chain without the margin division: Albert/pseudo-inverse Schur complements, ratio
closure; does the margin wall disappear as a requirement artefact?) was running at the time of
writing and is \emph{not} reported here.
...
width, gap $>2D$ needed against the twin-prime gap $0.003831$ at $521\to523$).\\[3pt]
...
handover margins; the two race slopes; every scaling
...
the T109 requirement chain divides by an artifact margin. Since T113 the mountain is the four-part
balance of Section~\ref{sec:certification}: [P1] semidefiniteness of the balanced form without a
gap, [P2] the certified cell-width power, [P3] --- leading --- the margin-free step certificate,
and [P4] $\theta\to0$ cost uniformity. T114 (\texttt{margin\_free\_step\_probe.py}, the chain
rebuilt without the margin division) was running at the time of writing.
...
$n=521$ --- finitely many zones either way). Not one constant in the chain is certified uniformly in
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T113}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $103$ parts T11--T113 that the current diary file records
($104$ runs --- T104 was a double run with two independent arms --- and $2636$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T113: \textbf{$113$ probes, $2828/2828$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T114
(\texttt{margin\_free\_step\_probe.py}, contract \texttt{MARGIN.FREE.STEP}) was still running when
