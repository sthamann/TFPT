...
input --- and then, once that margin was measured to be an artifact of the requirement, down to a
two-object core. It is a
...
$114$ preregistered probes with $2850$ machine checks, which localized the residual object. That
...
four parts, led by a margin-free step certificate. The last part builds that certificate and the
...
$O(0.1)$ Schur complement between grids. \textbf{No claim of progress on the Riemann Hypothesis is
...
% =====================================================================
\section{The certification arc (T102--T114)}\label{sec:certification}
% =====================================================================
The last thirteen parts stopped improving measurements and started converting them into certificates ---
...
\node[nxt] (n) at (0,-6.1) {T104 $\to$ T105 $\to$ T106 $\to$ T107 $\to$ T108 $\to$ T109 $\to$ T110 $\to$ T111 $\to$ T112 $\to$ T113 $\to$ T114\\[2pt]
...
\noindent T115 (\texttt{schur\_transport\_probe.py}, contract \texttt{SCHUR.TRANSPORT} --- a
transport certificate plus multi-resolution compression, and a deep run beyond $n=1331$) was
...
width, gap $>2D$ needed against the twin-prime gap $0.003831$ at $521\to523$), and the T114
...
block scale --- no margin division anywhere).\\[3pt]
...
$27/27$ steps and the $D$-powers of $\varepsilon$ and $\kappa$ (fits); every scaling
...
Schur criterion), Wilkinson/Higham backward error.
\end{keybox}
...
cost, never at a step, and the (R) demand itself is grid-bound and deleted ([P5]). Since T114 the
mountain is the two-object core of Section~\ref{sec:certification}: $\varepsilon>0$ with
controlled size relative to $\kappa$ (the exact Szeg\H{o} identity), and transport of the exact
$O(0.1)$ Schur complement between non-nested grids. T115 (\texttt{schur\_transport\_probe.py})
...
$n=521$, T114's margin-free ladder to $n=1331$ --- finitely many zones either way). Not one constant in the chain is certified uniformly in
...
% =====================================================================
\appendix
\section{Probe index, diary parts T11--T114}\label{app:index}
% =====================================================================
...
load-bearing artefact. The index covers the $104$ parts T11--T114 that the current diary file records
($105$ runs --- T104 was a double run with two independent arms --- and $2658$ green checks); the ten
earlier probes T1--T10 predate that file and are not indexed here. Series totals as recorded by the
diary at T114: \textbf{$114$ probes, $2850/2850$ sandbox checks}, plus \vref{v535}--\vref{v541}
promoted. T115
(\texttt{schur\_transport\_probe.py}, contract \texttt{SCHUR.TRANSPORT}) was still running when
