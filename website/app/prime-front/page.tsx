...
export const metadata: Metadata = {
  ...
  description:
    "A plain-language research diary of TFPT's prime / zeta investigation: signed E8 census, Hecke from geometry (v535–v537), the relative-trace identity (v538), Weil structure with two isolated obstructions (v539), the amplitude route with the open boundary λ* (v540), the matching-lemma/transport-ledger package (v541), the margin-chain identities of phase 2 (v542), and the induction sprint T102–T125 — a reduction cascade from one matrix inequality down to one sign and one accounting convention, assembled end to end in the finale, with certified margin-free steps to zone 155,921. Series complete at 125 parts; phase 2 (the full proof, T126+) is open — the seam architecture is finished, both remaining inequalities are proof-shaped, three of the four resulting points stand at their preregistered bars, the kappa law falls once while the curvature theorem underneath it stands on all 436 transports (T129), the graded-to-uniform bridge stands as an identity while the curvature bound is reduced to one exponent (T130), and the self-supply loop is built one number short of closed — the epsilon-to-floor sandwich and Perron sign constancy are theorems, M25 reduces to the pole-free section (T131); then the phase reversed direction — the identity block is promoted as v542, the Beurling–Deny triad became an operator discriminator for the seam DtN (T132), a certificate audit hardened v379 to an exact positive-mixture Gram certificate (T133), the pole-free floor closed its existence half with every cheap route failing by sign (T134), the seam DtN showed a bounded faithful state where the Weil window provably has none (T135), the M-matrix pair closed the a-priori radius item by Varga's identity while the exact D-bookkeeping put the whole loss in the margin (T136, promoted as v543), the long lags turned out to be an arithmetic stripe set whose absolute-value envelope family is certified dead (T137, promoted as v544), the compensation itself proved geometric — the sign of every inter-stripe coupling follows the interval geometry, and the m-paired Neumann certificate removed the arithmetic wall on all 77 dead blocks (T138) — the classical decay lemma was refuted at its hypothesis for an arithmetic reason, with the sign law now DERIVED from one exact telescoping identity and the measured core shrunk to one signed inequality at stripe distance b ≤ 16 (T139) — and that inequality now has an exact finite core per zone: the telescope identity lifts to the form level, ρ(W) is an exact eigenvalue of a closed-geometry coverage kernel times a mass-plus-Dirichlet form, the checkerboard split replaces the O(nb) Weyl steps by three D-independent ones, and all the D-dependence sits in the geometry (T140) — and the Hardy ingredient itself resists while acquiring an address: four exact identities put it in classical two-weight shape, the certified constant is not zone-uniform (D^−0.366) while the object it bounds is (D^−0.229), the additive shape is dead at its own exact Weyl floor and the joint shape fails at the normalisation alone, so the rest collapses to one closed conductance profile (T141, promoted with T140 as v545) — and that profile is then constructed rather than guessed: the capacity decomposition gives the optimal Hardy weight exactly, with Omega = 1 by a projection identity where T141 had guessed 20.7–2724, the failure shrinks to a constant factor ~2.3 (flat in D), and the rank ladder closes the whole comparison path — no comparison argument can deliver D-uniformity, so the next move is the sharp capacity-Rayleigh route (T142) — and that sharp route carries: the exact capacity-Rayleigh form is an identity on all 26 windows, Maz'ya's capacity criterion applied to the gap form lands inside its window [1/4, 1] with a zone-uniform loss factor (D^−0.048 ± 0.010), the supremum lives on closed families — intervals in node coordinates, certified by full enumeration on the small border blocks — and D-uniformity is reduced to ONE named inequality, a non-Markovian Maz'ya capacity bound, pointing at Muckenhoupt's 1972 two-weight calculus (T143) — and that interval route then carries: the interval class is exhausted exactly (11.4 million intervals via a Cholesky prefix-sum identity), the closed two-weight sup lands inside the Maz'ya window flat in D, the family restriction falls entirely by a max-density-subgraph bound over all 2^m sets, the Markov perturbation route is certified dead, and the certified chain has exactly ONE unproven input — the absolute Maz'ya constant c₀, its sharpest shape a Muckenhoupt-type hypothesis (T144) — and the Maz'ya proof itself then transcribes step by step: the Markov property sits in exactly one line (M4), that line splits, the mass half is a theorem and dominates (σ_tot = 0.215–0.443 < 1 everywhere), c₀ becomes explicit (2.25–4.23, flat in D) and S1′ is certified on 64/64 windows — what remains is ONE named lemma (L1, an a-priori delocalization bound for the minimiser's layer cake), and an explicit no-go proves no weaker hypothesis can replace it (T145; the identity spine of the capacity cycle is promoted as v546) — and that level lemma then stands: L1 closes on the measurement surface as a chain of theorems and certified window inequalities with no step reading the minimiser, the proof lever the resolvent identity ψ = λRψ itself (the Θ(D³) smallness turns from obstacle into tool), Davis–Kahan instrumented and discarded, Maz'ya's classical dyadic 8 falling to 2 with the cake base free, and the a-priori constant c₀^ap = 3.90–4.85 landing at the size of Maz'ya's Dirichlet value 4 — the one genuine remainder is D-uniformity for ALL D, the asymptotic delocalization of the Green columns (T146; the a-priori core is promoted as v547) — and that asymptotics then acquires an exact shape: the last open object becomes the identity Γ = √Q★ · Sw, splitting all-D uniformity into a purely spectral and a purely geometric factor, both certified on the surface, the classical decay route computationally dead — delocalization itself IS the bound — and the mechanism named: a Toeplitz-minus-Hankel section whose bottom eigenvectors are Fourier modes (Szegő/Widom), with the sharp prediction Q_B ≤ 2|B| holding; one statement, a Szegő theorem for the reweighted section, lifts the chain to all D (T147) — and that statement is then dissected: the second factor Sw closes on the surface via an LDLᵀ layer-cake certificate (Sw ≤ 1.9587, flat), at the price of an honest negative — the Toeplitz symbol is refuted at 0, so positive definiteness is a finite-section effect — while the lifting statement resists with its hypothesis reduced to a single named scalar, the total variation of the log-whitening weight, proved to be the roughness and not the conditioning by a controlled weight-class experiment; first end-to-end number: the certified chain delivers 8.4–15.9% of the true gap on all 48 windows (T148; the identity/certificate core of T147+T148 is promoted as v548) — and that scalar is then attacked through the gauge freedom: the whitening diagonal is a free choice, and the constant gauge ELIMINATES the total variation exactly (0.0000) at a certified sandwich price — thereby refuting the hypothesis, since gauges that kill the flutter move the smoothness functional by at most 0.9%: the roughness sits in the form itself, and the missing input relocates to the deep modes of the pure Toeplitz-minus-Hankel section, with the Dirichlet control correcting the question to a ladder bound ν_k ≤ C·k² with m-free C (T149) — and that ladder then names the mechanism: the form is exactly the compression of the full symmetric Toeplitz section onto its antisymmetric parity sector, and the sign-changing symbol's entire negative inertia sits in the even sector (certified 72/72) — T148's honest negative explained rather than worked around; the flutter amplitude becomes a certified form functional with a closed atom budget, and the arithmetic atoms are co-responsible for positivity (the archimedean section alone would be indefinite); one gate remains — the ladder constant still grows (13.81π, x^0.258 against the bar 0.25), the gap from π exactly the arithmetic leakage of the bottom mode (T150) — and that one inequality then closes by rerouting off the ladder: the odd grid steps over the symbol's negative window (θ_c/θ_1 = 0.328–0.407 on 72/72 — positivity is a grid fact), the bottom spectrum is certified against the parity Laplacian, and a discrete Sobolev step at the odd sector's virtual node yields a per-mode bound LINEAR in k with a non-growing constant (C_S = 11.51–19.57, trend x^0.020) where the quadratic ladder grew; two of three rest items settled (the archimedean minimum attained exactly; the symbol route a computed dead end — the local model is a matrix statement), one scalar remains a fit: the bottom pencil ratio R = K_bot/κ = 3.36–9.71, flat, exactly where the no-go breaks (T151; the identity/certificate core is promoted as v550) — and that scalar is then attacked head-on, refuting both hoped-for gifts from the smooth kernel: the archimedean part is itself negative in the odd sector and the atom part too, so positivity is a cancellation between geometry and arithmetic, not a property of either half — while the floor nevertheless closes structurally (a Schur two-block criterion with a fixed 16-mode block certifies the pencil floor κ ≥ 0.225–0.250, flat across all windows), leaving the ratio certified on both ends (R ≤ K_bot/t = 4.41–8.47) with one term missing (an m-free ceiling on K_bot), and the Psi map locating 89–92% of the dominant loss on the eight modes the certified ladder already controls — a 3.5–5.3× end-to-end reserve (T152) — and that rebuild is then carried out and refuted by exactly its own factor: Psi is pinned to within 1.20–1.30× by the first parity sine (a₁ = 8/π²), so the reserve never existed, while Psi closes as a term (the theorem-grade collapse needs nothing beyond the certified floor), the archimedean part turns out POSITIVE on the bulk parity block, and the two remaining terms lose to the same missing object — a Green/alignment estimate of two inverse-iteration columns (T153) — and that estimate is then attacked head-on, and the ceiling closes exactly: a fixed-size sixteen-column certificate (the parity sines plus one Green step) needs no residual argument at all — Ritz values are upper bounds by Courant–Fischer — and agrees with the inertia-certified true constant to 5.2e-7 on every window; the obstruction acquires a geometry — seven of eight bottom directions agree with the parity Laplacian to a degree, ONE sits at 83–90°, and that single misalignment IS the collapse price, recovered in full per window by one Cholesky, moving per-window end-to-end to 4.5e-2–3.1e-1, inside the target band; both remaining terms are now uniformity terms with certified per-window numbers, the most accessible arithmetic-free (T154; the certificate core is promoted as v551) — and those two floors are then attacked head-on: the complement floor becomes an EXACT fixed-size certificate — a 12×12 problem in the exact Kac–Murdock–Szegő numbers reproduces the size-m eigenproblem to seven decimal places on every window, the 83–90° direction is demystified (it lives on modes 9–12, which is exactly why K = 12), and the collapse price is recovered 78.8–100% at fixed size — while on the block side the full symbol infimum turns out negative on every window against a positive block minimum, so NO symbol argument can ever deliver that floor: the mechanism must be Fejér cancellation in the finite section (T155) — and those two rest objects are then attacked head-on, and both become SINGLE SCALARS: the t₁-loss of the bottom Ritz mixing is an exact closed function F(P, r) of two dimensionless numbers (verified to 2e-16), its two-line ceiling r ≤ 1/(Ls) ≤ 1/p₁ tight to 1.03–1.16, so the first term is ONE m-free lower bound on the angle p₁ = 0.2010–0.3282 (55–63°, flat), while the block positivity is re-identified as an ALIGNMENT fact, not Fejér mass damping — on the minimiser an arch part and an atom part of nearly equal size cancel, the expected arch inequality ≥ 1 is refuted and the additive split is divergent; the no-go family breaks on three axes including a collapse of the angle itself (T156) — and those two angles are then attacked head-on, and neither falls, but both change shape: a new theorem (the sine-block confinement) derives from the certified floor and ladder alone that the bottom eigenvector lives 97.1–98.4 percent inside the first sixteen parity sines, the resolvent route pins 97.9 percent of the measured angle with one measured fixed-size scalar left, the whole first term becomes a bound on ONE diagonal entry of a 16×16 Schur complement the chain already builds (Cauchy–Schwarz misses it by up to five orders — the cancellation is nearly complete), and the arch half is uniformly certified via an executed adaptive Lipschitz ceiling; the no-go family breaks on five axes (T157). 164 parts, 4295 sandbox checks. Not RH evidence.",
  keywords: [
    "TFPT prime front",
    "E8 census",
    "Hecke from geometry",
  ...
  openGraph: {
    ...
    description:
      "Primes, the E8 census, and load-bearing results v535–v542. Program status: the relay induction is certified step by step to zone 155,921, and the T125 finale assembles the chain end to end — a load-bearing spine that is 96.2% identity-or-Cholesky with one declared accounting convention. Series complete at 125 parts. No claim of progress toward the Riemann Hypothesis.",
  ...
  twitter: {
    ...
    description:
      "Research diary of the prime / zeta line. Machine-verified: v535–v561. Sandbox T102–T125: one matrix inequality compressed to one sign plus one accounting convention and assembled end to end; phase 2 (T126+) open, the bridge stands as an identity (T130), the self-supply sandwich is a theorem (T131), the identity block is promoted as v542, the reverse flow landed (T132/T133), the pole-free floor closed as existence with a sign anatomy (T134), the seam admits a bounded state (T135), Varga's identity closed the a-priori radius item (T136 → v543), the absolute-value envelope family is certified dead (T137 → v544), the sign law is geometric and the m-paired certificate removes the arithmetic wall (T138), the decay lemma is refuted arithmetically — the core shrinks to one signed inequality (T139), that inequality acquires an exact finite core per zone with all the D-dependence in the geometry (T140), the Hardy ingredient resists with an address — the certified constant is not zone-uniform while the object it bounds is, and the rest collapses to one closed conductance profile (T141 → v545), and that profile is found exactly: the capacity decomposition gives Omega = 1 exactly, the failure is a constant factor ~2.3, and the rank ladder closes the comparison path (T142), and the sharp route carries — Maz'ya's criterion lands inside its window on all 26 windows with a zone-uniform loss factor, reducing D-uniformity to one named capacity inequality (T143), and the interval route carries — the interval class is exhausted exactly, the closed two-weight sup sits inside the Maz'ya window flat in D, the family restriction is removed by a max-density-subgraph bound, leaving the absolute Maz'ya constant c₀ as the chain's single unproven input (T144), and the Maz'ya proof transcribes step by step — the Markov property is exactly one line, that line splits, the mass half is a theorem, c₀ becomes explicit (2.25–4.23, flat) with S1′ certified on 64/64 windows, and one named lemma L1 remains, proved necessary by an explicit no-go (T145; identity spine promoted as v546), and that lemma then stands — L1 closes as a theorem chain on all 64 windows with an a-priori constant (3.90–4.85) read off the form alone, the resolvent identity as the lever, Davis–Kahan discarded, and Maz'ya's 8 falling to 2; the remainder is D-uniformity for ALL D (T146 → v547), and that remainder becomes an exact identity — Γ = √Q★ · Sw, a purely spectral times a purely geometric factor, both certified, the decay route computationally dead, the mechanism named (Szegő/Widom) with Q_B ≤ 2|B| holding, one Szegő-type statement lifting the chain to all D (T147), and that statement dissected — Sw closes on the surface (≤ 1.9587, LDLᵀ layer cake), the symbol is refuted at 0, the lifting hypothesis is one named scalar (TV of the log-whitening weight, the roughness and not the conditioning), and the certified chain delivers 8.4–15.9% of the true gap (T148 → v548), and the smoothing freedom then eliminates that scalar exactly — TV(log Λ̃) = 0 in the constant gauge at a certified sandwich price — refuting the hypothesis and relocating the missing input to the deep modes of the pure Toeplitz-minus-Hankel section (T149), the mechanism then acquires its name — parity, with the entire negative inertia in the even sector and one growing gate left (T150 → v549), and the odd-sector bound closes by a Sobolev reroute — linear per-mode with a non-growing constant, one scalar (the bottom pencil ratio) left as the fit (T151 → v550), and the pencil-ratio probe then refutes both archimedean gifts — the arch section is itself negative in the odd sector, so positivity is a cancellation, not a property of either half — while closing the floor structurally via a Schur two-block criterion with a fixed 16-mode block, leaving one term (an m-free ceiling on K_bot) and a mapped 3.5–5.3× Psi reserve (T152), which the Psi-ladder probe then refutes by its own factor — Psi is pinned to within 1.20–1.30× by the first parity sine (8/π²), the reserve never existed — while closing Psi as a term, reversing a T152 sign (the arch part is positive on the bulk block) and leaving two terms that lose to one Green/alignment estimate (T153), which the green-align probe then closes on the ceiling side — a fixed-size sixteen-column certificate attains the inertia-certified constant to 5.2e-7, leaving two uniformity floors (T154 → v551) — and the bottom-floor probe then makes the complement floor an exact 12×12 certificate recovering the collapse price 79–100% at fixed size, while the full symbol infimum turns out negative on every window: no symbol argument can deliver the block floor (T155), and the twelve-by-eight probe then reduces both remaining terms to single scalars — the t₁-loss is an exact closed function of two dimensionless numbers whose ceiling makes the first term one m-free lower bound on an angle (p₁ = 0.2010–0.3282), and the block positivity is an alignment fact, not Fejér mass damping (T156), and the angle-floor probe then attacks both angles — neither falls, but both change shape: the sine-block confinement theorem replaces a measured statement (the bottom eigenvector is 97.1–98.4% inside the first sixteen parity sines, from the certified floor and ladder alone), the resolvent route pins 97.9% of the measured angle, the first term becomes a bound on one diagonal entry of a 16×16 Schur complement (Cauchy–Schwarz off by up to five orders), and the arch half carries an executed adaptive Lipschitz certificate; the no-go breaks on five axes (T157 → v552), and the cancellation-seeing bound then exists as a theorem — the Thomson dual form makes every trial vector bound the entry from the right side, a positive Cholesky ladder pins it to 1.13–1.27 flat, the entry is two-dimensional once one Green column is granted, and the growth pointer is refuted (T158), and the cancellation is then executed algebraically — seven machine-checked identities including the gauge identity (the form is exactly blind to lag mass, which is where the h² halves live) and closed Dirichlet-kernel weights; the honest negative: the h² does not telescope away, it is the common weight of both halves; the new structural win: the archimedean block is a raw symmetric Z-matrix with a closed sign-based floor above target, while the atom block obeys no sign law; the theorem count jumps 6 → 13 (T159 → v553), and the pairing is then attacked through its correlation structure — the watershed of phase 2: a machine-checked sampling identity shows the last term's atom half IS a Lambda-weighted prime sum at 32 explicit frequencies, needed to depth h^−2, while the geometric half is evaluated down to the double-precision floor via closed moment laws — the h² cancellation is the intrinsic arithmetic hardness of the problem, not an assembly artefact (T160), and the circularity triage then returns its most consequential answer — the chain is NOT circular: the required cancellation at the 32 frequencies (now identified exactly as the Fourier harmonics of the log-window, a theorem) has δ = 1.148–1.881 against RH strength 1/2 and sits below the boundary term of every partial summation, so the chain needs MORE than RH-strength input would supply — the h² sits in the SPLIT, not in the primes; the analytic half of the arch bound closes m-free (scale-free Bernstein rate (3+√5)/2, closed head split Ψ, degree schedule K(h) = O(log h)), the sign inequality R-B is refuted in all four readings and replaced by a certified off-diagonal fraction bound with constant 1/4, and a re-split moves the demand to δ_eff = 0.98–1.38 — still above 1/2 (T161 → v554); and the third-split probe then delivers the split and measures its end — a Mellin ladder lowers the demand 1.88 → 1.38 → 0.93 but saturates at its second term, the Fejér split goes BELOW the RH threshold 1/2 on all 18 windows at a price growing h^2.86 (relocated, not closed), R-A′ closes via a Lerch/Frullani integral at machine precision, R-B′ is refuted (the Gram form is indefinite), and the remaining question is a Pareto front (T162 · DELTA-REDUCED); and the front-survey probe answers it with a theorem: the exchange law makes price and demand two coordinates of one identity, a four-line theorem gives every admissible trial vector a total-variation floor ~ h² (the hardness was never in the primes — it is the spectral gap of the parity Laplacian meeting the entry normalisation), R-C‴ closes negatively, and the theorem cores of T162+T163 are promoted as v555 (T163 · FRONT-RESISTS); and the sector-change probe decides both arms of the successor: the chain spends its ceiling at power exactly one (no free tolerance) yet the O(1) gate is discharged window by window by a Cholesky identity — the last analytic term collapses onto the single quantifier sup_m 1/g₁₆(m) < ∞ — and no sector can help, as a theorem: the entry normalisation is a gauge, floor times transfer reproduces the h² identically, and the new gauge-invariant, prime-free successor is the alignment efficiency between the weight increments and their partial sums (T164 · TOLERANCE-CARRIES / SECTOR-RESISTS); and the alignment probe closes that successor by certificate: a new gauge-invariant identity — P_pr = g₁₆·R·(TV/(t₁v)²)/μᴾ₁ — makes demand and price one equation, so any crossing vector pays the old KMS h², the alignment question was never independent (its m-uniform form IS the quantifier), the price exponent decomposes exactly (61% the same h² since T152), the quarter-bar drift is settled as zone depth on a decoupled surface, and exactly ONE genuine open object remains — a lower bound on the 16-step Schur-cascade gain, a cancellation beyond absolute-value budgets — with the theorem cores of T164+T165 promoted as v556 (T165 · ETA-RESISTS); and the cascade probe then dissects that lower bound completely: the whole gain is a near-collinearity (g_K/g₁ = 1/(1 − R_K²)), one rung carries most of it, the gain is invariant under the spectral normalisation — the h³ is a property of the arithmetic Gram block alone — the cancellation lives in a 2×2 Gram determinant (the same arch-against-atom mechanism as the sampling identity, one level up), an anti-fitting scramble destroys the effect by a factor 4569, and the best closed route reaches h^+1.32 against the target h^+3.11 — an exponent gap: the one missing inequality IS the cancellation, now stated in three equivalent dresses (T166 · CASCADE-RESISTS); and the null-vector probe then delivers the strongest reduction of the series: at K = 2 the closed vector (1, −Q₂₁/Q₂₂) is EXACT — a theorem, so the third dress collapses onto the second — and a machine-checked identity unifies all three dresses (eps_ent(K) = ρ·(1/g_K)/S_K: the determinant ratio, the scalar 1 − r₁₂² and the entry threshold are ONE inequality, not three); against expectation the threshold is mildest exactly where the vector is free (K = 2, not the first flat K = 6), perturbation theory dies for a remarkable reason (the Kato series converges fast — to the WRONG object), and the scramble control now shows a change of TYPE, not a factor (with scrambled prime positions the 2×2 block itself loses positivity); the whole programme rests on one scalar built from three closed lag sums, and the theorem cores of T166+T167 are promoted as v557 (T167 · VECTOR-RESISTS); T168 then ran the Lagrange identity on that scalar — the sum of squares is real (the arithmetic kernel is positive definite on all 63 windows, hard-fenced per window, no Weil test), the Wronskian minors are closed and provably maximal in norm, and the one open factor is the single ratio t* = Q₁₂/Q₁₁ — by T168-TH7 the target itself rewritten, the hardness self-similar under exact reformulation (T168 · MINORS-RESIST); and T169 then PROVED that self-similarity: every genuinely closed candidate misses by O(1) and diverges at h^+1.99, while the only accurate one, √(â₂₂/â₁₁), collapses by a new identity (T169-TH7) back onto det Â — the loop closes as an identity — with real gains: the first CERT-UNIF in weeks (Gershgorin on ν₁, unconditional), an R4-free chain that never touches the Weil-shaped positivity (frame-A trend h^−2.948, ε = 0.052), and R1 in standard analytic shape for the first time, a bilinear von Mangoldt sum against closed Dirichlet weights (T169 · RATIO-RESISTS); and T170 then ran the unconditional sieve toolbox on that bilinear form and completed the classification as a theorem — the form is written down exactly, collapses back onto three linear Λ-sums by a rank-3 theorem and a Type II collapse, no unconditional route exceeds δ = +0.996 against the target 3.0, and R1 is finally classified as a NEAR-DEGENERACY, not a size, with the theorem cores of T168+T169+T170 promoted as v558 (T170 · SIEVE-RESISTS); and T171 then assembled the capstone, and the map is complete: all sixteen links of the reduction chain reproduce in one connected run (thirteen theorems, three certificates), all eight classified no-go routes fail on instances exactly as classified, and the precision ledger closes — the needed joint precision stands five orders beyond the RH yardstick and three beyond the best unconditional bound; the file adds zero new uniform statements, phase 2 is a certified map with one open object, and the capstone is promoted as v559 (T171 · MAP-COMPLETE); T172 is testing how far the map carries beyond frame A; 4557 sandbox checks. Residual: I5 — not almost-RH.",
  },
};
    ...
    <>
      <section className="relative isolate overflow-hidden pt-12 pb-8 sm:pt-16">
        ...
        <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-8 font-mono text-[11px] uppercase tracking-[0.2em] text-sky-300/90">
            Research diary · Teile 11–177 · 4765 sandbox checks
          </p>
      ...
      >
        ...
        <p className="rounded-xl border border-slate-700/40 bg-slate-900/40 px-4 py-3 text-sm text-slate-400">
          ...
          sieve. T176 (dense_limit_probe.py) is running — the larger
          sieve. Sandbox; not RH evidence.
        </p>
      </DiarySection>
      ...
      <footer className="border-t border-slate-800/60 py-10">
        <div className="mx-auto max-w-5xl px-4 text-sm text-slate-500 sm:px-6 lg:px-8">
          ...
          <code className="font-mono text-slate-400">experiments/next.txt</code>{" "}
            (2026-07-23…29 diary, Teile 11–175) and the promoted modules{" "}
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <aside className="mt-8 rounded-2xl border border-rose-400/35 bg-rose-500/[0.08] p-5 sm:p-6">
          ...
          <ul className="mt-3 space-y-2.5 text-sm leading-relaxed text-slate-300">
            ...
            <li>
              ...
              This entire diary is exploratory. Only the twenty-seven promoted
              modules{" "}
              <Link
                href="/verification"
        ...
        <div className="mt-8">
          ...
          <dl className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <BigPictureStat
              term="176"
              desc="diary parts — series complete at 125, phase 2 a certified map"
              tone="sky"
            />
            <BigPictureStat
              term="4765"
            ...
            <BigPictureStat
              term="27"
              desc="machine-verified modules (v535–v561, incl. the T177 backflow)"
              tone="emerald"
            />
            <BigPictureStat
              term="96.2%"
  ...
  return (
    <aside className="rounded-2xl border border-violet-400/35 bg-violet-500/10 p-5 sm:p-6">
      ...
      <p className="mt-3 font-serif text-lg leading-relaxed text-violet-50 sm:text-xl">
        After 143 probes and eleven promoted modules, what remains TFPT-specific
  ...
  return (
    <aside className="rounded-2xl border border-violet-400/35 bg-violet-500/10 p-5 sm:p-6">
      ...
      <p className="mt-4 text-xs leading-relaxed text-slate-400">
        ...
        does not perform one. 2428/2428 sandbox checks at T101 (4765/4765 today).
