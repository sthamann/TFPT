"""v739 -- PRIME.RANK3JET.01: the review simplification of the
rank-3 wall, tested exactly (RANK3-JET-DEAD).
...
PROVENANCE: discovery probe rank3_jet_probe.py (2026-08-04, 20/20,
verdict RANK3-JET-DEAD: the jet read is exact -- the lock block is
the first jet of ONE function to 4.6e-16, with Loewner coordinates
and the Schwarzian form [1 - r12^2]_odd = (3/8) S[g~](1) eps^4 on the
odd sector -- BUT the review kill fires: the eps^0 coefficient of
1 - r12^2 is identically 1 (not 0), the even sector exceeds the net
entry by 1.15-6x, and the pure jet block carries no collinearity
(factor >= 1.9e3): the third power is even/odd COMPENSATION (T-B
substance), not a jet identity; class (c) for the envelope route;
KREIN-LINK-WEAK -- the section-8 conjecture is weakened (correlation
0.678 < control 0.946)).  Promoted verbatim (sibling imports resolve
against the promoted v563/v683/v696 modules); numbers unchanged.
"""
import ast
import math
import os
