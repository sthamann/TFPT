#!/usr/bin/env python3
"""v679 -- QGEO.ORBOS.01: the FIRST CONTINUUM-OS SLICE of the seam
orbifold B -- the last big [C] of the QGEO orbifold front.  The
Osterwalder-Schrader reconstruction needs (i) Euclidean correlators
that CONVERGE as the lattice is removed, (ii) reflection positivity IN
THE LIMIT (not only per lattice), (iii) the cluster property.  This
module measures all three at the convergence level, with RATES, plus
the continuum characters from the exact level extrapolation.
...
FIREWALL: GATE.QGEO does not move; no marker changes; the formal
continuum limit stays typed open (O5.1: uniform bounds over all point
configurations, tightness/equicontinuity, the GNS limit Hilbert space,
operator convergence, the R/parity sectors, the assembled orbifold-B
correlators beyond the twist blocks) -- named, not claimed.
Python-only, counted per GATE.WOLFRAM.02.

Conventions inherited read-only from v639_twist_ope.py,
v645_klein_rp.py, v650_orbifold_modular.py, v651_orbifold_assembly.py,
v652_orbifold_arf.py, v653_bond_defect.py, v622/v623/v628.

PROVENANCE: discovery probe orbifold_continuum_os_probe.py
(2026-08-02, 19/19, verdict ORB-OS-CONTINUUM-SLICE).
"""
