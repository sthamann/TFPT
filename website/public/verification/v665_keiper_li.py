"""v665 -- PRIME.KEIPERLI.01: SLICE A of the RH-criteria scan -- the
Keiper-Li sequence lambda_n computed on TWO INDEPENDENT ROUTES,
binding the zero comb (v589), the atom-table constants (v563:
gamma_E, log pi), the archimedean kernels (psi at 1/2 <-> v563 arch
block / v643 W_sm) and the pole block (v591/v643: the two poles
s = 0, 1 <-> the scalar +2) into ONE scalar sequence.
...
FIREWALL: no verification/ imports; NO RH claim (lambda_n > 0 for
n <= 64 is a FINITE verified statement, not Li's criterion); the
zero-side data are DIAGNOSTIC here, per the declared exception of the
zero-comb line (v589 convention) -- no positivity claim feeds any
prime-side module.  Zero data: mpmath zetazero only (dps 15,
declared; gamma error ~1e-13 propagates < 1e-12 into lambda_n, far
below every budget), cached in the committed
experiments/tfpt-discovery/zero_comb_cache_n2000.json (regenerated
locally if absent -- a full rebuild takes ~1 h of zetazero calls).
Verdict enums (frozen): KEIPER-LI-CONSISTENT (all checks incl. the
A3 budget), BUDGET-BREACH (A3 fails), MIXED.  Python-only, counted
per GATE.WOLFRAM.02.

PROVENANCE: discovery probe keiper_li_probe.py (2026-08-02, 13/13,
verdict KEIPER-LI-CONSISTENT).
"""
import json
import math
import os
...
# mirror / standalone use), else rebuild locally.
...
def run():
    ...
    print("FIREWALL: zetazero comb declared (dps %d, diagnostic zero-"
          "side line); tail budget via RvM density + |S| <= %.1f "
          "band; NO RH claim -- lambda_n > 0 for n <= %d is a finite "
          "statement, not Li's criterion"
          % (DPS_ZEROS, S_BOUND, N_MAX))
    print("--- v665_keiper_li: %d passed, %d failed ---"
