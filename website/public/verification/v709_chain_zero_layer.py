#!/usr/bin/env python3
"""v709 -- PRIME.ZEROLAYER.01: S-G, THE LAYERING HYPOTHESIS
...
PROVENANCE: discovery probe chain_zero_layer_probe.py (2026-08-03,
10/10 PASS, verdict LAYERING-REJECTED in the tested projections: the
S-F selection residual carries NO zero signature (best corr 0.042 =
the scramble level); ~64% of the residual is window geometry; the
zero data enter through a TEMPORAL SHA256 firewall (the residual is
hashed before any zero is read)).  Promoted verbatim; numbers
unchanged.
"""
import hashlib
import json
...
def _cache_path(fn):
    """zero caches live in experiments/tfpt-discovery (committed);
    local fallback for standalone copies."""
    repo = os.path.join(_DISC, fn)
    return repo if os.path.exists(repo) else os.path.join(_here, fn)
...
def run():
    ...
    for fn, key in (("zero_comb_cache_n2000.json", "gammas"),
        ...
        with open(_cache_path(fn), "r") as fh:
