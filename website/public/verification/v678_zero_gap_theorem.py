"""v678 -- PRIME.ZEROGAP.01: THE ZERO-GAP THEOREM ROUTE -- an EXPLICIT
...
delta(t) = kappa H_min(t).  This module measures and types; it moves
no marker.

CONTEXT (the two parents, both 2026-08-02).  fejer_density_bound_probe
(promoted as v669) derived the exact identity s_tot = 2 pi (F_a * dN)
...
packet_garding_probe (promoted as v674) built the packet norm and
...
zero-gap input.  THIS module supplies the zero-gap input and measures
...
SLICES AND BARS (declared BEFORE the numbers):
  ...
  Z3.3 [MEASURED, typed residual -- inverted expectation] the adaptive
       packet Garding ladder: per kappa in {2, 3} and C in {0.5, 1, 2}:
       c_X(M, C) on M = 92/184/368/736.  STABILIZATION BAR per
       (kappa, C): all c_X > 0 AND last relative increment <= 0.03
       (task bar).  TWO-MODEL BAR: affine b + a/L vs pure a/L
       (L = log(2 + pi/D)): rms(pure) >= 3 x rms(affine) AND b > 0.
       THE INVERTED EXPECTATION (v642/v662/v674 pattern; the discovery
       probe's declared FAIL at this literal bar IS the finding,
       numbers unchanged): stabilization MUST fail at every (kappa, C)
       AND the pure-1/log model MUST stay competitive at every
       (kappa, C) -- the pointwise obstruction is re-partition-
       invariant, adaptive widths do not remove it (the minimizer
       stays single-mode tight, c_X/single-mode = 0.898).
       Fixed-partition baseline printed; midpoint-weight column
       printed; minimizer forensics (dominant mode, single-mode floor,
       tightness) at (736, C = 1); kappa_X = lambda_max(H_X_adapt,
       H_log) <= 1.5 uniformly on the ladder (X -> H_log transfer,
       recalibrated read -- increments printed, see CALIBRATION
       HISTORY); pencil (H_X_adapt, Mass) == adaptive weights to 1e-8
       at 736.
  Z4.1 [C] typing + contract note (report only; nothing written).

Verdict enums (frozen, precedence top-down; the EXPECTED verdict of
this module is ZEROGAP-FLOOR-ONLY -- the honest split):
  ...
  ZEROGAP-GARDING-CLOSED -- Z2 + Z3.1 + Z3.2 bars pass AND for some
                            ...
                            AND the kappa_X bar;
  ZEROGAP-FLOOR-ONLY    -- Z2 + Z3.1 + Z3.2 bars pass, the projection
                            closure does NOT fire: the zero-gap
                            theorem gives the unconditional
                            packet-Boden (frame form), the projection
                            form still fails -- the honest split;
  ZEROGAP-NO-GAIN       -- otherwise.

FIREWALL (INVERTED, declared -- fejer convention).  This module
...
packet_garding_probe / v674); no zero enters the operator pipeline.
Zero data: the shared Turing-certified cache
zero_comb_cache_n2000.json (+ the c1 extension cache, provenance
printed, no bar).  TYPED-RESIDUAL DISCIPLINE (the v642/v662/v674
pattern): the discovery probe ended 9 PASS + 1 declared FAIL at the
literal Z3.3 stabilization bar -- this module formulates the SAME
measurement as a typed-residual check with inverted expectation (the
adaptive packet PROJECTION ladder MUST fail stabilization at every
(kappa, C) AND keep the pure-1/log model competitive: the minimizer
is single-mode tight, c_X/single-mode = 0.898 -- re-partitioning by
adaptive zero-gap widths re-labels the pointwise obstruction instead
of removing it; the theorem-shaped packet object is the Z3.2 frame
Boden, which is now UNCONDITIONALLY positive), numbers unchanged; the
verdict ZEROGAP-FLOOR-ONLY is the honest split.  No marker moves;
Python-only per GATE.WOLFRAM.02.

PROVENANCE: discovery probe zero_gap_theorem_probe.py (2026-08-02,
9 PASS + 1 declared FAIL, verdict ZEROGAP-FLOOR-ONLY);
fejer_density_bound_probe.py / v669 + packet_garding_probe.py / v674
(machinery + the typed OPEN residue), zero_comb_cache_n2000.json
(turing_cert_probe / v666: TURING-COMPLETE-BAND),
c1_zero_ext_n2500.json (c1_mechanism_probe / v676 extension),
...
def run():
    ...
    # TYPED RESIDUAL (v642/v662/v674 pattern, inverted expectation):
    # the probe's declared FAIL at the literal Z3.3 bar IS the finding
    # -- the adaptive packet PROJECTION ladder must fail stabilization
    # at every (kappa, C) (measured last increments 0.065..0.121 >
    # 0.03) AND keep the pure-1/log model competitive at every
    # (kappa, C) (measured rms ratios 1.70..1.77 < 3.0), with the
    # minimizer single-mode tight (c_X/single-mode = 0.898):
    # re-partitioning by adaptive zero-gap widths re-labels the
    # pointwise obstruction, it does not remove it.  The kappa_X
    # uniform bound and the pencil identity remain positive
    # requirements.  The theorem-shaped packet object is the Z3.2
    # frame Boden.  Numbers unchanged.
    z33_resid = (not any(stab.values())) and (not any(sep.values())) \
        and kap_ok and pack_eig_dev <= BAR_PACK_EIG
    check("Z3.3 [MEASURED, typed residual -- inverted expectation] "
          "adaptive packet Garding: stabilization MUST fail and the "
          "1/log model MUST stay competitive per (kappa, C): %s (all "
          "must be MISS -- the probe's honest FAIL, promoted as the "
          "finding); kappa_X worst %.4f <= %.1f (uniform bound) %s; "
          "pencil (H_X, Mass) == adaptive weights to %.1e <= %.0e -- "
          "the projection form with adaptive widths %s: the pointwise "
          "obstruction is re-partition-invariant (the theorem-shaped "
          "object is the Z3.2 frame Boden)"
          % ({kap: ["C=%.1f:%s" % (C, "ok" if stab[(kap, C)]
             ...
             BAR_PACK_EIG,
             "CLOSES" if z33_ok else "does NOT close (measured)"),
          z33_resid)
