#!/usr/bin/env python3
"""v720 -- PRIME.MOONSHOT.06: MOONSHOT K2+K3 slice -- die zwei
Konvergenz-Saetze der Ledger-Liste.
...
PROVENANCE: discovery probe moonshot_k2k3_probe.py (2026-08-03,
14/14 PASS, verdict MOONSHOT-K2K3-COLLAPSED-CLASSICAL: K3 collapses
to measured tightness (compacta Cauchy, band edge = the smooth omega
density, p_0 = 0.981 log(1/D) - 1.306 -- sigma-finite, vague
topology); K2 is classical analysis on s > 1/2 (unconditional
Chebyshev majorant c_RS = 1.03883 machine-verified in range, tail law
slope = -(s - 1/2) exact, discretization ~ C D^2); the boundary is
EXACTLY s = 1/2 (Mertens slope 1.9994 == 2 at t = 0, t != 0 bounded);
the remaining hard substance is (L1) line identification + (L2) K1
node convergence, (L3) routine).  Promoted verbatim (sibling imports
now point at v716..v719); numbers unchanged.
"""
