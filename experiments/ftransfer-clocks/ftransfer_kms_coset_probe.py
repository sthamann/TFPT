...
def main() -> int:
    ...
    out = {
        "probe": "ftransfer_kms_coset",
        "executed": time.strftime("%Y-%m-%d %H:%M:%S"),
        "freeze": {"yaml": YAML_SHA256, "gstar": GSTAR_SHA256, "alphas": ALPHAS_SHA256},
        "criterion": "time-1 PGL2 holonomy class + detailed-balance multiplier "
                     "cross-ratio (no Schwarzian, no derivatives)",
        "theorem": "S_transported = -(ln lambda)^2/2",
        "blind_assignment": {nm: {"typing": blind[nm][0],
                                  "ln_mult_abs": str(blind[nm][1]),
                                  "witness_ok": blind[nm][2]} for nm in blind},
        "unblinding": {nm: str(sp.simplify(S_meas[nm])) for nm in S_meas},
        "controls": {"C1_fake_caught": c1, "C2_affine_invariant": c2,
                     "C3_moebius_positive_case": c3},
        "verdict": verdict,
        "checks_total": len(CHECKS), "checks_failed": n_fail,
        "checks": CHECKS,
    }
    os.makedirs(os.path.join(HERE, "results"), exist_ok=True)
    out_path = os.path.join(HERE, "results", "ftransfer_kms_coset.json")
