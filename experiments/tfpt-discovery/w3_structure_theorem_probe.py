"""Discovery probe: THE W3 STRUCTURE THEOREM -- what the third rung IS.
...
FIREWALL (INVERTED for identification, declared -- as in the fejer
density probe).  The window forms and mode reads are assembled from
primes + digamma ONLY (v563 machinery verbatim; no zero enters any
assembly); Riemann zeros enter ONLY the S2 identity VERIFICATION
(the shared Turing-certified cache) and the discussion; the Epstein
zero census is a measured OUTPUT (argument principle on E built from
lattice data), never an input table.  experiments-only;
verification/ read-only (v563 import); no marker moves; no RH
statement; Python-only per GATE.WOLFRAM.02.

Provenance: fejer_density_bound_probe.py (2026-08-02, the s_tot =
2 pi (F_a * dN) identity + machinery conventions),
epstein_firewall_probe.py (the lab, Lambda_E machinery, box),
ihara_ground_truth_probe.py (K* s ~ 2-3 detection law),
w3_resonance_landscape_probe.py / w3_uniform_bound_probe (the W3
margin context), v563_paper2_readouts (window machinery,
lag_weights_from_v = the T163 correlation theorem),
zero_comb_cache_n2000.json (turing_cert provenance),
