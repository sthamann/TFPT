"""PRIME.CHEBLOEWNER -- the second review's Toeplitz strand, audited: the
polarized cross-weight formula is EXACT against the corpus definition, the
edge read block is EXACTLY a scaled Chebyshev-Loewner matrix (a two-line
consequence, not an asymptotic), the first edge read is rank one with the
Pythagorean null profile (5,-3,4), and the claimed N^-4 defect and
pair-kernel sign polynomial are EXACT series coefficients.

THE CLAIMS (external review, 2026-07-31 II):
  (1) cross weights  w_d^{(ij)} = (2/N)[sin wj sin(d wi) - sin wi
      sin(d wj)]/(cos wi - cos wj),  N = 2h+1, wk = 2 pi k/N;
  (2) at the reflection edge d = N-s the block is the Loewner matrix
      W^(s)_{ij} = -(2/N) sin wi sin wj U_{s-1}[x_i, x_j]  (x = cos w),
      hence rank <= s-1; s = 2 gives rank ONE exactly;
  (3) the leading edge profile is -(2s(s^2-1)/(3N)) w w^T, whose modes-(1,2)
      Lorentz image is the Pythagorean null triple (5,-3,4) -- the SAME
      triple as (g_car, N_fam, |mu4|) via Euclid's parametrisation;
  (4) det W^(s)/(W11 W22) = -(3 pi^4/175)(s^2-4)(11 s^2+6) N^-4 + O(N^-6);
  (5) K(W^(s), W^(z)) = (256 pi^8/525) N^-10 s z (s^2-1)(z^2-1) P(s,z) +
      O(N^-12) with P = 5s^4 - 21 s^2 z^2 + 19 s^2 + 5 z^4 + 19 z^2 + 24
      (sign transition near s/z ~ 0.503 / 1.987);
  (6) the macroscopic two-scale kernels W^inf(sigma).

CONSTRUCTION BASE: the corpus definitions verbatim (v563 parity_basis,
lag_weights_from_v -- the T163 correlation theorem's weights), READ-ONLY.
FIREWALL: exact identities and series coefficients about FINITE trigonometric
objects; NO uniformity claim, no rate claim beyond fixed s, NO RH statement;
the (5,-3,4) bridge is typed COMPRESSION (no-free-pattern), not new evidence.
Verdict enums (frozen): FORMULAS-EXACT, FORMULAS-FAIL, MIXED.
