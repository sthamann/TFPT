"""Discovery probe: CLOSING THE O(1) COVERAGE HOLE -- the last gap of the
W2 chain: the pinch-attack coverage map at the anchor a0 has ONE hole,
(2500, 7.27e6) = 3.46 decades, where neither the comb floor (comb ends
at 2515) nor any capture / Selberg chain is positive.  This probe
attacks the hole on the three paths NAMED by the parent and synthesizes
the gapless map.  It measures, derives and types; it moves no marker.

CONTEXT (the parent, pinch_attack_probe, 2026-08-03).  Verdict
...
FIREWALL (INVERTED, declared -- parent convention).  This probe
...
against mpmath).  experiments-only; verification/ read-only (v563
import); no marker moves; new file only; Python-only per
GATE.WOLFRAM.02.  A scan cache (coverage_hole_zscan_cache.npz, new
file in this directory) stores the deterministic stage-1 result.

Provenance: pinch_attack_probe.py + zero_gap_theorem_probe.py
(2026-08-02/03, machinery + the typed hole), zero_comb_cache_n2000.json
(turing_cert_probe: TURING-COMPLETE-BAND), c1_zero_ext_n2500.json,
...
CACHE_SCAN = os.path.join(HERE, "coverage_hole_zscan_cache.npz")
...
def run():
    ...
    print("   R2  RS scan (this probe)      (2515.3, %.5g]  %.5f"
