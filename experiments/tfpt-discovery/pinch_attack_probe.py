"""Discovery probe: THE PINCH ATTACK -- closing the 11.7% gap of the W2
pointwise-capture obstruction from OUR side: (A) extremal minorants
(Beurling-Selberg instead of the Fejer box) and (B) the literature /
empirical-S(t) state of the art.  This probe measures, derives and
types; it moves no marker.

CONTEXT (the parent, 2026-08-02).  zero_gap_theorem_probe (v678
candidate) supplied the explicit unconditional zero-gap theorem
...
fejer_density_bound_probe (v669 candidate) built the RvM+Trudgian
...
FIREWALL (INVERTED, declared -- v678 convention).  This probe
...
provenance printed, no bar).  experiments-only; verification/
read-only (v563 import); no marker moves; new file only; Python-only
per GATE.WOLFRAM.02.

Provenance: zero_gap_theorem_probe.py + fejer_density_bound_probe.py
(2026-08-02, machinery + the typed pinch), zero_comb_cache_n2000.json
