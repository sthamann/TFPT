"""Discovery probe: THE PACKET GARDING INEQUALITY -- the packet-to-point
gap of the W2 route, measured where the Fejer density theorem lives.
...
machinery REBUILT verbatim (no probe imports); NO zero of any
L-function is read (AST-checked); no marker moves; Python-only per
GATE.WOLFRAM.02.

Provenance: garding_probe.py (certified layerwise lags, H_log Gram
T = 3000, C-ladder, DST sweep), fejer_density_bound_probe.py (the
identity, the RvM chain, (c0, C0, t0), the certificate convention),
w2_mosco_probe.py (resolvent carriers, recovery + wiggle, A5(a)),
w1_theorem_probe P2.3 (layer certification), v563_paper2_readouts
(atom table, frame-A windows), Trudgian J. Number Theory 134 (2014),
Platt-Trudgian Bull. LMS 53 (2021), Iwaniec-Kowalski Thm 5.12,
Suzuki arXiv:2606.09096 Prop. 4.1.
"""
import ast
import math
import os
...
def run():
    ...
    check("P2.1 [MEASURED, central] the packet Garding ladder: "
          "stabilization (all c_X > 0, last increment <= %.2f) %s; "
          "two-model separation (rms ratio >= %.1f, b > 0) %s; "
          "c_X tracks the pointwise c_Hlog to %s -- the packet "
          "PROJECTION norm %s the pointwise obstruction"
          % (STAB_BAR,
             "holds for C = %s" % [C for C in C_LADDER if stab[C]]
             if any(stab.values()) else "FAILS for all C",
             SEP_BAR,
             "holds for C = %s" % [C for C in C_LADDER if sep[C]]
             if any(sep.values()) else "FAILS for all C",
             ["%.3f" % (cX[(M, 1.0)] / cH[(M, 1.0)]) for M in MS],
             "escapes" if p21_ok else "DOES NOT escape"),
          p21_ok)
