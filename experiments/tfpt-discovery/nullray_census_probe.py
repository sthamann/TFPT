"""PRIME.NULLRAY -- the null-ray locking census (the second review's
Priority 2): does the pencil locking direction align with the Pythagorean
null ray (2, -1) of the rank-one edge read?  Answer on the declared
surface: CORRELATED BUT PARTIAL -- the angle sits far below random,
falls slowly, and is destroyed by the scramble; the ray is a real
arithmetic structure but not the full locking carrier.

THE TEST (preregistered by the review, executed verbatim): for every
declared window, v_lock = the near-null eigenvector of Ahat = B - S
(the locking mode of the v569 pencil); measure angle(v_lock, v0) with
v0 = (2, -1) -- the exact null direction of the leading edge profile
[[1,2],[2,4]] (v576) -- in the raw mode coordinates AND in the B-metric.
SUCCESS (review): angle falls uniformly to zero.  KILL (review): no
decline, zone wander, or scramble blindness.

FIREWALL: the declared v563 T170 frame-A surface, read-only; MEASURED
statements with their ladders; no uniformity claim, no rate claim, NO RH
statement.  Verdict enums (frozen): NULL-RAY-LOCKED (clean convergence),
NULL-RAY-CORRELATED (below-random alignment, slow decline, scramble-
sensitive -- partial carrier), NULL-RAY-DEAD (kill criteria fire).
