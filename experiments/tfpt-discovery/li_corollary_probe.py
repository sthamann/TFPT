"""li_corollary_probe -- SLICE A of the Li-strand: the W1 => Li corollary
SPELLED OUT.  The scan thesis was: "W1 probably already implies finite
Li positivity for the test functions spanned by our windows."  This
probe makes that precise and measures it on the LARGEST complete
frame-A window (h = 1433, the v563/v648 surface).
...
FIREWALL: experiments-only; v563 imported READ-ONLY; no ledger/paper/
website edits; zero data = the shared zetazero cache (declared);
...
def run():
    ...
    print("FIREWALL: experiments-only probe; v563 READ-ONLY; shared "
          "zero cache (dps %d); finite statements only; NO RH claim"
          % DPS_ZEROS)
    print("--- li_corollary_probe: %d passed, %d failed ---"
