"""Discovery probe: THE BLOCK DEFLATION -- the named successor of the
FAILED rank-1 rotation predicate (theta_rotation_predicate_probe,
2026-08-02, THETA-PRED-FAIL): replace the eigenvector-free 2x2
...
v618/v591/rid/landscape/theta machinery REBUILT verbatim, no other
imports); no marker moves; no positivity claim; no RH statement; NO
zero of any L-function is read (AST-checked).  Python-only, per
GATE.WOLFRAM.02.

Provenance: theta_rotation_predicate_probe (2026-08-02, THETA-PRED-
FAIL + the K2 secular machinery + the run-2 quotes), w3_resonance_
landscape_probe (52 peaks + regime typing), rid_alignment_probe
(lock geometry), v618 (C = 1 reads), v591 (closed lock direction).
"""
import ast
import math
import os
