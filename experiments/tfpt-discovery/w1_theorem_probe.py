"""Discovery probe: W1 AS A THEOREM at the measure level -- the precise
statement on SUZUKI'S TRUE SCREW FUNCTION, the L^2_0 projection
bookkeeping (the last open D6 item of w1_dictionary_probe), and the full
form equality on the common odd sector.
...
explicit; portability of the measured ratios per w1_frozen_windows_probe
...
against exact kink-split integrals, cf. w1_matrix_identity_probe M1.1),
...
FIREWALL: experiments-only; v563 read-only; no marker moves; no
positivity claim, no RH statement.  Python-only (GATE.WOLFRAM.02).
"""
import math
import os
import sys
