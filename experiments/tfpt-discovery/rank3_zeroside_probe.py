"""Discovery probe: OFFENSIVE 2b -- THE K_M FUNCTIONAL THROUGH THE
EXPLICIT FORMULA, AND THE ZERO-FREE-ZONE DELIVERY.

Parent (rank3_functionals_probe, 2026-08-03): the whole first-order
...
inequality forbade the internal cancellation).  THIS probe treats the
...
ZERO INPUT (declared openly -- this probe READS ZETA ZEROS; that is
the point of the explicit-formula side; experiments-only, no marker
move, no verification surface touched):
...
FIREWALL: verification/ strictly read-only (v563 import); the parent
probe rank3_functionals_probe is imported for its helpers (same dir,
...
Provenance: rank3_functionals_probe.py (K_M lift, R3.PAIR);
v563_paper2_readouts.py (windows, band_fit); v589_zero_comb /
zero_comb_cache_n2000.json + c1_zero_ext_n2500.json (precise zeros);
coverage_hole_probe.py cache (count oracle + tail); pinch_attack_probe
(Riemann-Siegel machinery pattern, Gabcke remainder); Platt-Trudgian
2021 (RH verified to 3e12, CITED); parity_toeplitz_classification.tex
...
def run():
    ...
    npz = np.load(os.path.join(_here, "coverage_hole_zscan_cache.npz"))
  ...
  WHAT STAYS OPEN (honest): T-B (det Ahat >= 0, margin ~ 2e-5 of
    ...
    still needs the collective cancellation (prob:R1); this probe
