"""PRIME.PAIRBAND -- the dominance is LONG-RANGE: the pair-level anatomy of
det S on the declared surface -- every near band drags negative, the whole
positive excess lives beyond 16 lag cells, and its net is stable while the
gross masses grow.

THE QUESTION (the v570 open question, next step): v570 reduced the
spectator separation to the dominance det S >> det B and killed the cheap
two-atom witness.  WHERE does the dominance live?  If it were near-diagonal
mass, Chebyshev-type budgets could reach it; this probe measures the
pair-band anatomy to type the right target.

FIREWALL: declared T170 frame-A surface only (v563, read-only); a DECLARED
subset of windows for the O(n^2) kernel dissection (every 12th window with
n <= 16000 atoms, plus the h = 540 reference); MEASURED statements carry
the surface ladder; no rate, no bound, NO RH statement.  Verdict enums
(frozen): LONG-RANGE (near bands negative, far band carries the whole
excess, stable net), NEAR-RANGE, MIXED.
