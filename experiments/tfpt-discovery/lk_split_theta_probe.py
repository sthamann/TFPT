"""Discovery probe: THE L+K SPLIT AND THE RELATIVE FORM BOUND THETA --
OFFENSIVE 1 of the uniform-operator-positivity program: construct the
NON-CIRCULAR split B_a = L_a + K_a of the deployed window form and
MEASURE the relative bound |<K v, v>| <= theta <L v, v> on the odd
sector, over the 5-window family x an M-ladder.  This probe measures,
derives and types; it moves no marker; NO RH claim.
...
no marker moves; no positivity claim beyond the measured tables; no
RH statement; NO zero of any L-function is read anywhere (assembly =
primes + digamma + closed exponentials only; AST-checked).
Python-only, counted per GATE.WOLFRAM.02.

Provenance: v563_paper2_readouts (window machinery),
...
pinch_attack_probe / coverage_hole_probe (5-window garding
selection), Weil 1952, Yoshida 1992, Kac-Murdock-Szego 1953,
