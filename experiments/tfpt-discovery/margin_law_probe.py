#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""margin_law_probe -- PRIME.MARGIN.LAW.01
(EXPLORATION ONLY, experiments/; the margin itself as a mathematical
object, after SKELETON-CERTIFIED, 2026-08-07).

THE OBJECT: the certified margin series tau_X (5.98e-4 .. 1.71e-5)
on the 67 deployed rungs, in identified-corner coordinates tau_X =
lambda_min(S + C) (== lambda_min(Ah) to 4.9e-13, measured).  Four
frozen questions -- measure, don't narrate.

Q1 THE LAW (fit-free): tau vs (h, alpha, mass).  Fit-free internal
exponent: NEAR-EQUAL-ALPHA RUNG PAIRS (|d alpha| <= 0.05, |d ln h|
>= 0.4) give pairwise slopes d ln tau / d ln h with alpha frozen --
the h-exponent measured without any global fit.  THE ENVELOPE
CONSISTENCY: the floor strand certified rho = tau/tau_pnt >=
4.335 h^{-3/2} (73/73 frames; v818/v829, tau_pnt = the explicit
zero-free PNT-density transversal, v583 convention U0 =
2 ln(-C_TH/4), GRID_PER_D = 4, umax = 2 alpha).  The ward: the
relational-coordinate margin IS the same object (tau agreement
already at 4.9e-13); reproduce e1 = rho h^{3/2} >= 4.335 on all 67
rungs and TYPE the exact relationship: tau = e1(alpha) h^{-3/2}
tau_pnt(alpha) -- the h-law is the envelope's, the alpha-decay is
tau_pnt's (its segment slopes measured).

Q2 THE TOWER RECURSION: the Hjelmslev reduction maps act as the
IDENTITY on the GL1 corner reads (cp_tower: compatibility defect
identically zero), so the tower depth m enters the margin only
through the dyadic mass scale 4^m = e^{2 alpha_m} -- the well-posed
induced recursion is on the dyadic rung subsequence.  Frozen level
rungs (mass closest to 4^m; exact at m = 4, 6, 7, 9):
    m=4: kz9 (256 = 4^4)   m=5: kz16 (961 ~ 4^5)
    m=6: kz26 (4096 = 4^6) m=7: kz43 (16384 = 4^7)
    m=8: kz70 (66049 ~ 4^8) m=9: kz116 (262144 = 4^9).
Recursion object: the h-free level margin e1(m); ratios r(m) =
e1(m+1)/e1(m); defect series delta(m) = |r(m+1) - r(m)|.
RECURSION-EXISTS bar (frozen): delta non-increasing AND final
delta <= 0.05.  Extrapolation (typed HEURISTIC, not certified):
e1 at m = 12 via r-band, tau_pnt at alpha = 12 ln 2 via last-third
log-slope band -> the margin prediction beyond the ladder with
error control.

Q3 THE EULER FACTORIZATION: per rung, v = bottom eigenvector of
A = S + C; the relational cells are the events (prime powers):
cell_j = -w_j lam_j (v Xn_j v) with w_j = -1 deployed (Xn = the
shipped per-event spline reads = -q3, ward Sum cells = v (A - B) v
to 1e-8 rel).  Census on ALL rungs: sign census, cancellation
index CI = Sum|cell| / |Sum cell|, top-share (concentration),
class aggregates (p = 2 ro/re, p = 1 mod 4, p = 3 mod 4; k = 1 vs
k >= 2), per-prime aggregates C_p (exact mass classification via
own spf sieve).  FACTORIZES bar (frozen): C_p > 0 for ALL primes
on ALL rungs AND median CI <= 1.5 (additive per-prime control --
typed: the native structure is ADDITIVE over primes, a product
form is not claimed).  RECURSES-THE-WALL bar: median CI >= 10.
Also measured: the alignment gap v S v - lambda_min(S) (how much
of the excess the cells do NOT see).

Q4 THE COMPILER CONNECTION (one cheap section, Bonferroni-honest):
predeclared candidates {1/(8 pi), 8 pi, 5, 3/2, ln 2, pi^2/6,
gamma_E, zeta(1/2), |C_TH|, phi0 = 0.0531718, 137.03599, 4.335}
x measured targets {median e1, min e1, mean r, tau_pnt slope, tau
slope, median CI, median top-share, median C2-share}; match iff
|t/c - 1| <= 1e-3 (96 comparisons, expected false hits ~0.2; a
null result is expected and fine).

CONTROLS: skeleton regressions (tau refs, EXC refs, Ah gap);
the SCRAMBLE margin series on all rungs (per-event spline reads at
scrambled positions; must break the envelope and turn the margin
negative); the h=2 EPSTEIN routed margin at the anchors (negative
AND below its own unrouted baseline -- the Euler-product violation
carries the sign; routing from the EXACT Lambda_F recursion,
Fractions).  FIREWALL: no zeros
(zeta(1/2), zeta'(1/2) are zero-free constants, the deployed v583
convention); own sieves; v563 READ-ONLY.

VERDICT (frozen enum, strongest that holds): MARGIN-RECURSION-
EXISTS / MARGIN-FACTORIZES / MARGIN-RECURSES-THE-WALL /
MARGIN-LAW-ONLY.  NO RH claim; writes nothing.

Run:
    experiments/tfpt-discovery/.venv/bin/python \
        experiments/tfpt-discovery/margin_law_probe.py
"""
