"""Discovery probe: THE GARDING ENVELOPE INEQUALITY -- does the total
window symbol obey  s_tot(t) >= c0 log(2 + t) - C0  with a-uniform
constants, or is the W2 norm choice wrong?
...
predecessor machinery REBUILT verbatim (no probe imports); no marker
moves; NO zero of any L-function is read (AST-checked) -- all spectral
structure quoted below is measured from the probe's own symbol.
Python-only, per GATE.WOLFRAM.02.

Provenance: garding_probe.py + garding_edgeband_probe.py (2026-08-02,
the drift diagnosis and the symbol mechanism), w2_mosco_probe (A3
H_log convention, A5(a) typed remainder), v563_paper2_readouts (atom
table mu_n = 2 Lambda/sqrt(n), B_PSI = 1.03883, frame-A windows),
Suzuki arXiv:2606.09096 (Prop. 4.1 full-text read).
"""
import ast
import math
import os
