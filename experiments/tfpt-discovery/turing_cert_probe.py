"""turing_cert_probe -- SLICE B of the RH-criteria scan: the
COMPLETENESS CERTIFICATE of the loaded zero comb via the Riemann-von
Mangoldt counting formula and Turing's integral refinement.
...
no ledger/paper/website edits, NO RH statement (the certificate says
"the loaded list is exactly the first N_ZEROS zeros up to the cited
S-band and Turing bound", nothing more).  Zero data: mpmath zetazero
(dps 15) via the shared cache zero_comb_cache_n2000.json.  Verdict
...
def run():
    ...
    print("FIREWALL: standalone probe; shared zetazero cache (dps %d)"
          "; certificate = completeness of the loaded list, NO RH "
          "statement; verification suite untouched" % DPS_ZEROS)
    print("--- turing_cert_probe: %d passed, %d failed ---"
