#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""conditional_expectation_probe -- PRIME.CORNER.EXPECTATION.01
(EXPLORATION ONLY, experiments/; escalation (3) of the DOORS-CLOSED
verdict: beyond rank-1 corners, 2026-08-07).

THE INPUT (frozen, read-only): the open-doors probe proved the
dichotomy VISIBILITY <=> IDENTITY-DEFECT for rank-1 character corners
e_chi a e_chi of C[G], G = C2 x F2^4 x Z4 (and the level-2 probe
extends it to m <= 3).  THE FROZEN QUESTION HERE: does the dichotomy
FAIL anywhere in the richer compression toolkit -- or does it extend
(the definitive class closure)?  Tested in order of generality:

(i) CONDITIONAL EXPECTATIONS E_H: C[G] -> C[H] onto the group
    algebras of subgroups H <= G (u_g -> u_g if g in H else 0; the
    standard expectation for group inclusions).  Since G is abelian,
    E_H = the average over the annihilator A = Ann(H) < Ghat of the
    character-multiplication *-automorphisms -- warded EXACTLY -- and
    the compressed object decomposes into |H| COMPONENTS indexed by
    the A-cosets of Ghat, each reading the COSET AVERAGE of the
    corner coefficients chat(chi).  The FULL subgroup lattice of G is
    enumerated (BFS closure over the 128-element group, every
    subgroup); per subgroup: does the compressed object retain a
    deployed-form component (identity: the GL1-coset average == -1
    for all events -- since chat in [-1, 1] this forces chat == -1 on
    the whole coset, i.e. A inside the pinned pair {1, psi0},
    psi0 = (0, w0, 0)); the componentwise dichotomy (no component
    with identity AND visibility) is checked for EVERY component of
    EVERY subgroup; placement per component on 4 predeclared
    representative subgroups (G itself = the identity map; ker psi0 =
    the largest proper identity-preserving expectation; C2 x V x {0}
    = the mu4-killing expectation; {e} = the full average, whose
    single component reads EXACTLY ZERO -- the C2 average
    annihilates the carrier, typed).
(ii) PINCHINGS Sigma_chi e_chi a e_chi (the full abelian character
    pinching): completeness Sigma e_chi = 1 EXACT; the pinched matrix
    is (G abelian) precisely the DIRECT SUM of the 128 corners, so it
    carries the identity (GL1 component) and placement (other
    components) ONLY as a direct sum, never in one reading.  The
    decisive joint-use test: the POSITIVITY CENSUS of the pinched
    packet operator at the deployed weights -- the full h x h sector
    window per character class; if the only PSD components are the
    pinned ones (chat == -1, window == the deployed window), then no
    positive object has the deployed pinching and every positivity-
    transporting use of the pinched family reduces to the GL1 corner.
(iii) STINESPRING-SIDE COMPRESSION: the 105-leg dilation of the
    deployed flag channel, W: C^15 -> C^15 (x) C^105, W|x> =
    sum_{y ~ x} xi_xy |y> (x) |leg xy> (uniform xi = 1/sqrt 7 gives
    the deployed channel -- dilation identity warded).  The dilated
    corner reads the POSITIVE leg functional c(n) = -sum_x p_n(x)
    s(x), s(x) = sum_y |xi_xy|^2: the flags are explicit basis legs
    there, so xi can be position-weighted.  Variants (predeclared):
    xi-uniform (regression: the base corner), xi-raw (legs weighted
    by the tradeoff-probe placement field, NOT row-normalized),
    xi-normalized (same field, rows renormalized to isometry).  The
    frozen question: does the dilated corner see placement where the
    base corner cannot -- and if so, at what identity cost.

GATES per structure (the open-doors gates verbatim): identity =
deployed-form defect <= 1e-9 scale (weight-generic, coefficient
level); visibility = block/excess differs from the undressed GL1
block by > 1e-9 scale; placement = the excess moves under the
scramble (seed 1) AND the Epstein comb; CP/positivity wards per
structure.  CONTROLS: the GL1 anchor chat = -1 exact (all union
events); the product law verified against the full carrier Fourier
(all 128 characters x all primary events, exact); tau_X reproduction
on 3 windows; the rank-1 regression (identity cells of the none-row
== {(1,0,0), (1,w0,0)}).

FROZEN VERDICTS: EXPECTATION-CARRIES (a named structure carries
identity + placement in ONE reading -- the route reopens with a
concrete object) / EXPECTATION-CLOSED (the dichotomy extends to the
full compression toolkit) / EXPECTATION-PARTIAL.
...
Run:
    experiments/tfpt-discovery/.venv/bin/python \
        experiments/tfpt-discovery/conditional_expectation_probe.py
"""
...
def main():
    ...
    for kz, rr in wins:
        ...
        q3w, tBw, _ = reads_pair(rr, unit_rows(rr))
    ...
    for p, uu in (("t", uu_t), ("s", uu_s), ("e", uu_e)):
        rows_p = rows_t if p == "t" else unit_rows(rr0, positions=uu)
        q3[p], tB[p], c_ar = reads_pair(rr0, rows_p)
        if p == "t":
            c_ar0 = c_ar
            rows_true = rows_p
