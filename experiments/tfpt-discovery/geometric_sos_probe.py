"""geometric_sos_probe -- OFFENSIVE 3 / PRIME.GEOMSOS.01: the FIRST
FEASIBILITY STRIKE at the geometric positivity source -- the contract
"Q_a^W(v) = sum_j |l_{a,j}(v)|^2 from TFPT geometry, without zeta,
without zeros".  This probe takes stock of the four demanded
...
THE FOUR INGREDIENTS, what this probe establishes:
...
G2 [<,>, the polarization ingredient]: the cover carries J = -h per
  ...
  the odd hat sector (this probe's miniature: h = 23 odd modes -> 276
...
Verdict enums (frozen): GEOMSOS-TRACE-EXACT-TRANSPORT-OPEN (all
...
v625 theta glue, the v643 screw layers and the v624/v613 matrices are
re-implemented / hard-coded inline with provenance); zero data = the
shared zetazero cache (declared, dps 15), used ONLY as verification
target; finite statements only; NO RH claim; no marker moves.

PROVENANCE: v625_prime_shadow (Theta_E8 = E_4, Hecke channels),
...
def run():
    ...
    check("G1.5 [C] K1/K3 AUDIT of the Tr ingredient: construction chain "
          ...
          "probe's certificate is; typed, not hidden" % N_REC, True)
   ...
   ------------------------------------------------------------------""")
    check("G4.3 [C] the contract draft PRIME.GEOMSOS.01 is printed with "
          ...
          "marker moves, no ledger row, no paper edit from this probe "
    ...
    print("FIREWALL: standalone probe (no verification/ import); zero "
