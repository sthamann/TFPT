#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""tiny_checker -- independent auditor of the PRIME.COFINAL.PACKAGE.01
...
with c_r > 0, (d) sign/order comparisons, (e) sha256 integrity.
AXIOM SURFACE (all it trusts): python stdlib integer/Fraction
arithmetic and the package file -- nothing else.  WHY these identities
and signs imply positive definiteness is the package's Part-1 RELATION
CERTIFICATE (sympy-verified upstream); only its statement hashes are
re-checked here.  Fail-fast: exit 1 at the FIRST failing check,
naming it.  Usage: python tiny_checker.py <package.json>
"""
import hashlib
import json
import sys
...
def check_cert(tag, cert):
    ...
    if cert_sha(beta, lce, cxs, gm0, gm1) != cert["hash"]:
...
def check_cell(tag, cell, eta):
    ...
    beta, lce, cxs = check_cert(tag + " cert", cell["certificate"])
...
def main():
    if len(sys.argv) != 2:
        fail("usage: tiny_checker.py <package.json>")
    ...
    for key in sorted(glob):
        check_cert("global-%s" % key, glob[key])
    cells = pkg["part2_positivity"]["cells"]
    ext = pkg["part2_positivity"]["extension_cells"]
    for grp, lst in (("cell", cells), ("ext", ext)):
        for cell in lst:
            check_cell("%s %s" % (grp, cell["id"]), cell, eta)
    ...
    print("CHECKER CENSUS: statements %d/%d, global %d/%d, cells %d/%d,"
          " extension %d/%d, coverage OK -- ALL PASS"
