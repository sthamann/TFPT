...
HONESTY MARKER (updated r305, the wave-10 Lean round): the declared
mathlib-based second stage is now largely DONE.  PROVED in this file:
...
The ONE remaining `sorry` of this file is `crossing_budget` (the Jacobi
inertia theorem T2): mathlib carries neither Jacobi's minor-sign rule
(#negative pivots = #sign changes of the leading-minor sequence) nor
Sylvester's law of inertia in matrix-congruence counting form, and both
are needed; this is a genuine formalization gap, not a to-do of a known
five-line proof.  The statement stays machine-certified exactly
(rationals, five frozen signed measures) in v962 T2 / r279.
...
/-- **T2, the crossing budget theorem** (v962 / r279; Jacobi's
minor-sign rule + the Sylvester congruence `G_S = V W V^T ~ W`).  For a
nondegenerate `S`-atom signed measure whose Sylvester chain `h` has no
vanishing entry below `S`, the number of negative pivots over the full
algebraic continuation equals the number of negative atoms: the Maslov
crossing budget is world-blindly fixed by the source -- the entire
arithmetic of the wall is WHERE this fixed budget is spent
(`free_window_positivity`, RH/Window.lean).

ROADMAP (`sorry` -- the ONE remaining sorry of this file after the
wave-10 / r305 cleanup): certified exactly (rationals, five frozen signed
measures; real-side w9 104 / w13 98 / controls 141/94/6 / kz15 121 /
kz52 551) in v962 T2 / round 279 (oriented_theorem_probe.py, SPEC_SHA
9107709b4f4a65d1); ledger `PRIME.WALL.HALFFILLING_PINNING_THEORY.01`
[E].  WHY it stays open (r305 adjudication, honest): the proof needs
(a) Jacobi's minor-sign rule (#negative pivots of a symmetric matrix
with nonvanishing leading minors = #sign changes of the leading-minor
sequence) and (b) Sylvester's law of inertia in matrix-congruence
COUNTING form (negative-eigenvalue count invariant under `AᵀMA`,
applied to the Vandermonde factorization `H_S = VᵀWV`).  mathlib v4.29.1
carries neither (its inertia material lives at the quadratic-form
signature level without the pivot/minor dictionary); both are genuine
formalization projects, not wiring.  This is a mathlib gap, not a gap in
the mathematics -- the exact-rational certificate stands. -/
