#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""v806 -- HECKE.LOCAL.CLIFFORD.01: the analytic activation-ordering hypothesis dies, the rule map gains two instances -- the honest kill with a positive side-result, one probe (16/18 checks with the two FAILs being the preregistered decider gates themselves; controls 4/4; ~8 s; discovery probe hecke_local_clifford_probe.py, 2026-08-06, verdict LOCAL-CLIFFORD-BREAKS, non-PSD class sectors [3, 7, 5, 1]; FROZEN_SPEC v2 -- the one declared run-1 bar re-freeze on control C1b, 1e-2 mis-frozen -> 1e-3 with an added shape clause, carried verbatim).  THE CONSTRUCTION IS EXACT: the four p mod 8 class combs decompose EXACTLY into the mod-8 character system {chi_0, chi_-4, chi_8, chi_-8} (chi-exact prime-power convention, decomposition Ward 4.4e-16); the completion Wards hold to 1.7e-15; GL1 native == deployed at 1.7e-15.  POSITIVE SIDE-RESULT: the Gamma_R conductor rule of v793 EXTENDS to the full mod-8 dual group -- chi_8 {mu = 0, q = 8, EVEN} PSD +8.70e-5 -> +1.50e-5 and chi_-8 {mu = 1, q = 8, odd} PSD +8.84e-5 -> +1.84e-5 on the whole ladder, nothing tuned: two NEW instances for the conductor-functoriality contract with exact parities.  THE DECIDER BREAKS (the two honest FAILs, S4.3/S4.4): ALL FOUR class sectors fail PSD with their canonical completions -- c = 3, 5, 7 at O(1) with negativity GROWING in X (signed character combinations carry no completed L-function: no positivity target); c = 1 at only O(0.1), pinned to the IMPRIMITIVITY of the trivial-mod-8 component (the (1 - 2^{-s}) Euler zero line Re s = 0 off the critical line).  THE EULER REPAIR: moving the 2-adic atoms back makes class 1 PSD EXACTLY (lambda_min +1.4e-5) -- class 1 (the all-plus signature = trivial coset) is the UNIQUE positivity-completable class, the structural remnant of the activation reading.  ORDERING: margin order c5 < c3 < c7 << c1 on 7/7 rungs, Spearman(depth, margin) = +0.40 -- NO canonical order: the analytic margin ordering does NOT realize the proven 2-adic depths (5, 6, 7, 8); the Clifford activation reading stays [H] DECORATIVE analytically.  The arithmetic multirate theorem is UNTOUCHED (re-anchored at 2e5: v2(D_p) >= d(class) for all 17983 odd primes, minima exactly (5, 6, 7, 8)).  NAMED NEXT FALSIFIER: an analytic shadow of the activation code must live in the PRIMITIVE sector data themselves, not in class indicators.  Controls: wrong-conductor shift == (ln2/4) chi_8(c) e_0 to 4.7e-16; wrong-parity profile smooth 7.8e-3; scramble Ward defect 0.615; poleless chi-weighted Epstein windows all < 0.  No marker move, NO RH/GRH/Chebyshev-bias claim.  Python-only per GATE.WOLFRAM.02.

PROVENANCE: discovery probe hecke_local_clifford_probe.py (2026-08-06, run 2 after the declared C1b bar re-freeze, 16/18 with the two FAILs the decider gates themselves, ~7 s, LOCAL-CLIFFORD-BREAKS); re-run identically at promotion.  Promoted verbatim; the machinery imports (v563/v716/v755/v766 read-only plus the sibling probes f8_sector_continuum_probe, epstein_firewall_probe, hecke_check32_probe, hecke_multirate2adic_probe) resolve against experiments/tfpt-discovery on sys.path -- exactly the probe's own import graph; a module-level _LAST verdict capture inserted (v791 precedent); numbers unchanged; run() encodes the honest-kill pattern EXPLICITLY: 16/18 with fails exactly {S4.3, S4.4} and verdict LOCAL-CLIFFORD-BREAKS (v757 precedent, negative-pattern form).

Original hecke_local_clifford_probe.py docstring (verbatim):
hecke_local_clifford_probe -- HECKE.LOCAL.CLIFFORD.01: the four
...
def run():
    """run_all entry point (v757 precedent, negative-pattern form):
    expected pattern 16/18 with fails EXACTLY the two preregistered
    decider gates S4.3/S4.4 and verdict LOCAL-CLIFFORD-BREAKS."""
    main()
    n_pass = sum(1 for _n, ok in CHECKS if ok)
    fails = sorted(n.split()[0] for n, ok in CHECKS if not ok)
    v = _LAST.get("verdict", "")
    controls_ok = all(CONTROL_FIRED.get(k, False)
                      for k in ("C1a", "C1b", "C2", "C3"))
    ok = (n_pass == 16 and len(CHECKS) == 18
          and fails == ["S4.3", "S4.4"]
          and v.startswith("LOCAL-CLIFFORD-BREAKS") and controls_ok)
    print("\n[%s] PATTERN GATE: expected 16/18 with fails exactly "
          "[S4.3, S4.4] and verdict LOCAL-CLIFFORD-BREAKS; got %d/%d, "
          "fails: %s, controls %s, verdict: %s"
          % ("PASS" if ok else "FAIL", n_pass, len(CHECKS), fails,
             controls_ok, v))
    print("\nCOMBINED ADJUDICATION: %s -- LOCAL-CLIFFORD-BREAKS (the "
          "preregistered decider fails honestly): all four p mod 8 "
          "class sectors are non-PSD with their canonical completions "
          "-- signed character combinations carry no completed "
          "L-function, and the trivial-mod-8 component is imprimitive; "
          "class 1 is the UNIQUE positivity-completable class (Euler "
          "repair exact, +1.4e-5); the analytic margin ordering does "
          "NOT track the proven activation depths (Spearman +0.40) -- "
          "the Clifford activation reading stays decorative "
          "analytically while the arithmetic multirate theorem is "
          "untouched.  POSITIVE SIDE-RESULT: chi_8/chi_-8 are two new "
          "PSD instances of the Gamma_R conductor rule (exact parities "
          "even/odd).  NO RH/GRH claim."
