"""v676 -- PRIME.C1MECH.01: THE C = 1 CONTRACTION MECHANISM -- the
structural derivation of the v618/v619 uniform constant (eps*h <= 1
on the complete-comb surface) from the theorem-level W1 operator
...
FIREWALL (INVERTED, declared -- like v669): this module DELIBERATELY
reads Riemann zeros; the zero side IS the question.  Structural
separation: q_r, q_m, the profiles, the PNT flip prediction and the
scramble moments are assembled from primes + digamma ONLY (v563/v618
machinery verbatim); zeros enter ONLY the zero-side identification
and the density integrals.  Zero data: the shared Turing-certified
...
convention) plus the committed live extension to n = 2500
(c1_zero_ext_n2500.json, guarded against the base cache).  v563
import read-only; no marker moves; no RH statement; Problem 7.1
untouched.  Python-only, counted per GATE.WOLFRAM.02.

PROVENANCE: discovery probe c1_mechanism_probe.py (2026-08-02, 12/12,
verdict C1-QUADRATURE-MECHANISM); v618_uniform_constant (surface +
C = 1), v619 (flips = truncation), v596 (1D lock projection),
v643_w1_theorem (measure-level W1), v591_pole_rank_one (the closed
lock law = pole null direction), margin_link_probe / v656 (T163
pairing q_r = w^T Ahat w), fejer_density_bound_probe / v669
(explicit-formula conventions + zero cache), Iwaniec-Kowalski
...
_REPO_EXT = os.path.join(_DISC, "c1_zero_ext_n%d.json" % N_EXT)
_LOCAL_EXT = os.path.join(HERE, "c1_zero_ext_n%d.json" % N_EXT)
