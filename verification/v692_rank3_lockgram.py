"""v692 -- PRIME.RANK3LOCKGRAM.01: OFFENSIVE 2d -- TYPING T-B THROUGH
THE ZERO SIDE: THE LOCK BLOCK AS AN ALMOST-COLLINEAR GRAM MATRIX.
...
FIREWALL: all parent modules are imported READ-ONLY; zeros are a
DECLARED INPUT (explicit-formula side; the certificate direction uses
them only through citations + the computed list).  Deterministic, no
RNG, no marker moves.

PROVENANCE: discovery probe rank3_lockgram_probe.py (2026-08-03, 7/7
PASS, T-B(block) typed as a quantified partial-RH tail statement:
closes 8/15 windows at the cited 3e12 alone, 13/15 under the named
density route, worst T* = 1.1e16); v677_w3_structure_theorem.py
(master identity, pole closed form, off-line gain);
v563_paper2_readouts.py (window, parity basis, T163 weights);
margin_link_probe.py (lock assembly, verbatim);
rank3_zeroside_probe.py / v684 (zero list machinery);
