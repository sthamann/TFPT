#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""v794 -- PRIME.CP.EXTENSION.01: the infinite-level CP extension gate -- the net formalization of the four-sector window system + the deep margin-trend decision (17/17 checks, controls 3/3, ~45 s; discovery probe cp_extension_gate_probe.py, 2026-08-05, adjudicating run 2, verdict CPGATE-UNDECIDED-FALLING; two declared run-1 -> run-2 repairs carried verbatim: the S2 halved-eta transcription bug caught exactly by its own ward S2.2, and the N3 recalibration -- the frozen e^{X/2} pole-growth prediction for the GL1 norm was WRONG, measured ln-slope 0.079: the comb cancels the pole lag by lag, so the whole family is near-uniformly bounded and the GL1 sub-gate uses the same uniform bar as the pole-free sectors).  THE NET PROPERTIES a CP-extension theorem consumes, MEASURED: N1 RESTRICTION EXACTNESS (the GL1 deep comb reproduces the deployed parent tower on the shared lag prefix at 1.7e-15; the chi4 comb with cap e^13 == cap e^18.375 on the shared prefix at 0.0 -- finite propagation: the sector Weil functional W_chi is well-defined on the union space V_oo WITHOUT any limit); N2 MONOTONE COMPATIBILITY (Cauchy interlacing, every sector); N3 UNIFORM BOUNDEDNESS (sup/first ||T||_2 = 3.24 / 2.68 / 1.79 / 1.76, all <= 5 -- the Helly/Montel data, sector-decorated).  THE EXACT LIMIT OBJECT typed with the v780 relation: per automorphic register character chi, the positivity of W_chi on V_oo -- inf_X lambda_min(T_{chi,X}) >= 0; the chi = GL1 instance IS the promoted Z1-COMPACTNESS object (gate A0: deep GL1 lambda_min(M = 1176) = 3.8825e-6 == the v780/qf-drainage frozen 3.882e-6, rel 1.3e-4, same 1e8 comb) -- the CP gate is Z1-COMPACTNESS SECTOR-DECORATED, one demand family indexed by the automorphic characters; 'state selection must settle' (v780) == 'the sector margins never cross' (here).  CERTIFIED CUSPIDAL DEEP LAYER: two-modulus CRT eta recurrence (p1 = 999983, p2 = 999979, both prime by own trial division) to N = 442500 (X = 13.0), the centered lift CERTIFIED by the Deligne bound n^{3/2} d(n) < p1 p2 / 2 (max 2.1e10 << 5.0e11), CRT values == the sibling exact-int64 eta on ALL odd n <= 22049 EXACTLY; Deligne on every odd prime (max |a_p|/2p^{3/2} = 0.999455 over 37134 primes -- near-extremal).  THE TREND (the decider; deep rungs to X = 18.375 for GL1/chi4, certified X = 13.0 for f8/twist): NO sector crosses zero anywhere in reach; GL1 5.3e-5 -> 3.9e-6 FALLING, decelerating 1.42x, POWER-LAW beta = -1.69 (rms 0.066 vs exp 0.123); chi4 SATURATES at ~1.5e-6 -- the FIRST MEASURED FLOOR (last-3 slope -0.043 < 0.05; flat 1.6e-6 -> 1.5e-6 over X = 16 -> 18.375); f8 FALLING to 1.8e-5, beta = -1.63 (the GL1 rate); twist FALLING, beta = -2.10; all margins 3e6..4e7 above the noise floor.  Depth budget predeclared and typed (the 1e9 comb declined; the v780 fence 'nothing claimed beyond X = 20.3125' inherited).  CONTROLS: C1 golden-rotation scramble -1.64e4; C2 conductor swap -0.693134 == margin - ln 2 (dev 2.2e-16); C3 Epstein -3.38e2.  The persistence demand stays open with measured power-law rates and one measured saturation.  No marker move, NO RH/GRH claim.  Python-only per GATE.WOLFRAM.02.

PROVENANCE: discovery probe cp_extension_gate_probe.py (2026-08-05, adjudicating run 2, 17/17, 42 s, CPGATE-UNDECIDED-FALLING; the two declared run-1 -> run-2 repairs carried verbatim in the original docstring below: (i) the S2 transcription bug -- the halved eta recurrence used d*e_d instead of the divisor sums, caught exactly by ward S2.2 -- and (ii) the N3 recalibration of the wrong frozen e^{X/2} pole-growth prediction; no bar was weakened); re-run identically at promotion (2026-08-06).  Promoted verbatim; the sibling imports (v563/v755 machinery, f8_sector_continuum_probe, conductor_functoriality_probe, epstein_firewall_probe) resolve against experiments/tfpt-discovery on sys.path -- exactly the probe's own import graph; a module-level _LAST verdict capture inserted at the single verdict assignment (v791 precedent); numbers unchanged; run() encodes the pattern (v757 precedent).

Original cp_extension_gate_probe.py docstring (verbatim):
cp_extension_gate_probe -- PRIME.POSITIVE_DESCENT.01, fourth
...
def run():
    """run_all entry point (v757 precedent): expected pattern 17/17 with
    verdict CPGATE-UNDECIDED-FALLING (no floor in three sectors, no
    crossing anywhere; chi4 saturates at the first measured floor)."""
    rc = main()
    n_pass = sum(1 for _n, ok in CHECKS if ok)
    fails = [n for n, ok in CHECKS if not ok]
    v = _LAST.get("verdict", "")
    ok = (rc == 0 and n_pass == len(CHECKS) == 17 and not fails
          and v == "CPGATE-UNDECIDED-FALLING")
    print("\n[%s] PATTERN GATE: expected 17/17 with verdict "
          "CPGATE-UNDECIDED-FALLING; got %d/%d, fails: %s, verdict: %s"
          % ("PASS" if ok else "FAIL", n_pass, len(CHECKS),
             fails or "none", v))
    print("\nCOMBINED ADJUDICATION: %s -- CPGATE-UNDECIDED-FALLING: the "
          "net data a CP-extension theorem consumes are measured "
          "(restriction exactness, Cauchy interlacing, uniform bounds -- "
          "the whole four-sector family near-uniformly bounded after the "
          "declared N3 recalibration), the exact limit object is the "
          "positivity of W_chi on V_oo = the v780 Z1-COMPACTNESS demand "
          "sector-decorated (anchor 3.882e-6 reproduced), and the deep "
          "margin trend decides: no sector crosses zero at reachable "
          "depth, GL1/f8/twist fall power-law (beta -1.69/-1.63/-2.10), "
          "chi4 SATURATES at ~1.5e-6 -- the first measured sector floor.  "
          "The persistence demand stays the honest open core.  "
          "NO RH/GRH claim."
