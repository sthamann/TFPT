#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""v817 -- TFPT.POSITIVE_DESCENT.MASTER.01: the abstract positive-descent master theorem, its kernel-checked Lean core, and BOTH instance checks -- G_net re-verified and the NEW prime packet-state summability number, one probe (14/14 checks incl. the 4 must-fire controls, ~1 s; discovery probe positive_descent_master_probe.py, 2026-08-06, verdict MASTER-BOTH-INSTANCES; construction SHA-frozen).  THE THEOREM (7 hypotheses frozen): (1) finite operator systems with unital inclusions, (2) positive states, (3) CP transitions, (4) finite symmetry group with character sectors, (5) SUMMABLE state defects, (6) a sector-adapted closure datum, (7) a Mosco form family => a unique positive limit state exists and every compatible sector projector has a well-defined positive limit sector; the load-bearing core -- (a) summable-defect telescoping => unique norm-limit with the quantitative Cauchy tail, (b) positivity closed under limits, (c) sector decomposition commutes with the limit -- is KERNEL-CHECKED in TfptCarrier/PositiveDescentMaster.lean (finite sector-weight form; GramCompactness.lean carries the matrix positivity closure, cited not re-proved); hypotheses (6)/(7) are per-instance closure data, typed, never gated.  INSTANCE G_NET (re-measured self-contained): exact Haar filtration (V*V = 1, support-coherence leak 0), the index-4 mu4 expectation is CP (Choi min eig >= -1e-10, unital, trace-preserving), defects summable at 0.267/doubling (envelope 8 delta_1 2^-k), states positive with sector traces (6,4,2,4) => w_1 = w_3 = 1/4 evenness exact.  INSTANCE PRIME/PACKET -- THE NEW NUMBER: the packet GNS state defects on the fixed 128-dim commutative register C[G] (state norm = the exact l1 norm over the 128 characters) are SUMMABLE: delta_k = 1.434e-2 -> 2.485e-3 over N_k = 256 * 2^k (ratios 0.66-0.74, fitted rate 0.701/doubling ~ the sqrt-dilution law 2^{-1/2} = 0.707, envelope 2 delta_0 (3/4)^k, Cauchy tail on every rung) -- the STATE norm behaves strictly better than the corner increments; the packet layer exact (theta heads (52,64,60,64), glue identity Sum Th_j = 240 sigma3 for ALL n <= 16500, Th0 - Th2 = -8 f8 on ALL odd n int64-exact; mh2(p) = -1/15 EXACTLY for every odd prime -- an exact Eisenstein ratio, zero variance); ladder transitions convex/CP with unit wards.  THE PAYOFF: MASTER.01 applies to BOTH instances at finite level -- the unique limit state exists on each side by telescoping alone; the SELECTION problem is eliminated on both sides; what remains per instance is NOT existence but G_net normality/split and the prime SECTOR-FLOOR question (the GL1 sign sector) -- cross-ref PRIME.FLOOR.RATIO.01.  CONTROLS (all fire): scrambled sea O(1) defects; label scramble and Epstein-type drift break summability; K4 -- an update family with SUMMABLE defects but one non-positivity-preserving step has a NON-positive limit: positivity is a SEPARATE hypothesis, not implied by (5).  No marker move, NO RH claim.  Python-only per GATE.WOLFRAM.02.

PROVENANCE: discovery probe positive_descent_master_probe.py (2026-08-06, 14/14, ~0.5 s, MASTER-BOTH-INSTANCES); re-run identically at promotion.  Promoted verbatim (self-contained, no imports beyond numpy); a module-level _LAST verdict capture inserted (v791 precedent); numbers unchanged; run() encodes the pattern (v757 precedent).  Lean core: experiments/lean4-carrier-rigidity/TfptCarrier/PositiveDescentMaster.lean (kernel-checked, no sorry, carried in the shipped Lean manifest).

Original positive_descent_master_probe.py docstring (verbatim):
TFPT.POSITIVE_DESCENT.MASTER.01 -- the abstract positive-descent
...
def run():
    """run_all entry point (v757 precedent): expected pattern 14/14 with
    verdict MASTER-BOTH-INSTANCES."""
    rc = main()
    n_pass = sum(1 for _n, ok in CHECKS if ok)
    fails = [n.split()[0] for n, ok in CHECKS if not ok]
    ok = (rc == 0 and n_pass == len(CHECKS) == 14 and not fails
          and _LAST.get("verdict") == "MASTER-BOTH-INSTANCES")
    print("\n[%s] PATTERN GATE: expected 14/14 with verdict "
          "MASTER-BOTH-INSTANCES; got %d/%d, fails: %s, verdict: %s"
