#!/usr/bin/env python3
"""v715 -- QGEO.CARLIMIT.01: QGEO-CAR -- the quasi-free CAR route to the formal
...
FIREWALL: self-contained (no verification/ import needed);
GATE.QGEO does not move; no marker changes; the lemma is NAMED, not
claimed; deterministic (no RNG).

PROVENANCE: discovery probe qgeo_car_continuum_probe.py (2026-08-03,
22/22 PASS, verdict QGEO-CAR-RATES-SUMMABLE: all 6+1 deck/twist
channels are Schatten/shadow-norm Cauchy (N up to 3072 / 6144), the
FH renormalization is fit-free lattice-derived (Barnes G, 8.8e-7),
the RP cone is stable along the ladder, and the majorant lemma
(L1)-(L3) is stated with constants and the classical proof route --
GATE.QGEO does NOT move).  Promoted with a run_all entry point (the
checks execute at import, module level); numbers unchanged.
"""
