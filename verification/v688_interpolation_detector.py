"""v688 -- PRIME.INTERP.01: THE ALIAS-INTERPOLATION FALSIFIER -- turning the
...
xi_min = 2 alpha s_min ~ 1.4-2.6).  This module measures, derives and
...
the D5 lemma inventory.  The v677 caveat this module attacks: (a)
...
this module is INTERP-FALSIFIER-CONSTRUCTIVE -- the in-kernel matched
filter detects every off-line quadruple from 2 alpha delta >= 1.974,
beating the v677 mode map x2; masking fully adjudicated 46 broken + 2
provably sub-resolution):
...
FIREWALL (INVERTED, declared -- v677/v678 convention).  This module
...
certified provenance).  No marker moves; NO RH statement -- an
off-line zero is the HYPOTHESIS of every statement here; Python-only
per GATE.WOLFRAM.02.

PROVENANCE: discovery probe interpolation_detector_probe.py
(2026-08-03, 15/15 PASS, verdict INTERP-FALSIFIER-CONSTRUCTIVE);
w3_structure_theorem_probe.py (v677: master identity,
...
Platt-Trudgian Bull. LMS 53 (2021).
"""
import cmath
import json
import math
