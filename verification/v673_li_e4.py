"""v673 -- PRIME.LIE4.01: SLICE B of the Li-strand -- Li additivity
for L(E_4, s) = zeta(s) zeta(s-3) as the EXACT consistency test
between the E8 strand (v625: Theta_E8 = E_4, shell counts
240 sigma_3(n)) and the comb strand (the shared 2000-zero cache).
...
FIREWALL: standalone (NO other verification/ import; the v625
theta-glue construction is re-implemented inline and checked against
240 sigma_3); zero data = the shared committed zetazero cache
(experiments/tfpt-discovery/zero_comb_cache_n2000.json, dps 15;
completeness certified by v666_turing_cert.py; diagnostic zero-side
line, v589 convention); finite statements only, NO RH claim;
Python-only (GATE.WOLFRAM.02).

PROVENANCE: discovery probe li_e4_probe.py (2026-08-02, 12/12,
verdict LI-E4-ADDITIVE-CONSISTENT); v625 (Theta_E8 = E_4, the 240
sigma_3 shell counts), keiper_li_probe / v665 (binomial assembly +
tail budget B_fl), li_corollary_probe / v672 (SLICE A), Lagarias
(Li coefficients for automorphic L-functions, additivity).
"""
import json
import math
import os
...
def run():
    ...
    print("FIREWALL: standalone (v625 logic re-implemented, no other "
          "verification/ import); shared committed zero cache (dps "
          "%d, diagnostic zero-side line); finite statements only; "
          "NO RH claim" % DPS_ZEROS)
    print("--- v673_li_e4: %d passed, %d failed ---"
