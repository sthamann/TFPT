...
specificity question is stage 2.  \emph{Stage 2 (the next gate,
running separately):} (i) glue the arch --- the deck-sector trace
(\veri{v705\_chain\_deck\_sector.py}) and the tower must come from
one groupoid, not a direct sum; (ii) specificity --- show the tower
singles out E8 among lattices; (iii) derive the declared ingredients
internally.  \emph{Kill criteria (from the strategy council):} (K1)
the gluing needs zeros or an RH-equivalent input $=$ renaming; (K2)
the $\sqrt{n}$ half-density cannot be derived groupoid-internally
$=$ the construction is a repackaging of the explicit formula; (K3)
the specificity census shows other lattices work equally well $=$
the E8 anchor is decorative; (K4) the glued object fails positivity
on a deployed window $=$ dead.  A research contract, \emph{not} a
claim; no marker move; no RH statement.
