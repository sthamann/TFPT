...
constraints).  No marker move; no RH statement.]}\par}
...
handover content.  No RH statement.]}\par}
...
(\texttt{FTRANSFER.CLOCKS.01}) --- preregistered, executor pending.}
...
the surprise.  \emph{Executor: pending} --- a future
\texttt{v752\_ftransfer\_clock\_jets.py}-pattern module (sympy-exact
kernel; the FTC.01 data interfaces need the frozen $g_{*s}$
tabulation and the PDG $\alpha_s(Q)$ overlay data curated first).
Nothing here upgrades \texttt{CONTRACT.F.01}: the four observables
stay \mC/\mO; no marker move; no RH statement (unrelated surface;
discipline note).
\begin{figure}[t]
\centering
