...
physics, not missing dynamics (\texttt{RESIDUAL.CERTIFICATION.01}). The two kind-(A) external-math
targets, \texttt{SEAM.EQUIV.01} and \texttt{ALPHA.QUILLEN.EXACT.01}, are additionally packaged as
self-contained \emph{externalization contracts} (statement, hypothesis list, runnable finite models,
Lean axiom surface, must-fail controls, acceptance tests) in their own sections at the end of this
document --- hand-off documents for outside specialists; both stay \mO, no marker moves.  A third
hand-off document in the same format, \texttt{PRIME.RESIDUE.EXTERNAL.01}, packages the
prime-front's terminal \emph{open statements} for external specialists in analytic number theory
--- it offers open problems only, asserts no progress toward the Riemann Hypothesis in either
...
the residual \mO\ is the global BCOV integral beyond the fibre zero-mode factor (the declared
measure is since shown \emph{unique on the computable v508/v515 sector} up to exactly one
named flat direction --- sector rigidity, not a BCOV derivation,
\veri{v973\_seam\_route\_narrowing.py}).
...
cone formalization is dead, the protection is nonperturbative); the law is since refined to the
decidable Jaffe--Pedrocchi \emph{odd-split} form, and the quartet-avoiding cut family of the
surviving member carries a positive OS/GNS structure --- an OS-viable cut geometry exists, the
kill-test-2 counterweight narrowed, not removed
(\veri{v972\_seam\_interaction\_front.py}, toy-level). Both contracts stay \mO; no
marker moves.
\end{keybox}
...
axioms, no \texttt{sorry} (\texttt{FORM.QGEO.04}). \mO\ the one premise stays the honest axiom
(\texttt{QGEO.OBLIG.01}).
...
documented in this section --- \emph{closed modulo cited theorems}; its \mO\ residual, formerly the
abstract continuum existence \vref{v336}, is since \emph{carved into a named list} against the
cited Osborne--Stottmeister coverage (\veri{v973\_seam\_route\_narrowing.py}): \emph{covered} is
the $\mathfrak{so}(16)_1$ fermion-bilinear current algebra --- strong convergence, Virasoro and
correlator control, polynomial errors; \emph{not covered}, and now the named residual, are
N1 the $\mu_4$/GSO extension $SO(16)_1\to(E_8)_1$ as a net statement, N2 the twist/spin-field
sector, N3 the one-sided chiral edge net, and N4 the OS/holomorphy selector --- with a measured
collar convergence rate $p=3.48$ inside the preregistered band while the trivial $M{=}3$ control
does not converge), and \texttt{SEAM.EQUIV.TWISTOR.01}, the open
Costello--Li construction (prepared by \texttt{CELEST.SEAM.01}, bridged to physics only by
\texttt{WOIT.OS.TWISTOR.01}; \mO\ --- its declared measure is since shown \emph{unique on the
computable v508/v515 sector} up to exactly one named flat direction, the uniform sector-weight
shift onto the untwisted Okubo square (constraint ranks $9/8/2/2$, joint rank $13$ on $14$
moduli): sector rigidity, \emph{not} a BCOV derivation,
\veri{v973\_seam\_route\_narrowing.py}). The parent claim \texttt{SEAM.EQUIV.01} remains, with exactly this
...
\par\smallskip\noindent\textbf{The two routes share an executable OS skeleton (2026-08-27,
\texttt{SEAM.EQUIV.SKELETON.01} \mE\ new; \veri{v973\_seam\_route\_narrowing.py}; no marker
moves --- \texttt{SEAM.EQUIV.01}, \texttt{SEAM.EQUIV.MMST.01} and
\texttt{SEAM.EQUIV.TWISTOR.01} all stay \mO).} The two routes are bridged by an OS skeleton
that \emph{runs}, $5/5$: all WOIT kill-test main invariants hold verbatim on the $p{+}ip$
collar edge, every divergence is control-typed to the checkerboard mechanism --- confirming
the v519-R4 finding from the second route --- and the collar top edge realizes the
anti-chiral state exactly. Toy/census level; nothing here is a continuum statement.
...
\emph{[2026-08-07 round-29 margin-law note:} the envelope
constant is now \emph{doubly derived}
(\veri{v843\_margin\_law\_excess\_lean.py},
\textsc{margin-recurses-the-wall}): the identified-corner
(relational) coordinates reproduce this contract's floor-strand
law exactly --- $e_1 = (\tau/\tau_{\rm pnt})\,h^{3/2} \in
[4.855, 24.209]$ on all $67$ rungs, $\min \ge 4.335 =
c_{\rm cert}$, with $\tau$ agreeing with $\lambda_{\min}(A_h)$
at $2.1\times10^{-12}$ --- two independently derived laws, one
object: the consistency itself is a nontrivial ward.  The typed
law $\tau(\alpha, h) = e_1(\alpha)\, h^{-3/2}\,
\tau_{\rm pnt}(\alpha)$ puts the entire $\alpha$-decay in
$\tau_{\rm pnt}$ and the arithmetic in $e_1$; the fit-free
internal $h$-exponent measures median $-1.341$ against the
frozen bar edge $-1.35$ --- the $h^{-3/2}$ law confirmed at
$\sim1\%$ grade, the $0.009$ miss kept as the one
preregistered-honest FAIL (not refit).  The margin-law attack
class (tower recursion / per-prime factorization of the excess)
is measured dead and stop-listed per the cell-level
self-similarity typed in \texttt{PRIME.RELATION.SKELETON.01}.
Kill criteria unchanged; still [O].]
...
theorem; no marker move; no RH statement.
\emph{[2026-08-07, round 27:} both corner-era routes at this wall
are now decided (see the two entries below).  The corner route's
\emph{structural} half is closed weight-generically --- the
character-corner identity is polynomial algebra in free weights
and the Hjelmslev CP tower carries it verbatim through five
levels at certified defect zero --- but identity and state corner
are comb-blind and the level-$m$ motion is the pure
register-dilution law $16^{1-m}$: the wall \emph{relocates into
the identification step} and stays this contract.  The commutant
SOS route is closed definitively by exact certificates (unique
trivial feasible point; forced $G = \mathrm{diag}(1,-1,-1)$ with
dual point $q(0,1,0) = -1$).  Both routes stop-listed;
register-side Kraus dressings provably cannot carry the
identification (\textsc{position-kraus-tradeoff}, probe-level) ---
the one live tower door is position-dependent Kraus data on the
level-$m$ flags.  Demands, kill and close conditions
unchanged.]
\emph{[2026-08-07, round 28:} the wall acquires its full
coordinate system.  (i) \emph{The four closure coordinates are
complete} (\veri{v837\_corner\_closure\_quantifier.py},
\veri{v838\_corner\_expectation\_position.py}): register closed
(the $90$-cell class map has no
identity$\wedge$visibility$\wedge$placement cell), tower
level-exact ($m \le 3$; jet characters position-blind),
compressions closed (all $5276$ subgroup expectations, the full
pinching, Stinespring-side compressions --- identity and new
placement mutually exclusive cell by cell), and carrier-side
position dependence \emph{extremally pinned}
($\hat c_{\rm GL1} = -1$ locks the entire $C_2$ mass;
identity-true carriers read zero on every self-consistent comb)
--- stop-list entries \texttt{PRIME.CORNER.OPENDOORS.01} /
\texttt{PRIME.CORNER.LEVEL2.01} /
\texttt{PRIME.CORNER.EXPECTATION.01} /
\texttt{PRIME.CARRIER.POSITION.01}.  (ii) \emph{The zero side is
quantified} (\veri{v839\_paircorr\_bridge\_saturation.py},
\veri{v840\_gue\_ablation\_loopgain.py}): the demand in Montgomery
$F(\alpha)$-form with the named minimal missing input
(unconditional $F(\alpha, T) \le C_F$ out to $\alpha \approx 3$);
the demand/GUE ratio saturates structurally at $R_\infty = 1.11$
(tight band unfolded $\alpha \in 1..2$, pinned across the
admissible family); the bootstrap loop is short ($g = 1/(k^2 R^2
c_{\rm sup})$; even ideal supply gives $0.655 < 1$; Poisson ward
passed) --- the wall's self-conservation \emph{is} the
saturation, and the certified ladder doubles as a calibrated
form-factor instrument ($\hat F(\mathrm{win}) = 0.44 \pm 0.14$).
(iii) \emph{The relation route opens} (the two registrations
below): the identification carrier exists with the
relational/multiplicative input, the identified corner's excess
is positive on all $67$ rungs, and the certified skeleton
encloses $\tau_X$ strictly positively --- the wall in its
sharpest form, $\tau_X = \lambda_{\min}(\text{structural}) +
\text{EXCESS}$, with the SHARPENED DEMAND: a uniform lower bound
on the excess margin (a finer-than-statistical datum that no
finite table settles).  Forward/backward demands, kill and close
conditions of this contract unchanged.]
\emph{[2026-08-07 round-31 transport note
(\veri{v855\_invariance\_atlas.py}):} the pair-type object this
registration names --- the alias second moment / the form-factor
band $\alpha \in 1..2$ --- is \emph{exactly} the bilinear chain
fluctuation $d\mathrm{Chain}_2$ in Selberg-hierarchy coordinates
(the demand concentrates there; the level-2 head is
elementary-controlled by Selberg symmetry with on-range-verified
constants) and the comb block's own soft direction at
compression level (the minimizer tracking law, $0.0056$ rad);
the sieve input class reaches the band in \emph{scope}
(window-free, deterministic) but misses the \emph{grade} by
$205\times$--$1.1\times10^4\times$ --- input-class-invariant,
the same wall; still [O].]
\emph{[2026-08-08 round-32 note
(\veri{v856\_connected\_current\_lean.py},
\textsc{connected-covariance-partial}):} the pair-type object
now has an explicit \emph{zero-free construction} --- the
centered KMS correlator of the M\"obius commutator current is
the pair-correlation form (its stationary part carries the
deployed pair weights $\lambda_m^2 = \Lambda(m)^2/m$; the
centered main term \emph{is} the deployed smooth half-line
subtraction, corr $0.99$; the state is anchored to the critical
line at $1/2$, the wrong half-weight losing the pair-weight
carrier), it \emph{evades} the nuclear tax (relapse ratio
$\approx 0.3$ --- genuinely different bookkeeping from the
event-local completions), and the gap to the deployed form is
the \emph{grade gap} itself: the window form is linear in the
comb and routing it through the correlator costs the variance,
$2.5$--$4.1\times10^5\,\tau$ --- the alias second moment is
reachable by construction, the floor demand is not payable from
it.  The instrument (zero-free, state-anchored, exact main
subtraction) is typed for measurement use; still [O].]
\emph{[2026-08-08 round-33 note
(\veri{v861\_krein\_normalform\_lean.py} --
\veri{v866\_source\_contractor\_formula.py}):} the floor demand
of this contract now reads, in Krein coordinates, $\|C\| \le 1$
for the \emph{explicit} closed-form source contractor $C =
W_-FG_+^{-1}F^{\mathsf H}W_+$ (Loewner form $G_- \preceq G_+$;
the $\tau$-margin is the \emph{single} soft defect mode's
contraction margin --- \textsc{defect-one}), the scalar shadow
is the pole-port impedance with the closed-form Poisson/Cauchy
$a$-term at $s = 1/2$ and $\kappa - 1 \approx 6e^{-\alpha/2}
\to 1$, and the absolute-bound route is \emph{provably} closed
(Perron $\rho(|C|) > 1$ everywhere): the bridge input this
contract demands must control the \emph{phases} of the pair
kernel, not its magnitudes --- the pair-correlation substance
in its sharpest operator clothing
(\texttt{PRIME.KREIN.CONTRACTOR.01}); still [O].]
\emph{[2026-08-08 round-34 note
(\veri{v869\_cdcore\_clarkphase.py} --
\veri{v872\_damping\_compensation.py}):} the wall now reads as
\emph{one} two-term inequality at \emph{one} named direction,
both terms source-built --- the exact split $I - C^*C = T_1 +
T_2$ (the indefinite cross-measure Jacobi geometry plus the PSD
Christoffel damping, warded at $10^{-12}$ on all $42$ rungs)
makes the floor demand $v^{\mathsf H}T_2v > |v^{\mathsf H}T_1v|$
at the soft direction (kz 9: $-0.0328 + 0.0330 = 1.68\times
10^{-4} = \tau$; $s = \kappa\tau$ with the certified bulk floor
at $98$--$100\%$ of $\lambda_2$), and the kernel behind both
terms is \emph{named}: the Christoffel--Darboux kernel of the
explicit source measure --- one Jacobi chain plus arm weights,
the controls' chains $O(1)$ different.  The pair-correlation
substance this contract types is now the deviation of the source
Jacobi chain and Christoffel weights from their free
counterparts, read at a single named direction; the classical
escapes are stop-listed (pole transformations at $x_{\rm pole}$,
finite-rank cross-defect corrections, $\mu_4$-offset Clark
kernels); the preregistered next test is
\texttt{PRIME.PRUEFER.COMPENSATION.01} (see the contractor
block); still [O].]
\emph{[2026-08-08 round-35 closing note
(\veri{v873\_pruefer\_cotlar\_decision.py}, the executed
preregistration):} the Cotlar decision is in --- the round-34
frozen contract was executed under the full provenance chain
(committed pre-execution in \texttt{526ca3eb}; two documented
mechanical fixes with the spec literal SHA byte-identical
throughout; the honest FAILs pattern-gated, nothing refit) and
returns \textsc{cotlar-growing} by its own rules: the blockwise
Cotlar--Stein / phase-cell class on source-only Pr\"ufer phases
is \emph{closed} and stop-listed --- the battery is flat ($S_U$
$5.55$--$6.30$) but the frozen deep holdouts explode (kz 142:
$S_U\,32.07$/$S_C\,58.78$; slopes $0.209/0.408 \ge 0.20$, both
channels \textsc{growing}; kills 2 $+$ 5 fire).  The demand
narrows: what survives is a \emph{non-stationary} phase bound on
the explicit kernel --- the predeclared $\pi$-resonance cells
$d = 7$--$8$ are the known critical structure (the envelope is
bimodal exactly there), and the typed growth law constrains
every candidate: the explosion tracks kz-\emph{depth} ($\alpha$),
not window size $h$ (kz $90/116$ at $h = 1430/1433$ stay in
band; kz 177 at smaller $h = 1219$ explodes) --- a uniform
certificate must be $\alpha$-aware.  The carry-forward assets:
the danger geometry is real (source-only cells capture
$0.98$--$0.61$ of $\lambda_{\min}(T_1)$ without the soft
eigenvector) and the discrimination stands (Epstein $3927\%$,
scramble $314462\%$); still [O].]
\emph{[2026-08-08 round-36 correction note
(\veri{v877\_complete\_comb\_reversal.py}):} the round-35 note's
holdout data was comb-truncated ($\texttt{ATOM\_MAX}$ artifact;
complete-comb floors strictly positive) --- on complete combs
the identically frozen rule returns \textsc{bounded}, the class
closure and stop-list entry are amended, the $\alpha$-not-$h$
law is retired, and the route reopens
(\texttt{PRIME.PRUEFER.COMPENSATION.02}); the floor demand in
embedding coordinates is now the registered target
\texttt{PRIME.CARLESON.PRIME.01} [O] (the Carleson embedding of
$\tilde\nu_-$ into $\tilde\mu_+$ at degree $h{-}1$ with constant
$1 - \tau_h$, machine-exact reduction
\veri{v876\_carleson\_polygon\_wave.py}); still [O].]\par}

\par\smallskip\noindent\textbf{The character corner and the
Hjelmslev tower (\texttt{PRIME.CORNER.CHARACTER.01} /
\texttt{PRIME.HJELMSLEV.CPTOWER.01}).}
{\sloppy
\emph{Registered} 2026-08-07
(\veri{v835\_corner\_hjelmslev\_tower.py}, $21/21 + 20/20$,
\textsc{corner-identity-symbolic} $+$
\textsc{hjelmslev-structure-only}).  \emph{Certified/exact:} the
corner identity $T_{\mathrm{GL1},X} = V^* e_{\mathrm{GL1}}
\pi(P_X) e_{\mathrm{GL1}} V$ as polynomial algebra in free event
weights (symbolic toy leg exact; all three deployed windows on
two independent code paths); $\hat c_j = -1$ exactly for all
$136$ events, consuming only the glue identity
$\sum_m \Theta_m = 240\,\sigma_3$ and the uniform $H^*$ mass;
$e_{\mathrm{GL1}}$ a rank-one projector, corner compression
single-Kraus CP; the chain-ring ladder $\Z[i]/(1+i)^m$, $m=1..5$,
with exact counts $15\cdot8^{m-1}/35\cdot16^{m-1}/105\cdot
32^{m-1}$, strictly projective CP channels (certified cb defect
identically zero) and level-exact corner coefficients.
\emph{Typed (the decision):} identity and state corner are
comb-blind (scramble control); the level-$m$ corner motion is the
pure register-dilution law $16^{1-m}$ --- no register/geometry
lift carries the prime placements; the arithmetic wall relocates
into the identification step and stays
\texttt{PRIME.FLOOR.PAIRCORR.01}.  Register-only tower lifts are
stop-listed; the live door is position-dependent Kraus data on
the level-$m$ flags (register-side dressings provably cannot
carry it: \textsc{position-kraus-tradeoff}, $13/13$,
probe-level).  \emph{Kill:} any level breaking the exact
counts/defect-zero wards, or a register-only lift moving the
comb-specific gap.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The commutant SOS closure
(\texttt{PRIME.COMMUTANT.SOS.01}).}
{\sloppy
\emph{Registered} 2026-08-07, closed at registration
(\veri{v836\_commutant\_sos\_closure.py}, $29/29$,
\textsc{commutant-sos-infeasible}; the declared SPEC v1$\to$v2
witness-triple repair documented, intent/bars/verdict rule
unchanged).  \emph{Certified/exact:} the five commutant
coordinates $\Pi_A(l)$, $l \in \{0,1,4,7,9\}$ (exact Lagrange
projections, traces $\{5,2,5,2,1\}$, structure constants
$\Pi_a\Pi_b = \delta_{ab}\Pi_a$ on all $25$ pairs,
$\dim C(\{K,\sigma\}) = 39$ exact); the reduction lemma (abelian
conjugation reads only Gram diagonals), collapsing the SDP to a
rational LP with the \emph{trivial diagonal rewriting} as unique
feasible point (Vandermonde kernel $\{0\}$) --- no cross-sector
positive transfer; the flag-space Gram lift forced to
$G = \mathrm{diag}(1,-1,-1)$ (not PSD) with the rational dual
certificate $q(0,1,0) = -1 < 0$ and achievable $q$-negative
directions (including $832/2799$ nonneg-cone truncation cuts).
\emph{Typed:} \textsc{fence-clean} on the algebra layer,
\textsc{fence-hit} on the positivity branch --- the nontrivial
version of the route is pair-correlation substance
(\texttt{PRIME.FLOOR.PAIRCORR.01}).  Route closed; stop-list
entry.  No marker move; no RH statement.\par}
...
{\sloppy
\emph{Registered} 2026-08-07, closed at registration (stop-list
entries; \veri{v837\_corner\_closure\_quantifier.py},
$19/19 + 17/17 + 3$, \textsc{doors-closed} $+$
\textsc{level2-closed};
\veri{v838\_corner\_expectation\_position.py}, $13/13 + 13/13$,
\textsc{expectation-closed} $+$
\textsc{position-carrier-tradeoff}).  \emph{Certified/exact:} the
$18$-class collapse of all $128$ characters (product law verified
on the full carrier Fourier, exact Fractions); the generalized
mass ward (the character corner reads one position-blind carrier
mode per event); the level-$m$ duals and polar annihilators
($m \le 3$; identity characters exactly
$\mathrm{Ann}(\langle S_m\rangle)$, Vfac spectra
$\{1, 0, -1/7\}$); the full subgroup-lattice census ($5276$
subgroups, $74\,259$ components; the pinning criterion
$\mathrm{Ann}(H) \subseteq \{1, \psi_0\}$ exact); the pinching
completeness; the exact tower address (period $4$ on rational
integers).  \emph{Typed (the closure):} identity $\Rightarrow$
not visible in every cell of every class map; the only
position-reading slot is exactly the slot the identity freezes;
the deployed-form identity is an \emph{extremal} state condition
(\textsc{extremal pinning}) --- no state-preserving, identity-true
carrier map $F(n, u)$ separates self-consistent combs.  The
structural half kernel-checked
(\texttt{TfptCarrier/SectorPositiveDescent.lean};
\texttt{IdentificationPositivity} the single named hypothesis).
\emph{Kill (of the closure):} a cell of any measured class map
carrying identity, visibility and placement together, or an
identity-true state carrier separating the self-consistent null.
The wall stays \texttt{PRIME.FLOOR.PAIRCORR.01}.  No marker move;
no RH statement.\par}

\par\smallskip\noindent\textbf{The bridge map, the saturation and
the loop (\texttt{PRIME.FLOOR.BRIDGEMAP.01} /
\texttt{PRIME.FLOOR.GUESAT.01} /
\texttt{PRIME.FLOOR.GUEABLATION.01} /
\texttt{PRIME.FLOOR.LOOPGAIN.01}).}
{\sloppy
\emph{Registered} 2026-08-07, evidence lines feeding
\texttt{PRIME.FLOOR.PAIRCORR.01}
(\veri{v839\_paircorr\_bridge\_saturation.py}, $9/9 + 7/7$,
\textsc{bridge-fully-beyond} $+$ \textsc{gue-saturating};
\veri{v840\_gue\_ablation\_loopgain.py}, $10/10 + 8/8$,
\textsc{saturation-structural} $+$ \textsc{loop-short};
scholarship correction throughout: Montgomery's $F(\alpha)$ on
$|\alpha| < 1$ is RH-conditional, every supply row tagged).
\emph{Measured:} the demand ledger in Montgomery
$F(\alpha)$-form on all $73$ frames; the two-tier gap
(unconditional supply misses by $52\times$; conditional window
covers $43$--$86\%$; full-GUE rms closes $39/73$); the named
minimal missing input ($F(\alpha, T) \le 0.33$--$1.7$ out to
$\alpha \approx 3$); the saturation plateau $R_\infty = 1.11$
inside the frozen band, structural across the Guinand-admissible
family and pinned in the \emph{unfolded} coordinate; the
calibrated form-factor instrument
$\hat F(\mathrm{win}) = 0.44 \pm 0.14$ at $T \le 2\times10^4$;
the loop-shortfall law $g = 1/(k^2 R^2 c_{\rm sup})$ with
headline $g(k{=}1) = 0.529$ and the ideal-supply bound
$1/R^2 = 0.655 < 1$; the Poisson circularity ward.
\emph{Typed:} necessary-side consistency and instrument
calibration only --- the saturation is the strongest form of
comb-zero bridge consistency a necessary-side battery can
deliver, the loop is \emph{not} a proof architecture, and the
measured positivity everywhere is finer-than-statistical
information.  \emph{Kill:} a Guinand-admissible variant landing
off-plateau with positivity intact (the structural reading
fails), or the instrument's Poisson control drifting from $1$.
No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The relation carrier
(\texttt{PRIME.RELATION.MULT.01} [O], carrying
\texttt{PRIME.RELATION.LADDER.01}).}
{\sloppy
\emph{Registered} 2026-08-07, OPEN
(\veri{v841\_relation\_carrier\_ladder.py}, $18/18 + 13/13$,
\textsc{relation-carrier-exists} $+$ \textsc{excess-nonnegative}).
\emph{The finding:} the identification carrier \emph{exists} with
the relational input --- the first structure in the program to
pass all four gates (state, identity at truth by degeneration,
placement, and the self-consistency null that killed every
compression and every single-event carrier).
\emph{Certified/exact:} $\mu * 1 = \delta$ and
$\Lambda = \mu * \log$ as exact structure in the truncated
convolution frame (the relational input \emph{is} the
Euler-product datum); the four-comb discriminator (true comb $0$
exactly; $\zeta_{\Q(i)}$ and $\chi_4$ blind ---
Selberg-class-correct; the $h = 2$ Epstein comb $x^2 + 5y^2$
separates at $200.0$ on the class-group obstruction sites); the
amplitude wiring ($L$-safety automatic); support and identity
wards on all $67$ reachable rungs.  \emph{Measured:} the
identified corner's excess over its comb-blind structural
skeleton is positive on all $67$ rungs ($+2.29$ to $+3.70$) while
$\tau_X$ decays to $1.7\times10^{-5}$ --- the sharpest wall form
$\tau_X = \lambda_{\min}(\text{structural}) + \text{EXCESS}$.
\emph{Demand (sharpened):} a \emph{uniform} lower bound on the
excess margin along the ladder, with its arithmetic entry point
declared (\texttt{PRIME.FLOOR.PAIRCORR.01} discipline binding ---
no hidden circularity); interim: amplitude-grade wiring for combs
with claimed coefficient data beyond the four frozen test combs.
\emph{Kill:} a reachable rung with certified-negative excess at
the true comb, or an $L$-function comb falsely flagged by the
amplitude reading.  \emph{Close:} the uniform excess bound
delivered and typed.  Explicitly \emph{not} promised: positivity
along the ladder, or any RH-relevant bound.  No marker move; no
RH statement.\par}

\par\smallskip\noindent\textbf{The certified skeleton
(\texttt{PRIME.RELATION.SKELETON.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-07, OPEN
(\veri{v842\_excess\_certified\_skeleton.py}, $9/9$,
\textsc{skeleton-certified}).  \emph{Certified:} strictly
positive interval enclosures of $\tau_X$ on \emph{all} $67$
reachable rungs ($\alpha = 2.773$--$6.304$, masses to
$3\times10^5$; widths $5.4\times10^{-11}$--$6.6\times10^{-9}$,
three to five orders below the margin; chol\_cert discipline
above the deployed float data; certified midpoints reproduce the
float ladder at $4.9\times10^{-13}$); the certified discriminator
(the $h = 2$ fake fails its margin certificate on every anchor,
routed-corner enclosure strictly negative, exact rational
$\Lambda_F$ routing).  \emph{Typed (the scaling law):}
certifiability degrades faster than the margin decays (width
slope $+0.62$ vs margin slope $-0.52$ per $\alpha$), so the
coordinate system has a finite certification horizon --- with
large headroom: $\alpha^* \approx 9.1$ (mass
$\sim 7.5\times10^7$), far beyond the deployed ladder end;
certifiability is not the binding constraint on any reachable
rung.  \emph{What stays open (the wall in its sharpest
coordinates):} the infinite quantifier over the sequence of
strictly-positive certified enclosures --- the sharpened demand
of the relation route: a \emph{uniform} lower bound on the excess
margin, a finer-than-statistical datum by the GUE-side findings;
no finite table, certified or not, settles it.  \emph{Kill:} a
reachable rung whose certified enclosure at the true comb loses
strict positivity under the frozen budget discipline, or a
certified-positive enclosure for the $h = 2$ fake.  \emph{Close:}
the uniform excess bound delivered with its entry point declared.
Explicitly \emph{not} promised: an unconditional proof.
\emph{[2026-08-07 round-29 closing note:} the quantifier is in
its final coordinates
(\veri{v843\_margin\_law\_excess\_lean.py}): the demanded object
sits on the one scalar series $e_1(m) =
(\tau/\tau_{\rm pnt})\,h^{3/2}$, certified $\ge 4.335$ on every
reachable rung and doubly derived (the consistency ward); the
wall is \emph{self-similar at cell level} --- the excess is
itself a growing cancellation (CI median $15.6$, rising
$5.65 \to 29.46$; $22\,287$ of $351\,122$ per-prime aggregates
nonpositive --- no per-prime positive control); the tower gives
scale, not recursion; the compiler connection is null ($0/96$).
The finite-table limit is now \emph{kernel-checked}:
\texttt{TfptCarrier/ExcessSkeleton.lean} proves
\texttt{pointwise\_pos\_not\_uniform} (per-rung positivity at
every rung does not yield a uniform bound) and the bridge
theorem \texttt{excess\_floor} consumes
\texttt{UniformMarginBound} as the single named hypothesis ---
this registration's ``no finite table settles it'' is a theorem,
not a typing.  Same-class margin-law re-attacks are stop-listed
per the measured self-similarity.  Demand, kill and close
conditions unchanged; still [O].]
\emph{[2026-08-07 round-30 closing note (the three-architecture
decision):} the three v5.4 proof-architecture candidates aimed at
this registration's demand are measured and typed \emph{closed}
at three walls that are one wall
(\veri{v846\_schur\_spectral\_mother.py},
\veri{v847\_wedge\_cellcone\_transport.py}): (i) the relational
Schur--Gram route dies structurally at the forced Cauchy--Schwarz
diagonal price (price$/\tau \sim 3.1\times10^4$--$4.5\times10^4$
at the density-anchored floor; trace \emph{and} form; the
untouched-projection deviation \emph{is} the price), and the
price is \emph{geometry-independent} --- the unitary spectral
mother ($J^2 = I$; $U_dU_m = U_{dm}$ exact) gains $20$--$33\times$
at symbol grade but never at Gram grade (the modulus bound);
harvesting interference is Fej\'er--Riesz of the total symbol =
the positivity itself; (ii) the sign-register wedge lift
\emph{exists} as exact algebra (nonnegative wedge weights per
frame, where the naive signed Lagrange provably cannot) but the
frame-uniform law fails at ${\sim}15\%$ against the
$4.4\times10^{-5}$ floor need --- the commutant uniformity wall
transported; (iii) the completed-cell Lorentz transport breaks
the cone at the $n = 2$ cell on $67/67$ rungs under both
predeclared groupings (endpoint back in-cone $=$ the certified
$\tau > 0$; ray cascade to $(5,-3,4)$ confirmed, Kendall
$\pm1.000$), with the inverted Epstein signature kept as its
frozen FAIL: the fake stays in-cone --- the violations are the
true comb's own $\psi(x) - x$ fluctuation; the arithmetic sits
\emph{in} the violations.  \emph{Stop list (extended):} diagonal
relational mothers and manifest-Gram interference harvesting;
constant (frame-uniform) wedge weight laws (the named next
object is an $h$-dependent, analytically controlled law);
unrenormalized small-$n$ completed-cell transports (the named
next object is a renormalized cell map absorbing the $n = 2$
fluctuation).  The extraction side is now separated and complete
(\texttt{PRIME.EXTRACTION.CHAIN.01} below): the wall is exactly
hypothesis (H) on a ladder cofinal in the mesh-refinement
order.  Demand, kill and close
conditions unchanged; still [O].]
\emph{[2026-08-07 round-31 closing note (the invariance
atlas):} the wall this registration names is now proven
\emph{invariant} across four axes
(\veri{v850\_completion\_tax\_flow.py},
\veri{v851\_cluster\_kernel\_field.py},
\veri{v855\_invariance\_atlas.py}): (i) \emph{compressions} ---
the nuclear-norm tax theorem closes the entire positive-dilation
completion class with fixed comb coupling (exact optimum
$\ge 4057.6\times\tau$ on every rung, diverging $\sim h^{0.24}$
against $h^{-3/2}$; zero-gap dual certificates; the measured
v846 designs were suboptimal by only $13.7$--$20.4\times$ ---
optimality could not have saved them), the finite-dimensional
spectral-flow index is a reformulation of the endpoint pivot
signs (distance-to-crossing $=$ metric margin $/$ flow velocity,
ratio $1.00$), and the $2$-dim corner cluster expansion is blind
to multiplicativity (position-geometric weights); (ii)
\emph{geometries} --- the tax is basis-invariant
($\le 1.1\times10^{-15}$): the round-30 TV-price
geometry-independence is now a corollary; (iii) \emph{criteria}
--- Weil-window $=$ Li $=$ Nyman--Beurling (BD-constant $=
2\lambda_1$ at $4.9\times10^{-17}$; the NB span \emph{is} the
spectral-mother geometry; the Li transfer fails by cone
geometry: certified partial data everywhere, one uniform
statement missing everywhere); (iv) \emph{input classes} --- the
sieve toolbox reaches the $\alpha \in 1..2$ band in scope but
misses the grade by $205\times$--$1.1\times10^{4}\times$, and
the demand at truth exceeds the factor-$2$ gate on every rung:
unclosable by \emph{any} upper-bound input class.  \emph{The one
object in four languages:} the uniform $e_1$ bound $=$ the
form-factor band $\alpha \in 1..2$ $=$ the bilinear chain
fluctuation $d\mathrm{Chain}_2$ $=$ the comb block's soft
direction (the minimizer tracking law, $0.0056$ rad).
\emph{Stop list (extended):} positive-dilation completions with
fixed comb coupling, in any basis; finite-dimensional
index/spectral-flow reformulations; $2$-dim corner cluster
expansions; elementary upper-bound supply rows against the
factor-$2$ gate.  The graded kernel law
(\texttt{PRIME.CELLCONE.GRADEDKERNEL.01} below) is the round's
positive core; the minimal hypothesis is now $(H_{\rm cof})$
(\texttt{FORM.PRIME.COFINAL.WEIL.01} below ---
\texttt{UniformMarginBound} demoted to an over-strong sufficient
lemma).  Demand, kill and close conditions unchanged; still
[O].]
\emph{[2026-08-08 round-32 note (the grade law):} the graveyard
this registration accumulated is now \emph{one} kernel-checked
homogeneity theorem
(\texttt{TfptCarrier/GradeNoGo.lean},
\texttt{FORM.PRIME.GRADE.NO\_GO.01} [F]; numeric mirror
\veri{v859\_grade\_no\_go\_elevator.py}): free positivity lives
at \emph{quadratic} grade (Gram squares of linear maps are
2-homogeneous), the deployed target at \emph{linear} grade, and
no exact match exists on any scaling ray --- with the affine
escape priced exactly (the representation error is the sum of
two PSD prices) and the background killed too.  The measured
exchange rate is $10^4$--$10^5$ across four instances: the
Schur--Gram diagonal price ($3.1$--$4.5\times10^4\,\tau$), the
class optimum ($\ge 4057.6\,\tau$), the covariance variance
price ($2.5$--$4.1\times10^5\,\tau$,
\veri{v856\_connected\_current\_lean.py} --- the first design
that \emph{evades} the nuclear tax, dying on the grade gap
instead), and the wedge uniformity gap.  The only licensed
grade-1 mechanism is the \emph{rates elevator}
(\texttt{tangent\_psd\_on\_kernel\_null}, kernel-checked), and
its deployed instantiation is verified real but the
L\'evy/conditional-positivity class is \emph{closed on both
deployed sides} by signed spectral densities (the comb at
exactly $50\%$ negative mass; the arch on the digamma band
ending at $\tau^* = 6.27$ where
$\mathrm{Re}\,\psi(\tfrac14 + i\tau/2) = \log\pi$).  The
finite-head phase-rescue family is closed by \emph{theorem}
(\veri{v860\_phase\_lever\_closure.py}: private-best-phase
budget margins $\ge 1.3894$ / $0.6296$ on all 67 rungs; the
D-frontier saturates at $0.73 < 1$).  \emph{Stop list
(extended):} finite-head phase rescues on the deployed frame
family; amplitude-Gram constructions without grade accounting
(exact matching of the linear window read by any Gram square,
affine backgrounds included); L\'evy/conditional-positivity
elevators on the deployed sides.  The rates door itself stays
licensed and typed; demand, kill and close conditions
unchanged; still [O].]
\emph{[2026-08-08 round-33 note (the wall's operator form):}
the demand now has exact operator coordinates
(\veri{v861\_krein\_normalform\_lean.py} --
\veri{v868\_divisor210\_audits.py}): the deployed window form
is the Krein normal form $Q = \|B_+t\|^2 - \|B_-t\|^2$ with the
Douglas pencil identity $1 - \|C\|^2 = $ the $\tau$-margin, the
defect has rank \emph{one} ($\lambda_1 = $ the $\tau$-margin
exactly --- one arithmetic-carrying soft mode), the contractor
factors \emph{exactly} as the target-free closed-form source
expression $C = W_-FG_+^{-1}F^{\mathsf H}W_+$ (blind holdouts,
certified defect transfer), the norm statement is the exact
reformulation $\|C\| \le 1 \Leftrightarrow G_- \preceq G_+
\Leftrightarrow K \succeq 0$, and the theorem frame is
kernel-checked (\texttt{FORM.PRIME.KREIN.DEFECT.01} [F]:
\texttt{SourceContractor} the named hypothesis, composed to
Weil nonnegativity).  What the campaign \emph{proves} about the
tool space: the absolute half of the classical toolbox is
closed (the Perron value $\rho(|C|) = 2.4$--$3.1 > 1$
everywhere --- no absolute-value inequality can ever certify:
the certification must be \emph{phase-aware}); quadrature does
certify $s > 0$ on every reachable rung (the Neumann frontier
killed) but the depth law $m^* \sim \sqrt{\mathrm{cond}(G)}$ is
\emph{intrinsic} (no source preconditioner waives it --- the
soft end is a continuum edge of distributed arithmetic
structure); the scalar shadow is the pole-port impedance with
the closed-form $a$-term and $\kappa \to 1$; the register is
canonical up to \emph{proven} gauge.  \emph{Stop list
(extended):} source-word searches at all lengths (the monomial
closure theorem: every word of any length misses by
$\ge 0.87$); absolute-value bounds on the weighted frame kernel
(the Perron closure); fixed-depth quadrature certificate
families without a conditioning bound; chirality deciders
through quadratic readouts.  The named demand is sharpened to
\texttt{PRIME.KREIN.CONTRACTOR.01} [O] (below): one phase-aware
bound on the closed-form kernel; still [O].]
No marker move; no RH statement.\par}
...
{\sloppy
\emph{Registered} 2026-08-07
(\veri{v843\_margin\_law\_excess\_lean.py}, $10$ checks with the
one preregistered-honest S2.2 FAIL kept and pattern-gated, $+4$
Lean-mirror checks, \textsc{margin-recurses-the-wall}; Lean core
\texttt{TfptCarrier/ExcessSkeleton.lean}, lake build green, no
\texttt{sorry}, carried in the shipped Lean manifest).
\emph{Measured (the law):} $\tau = e_1(\alpha)\, h^{-3/2}\,
\tau_{\rm pnt}(\alpha)$ with the envelope constant doubly
derived ($e_1 \in [4.855, 24.209]$ on all $67$ rungs,
$\min \ge 4.335$); $\tau_{\rm pnt}$ carries the entire
$\alpha$-decay, $e_1$ the arithmetic; the fit-free $h$-exponent
median $-1.341$ vs bar edge $-1.35$ (the one honest FAIL, not
refit).  \emph{Measured (the negatives, typed):} the tower gives
scale, not recursion (dyadic-level ratio defects non-decaying);
the excess is a growing cancellation --- the wall self-similar
at cell level (no per-prime positive control, no
Euler-factorized positivity); the compiler connection is null
($0/96$, Bonferroni-honest).  \emph{Kernel-checked [F]:} the
two-giants decomposition, the interval-certificate and
certified-negative discriminator lemmas, the finite-ladder
quantifier, \texttt{excess\_floor} with
\texttt{UniformMarginBound} the single named hypothesis
(implying the per-rung \texttt{IdentificationPositivity}
instances; the converse fails), and
\texttt{pointwise\_pos\_not\_uniform} --- no finite certificate
table discharges the uniform bound.  \emph{Stop list:}
same-class margin-law re-attacks (tower-recursion /
cell-factorization of the excess).  \emph{Demand (unchanged,
final coordinates):} a \emph{uniform} lower bound on $e_1$, with
its arithmetic entry point declared
(\texttt{PRIME.FLOOR.PAIRCORR.01} discipline binding).
\emph{Kill:} a reachable rung with $e_1 < 4.335 \times 0.999$,
or a kernel-checked discharge of \texttt{UniformMarginBound}
from structural input alone (which would contradict the typed
boundary and force a re-audit).
\emph{[2026-08-07 round-31 demotion note
(\texttt{FORM.PRIME.COFINAL.WEIL.01}):}
\texttt{UniformMarginBound} is from this round \emph{formally
demoted to an over-strong sufficient lemma} in the load-bearing
chain --- the minimal H theorem
(\texttt{TfptCarrier/CofinalWeil.lean}, numeric mirror
\veri{v849\_cfin\_unique\_cofinal\_lean.py}) proves uniform
$\subsetneq$ pointwise $\subsetneq$ cofinal kernel-checked in
both directions, and the extraction chain consumes exactly the
strictly weaker $(H_{\rm cof})$: PSD at the rungs of one
pre-fixed cofinal ladder, no uniform $\delta$.  This block's
lemmas and \texttt{excess\_floor} remain kernel-checked and
load-bearing as the \emph{sufficient} side; the named wall moves
to $(H_{\rm cof})$.  Still [F]/[O] as stated.]
No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The continuum extraction chain
(\texttt{PRIME.EXTRACTION.CHAIN.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-07, OPEN
(\veri{v848\_extraction\_chain.py}, $19/19$,
\textsc{extraction-chain-complete}; hypothesis (H) is \emph{never
evaluated} --- no positivity statement about any actual tower
form or the actual Weil functional is made).  \emph{Measured:} on
the canonical $(D, X)$ ladder, cofinal in the
\emph{mesh-refinement} order ($D_j = 2^{-j}$,
$j = 4..11$, $68$--$8704$ lags, the full pinned prime-power
table) every element of the canonical dense family identifies its
independently computed Weil value $Q_W = \mathrm{POLE} +
\mathrm{ARCH} + \mathrm{ATOM}$ (exact piecewise-cubic
autocorrelations in $\Q$, fifth-point certificates; Suzuki
normalization, W1-certified layers) with rates ${\sim}4^{-j}$
(median $-1.818$ per level), a geometric ledger (median ratio
$0.302$, decay law $-1.752$), and the $X$-direction \emph{exactly}
stable once the window covers the element ($44$ cutoff pairs at
float zero); the pure-box class carries the \emph{exact} Weil
value at every finite level (maximal error $5.3\times10^{-14}$
--- the sharpest dyadic-inheritance statement); recovery is
constructive and Cauchy (the exact strong-$L^2$ law in $\Q$);
the gated far-spike liminf family has strictly positive margins,
while the band directions are typed and \emph{never} gated ---
wall content under (H), refused by design.  Both controls fire
(the density-only wrong limit stalls at ratio $1.6\times10^6$;
the scrambled tower inflates the ledger by $5.5\times10^5$).
\emph{The deliverable (the quantifier reduction):} under (H) ---
finite positivity of the tower forms along a ladder cofinal in
that same mesh-refinement order --- every ladder
value is nonnegative and the limit passage is arithmetic-free:
\emph{no} Mosco compactness, \emph{no} uniform $\delta > 0$,
\emph{no} diagonal argument enters the implication; per-element
convergence is the only analytic input, and the three
infinite-level ingredients of \texttt{PRIME.MOSCO.SELECTION.01}
migrate to the operator-level selection programme, where they
stay typed.  With the classical legs cited (density; the
continuity of $Q_W$ at fixed support --- \emph{corrected} in the
round-CCCLXV note below, the pure sup-norm $C^0$ form of this
leg being false; Weil 1952, Bombieri
2000, Suzuki arXiv:2606.09096, Iwaniec--Kowalski Thm~5.12), the
chain \emph{cofinal finite positivity} $\Rightarrow$ \emph{Weil
positivity} $\Rightarrow$ (via Weil's criterion) \emph{the
target} is theorem-grade modulo the named citations.  \emph{What
stays open (by design, the whole wall):} hypothesis (H) ---
equivalently the uniform demand of
\texttt{PRIME.RELATION.SKELETON.01} restricted to a ladder
cofinal in the mesh-refinement order --- and \emph{nothing
else}.  \emph{Kill:} an element of
the dense family whose ladder values converge to a value
different from the independently computed $Q_W$, a non-summable
ledger on the true tower, or a failure of the $X$-stabilization
on a covered element.  \emph{Close:} any discharge of (H), which
via this chain yields Weil positivity and, via the criterion,
the target.  Explicitly \emph{not} promised: an unconditional
proof.
\emph{[2026-08-07 round-31 note:} step 2 of the chain --- the
arithmetic-free limit passage under (H) --- is now
\emph{kernel-checked} (\texttt{TfptCarrier/CofinalWeil.lean}:
the $\varepsilon/2$ argument, the no-diagonal dense-family form,
the assembly; axioms clean), and hypothesis (H) is formalized as
\texttt{CofinalHypothesis} $(H_{\rm cof})$,
preregistration-shaped (the ladder is data, never mined from
measured signs) and proven strictly weaker than
\texttt{UniformMarginBound}
(\texttt{FORM.PRIME.COFINAL.WEIL.01} below).  The genuinely open
item is unchanged: $(H_{\rm cof})$ itself; still [O].]
\emph{[2026-08-13 round-CCCLXV note}
(\veri{v912\_form\_convergence\_theorem.py}, $35/35$,
\texttt{PRIME.FORM.CONVERGENCE.THEOREM.01}): \emph{two of the
inputs above change type.}  (i) Per-element form convergence ---
the ``only analytic input'' of the quantifier reduction --- is no
longer measured but \emph{proven unconditionally} on the
deployed dyadic cellwise-affine family, with the explicit rate
$O(D^2\log(1/D)) = O(2^{-2j}j)$ and explicit constants
($\Theta_0 = 3\log 2 + \pi/2 + \gamma + \log\pi$; log
coefficient exactly $7/24$ for continuous $f$); the load-bearing
step is the exact defect lemma
$k_d - K(dD) = -(D^2/12)G(dD)$, verified with zero deviation in
exact rationals, and the only cited arithmetic input is
Rosser--Schoenfeld $\psi(x) < 1.03883x$ for support uniformity.
(ii) The continuity leg as cited above is \emph{false} in the
pure sup norm: the explicit even Lipschitz family
$e_n(w) = \tfrac1n\min(1, we^{n^2})(1 - w/2)_+$ has
$\|e_n\|_\infty \to 0$ while $|A[e_n]|$ grows linearly
($2.57 \to 12.09$).  The correct hypothesis --- uniform
convergence plus an equi-Lipschitz (Dini) condition at the
origin, supplied by the admissible even compactly supported BV
class --- replaces it here and wherever the contract cited it;
the \emph{density} leg in that topology stays cited.  This is a
correction of a wrong cited statement, not a downgrade.  What
stays open is unchanged: $(H_{\rm cof})$, still [O], still of RH
strength.]
No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The finite-compiler compressions
of round 30 (\texttt{MESSAGE.LADDER.01},
\texttt{DOILY.PASCAL.RANK.01}, \texttt{NORMALFORM.CFIN.01}).}
{\sloppy
\emph{Recorded} 2026-08-07 (evidence lines, not contracts; exact
identity grade; no marker moves).  The message ladder
$M_n = 15\cdot2^n = (15, 30, 60, 120, 240)$ with one named
structure per rung and $h(E_8) = 30$ derived from the rebuilt
root system, and the Doily--Pascal rank theorem
$NN^{\mathsf T} = B + 2I$ with $\operatorname{rank} N = 10 =
A_\Lambda$, $\dim\ker N = 5 = \gcar$ and recovery value $2/3$
(\veri{v844\_message\_doily\_rank.py}, $23/23 + 16/16$); and the
finite compiler normal form $C_{\rm fin} = (V, \hbar, q^{*},
\sigma, \iota)$ assembled as one object with the code-to-matter
kill built in and P2 narrowed to the \texttt{v799} residual
R1$'$ (\mC), not eliminated
(\veri{v845\_cfin\_normal\_form.py}, $28/28$, SPEC v2).  Full
statements and keyboxes in Paper~1; audit cards in Paper~3.  The
$(1/3)^6$ typed gap of the six-step decision is re-affirmed with
no upgrade.\par}

\par\smallskip\noindent\textbf{The minimal H theorem
(\texttt{FORM.PRIME.\allowbreak COFINAL.\allowbreak WEIL.01}
[F]).}
{\sloppy
\emph{Registered} 2026-08-07, FORMAL
(\texttt{TfptCarrier/CofinalWeil.lean}, $11$ declarations,
kernel-checked --- lake build green at $3415$ jobs, no
\texttt{sorry}, no \texttt{native\_decide}, axioms
propext/Classical.choice/Quot.sound only, verified by
\texttt{\#print axioms} on all eight theorems; numeric mirror
\veri{v849\_cfin\_unique\_cofinal\_lean.py} part B; carried in
the shipped Lean manifest at $87$ files).  \emph{Proven:} the
cofinal hypothesis $(H_{\rm cof})$ --- a \emph{pre-fixed}
strictly monotone ladder, cofinal in the \emph{mesh-refinement}
order in which the form convergence holds and not in the window
or cap parameter alone (at a fixed mesh the read is
cap-independent, hence eventually constant, and its limit
carries the frozen quadrature defect of that mesh), along which
the tower matrices are PSD;
preregistration-shaped: the ladder is \emph{data} of the
structure, supplied before any certificate, never mined from
measured signs --- and the minimal implication:
\texttt{limit\_nonneg\_of\_cofinal\_seq} (the $\varepsilon/2$
argument at sequence level) and
\texttt{weil\_nonneg\_of\_cofinal} (per-element form convergence
$+ (H_{\rm cof}) \Rightarrow$ the limit functional nonnegative
on the \emph{whole} dense family, with \emph{no} diagonal
argument --- one PSD certificate per rung covers every element
simultaneously); the strict hierarchy
\emph{uniform $\subsetneq$ pointwise $\subsetneq$ cofinal},
kernel-checked in both directions (witnesses $1/(m{+}1)$ ---
reusing \texttt{pointwise\_pos\_not\_uniform} --- and $\pm1$ on
the even ladder); and the assembly \texttt{cofinal\_weil}.
\emph{Absent by design:} no uniform margin, no
inverse/resolvent, no Mosco coercivity, no limit operator, no
diagonal argument.  \emph{The formal demotion (the round's
status statement):} the load-bearing chain of
\texttt{PRIME.EXTRACTION.CHAIN.01} consumes \emph{exactly}
$(H_{\rm cof})$; \texttt{UniformMarginBound} implies
$(H_{\rm cof})$ and the converse fails kernel-checked ---
\texttt{UniformMarginBound} is from this round an over-strong
\emph{sufficient lemma}, no longer the named wall; the wall
content has strictly decreased to cofinal PSD on one pre-fixed
mesh-refining ladder.  \emph{The honest boundary:}
$(H_{\rm cof})$ stays a
hypothesis everywhere; nothing proves, gates or evaluates
positivity of any actual tower form; no continuum theorem, no
RH statement, no arithmetic identification is formalized or
claimed.\par}

\par\smallskip\noindent\textbf{The graded kernel $+$ tail
theorem candidate
(\texttt{PRIME.CELLCONE.\allowbreak GRADEDKERNEL.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-07, OPEN
(\veri{v851\_cluster\_kernel\_field.py} part C, $7/7$, verdict
\textsc{kernel-graded}).  \emph{The candidate statement}
(measured on the $67$ deployed rungs under the G1/Stieltjes
completion; \emph{not} proven, \emph{not} uniform in
$\varepsilon$, not an RH claim): for every $\varepsilon > 0$
there exist finite $P_0(\varepsilon)$ and $X_0(\varepsilon)$
such that for every window $X \ge X_0(\varepsilon)$, every state
of the completed-cell flow beyond $n = P_0(\varepsilon)$ has
relative depth $-\lambda_{\min}/\lambda_{\max} \le \varepsilon$.
\emph{Measured witnesses:} $P_0(10^{-1}) = 0$ on all rungs;
$P_0(10^{-2}) \le 73$, collapsing to $\{2, 3\}$ above $\alpha
\approx 3.9$; the skin-depth law $D(X) = 0.754\,X^{-0.548}$
($R^2 = 0.99$, Kendall $-1.00$) predicts $X_0(\varepsilon) =
(\varepsilon/0.754)^{-1/0.548}$ ($X_0(10^{-3}) \sim
1.8\times10^{5}$); the strict ($\varepsilon \to 0$) census
wanders with the horizon ($P_0 \sim X^{1.03}$ at relative
position $0.98$) --- the wandering is vanishing-depth skin,
\emph{not} kernel growth; the G2 mechanism typed by contrast
(centroid offset $1653\times$, running imbalance $39\times$ ---
delayed compensation).  \emph{Honest scope:} ``resolved'' is an
in-range statement; the $\varepsilon < 10^{-2}$ rows are
extrapolated only through the fitted law.  \emph{Kill:} a rung
family on which the depth-$10^{-2}$ kernel grows without bound,
a measured break of the skin-depth power law, or a G1 state
beyond the predicted $P_0(\varepsilon)$ with depth $>
\varepsilon$ inside the verified range.  \emph{Close:} a proof
of the candidate statement for the canonical completed-cell flow
--- the graded weakening of the cone-transport demand; note it
does \emph{not} by itself give $(H_{\rm cof})$: the graded
statement bounds depth, not sign.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The code-layer compressions of
round 31 (\texttt{CFIN.UNIQUE.01},
\texttt{ARF.VACUUM.SYNDROME.01}, \texttt{OVOID.DECODER.01},
\texttt{ARF.BENT.CSS.01}, \texttt{PRIME.RELATION.MANGOLDT.01}).}
{\sloppy
\emph{Recorded} 2026-08-07 (evidence lines, not contracts; exact
identity grade; no marker moves).  The uniqueness theorem: every
admissible compiler tuple is isomorphic to $C_{\rm fin}$ (census
$14400$, one $Sp(4,2)\times S_5$ orbit,
$\mathrm{Aut}(C_{\rm fin}) \cong C_6$; the strict
terminal-object reading fails honestly --- unique up to
\emph{non}-unique isomorphism; P2 stays narrowed to the
\texttt{v799} residual R1$'$)
(\veri{v849\_cfin\_unique\_cofinal\_lean.py}, $22/22$).  The
Arf--vacuum syndrome theorem and the ovoid decoder: the two-bit
syndrome table $15/20/45/60$ on the Hamming code with the local
Hecke theorem $12/16 \to 28/{-4}$ at all $15$ points, the code
pencil with the canonicity lemma and the vacuum bit as the
$\mathrm{RM}(2,4)$ parity bit; $\mathrm{SNF}(N) =
\mathrm{diag}(1^{10}, 0^5)$ torsion-free with the integral
decoder $N^{+} = N^{\mathsf T}/4 - J/36$ and the $S_6$ standard
representation; the three-ring correspondence
(\veri{v852\_arf\_vacuum\_ovoid.py}, $24/24 + 23/23$).  The bent
CSS pair and the commutator theorem: $\hat s_q = -4\,s_q$, the
$(16,6,2)$ difference set, $[[15,5,3]] \to [[16,4,4]]$ with the
vacuum transformation typed, the $-4$ triple point (Hecke $=$
Gauss $=$ Walsh $= a_3$); and $\mathcal L = -M[D,Z] = -[D, \log
Z] = T(\Lambda)$ exact with the four-comb discipline at operator
level and the carrier bridge (typed honestly as classical
incidence algebra)
(\veri{v853\_bent\_css\_mangoldt.py}, $22/22 + 30/30$).  Full
statements and keyboxes in Paper~1; audit cards in Paper~3.\par}

\par\smallskip\noindent\textbf{The grade barrier as a class
closure (\texttt{FORM.PRIME.\allowbreak GRADE.\allowbreak
NO\_GO.01} [F]).}
{\sloppy
\emph{Registered} 2026-08-08, FORMAL
(\texttt{TfptCarrier/GradeNoGo.lean}, $13$ declarations ($10$
theorems), kernel-checked --- lake build green at $3415$ jobs,
no \texttt{sorry}, no \texttt{native\_decide}, axioms
propext/Classical.choice/Quot.sound only, verified by
\texttt{\#print axioms} on all ten theorems; numeric mirror
\veri{v859\_grade\_no\_go\_elevator.py} part A; carried in the
shipped Lean manifest at $87$ files).  \emph{Proven:} the core
no-go \texttt{grade\_no\_go} --- a 1-homogeneous map and a
2-homogeneous map that agree on a scaling ray agree with $0$ at
the base point (evaluate at $t = 1$ and $t = 2$: $2A(m) =
4A(m)$; two lines --- a \emph{class} closure, not a
construction failure); \texttt{gram\_two\_homogeneous} and
\texttt{grade\_no\_go\_gram} (Gram squares of linear maps are
2-homogeneous, so exact matching annihilates target \emph{and}
factor: grade 1 cannot live inside grade 2); the affine
trilemma --- \texttt{affine\_gram\_expansion} (the exact split
into constant $+$ cross $+$ quadratic components, the cross
term the \emph{only} grade-1 slot a Gram square owns),
\texttt{affine\_gram\_tax} (the representation error against
the pure cross term is \emph{exactly} the sum of two PSD
prices: the tax), and \texttt{affine\_grade\_no\_go} (exact
affine matching forces target, background \emph{and} factor to
vanish); and the elevator lemma
\texttt{tangent\_psd\_on\_kernel\_null} --- derivatives of PSD
families at kernel-null directions are positively signed:
\emph{rates, not amplitudes}, the only grade-1 mechanism the
no-go does not exclude, proven once, abstractly.  \emph{The
unifying diagnosis:} the four graveyard instances (the
Schur--Gram tax, the covariance grade gap, the wedge uniformity
failure, the phase-lever closure) are instances or relatives of
this one law; the measured exchange rate is $10^4$--$10^5$.
\emph{Honesty notes:} the no-go excludes exact Gram matching on
scaling rays and nothing more --- denominator representations
died separately by measurement (cited); non-Gram mechanisms are
outside the class closure; the trilemma exhaustiveness outside
the affine family stays an informal doc-level reading, typed.
No continuum theorem, no RH statement, no arithmetic
identification.\par}

\par\smallskip\noindent\textbf{The conditional route to the
cofinal hypothesis (\texttt{FORM.PRIME.\allowbreak
COFINAL\_CURRENT.01} [F]).}
{\sloppy
\emph{Registered} 2026-08-08, FORMAL
(\texttt{TfptCarrier/CofinalCurrent.lean}, $13$ declarations
($7$ theorems), kernel-checked --- lake build green at $3415$
jobs, no \texttt{sorry}, no \texttt{native\_decide}, axioms
propext/Classical.choice/Quot.sound only, verified by
\texttt{\#print axioms} on all seven theorems; numeric mirror
\veri{v856\_connected\_current\_lean.py} part C; carried in the
shipped Lean manifest at $87$ files).  \emph{Proven:} the
conditional theorem \texttt{cofinal\_current} ---
\emph{ConnectedTail} (every tail block PSD) $+$
\emph{PositiveHeadCell} (an open phase cell with a uniform
Rayleigh floor) $+$ \emph{PhaseRecurrence} (cofinal visits)
$\Rightarrow (H_{\rm cof})$, with the index sequence
\emph{extracted} from the recurrence; the composed corollary
\texttt{cofinal\_current\_weil} through the minimal H theorem:
the three hypotheses $+$ per-element form convergence force the
limit functional nonnegative on the whole dense family --- no
hidden wall behind the two analytic lemmas; the $\delta = 0$
relaxation (the chain consumes only PSD on the cell; the strict
floor is a stronger input than required, typed) and the
Rayleigh reading; and the counterexample
\texttt{connectedTail\_not\_sufficient} (a PSD-tail split with
everywhere-negative head admits no cofinal ladder --- the
hypotheses are jointly necessary).  \emph{Provenance typed, not
proven:} hypothesis 1 is what the connected-covariance
construction delivers structurally
(\texttt{PRIME.RELATION.CONNECTED\_COVARIANCE.01}); hypothesis
2 is what the phase-cell census \emph{measured empty at the
deployed frame} (\texttt{PRIME.COFINAL.PHASE\_CELL.01} ---
the frame awaits a head with a nonempty cell); hypothesis 3 is
the classical Kronecker/Weyl citation (arithmetic input:
log-prime independence, the two-line UFD lemma warded in the
probe).  \emph{The honest boundary:} nothing proves any of the
three hypotheses; a continuation contract, not a discharge; no
continuum theorem, no RH statement, no arithmetic
identification.\par}

\par\smallskip\noindent\textbf{The E8 audit compressions of
round 32 (\texttt{E8.SIMPLEX.FOURIER.01},
\texttt{E8.G31.CLOCK\_ALPHABET.01},
\texttt{FLAVOR.WINDING.QUADRATIC.01}).}
{\sloppy
\emph{Recorded} 2026-08-08 (evidence lines, not contracts;
exact identity grade; no marker moves).  The simplex-Fourier
character theorem: the census $240 = 15\times16$ \emph{is} the
spectral statement $\hat r = (240, -16^{15})$ --- the
uniform-nonzero channel $P = (J - I)/15$ with spectrum $\{1,
(-1/15)^{15}\}$; the \texttt{v817} packet numerator equals the
code Walsh transform at every level $\le 16$, and
$\hat m_2(n) = -1/15$ holds as an integer identity at every odd
$n \le 16500$ --- the per-prime measurement is the prime
restriction of a character theorem, typed spectral-only
(\veri{v857\_simplex\_fourier\_winding.py}, $26/26$; upgraded in
round 36 to \emph{all} odd shells, \texttt{E8.SHELL.GLOBAL.01}
--- see the round-36 record below).  The
winding quadratic: the whole \texttt{tfpt\_2} winding line is
one adjugate quadratic $q_{\rm wind} = t^2 - \gcar t + |\Z_2|$
with the triple lock $s = 6$, $\Delta(6) = 39200 = 2\cdot140^2$
and the reality threshold $s^* \approx 2.8250$ typed
(\veri{v857\_simplex\_fourier\_winding.py}, $20/20$).  The G31
clock alphabet: $\mathrm{Deg}(G_{31})/4 = \{2,3,5,6\}$ as a
normal form with the unique product-$46080$/sum-$64$ quadruple,
the $607$-group kill scan ($G_{31}$ the sole full-battery
passer) and the $W(D_5)\times W(A_3)$ fence re-killed by
computed centers $4$ vs $1$
(\veri{v858\_g31\_clock\_alphabet.py}, $19/19$).  Full
statements and keyboxes in Papers~1 and~2; audit cards in
Paper~3.\par}

\par\smallskip\noindent\textbf{The Krein-defect theorem frame
(\texttt{FORM.PRIME.\allowbreak KREIN.\allowbreak DEFECT.01}
[F]).}
{\sloppy
\emph{Registered} 2026-08-08, FORMAL
(\texttt{TfptCarrier/KreinDefect.lean}, $14$ declarations ($10$
theorems), kernel-checked --- lake build green at $3416$ jobs,
no \texttt{sorry}, no \texttt{native\_decide}, axioms
propext/Classical.choice/Quot.sound only, verified by
\texttt{\#print axioms} on all ten theorems; numeric mirror
\veri{v861\_krein\_normalform\_lean.py} part C; carried in the
shipped Lean manifest at $88$ files).  \emph{Proven:} the
defect representation \texttt{defect} / \texttt{formAt\_defect}
--- the quadratic form of $B_+^{\mathsf H}B_+ -
B_-^{\mathsf H}B_-$ \emph{is} $\|B_+f\|^2 - \|B_-f\|^2$
exactly; the main theorem \texttt{defect\_psd\_of\_contraction}
--- $B_- = CB_+$ with the Loewner certificate $(1 -
C^{\mathsf H}C) \succeq 0$ forces the defect PSD, via the exact
factorization $B_+^{\mathsf H}(1 - C^{\mathsf H}C)B_+$ and
congruence; the converse
\texttt{douglas\_contraction\_of\_defect\_psd} (full-rank case
proven, $C := B_-B_+^{-1}$; the singular case deliberately the
classical Douglas 1966 citation --- the range bookkeeping not
formalized, typed); \texttt{SourceContractor} --- the
\emph{named hypothesis}: contractor $+$ factorization $+$
\emph{independent} norm certificate as data a consumer must
supply from the source side, with the circularity warning typed
(a target-computed $C$ --- eigendata or the converse's
pseudoinverse --- is a reformulation of the positivity, not a
proof); and the cofinal composition \texttt{krein\_cofinal} /
\texttt{krein\_cofinal\_weil} --- contractors on a cofinal
ladder yield $(H_{\rm cof})$ (index extraction from the
recurrence) and compose through the minimal H theorem to Weil
nonnegativity on the whole dense family: no hidden wall behind
the contractor certificates.  \emph{Provenance typed, not
proven:} the contractor hypothesis is exactly what the Krein
source-algebra search must deliver;
\veri{v866\_source\_contractor\_formula.py} delivers its closed
\emph{formula} and proves the absolute half of the toolbox
cannot certify it --- the certificate demand is the
registration below.  \emph{The honest boundary:} nothing here
proves the contractor exists; no continuum theorem, no RH
statement, no arithmetic identification.\par}

\par\smallskip\noindent\textbf{The phase-aware contractor
certificate (\texttt{PRIME.KREIN.\allowbreak CONTRACTOR.01}
[O]).}
{\sloppy
\emph{Registered} 2026-08-08, OPEN
(\veri{v866\_source\_contractor\_formula.py}, $10 + 8/8$ with
the one frozen-honest FAIL kept and pattern-gated,
\textsc{sylvester12-angles-fail} $+$
\textsc{norm-reformulation}; with
\veri{v861\_krein\_normalform\_lean.py},
\veri{v862\_defect\_polar\_weld.py},
\veri{v864\_softport\_kappa\_law.py},
\veri{v867\_radau\_conditioning.py}).  \emph{The demand,
verbatim:} supply a \emph{target-free} certificate that the
explicit closed-form source contractor
$C = W_-\,F\,G_+^{-1}F^{\mathsf H}\,W_+$ (the factorization
warded at $1.4\times10^{-14}$ incl.\ blind holdouts; density
roots and window geometry only) is a contraction, $\|C\| \le
1$, uniformly along the deployed cofinal ladder ---
equivalently the Loewner comparison $G_- \preceq G_+$ of the
two weighted frame Grams, equivalently $K \succeq 0$ (the
Douglas equivalence, machine-verified) --- with the
certificate's constants independent of the measured target
signs.  \emph{What is proven about the tool space (the
registration's teeth):} (i) \emph{no absolute-value bound can
succeed} --- the Perron value $\rho(|C|)$ is the infimum over
all absolute Schur/Hilbert-type tests and it exceeds $1$ on
every heavy rung ($2.363 \to 3.103$, growing with depth): any
valid certificate must use the \emph{phases} of the kernel;
(ii) quadrature certifies $s > 0$ pointwise on every reachable
rung but the depth grows as $m^* \sim \sqrt{\mathrm{cond}(G)}$
and the conditioning is \emph{intrinsic} --- a uniform
certificate cannot be a fixed-depth quadrature family; (iii) no
source word of \emph{any} length reaches $C$ (the monomial
closure theorem) --- the certificate cannot be a word bound;
(iv) the discharge target is kernel-checked: per-rung
\texttt{SourceContractor} instances on a cofinal ladder compose
through \texttt{krein\_cofinal\_weil} to Weil nonnegativity.
\emph{The named candidate shape:} an oscillatory-integral /
stationary-phase bound on the entries of the weighted frame
kernel, strong enough to beat $1$ by the measured
$e^{\mathrm{slope}\,\alpha}$ margin (fit-free defect law, slope
$-2.40$).  \emph{Kill:} a rung with $\rho(|C|) \le 1$; a proof
that every phase-aware bound of the named shape degrades to an
absolute bound on the deployed kernel class; a measured break
of the factorization on any new rung.  \emph{Close:} the
certificate itself --- it would discharge the
\texttt{SourceContractor} hypothesis and with it
$(H_{\rm cof})$ for the defect ladder.
\emph{[2026-08-08 round-34 note
(\veri{v869\_cdcore\_clarkphase.py} --
\veri{v872\_damping\_compensation.py} $+$ the Pr\"ufer
preregistration):} \emph{the kernel is named} --- the closed-form
contractor's reproducing kernel $K_+$ is the
Christoffel--Darboux kernel of the explicit source measure $\mu
= \sum [2\sin^2(\varphi/2)\,d_+(\varphi)/L]\,
\delta_{2\cos(D\tau)}$ (rank-2 displacement collapse with a
$10^{13}$ gap on all rungs incl.\ blind holdouts; firewalled
chain-only reconstruction at ${\sim}10^{-13}$): the demand now
reads --- control \emph{one} source Jacobi chain and the arm
weights.  \emph{The compensation identity:} $I - C^*C = T_1 +
T_2$ exactly ($T_1 = I - U^*U$ indefinite Jacobi cross-geometry;
$T_2 = U^*(I - D_-^2)U$ PSD Christoffel damping) with the exact
global minimizer equal to the soft direction and the isolated
scalar $s = \kappa\tau$ --- the demand localized to the two-term
inequality at one named direction, both terms source-built.
\emph{The closures (stop-list entries):} the Gauss-node unitary
core does not exist (co-isometry defect $O(1)$; (H-U) the open
hypothesis, $D_-^G \le 0.997041$ the surviving asset ---
\veri{v870\_christoffel\_gauss\_frame.py}); classical
pole transformations at $x_{\rm pole} = 2\cosh(D/2)$ are dead
(typed residue: small-negative-mass Geronimus at $3.5$--$8.1\%$,
no $\lambda(D)$ law); finite-rank cross-defect corrections are
dead (rank grows linearly with $h$, Pearson $+0.996$ ---
\veri{v871\_pole\_uvarov\_crossdefect.py});
$\mu_4$-offset Clark kernels are dead (the equal-phase law).
\emph{The preregistration (the contract's next test,
preregistered pre-execution):}
\texttt{PRIME.PRUEFER.COMPENSATION.01} --- source-only Pr\"ufer
phases of the two arm chains, exactly $16$ fixed phase cells
(boundaries $k\pi/8$, entrywise pairing primary, danger families
predeclared), Cotlar--Stein decision rules and all success/kill
criteria frozen \emph{before} any deployed-data run, deep
holdouts kz $90/116/142/177/243$ never used by the CD wave, the
decisive analysis locked behind the operational unfreeze flag,
$8/8$ synthetic freeze wards green; committed byte-exact with
round 34: SPEC SHA-256
\texttt{4621b89958531d6d\allowbreak 3baae9fe762dbd10\allowbreak
d23dc22eed77a57a\allowbreak a1c6b179a7440811},
FILE SHA-256
\texttt{93208a1b2edebde3\allowbreak 1388a87192055c9b\allowbreak
f3e5a3de29af9fb3\allowbreak e34ddc904825d9fd}
(\texttt{experiments/\allowbreak tfpt-discovery/\allowbreak pruefer\_compensation\_probe.py};
any modification invalidates the preregistration).  Demand, kill
and close conditions unchanged; still [O].]
\emph{[2026-08-08 round-35 note
(\veri{v873\_pruefer\_cotlar\_decision.py}, the executed
preregistration):} the contract was executed as committed --- the
unfreeze flag created \emph{after} the round-34 commit
\texttt{526ca3eb} (audit line committed), the two mechanical
fixes documented verbatim (the $h \le 900$ battery filter the
spec's own text mandates; the overflow renormalization,
$\operatorname{atan2}$-invariant, validated at $\le
6.6\times10^{-4}$ against a $60$-digit \texttt{mpmath}
reference), the spec literal SHA byte-identical throughout, the
executed FILE SHA-256
\texttt{15d35b26c652ef41\allowbreak 44df713cafba7e04\allowbreak
9e2dcb9bb7a16846\allowbreak 2cd65735f5ffb4cc} --- and it returns
\textsc{cotlar-growing} by its own frozen rules: danger geometry
real (capture $0.98$--$0.61$ of $\lambda_{\min}(T_1)$,
source-only), local dominance and envelope decay FAIL (the
envelope bimodal at the predeclared $\pi$-resonance cells), the
battery flat but the frozen deep holdouts exploding (slopes
$0.209/0.408 \ge 0.20$, both channels; kz 142: $S_U\,32.07$/
$S_C\,58.78$), the contrast bars passing, kills 2 $+$ 5 firing,
RUN 3 correctly withheld.  The blockwise Cotlar--Stein
phase-cell route is closed and stop-listed; the surviving demand
is a non-stationary phase bound with the $\pi$-resonance as the
known critical structure and the $\alpha$-not-$h$ growth law as
the typed constraint (the explosion tracks kz-depth, not window
size).  The first fully preregistered uniformity test of the
program, executed and booked exactly as frozen; still [O].]
\emph{[2026-08-08 round-36 CORRECTION note
(\veri{v877\_complete\_comb\_reversal.py}, the complete-comb
re-decision):} the round-35 note above is corrected, dated ---
the RUN-2 deep-holdout data was \emph{comb-truncated}
(\texttt{core.build\_window} silently caps the atom table at
$\texttt{ATOM\_MAX} = 4\times10^{5}$; kz $142/177/243$ demand
support to $4.4\times10^{5}/8.0\times10^{5}/1.8\times10^{6}$;
\texttt{PRIME.DEEPALPHA.SIGN.01} certifies the complete-comb
floors strictly positive).  The v1 verdict was correct \emph{on
the data as measured} and \texttt{v873}'s booking stands as the
record of the preregistered v1 execution (spec SHA
\texttt{4621b899\ldots}, executed file SHA
\texttt{15d35b26\ldots}); but on complete combs the
\emph{identically} frozen v1 rule returns \textsc{bounded} in
both channels (v2 spec SHA \texttt{5fd6bf61\ldots}; kz 142:
$S_U\,32.07 \to 5.65$, $S_C\,58.78 \to 4.10$; slopes
$0.006/0.007$) and RUN 3 passes ($K_1/K_2$ $h$-stable on all
five holdouts).  \emph{The stop-list entry is amended:}
``blockwise Cotlar--Stein on Pr\"ufer phase cells'' is removed
--- the class is \emph{not} closed; the route reopens as
\texttt{PRIME.PRUEFER.COMPENSATION.02} with the remaining gap
typed (the Cotlar constant $B_C \sim 3.9$--$4.3$ vs the needed
$1$; the cell-local defects $\sum_r\varepsilon_r \sim 7$--$10$
--- the RUN-1 anatomy was in-cap and stands); the
$\alpha$-not-$h$ law is retired (it was the truncation's
signature).  The $\pi$-resonance critical structure and the
discrimination stand; the surviving demand is now carried in
embedding coordinates by \texttt{PRIME.CARLESON.PRIME.01} [O]
(registered below).  Demand, kill and close conditions
otherwise unchanged; still [O].]
Existence is \emph{not}
claimed; no RH statement.\par}

\par\smallskip\noindent\textbf{The 210 canonicity audits of
round 33 (\texttt{E8.DIVISOR210.\allowbreak CANONICITY.01}).}
{\sloppy
\emph{Recorded} 2026-08-08 (evidence lines, not contracts; no
marker moves).  The label register $\mathbb{F}_2^4$ \emph{is}
the divisor lattice of $210 = 2\cdot3\cdot5\cdot7$
(Walsh--Hadamard carries the lattice M\"obius function; the
vacuum column is the rank-one Redheffer closure with
$\det = \prod_p(1 - 1/p) = 8/35$ exact,
\veri{v863\_redheffer\_colligation.py}); the canonicity guard
quantifies the selection
(\veri{v868\_divisor210\_audits.py}, $36/36 + 12/12 + 15/15$):
the Boolean layer is measured generic ($210/210$), the two
Euler determinants pin $\{2,3,5,7\}$ uniquely, the anchor prime
$2$ is forced by ramification, and after the crossref exactly
two gauge classes remain --- the family-cycle chirality, then
\emph{proven gauge} by two exact no-go wards (the orientation
functional vanishes identically via the M\"obius complement
$d \mapsto 210/d$; quadratic readouts provably blind by
transposition); the $17 = 12{+}3{+}1{+}1$ moving-sector
identification honestly buried at space level (principal angles
${\sim}90^\circ$); the $6$-vs-$7$ honesty note stands
untouched.  Full statements and keyboxes in Paper~1; audit
cards in Paper~3.\par}

\par\smallskip\noindent\textbf{The Prime Carleson embedding
contract (\texttt{PRIME.CARLESON.PRIME.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-08, round 36
(\veri{v876\_carleson\_polygon\_wave.py}; the ward making the
reduction machine-exact at rel
$1.5\times10^{-13}/1.3\times10^{-15}$ on kz $9/13$).
\emph{The chain:} for the canonical window family, the Krein
floor $\tau_h > 0$ is \emph{equivalent} to the Carleson
embedding of the negative arm measure into the positive arm
measure at polynomial degree $h - 1$ --- $\|C_h\| \le 1
\Leftrightarrow G_- \le G_+$ (exact,
\texttt{PRIME.SOURCECONTRACTOR.NORM.01}) $\Leftrightarrow
\int|P|^2\,d\tilde\nu_- \le \int|P|^2\,d\tilde\mu_+$ for all
$P \in \mathcal P_{h-1}$ $\Leftrightarrow M(\tilde\mu_+ -
\tilde\nu_-) \ge 0$, with the embedding constant
$\lambda_{\max}(M_-) = \|C_h\|^2 = 1 - \tau_h$.  \emph{The
demand:} a proof of the embedding uniformly along a window
subfamily cofinal in the mesh-refinement order would discharge
\texttt{SourceContractor}
(\texttt{FORM.PRIME.KREIN.DEFECT.01}) and, through
\texttt{krein\_cofinal\_weil}, hypothesis $(H_{\rm cof})$.
\emph{The typed constraints a candidate must respect:} the
certificate must be phase-aware (the Perron closure of
\texttt{v866}); the $\pi$-resonance is the known critical
structure and it is \emph{thick} --- not removable by excision
(\texttt{PRIME.PI.RESONANCE.ANATOMY.01}); the polygon/two-gauge
compression is the \emph{wrong} compression of the Loewner
order (\texttt{PRIME.PHASE.POLYGON.01}); the constructive side
is the residual quadrature at full degree
(\texttt{PRIME.RESIDUAL.QUADRATURE.01}) under the complete-comb
constraint (\texttt{PRIME.TRUNCATION.AUDIT.01}).  Arrow (iv)
(a prefix-local scalar reduction) is typed \textsc{needs
proof}.  \emph{Kill/close:} closes if the embedding is proven
on a cofinal family, or if a certified negative canonical
window family is exhibited (none exists:
\texttt{PRIME.DEEPALPHA.SIGN.01}).  Existence is \emph{not}
claimed; no RH statement.
\emph{[2026-08-09 round-37 note
(\veri{v879\_envelope\_partition\_closure.py}): the constraint
set is extended, dated --- the partition class is closed
\emph{entirely} (\texttt{PRIME.PARTITION.CLOSURE.01}: Pr\"ufer
cells at every tested resolution with no predeclared accounting
below $3.568$ vs the bar $1$; honest tight wave-packet frames on
both natural phase spaces with diagonal mass $\le 18\%$ under any
pairing and a growing Schur constant $4.9$--$9.2$), so no
partition-based accounting can certify $\|C\| \le 1$: the
embedding certificate must be \emph{non-decompositional} ---
eigen-enclosure, Loewner-order or passivity arguments that treat
$M(\tilde\mu_+ - \tilde\nu_-)$ whole, all three measured
territory.  The new carried assets: the exact tridiagonal
multiplication identity on the odd frame (CD telescoping at
$1.3\times10^{-13}$, rank-2 boundary defect at $k = 0$), the
derived entrywise bound with $h$-uniform source constants
($C_{\rm int} \in [1.232, 2.328]$ from $h = 168$ to $1445$ ---
the route's first analytic uniformity statement), and their
source-law persistence as $h \to \infty$ as the named open
object.  Demand, kill and close conditions otherwise unchanged;
still [O]; no RH statement.]}\par}

\par\smallskip\noindent\textbf{The Carleson wave of round 36
(\texttt{PRIME.PHASE.POLYGON.01},
\texttt{PRIME.RESIDUAL.\allowbreak QUADRATURE.01},
\texttt{PRIME.PI.RESONANCE.\allowbreak ANATOMY.01}).}
{\sloppy
\emph{Recorded} 2026-08-08 (evidence lines with typed honest
FAILs; no marker moves;
\veri{v876\_carleson\_polygon\_wave.py}, $6/8 + 7/8 + 5/5$, the
three frozen-honest FAILs pattern-gated).  The polygon: Lemma A
exact (the difference of the two source canonical systems
\emph{is} a signed rank-one phase sum; round-trip
$2.4\times10^{-12}$), but $S_0(m) \ge |S_2(m)|$ breaks on all
$47$ rungs at $2\theta \approx 1.5\pi$ --- the two-gauge
compression is the \emph{wrong} compression of the Loewner
order, typed with both readings; holonomy null with inverted
contrast; the deep-holdout rows retype \textsc{numeric-void}
per the round-36 truncation audit (battery verdict unaffected).
The residual horizon law: the difference Jacobi chain
(row-scaled Wheeler $+$ mp escalation past the $53$-bit noise
wall) realizes the positive representing measure $\rho_h$ at
\emph{full degree} $h$ on all $44$ floor-positive rungs
($k^*/h$ median $1.000$, slope $0.974$, $r_h = 0.998$); a
single atom sits just beyond the pole port (mass beyond the
fold is \emph{required}: the Radau-anchored functional breaks
early, genuine); discrimination $7$--$9\times$; the three
sub-$h$ stalls were exactly the truncated windows --- a
validated sign detector.  The resonance anatomy: the resonant
set is \emph{thick} (a constant ${\sim}1/5$ of the minus nodes;
excision gains nothing; the soft direction disjoint) --- the
wall is phase-distributed at this partition resolution; the
$\alpha$-law addendum confounded by truncation, typed.  Full
narrative in the prime-front paper; audit card in
Paper~3.\par}

\par\smallskip\noindent\textbf{The artifact resolution and the
Cotlar reversal of round 36
(\texttt{PRIME.DEEPALPHA.SIGN.01},
\texttt{PRIME.PRUEFER.\allowbreak COMPENSATION.02},
\texttt{PRIME.TRUNCATION.\allowbreak AUDIT.01}).}
{\sloppy
\emph{Recorded} 2026-08-08 (the round's headline;
\veri{v877\_complete\_comb\_reversal.py}, $8/8 + 9/9$).  The
deep-alpha negative floors at kz $142/177/243$ were
\emph{comb-truncation artifacts} of the deployed atom table
($\texttt{ATOM\_MAX} = 4\times10^{5}$, silent cap;
the windows demand support to $1.8\times10^{6}$): with the
complete canonical comb the certified corner enclosures are
strictly positive
($+1.06\times10^{-5}/+1.52\times10^{-5}/+1.20\times10^{-5}$),
no negative canonical window exists, the certified ladder's end
at kz 121 was the atom-table \emph{data} boundary, and
$(H_{\rm cof})$ is unaffected; the silent truncation is typed
an infrastructure bug with the registered follow-up (the
builder must fail loudly beyond its table).  Under the
\emph{identically} frozen v1 bars the complete-comb deep sums
collapse into the battery band and both channels return
\textsc{bounded} (slopes $0.006/0.007$); RUN 3 unlocks and
passes ($K_1/K_2$ $h$-stable on all five holdouts) --- the
route reopens, with the remaining gap typed ($B_C \sim 4$ vs
$1$; local defects $7$--$10$).  The truncation audit re-types
every affected finding, dated (pruefer v1 confounded $\to$
re-decided; residual strengthened; resonance $\alpha$-law void;
polygon holdouts numeric-void; the other waves unaffected).
The dated corrections live on the
\texttt{PRIME.KREIN.CONTRACTOR.01} and
\texttt{PRIME.FLOOR.PAIRCORR.01} blocks above; \texttt{v873}'s
booking stands as the record of the preregistered v1
execution.  No RH statement.\par}

\par\smallskip\noindent\textbf{The round-36 compiler theorems
(\texttt{FLAVOR.FEEDBACK.\allowbreak NORMALFORM.01},
\texttt{E8.WINDING.\allowbreak DECODER.01},
\texttt{E8.SHELL.GLOBAL.01}; Lean:
\texttt{FORM.FLAVOR.\allowbreak FEEDBACK.01} [F],
\texttt{FORM.E8.SHELL.\allowbreak GLOBAL.01} [F]).}
{\sloppy
\emph{Recorded/Registered} 2026-08-08 (exact identity grade; no
marker moves; \veri{v874\_feedback\_decoder\_lean.py} $14/14 +
11/11$, \veri{v875\_shell\_global\_lean.py} $15/15$).  The
feedback normal form: $P = [\,2\cdot\mathbf 1 \mid e_3 \mid
R e_3\,]$ conjugates the whole winding line, symbolic in $s$,
to the gain-plus-companion form with $\chi_{R_s} = (t - s -
|\mu_4|)\,q_{\rm wind} - 12t$ and the honest $\Z_2^2$-not-$\mu_4$
typing load-bearing (keybox in Paper~2).  The winding decoder:
$q_{\rm wind}$ maps the clock alphabet to
$(-|\mu_4|,-|\mu_4|,|Z_2|,h(D_5))$, the decoder polynomial
$(y{+}4)^2(y{-}2)(y{-}8)$ carries budget coefficients
$(2,48,32,256)$, and $|\mu_4|\,q(|Z_2|)/|R(E_8)| = -1/15$ is
exactly the Walsh message eigenvalue --- typed \emph{audit}
theorem, not a functor (keybox in Paper~1).  The global shell
theorem: $\hat\Theta_u/\Theta_L = -1/15$ for \emph{all} odd
shells from $\mathrm{Sp}(4,2)$ transitivity $+$ $(1{+}i)$
doubling $+$ $\Theta_L = 240\,\sigma_3$; the $16500$ census
retained as the exact ward (keybox in Paper~1).  \emph{The Lean
records:} \texttt{TfptCarrier/FlavorFeedback.lean} ($16$
declarations, $11$ theorems --- the action identities, the
inverse-free conjugation $PF_s = R_sP$ over $\Z$ symbolic in
$s$, $\chi_{R_s}$ in $\Z[X]$, the SNF determinantal divisors)
and \texttt{TfptCarrier/GaussianShells.lean} (the
\texttt{ShellSystem} hypothesis bundle with provenance typed on
each field, named \emph{not} proven, $+$ $7$ consequence
theorems, every statement an exact integer identity, no
division) --- both kernel-checked: lake build green at $3418$
jobs, no \texttt{sorry}, no \texttt{native\_decide}, axioms
propext/Classical.choice/Quot.sound on all $18$ theorems
(\texttt{\#print axioms}), carried in the shipped Lean manifest
at $90$ files.  \emph{The honest boundaries:} no $\mu_4$
identification from the $\Z_2^2$ quotient; the decoder needs an
intertwiner to be causal; the shell theorem's three inputs stay
named hypotheses.  No RH statement.\par}

\par\smallskip\noindent\textbf{The ledger integrity guard and
the \texttt{FORM.QGEO.04} QA fix (\texttt{GATE.LEDGER.01}).}
{\sloppy
\emph{Recorded} 2026-08-08 (a QA/CI note, no new physics;
\veri{v878\_ledger\_integrity.py}, $10/10$).  The round-36
ledger audit found the first \emph{primary-key violation} in
\texttt{status\_ledger.csv}: the claim id \texttt{FORM.QGEO.03}
was double-booked (2026-06-15 the CohomologyGrading
formalisation; 2026-06-18 the SeamDeckClosure flat all-orders
closure).  The fix, dated: the SeamDeckClosure row is renamed
\texttt{FORM.QGEO.04} with a QA note in its claim text, and
every flat-closure reference is updated (this document, the two
2026-06-18 changelog mentions, \texttt{v279}/\texttt{v284}/%
\texttt{v286} $+$ \texttt{seam\_equivalence.json},
\texttt{make\_figures}, the website residual chain, registry
and \texttt{run\_all}); the CohomologyGrading row keeps
\texttt{FORM.QGEO.03}; claim contents unchanged.  The guard
enforces from this round: globally unique claim ids (the fix
pinned), existing supersession targets with an acyclic
supersession graph, unique \emph{active} supersession targets
modulo a frozen remove-only allowlist (the two legitimate
2026-06 joint closures), and the dependency cross-reference
census frozen remove-only ($13$ SCCs / $265$ ids ---
acyclicity enforced as \emph{no new cycles}; the
\texttt{audit\_baseline} precedent).\par}

\par\smallskip\noindent\textbf{The partition closure of round 37
(\texttt{PRIME.PARTITION.\allowbreak CLOSURE.01}; dated notes on
\texttt{PRIME.CARLESON.\allowbreak PRIME.01} and
\texttt{PRIME.PRUEFER.\allowbreak COMPENSATION.02}).}
{\sloppy
\emph{Recorded} 2026-08-09 (the compact closing round of the
reopened Cotlar route;
\veri{v879\_envelope\_partition\_closure.py}, $7$ checks with the
two frozen-honest FAILs T1.4/C.1 pattern-gated at exit 1 $+$
$6/6$; the contract chain \texttt{4621b899\ldots} $\to$
\texttt{5fd6bf61\ldots} $\to$ \texttt{27d9f0a3\ldots} $\to$
\texttt{2199e9b1\ldots} machine-verified at run time).  The
analytic core, saved: multiplication by $\cos\theta$ is
\emph{exactly} tridiagonal on the odd frame (mirror-closed
boundary, single rank-2 defect at $k = 0$; the CD telescoping
identity at $1.3\times10^{-13}$ on every rung incl.\ all five
complete-comb holdouts), and the derived pointwise bound carries
\emph{$h$-uniform source constants} ($C_{\rm int} \in [1.232,
2.328]$ from $h = 168$ to $1445$, slope $+0.106$;
$\sqrt{(G_+^{-1})_{00}} \in [0.661, 0.899]$) --- the route's
first analytic uniformity statement, at entrywise grade.  The
structural closure (the honest FAILs are the finding): the
cell-envelope decay is \emph{cancellation-carried}, provably not
entrywise-derivable (every Pr\"ufer cell contains near-diagonal
pairs; the analytic envelopes blow up as $h^2$); no predeclared
accounting below $3.568$ (bar $1$); $\sum_r \varepsilon_r =
6.9$--$12.9$ $\alpha$-uniform on clean complete-comb data; and
the honest tight wave-packet frames (both natural phase spaces;
$\|B\| = \|C^G\|$ exact) leave diagonal mass $\le 18\%$ under
any pairing with a Schur constant growing $4.985 \to 9.222$
(slope $+0.295$), while the Gabor envelope does decay in phase
space (the first partition to show decay) and the discrimination
is massive (truth $4.985$ vs Epstein $128.7$ vs scramble
$9006.6$).  \emph{The consequence:} the cancellation is
\emph{atomically global} --- no partition-based accounting
(cells, packets, any frame decomposition of this family) can
certify $\|C\| \le 1$; the certificate demand of
\texttt{PRIME.CARLESON.PRIME.01} is retyped
\emph{non-decompositional} (eigen-enclosure / Loewner-order /
passivity --- all three measured territory); wave-packet
partitions and entrywise envelope bounds are stop-listed on the
\texttt{PRIME.PRUEFER.COMPENSATION.02} block.  The surviving
assets: the tridiagonal identity, the $h$-uniform derived
constants, their source-law persistence (the named open object).
Full narrative in the prime-front paper; audit card in
Paper~3.  No RH statement.\par}

\par\smallskip\noindent\textbf{The rounds-38/39 sandbox campaign
(the operator geometry and the source law of the wall).}
{\sloppy
\emph{Recorded} 2026-08-09 (a campaign note; $25$ frozen sandbox
probes, every spec amendment fail-first on record; the promoted
verification modules
\veri{v880\_finite\_anchor\_theorems.py},
\veri{v881\_carleson\_port\_geometry.py} and
\veri{v882\_port\_source\_mellin.py} embed their discovery probes
byte-exact).  The campaign gives the Carleson wall its exact
operator geometry (rank-2 Jacobi displacement, gauge-exact
Carleson Gram with the classical testing diagonal, exact port
Schur reduction --- the port \emph{is} the wall --- and the
preserved IIKS integrable class), its universal source law (the
$\sqrt{n/X}$-uniformized prime measure, the PNT edge law $1 -
e^{-1/2}$ as the ${\sim}39\%$ port mass, the Mellin--Cauchy port
symbol, the closed criticality budget), and the hardness decision
(\texttt{PRIME.ERRORTERM.SCALE.01} below); the six ledger-only
items of the campaign are registered as the blocks that follow.
Full narrative in the prime-front paper; audit cards in Paper~3.
No marker moves; no RH statement.\par}

\par\smallskip\noindent\textbf{The error-term scale decision
(\texttt{PRIME.ERRORTERM.\allowbreak SCALE.01}, decided).}
{\sloppy
\emph{Recorded} 2026-08-09, DECIDED (sandbox probe
\texttt{errorterm\_demand\_curve\_probe.py}; the demand-curve
measurement behind the round-39 hardness statement).  \emph{The
finding:} the wall margin equals injected off-critical
perturbation \emph{energy} on the $\tau$ law exactly (ratios
$1.00$--$1.10$ across the tested grid); the perturbation
amplitude scales as $A^{*} \sim \sqrt{\tau}/\delta^{2}$ under the
frozen convention $\Delta\lambda = O(A^{2}\delta^{4})$; and the
kill localizes through the port block.  \emph{The decision:} the
reduction chain of the wall is an \emph{exact reformulation at RH
strength} --- there is no unconditional slack anywhere in it; the
identities are exact, and the one-sidedness is open at exactly
that strength.  Recorded as a hardness decision, not a contract
demand.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The port limit contract
(\texttt{PRIME.PORT.\allowbreak LIMIT.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN (sandbox probes
\texttt{port\_limit\_operator\_probe.py},
\texttt{port\_cocycle\_window\_probe.py},
\texttt{weyl\_m\_criticality\_probe.py}).  \emph{Measured:} the
compressed operator-norm convergence has slope $-2.80$ carrying
the wall exactly; $\lambda_{\max}$ increases to $1$ from below;
the atom-at-edge Weyl-$m$ class has $\sigma \to -1$ and $w_1 \to
0.72$.  \emph{The demand:} the fixed-space limit theorem --- the
existence and identification of the limiting port operator on a
fixed space, with the wall carried through the limit.
\emph{Kill:} a measured break of the convergence slope or a rung
family on which the compressed operator fails to carry the wall.
No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The scalar port inequality
(\texttt{PRIME.PORT.\allowbreak SCALAR.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN (sandbox probe
\texttt{port\_scalar\_schur\_probe.py}).  \emph{Measured:} the
wall is \emph{one} scalar inequality,
\[
\sigma_h \;=\; \big[\,1 - \tilde\nu_{m^{*}}K_h(y^{*},
y^{*})\,\big] - b^{\mathsf T}(I - E_{\rm rest})^{-1}b \;\ge\; 0,
\]
exactness $6.2\times10^{-11}$ on $42$ rungs, with $\sigma_h$
tracking $\tau_h$.  \emph{The demand:} one-sidedness --- a proof
that $\sigma_h \ge 0$ along a cofinal window family.  \emph{Kill:}
a rung on which the scalar reduction fails at certificate grade.
No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The port cocycle window
(\texttt{PRIME.PORT.\allowbreak COCYCLE.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN (sandbox probe
\texttt{port\_cocycle\_window\_probe.py}).  \emph{Measured:} a
fixed $12\times12$ window carries the wall exactly, with
compressed convergence slope $-2.80$.  \emph{The demand:} the
closed-form limit of the window cocycle.  \emph{Kill:} a depth at
which the fixed window stops carrying the wall.  No marker move;
no RH statement.\par}

\par\smallskip\noindent\textbf{The Case sum-rule contract
(\texttt{PRIME.CASE.\allowbreak SUMRULE.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN (sandbox probes
\texttt{case\_sum\_rule\_probe.py},
\texttt{testing\_mnt\_law\_probe.py}).  \emph{Measured:} the
deployed Jacobi chains are measured Killip--Simon class.
\emph{The demand:} prove a Case C0/C1 sum rule implying the
unconditional testing bound $T_h \le 1$; the target is exactly
the closed criticality budget of round 39 (PNT growth compensated
by window geometry plus Christoffel).  \emph{Kill:} a measured
departure of the deployed chains from the Killip--Simon class.
\emph{Route update (2026-08-09, round CVIII) --- retyped
conditional:} the free-tail bridge is numerically delivered
(sandbox \texttt{freetail\_case\_bridge\_probe.py}, $8/8$: the
finite Killip--Simon P2 identity exact at $5\times10^{-11}$; the
P2 energy $h$-uniformly bounded $0.63$--$0.75$ over all $42$
rungs), \emph{but} the route is retyped conditional by
\veri{v889\_route\_decisions.py}: \textsc{pnt-insufficient} (the
smooth reference violates the port margin on every rung) and
\textsc{homotopy-indefinite-arithmetic} (the required input is
the signed pair-correlation functional).  The demand becomes:
state and prove the weighted prime-pair correlation inequality,
or accept conditionality.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The port Riemann--Hilbert contract
(\texttt{PRIME.PORT.\allowbreak RIEMANNHILBERT.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN (sandbox probes
\texttt{port\_riemann\_hilbert\_setup\_probe.py},
\texttt{mellin\_model\_operator\_probe.py}).  \emph{Measured:}
the discrete $2\times2$ IIKS Riemann--Hilbert problem of the
dressed port operator is \emph{well-posed} --- the two generators
are explicit and the resolvent closure holds at
$1.2\times10^{-12}$.  \emph{The demand:} nonlinear steepest
descent for $h \to \infty$.  \emph{The calibration constraint
(load-bearing):} the smooth PNT continuum model \emph{violates}
the floor at finite depth ($\tau_{\rm model}$ $-1.26 \to -0.11$)
--- the prime discreteness is load-bearing, and the unperturbed
problem must keep the lattice.  \emph{Execution note (2026-08-24, rounds 244--255):} the
finite formulation this demand must consume is now certified
--- the bordered end-object is a canonical $2\times2$ RHP
$\ltimes$ a one-dimensional budget cocycle with exact
evaluation infrastructure
(\veri{v958\_bordered\_tau\_readout\_dictionary.py}); the
irreducible-$3\times3$ steepest-descent variant is dropped
without replacement, and the admissible target form is an
\emph{integrated} statement over the compressed CD readout
(no uniform $Y_n$ parametrix) --- the campaign continues as
\texttt{PRIME.PORT.RHP.FULLSOURCE.\allowbreak BASEFIBER.01}
(the block below).  This umbrella stays [O].  \emph{Kill:} a
failure of the
IIKS well-posedness on a deployed rung.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The rounds-40/41 flow corridor
(the determinant chain, the parametrix decision, the certified
head, and the complete-comb anatomy).}
{\sloppy
\emph{Recorded} 2026-08-09 (a campaign note; nine frozen sandbox
probes, every spec amendment fail-first on record; the promoted
verification modules
\veri{v883\_tau\_chain\_parametrix.py},
\veri{v884\_certified\_head\_positivity.py},
\veri{v885\_ladder\_flow\_anatomy.py} and
\veri{v886\_register\_gapcode\_null.py} embed their discovery
probes byte-exact).  The campaign gives the wall's sign an exact
finite determinant chain ending in a candidate tau function, the
decided parametrix constraint (the Euler-product comb is
globally rigid --- every smoothing-based parametrix is dead),
the first rigorous positivity statement of the program (exact
integer Sylvester certificates on the three head rungs), the
complete flow/induction anatomy of the ladder (degree-2 Lax
closure; no monotone one-prime induction in any insertion order
--- positivity is a property of the complete comb), and the
register-gapcode disenchantment.  The registrations and updates
follow as the blocks below.  Full narrative in the prime-front
paper; audit cards in Paper~3.  No marker moves; no RH
statement.\par}

\par\smallskip\noindent\textbf{The tau contract
(\texttt{PRIME.PORT.\allowbreak TAU.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN --- the finite-$h$
determinant chain delivered
(\veri{v883\_tau\_chain\_parametrix.py}, $5/5$,
\textsc{tau-chain-exact}).  \emph{Exact:} the Schur determinant
identities $\sigma_h = \det(I - E)/\det(B)$ and $\det(I - E) =
\det(I - R)\det(I - D_P)$ are machine-exact
($4.8\times10^{-13}$ in log space) with positive bulk factors,
hence $\operatorname{sign}\tau_h = \operatorname{sign}\sigma_h$
= the sign of the wall with $\tau_h := \det(I - D_P)$; the
Jacobi variational identity $\frac{d}{ds}\log\det(I - sD_P) =
-\operatorname{tr}[(I - sD_P)^{-1}D_P]$ holds on the true family
($10^{-6}$); the determinant wall indicator is false on Epstein
and scramble.  \emph{Measured:} the $\kappa/\rho$ readout ---
$\operatorname{slope}(\log\sigma) = -2.93$ vs $2\times$ the
Mellin fluctuation slope $-2.53$ ($\sigma \sim$ fluctuation$^2$
within $16\%$).  \emph{The demand (the symbolic half):} $\tau_h$
= the tau function of the discrete $2\times2$ IIKS
Riemann--Hilbert problem of the \texttt{v881} generators, for
arbitrary $h$ without numerics.  \emph{Execution note
(2026-08-24, rounds 224--226):} the demand is \emph{executed at
the finite-identity level} --- Sylvester as a full
characteristic identity, the $s$-dressed Schur family, the
confluent diagonal, the IIKS dressing identity with
source-explicit dressed generators, the Fredholm/JMU variational
identity in two independent times and the relative tau transfer,
all machine-exact on the actual builders
(\textsc{tau-iiks-exact}, $18/18$;
\veri{v955\_tau\_iiks\_toda\_dictionary.py}); per the note-DLI
claim splitting the finite theorem set is carved out as
\texttt{PRIME.PORT.TAU.FINITE.IIKS.01} [E] (the block below) and
the sign question stands alone as
\texttt{PRIME.PORT.TAU.NOPOLE.COFINAL.01} [O]; this umbrella
stays [O] for the fully symbolic arbitrary-$h$ statement.
\emph{Kill:} a heavy rung on
which the determinant chain fails at certificate grade.  No
marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The finite IIKS/Toda theorem set
(\texttt{PRIME.PORT.TAU.FINITE.\allowbreak IIKS.01} [E]).}
{\sloppy
\emph{Registered and delivered} 2026-08-24 (rounds 224--226,
notes DL/DLI/DLII;
\veri{v955\_tau\_iiks\_toda\_dictionary.py}, probes embedded
byte-exact, $18/18 + 15/15 + 17/17$ first pass).  \emph{Exact:}
the wall determinant is the exact IIKS Fredholm tau function at
the finite-identity level (Sylvester characteristic identity on
the whole $s$-grid; the $s$-dressed Schur family $\det(I - sE) =
\det(I - sR)\det(I - s\,D_P(s))$ with the fixed-kernel alias
named and blocked; the confluent diagonal $\le
3.2\times10^{-13}$; the IIKS dressing identity $\le
7.9\times10^{-13}$ with the dressed port generators
source-explicit in closed form; the Fredholm/JMU variational
identity in two independent times; the relative tau transfer
exact over $-212.84/-195.50$ log-unit collapses); the
$h$-direction carries an exact \emph{predictive} degree-1 Lax
dynamics (rank-1 $h$-step at $2.9\times10^{-16}$; blind
next-generator and tau-step predictions at $3.4\times10^{-16}$ /
$3.2\times10^{-11}$; zero curvature identically, world-blind;
mpmath wards at dps $80/120$); and the Toda dictionary holds:
$\tau_{w,n} = D_n(\mu - \nu)/D_n(\mu)$, $r_n = h_n(\mu -
\nu)/h_n(\mu)$, the Hirota coefficient source-pure from a scaled
signed Stieltjes recursion.  \emph{Typed honestly:} the sign is
\textsc{wall-equivalent} ($\hat\gamma_n > 0$ for all $n$ $\iff$
quasi-definiteness of the signed defect measure $\mu - \nu$
$\iff$ the wall; MAIN carries all $184$ $r_n > 0$, the controls
flip at $n = 25/21/27$); the $s$-time has no fixed degree
(\textsc{conditioning-only}) and across rungs there is no common
carrier; the general-$h$ statements are classical theorems
(Sylvester, Schur, CD confluence, IIKS dressing) instantiated on
the actual builders.  \emph{Kill:} any of the finite identities
failing on a deployed rung.  No RH statement.\par}

\par\smallskip\noindent\textbf{The tau sign contract
(\texttt{PRIME.PORT.TAU.NOPOLE.\allowbreak COFINAL.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-24, OPEN (the honest open half of the
note-DLI claim splitting).  \emph{The demand:} $\tau_h(1) > 0$
cofinally --- equivalently, no Malgrange divisor of the
$s$-family up to $s = 1$.  \emph{Equivalent coordinates (all
machine-exact dictionaries,
\veri{v955\_tau\_iiks\_toda\_dictionary.py} /
\veri{v956\_signedmoment\_halffilling\_duality.py}):}
quasi-definiteness of the signed defect measure $\mu - \nu$
through the full free moment window; all Christoffel scalars
$r_n > 0$; all Jacobi couplings $\beta_n > 0$ in every one of
the four source-pure coordinate paths; the positive orientation
of the signed Stokes multiplier of the two-sided midpoint
problem.  \emph{Measured seeds:} MAIN survives the entire free
moment window plus $O(1)$ forced-tail degrees (offsets
$0/2/2/3/1$) while the controls die at $11$--$15\%$ of it;
$\lambda_{\max}(Q_{N_w}) = 0.999832..0.999999 < 1$.  \emph{Kill
(of any mechanism candidate):} failing the frozen micro
falsifier --- predict the forced tail $0/2/2/3/1$ \emph{and} the
control flips $25/21/27$ blind.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The parametrix decision
(\texttt{PRIME.PORT.LATTICE.\allowbreak PARAMETRIX.01},
decided).}
{\sloppy
\emph{Registered and decided} 2026-08-09
(\veri{v883\_tau\_chain\_parametrix.py}, $3/3 + 3/3$,
\textsc{parametrix-measured (fluctuations-required)} $+$
\textsc{zones-measured (both-required)}).  \emph{Measured:}
four worlds through the identical pencil pipeline on every heavy
rung --- true comb $\tau = +1.7\times10^{-4}$ to
$+6.7\times10^{-7}$ (ward); lattice with fully smooth quadrature
masses $-78$ to $-1706$; lattice with exact local
$\pm\tfrac12$-log-unit mass sums $-78$ to $-5537$; plain
continuum $-0.11$ to $-1.26$ (ward) --- and the zone refinement
kills both ways (edge-only smoothing $-3.7$ to $-22.1$;
interior-only $-77$ to $-7551$).  \emph{The decision (a design
constraint for every RH analysis of the wall):} the exact
$\Lambda$-mass-position pairing --- the multiplicative
Euler-product structure --- is load-bearing in both zones; the
main/unperturbed object must carry the full von Mangoldt comb;
every smoothing-based parametrix is dead; the small parameters
live exclusively in the flow/deformation direction (ladder
order, isomonodromy/tau, $s$-deformation).  No marker move; no
RH statement.\par}
...
delivered).}
{\sloppy
\emph{Registered and delivered} 2026-08-09
(\veri{v884\_certified\_head\_positivity.py}, $5/5$,
\textsc{certified-head}; SPEC v2 amendments on record).
\emph{Proven:} $\sigma_h > 0$ on kz 9 ($h = 184$), kz 12 ($151$)
and kz 13 ($168$) --- the first rigorous positivity statement of
the program: independent \texttt{mpmath} rebuild of the window
lags (dps $60/100$; conservative per-entry error $\varepsilon_c
= 10\times$ dps-disagreement; cross-ward at $1.1\times10^{-14}$,
which caught a sign bug in the rebuilt Arch near-branch), matrix
shift by the total error budget (${\sim}2\times10^{-18}$,
${\sim}14$ orders below the margins), then exact integer
Bareiss/LDL on the $Q = 10^{20}$ grid: all pivots positive
(Sylvester) $\Rightarrow K \succ 0 \Rightarrow \sigma_h > 0$ via
the exact \texttt{v866} equivalence; certified floors
$\lambda_{\min} \ge 3.633/3.327/3.842\times10^{-4}$; the Epstein
control through the identical machinery refused at pivot index
$10$.  \emph{Typed honestly:} proven modulo the declared
conservative transcendental-evaluation error model
(validated-numerics standard); the ball-arithmetic closure and
the $42$-rung rollout were the named follow-ups --- both since
delivered (\veri{v887\_certified\_ladder\_complete.py},
\veri{v897\_certified\_interval\_ladder.py}: the informal error
model is retired from the ladder).  \emph{Kill:} a certified
pivot failure on a head rung under the frozen error
model.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The leading-sign contract
(\texttt{PRIME.PORT.LEADING.\allowbreak SIGN.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN (the moonshot of the
round-40 review).  \emph{The demand:} $\sigma_h = \kappa\rho_h +
o(\rho_h)$ with a closed-form $\kappa > 0$ via a relative
Case/Killip--Simon identity --- the one-sidedness of the scalar
wall as an asymptotic law.  \emph{Measured seeds:} the $\rho$
candidate is the squared lattice fluctuation scale ---
$\operatorname{slope}(\log\sigma) = -2.93$ vs $2\times$ the
Mellin fluctuation slope $-2.53$ (within $16\%$;
\veri{v883\_tau\_chain\_parametrix.py}); the parametrix decision
forces the $h$-ladder direction --- $D_P(h)$ as sections of one
arithmetic limit object, $\kappa$ from the cocycle increment
structure, not from comb deformations.  \emph{Kill:} a measured
break of the $\sigma \sim$ fluctuation$^2$ law on the deep
ladder.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The tail contract
(\texttt{PRIME.PORT.\allowbreak TAIL.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN (the final composition
contract of the port route).  \emph{The demand:} $|\sigma_h -
\kappa\rho_h| \le (\kappa/2)\rho_h$ for all $h \ge H$ explicit
(the tail half), plus the finite head by certified numerics,
plus the Lean composition through
\texttt{krein\_cofinal\_weil}.  \emph{Head status:} delivered
for kz 9/12/13 (\texttt{PRIME.PORT.CERTIFIED.HEAD.01},
\veri{v884\_certified\_head\_positivity.py}); the rollout to all
$42$ head rungs and the ball-arithmetic closure are the named
next steps; the tail half waits on
\texttt{PRIME.PORT.LEADING.SIGN.01}.  \emph{Promotion note
(2026-08-09, round CVIII):} the rollout is \emph{complete} ---
$\sigma_h > 0$ proven on all $42$ reachable rungs, promoted in
\veri{v887\_certified\_ladder\_complete.py}
(\texttt{PRIME.PORT.CERTIFIED.LADDER.01}; the $40$-rung parent
rollout stays sandbox, runtime-excluded); the remaining named
steps are the ball-arithmetic closure and the Lean composition.
\emph{Interval note (2026-08-09, round CXV):} the
ball-arithmetic closure is \emph{delivered} ---
\veri{v897\_certified\_interval\_ladder.py}
(\texttt{PRIME.PORT.BALLLADDER.01}) re-proves all $42$ reachable
rungs with rigorous interval-arithmetic shifts ($15$
exact-rational $+$ $27$ validated-precision) and retires the
informal lag-evaluation error model; the remaining named steps
are the Lean composition and the asymptotic tail.
\emph{Kill:} a certified
negative $\sigma_h$ on any head rung.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The isomonodromy-flow measurement
(\texttt{PRIME.PORT.\allowbreak ISOFLOW.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v885\_ladder\_flow\_anatomy.py}, $8/8$,
\textsc{isoflow-measured (flow-open)}).  \emph{Measured:} the
gauge-fixed IIKS generator flow over $41$ consecutive ladder
pairs closes in degree 2 (residual medians $0.244/0.248$ with
$\{Y^2 f, Y^2 g\}$; degree 1 fails at $0.498/0.457$) ---
isomonodromy-compatible, gauge stable (consecutive cos median
$0.943$), $\log\tau$ monotone $-58.6 \to -396.6$.  \emph{Typed
honestly:} the controls close similarly --- degree-2 closure is
a structural precondition, not a discriminator; the arithmetic
stays in the value.  \emph{The demand:} the closed-form degree-2
deformation polynomial in a well-conditioned coordinate (the
\texttt{LAX2} block below).  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The window-ladder transfer
(\texttt{PRIME.PORT.LADDER.\allowbreak TRANSFER.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v885\_ladder\_flow\_anatomy.py}, $5/5$,
\textsc{ladder-measured (perturbative $+$ non-monotone)}).
\emph{Measured:} over $40$ consecutive steps the first-order
transfer is marginally accurate (median $0.458$ at bar $0.5$)
--- the step is first-order graspable --- but raw $h$ order is
not a natural flow (only $52\%$ of steps increase $\lambda$;
$\|\Delta\|/\mathrm{gap} = 594$--$2026$: the accuracy comes from
port concentration, not step smallness).  \emph{The typed
lesson:} the induction needs a better order than $h$; the nested
one-prime ladder was the named candidate and is decided in the
block below.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The one-prime ladder decision
(\texttt{PRIME.PORT.\allowbreak PRIMELADDER.01}, decided).}
{\sloppy
\emph{Registered and decided} 2026-08-09
(\veri{v885\_ladder\_flow\_anatomy.py}, $13/13$,
\textsc{primeladder-measured}).  \emph{Measured (exact per-atom
lag rows; linearity warded at $0$; endpoint warded vs the true
floor at $2.5\times10^{-9}$):} the pure-Arch start is
catastrophically subcritical ($\tau(0) = -9.9\times10^{13}$ at
kz 9; $G_+$ not even PD at kz 13/26/40); insertion crosses into
positivity only at $u^{*}/U = 0.991$--$0.999$ in every insertion
order; the edge-first ordering reaches stable positivity only at
$100\%$ of the atoms (the ${\sim}39\%$ edge mass alone does not
carry the wall); per-atom increments $54$--$64\%$ positive; the
deterministic mass scramble drops the endpoint to $-474.7$
(control fires).  \emph{The decision:} positivity is a property
of the complete comb --- \texttt{v877}'s truncation reversal
sharpened to atom resolution; no monotone one-prime induction
exists in any order; induction must run in a flow coordinate,
not in comb content.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The degree-2 Lax coefficient flow
(\texttt{PRIME.PORT.\allowbreak LAX2.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v885\_ladder\_flow\_anatomy.py}, $11/11$,
\textsc{lax2-measured (flow-noisy / lax2-partial)}).
\emph{Measured:} the twelve deformation-polynomial coefficients
are extracted on all $41$ ladder steps, but normalized by the
$\alpha$ step the coefficient series behave as noise (median
roughness $1.365$ vs lawful bar $0.7$); the operator-level IIKS
prediction misses the bar narrowly ($0.350$ vs $0.3$); the
\emph{integrated} tau-transfer identity $\log\det(I + (I -
D_k)^{-1}\Delta D)$ holds exactly (warded at
$5.6\times10^{-11}$).  \emph{The named lead:} the dictionary
columns $f, Yf, Y^2 f$ are nearly collinear --- the raw
coefficient basis is ill-conditioned; a well-conditioned
reparametrization may expose a lawful flow the raw basis masks.
\emph{The demand:} the flow in a conditioned coordinate; then
couple to the \texttt{PRIME.PORT.TAU.01} variational identity.
\emph{Adjudication note (2026-08-24, round 225):} the demand is
adjudicated --- \textbf{the word ``2'' was a hypothesis}: with
the canonical basis-invariant conditioning the true minimal
degree in the closing direction is \emph{one}, and the closing
direction is $h$, not $s$ (\textsc{lax1-h-exact}: rank-1
$h$-step, blind-predictive degree-1 connection, zero curvature
world-blind; the $s$-time has no fixed degree,
\textsc{conditioning-only}, and across rungs there is no common
carrier --- \veri{v955\_tau\_iiks\_toda\_dictionary.py},
\texttt{PRIME.PORT.TAU.FINITE.IIKS.01} [E]).  Per the contract's
no-go rule, no further Lax cosmetics in the $s$/across
directions.  \emph{Kill:} a conditioned basis in which the flow
remains noise at the frozen bars (executed: the $h$-basis
closes exactly).  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The half-filling / free-moment
theorem set (\texttt{PRIME.FREEMOMENT.\allowbreak JFRACTION.01}
[E] $+$ \texttt{PRIME.JACOBI.DUAL.\allowbreak REVERSAL.01} [E])
and the positive-prefix contract
(\texttt{PRIME.FREEMOMENT.\allowbreak POSITIVEPREFIX.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-24 (rounds
228--231, notes DLIV--DLVII;
\veri{v956\_signedmoment\_halffilling\_duality.py}, probes
embedded byte-exact, $14/14 + 17/17 + 17/17 + 13/13$ first
pass).  \emph{Exact (JFRACTION):} the half-filling law $N_w =
\lceil \#\mathrm{supp}(\tilde\mu)/2 \rceil$ on all five windows
with the wall dying immediately past the cap (first $r$-flip at
$N_w + 0/2/2/3/1$ --- the wall is \emph{maximal}); the
complement identity and $h$-duality at dps 80; the free moment
window law (the largest Hankel window inside the $S$ free
moments is $n = (S+1)/2 = N_w$; MAIN exhausts the entire free
window, the controls die at $11$--$15\%$); the inertia theorem;
the four-path dictionary (moment prefix $=$ Jacobi chain $=$
Euclidean chain $=$ value chain, exact in rationals; Epstein's
first wrong Euclidean pivot exactly at its flip $n = 25$).
\emph{Exact (DUAL.REVERSAL):} the full chain reversal
($\alpha^{\#}_m = \alpha_{S-1-m}$, $\beta^{\#}_m =
\beta_{S-m}$; \textsc{no-source-reversal} --- not a builder
transformation) and the $L$-gauge midpoint connection theorem
($Y^{\#}_{N-1+l} = J\,Y_{N-1-l+1}\,W$, $W =
\mathrm{antidiag}(1/L, L)$, built from the node polynomial
alone; exact in rationals, real w9 at $9.1\times10^{-94}$, dps
100); the gauge is $h$-free --- the wall's orientation sits
exclusively in the $h$-chain it does not see
(\textsc{signed-stokes-wall-equivalent}).  \emph{The open
contract (POSITIVEPREFIX):} $\beta_n > 0$ through the entire
free moment window as a theorem with an identified mechanism ---
the wall itself in free-moment coordinates; half-filling is the
\emph{location}, its positive reachability is the result to be
proven; named sub-deliverables: the source-pure scalar
normalization beyond the node-log pole remover, and a
source-side critical filling $t_c$ as output.  \emph{Kill (of
any mechanism candidate):} the frozen micro falsifier --- a
parametrix must predict the forced tail $0/2/2/3/1$ and the
control flips $25/21/27$ blind; a model that only paints the
positive side is a fit, not a mechanism.  Design rules
unchanged: the full von Mangoldt comb stays in the main
problem, no PNT smoothing.  No marker move beyond the two [E]
registrations; no RH statement.\par}

\par\smallskip\noindent\textbf{The bordered-RHP / tau-readout
theorem set (\texttt{PRIME.PORT.RHP.BORDERED.\allowbreak
READOUT.01} [E]) and the full-source campaign contract
(\texttt{PRIME.PORT.RHP.FULLSOURCE.\allowbreak BASEFIBER.01}
[O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-24 (rounds
244--255, notes DLXXIII--DLXXXV;
\veri{v958\_bordered\_tau\_readout\_dictionary.py}, probes
embedded byte-exact, $23/23 + 29/29 + 24/24 + 20/20 + 22/22 +
26/26$ first pass, plus the module-own exact-rational PSD base
theorem).  \emph{Exact (BORDERED.READOUT):} the bordered-RHP
dictionary (the Uvarov border column carries $F_n$ as its
$z^{-1}$ coefficient; the budget is a CD-kernel functional of the
terminal $Y_n$ data alone, $S_{n-1} = \iint K_n\,d\sigma\,
d\sigma$; the Uvarov tau step and the $T$-sourced bordered
\textsc{lax1} flow, mp-exact through all $184$ w9 degrees); the
Schlesinger rank-1 insertion ($\det Y_3 = -h_{n-2}F_{n-1}$; IIKS
generator rank exactly $2$; canonical $2\times2$ RHP $\ltimes$
one-dimensional budget cocycle carrying the full pivot chain ---
the irreducible-$3\times3$ campaign dropped without replacement);
the centering congruence ($B^{\circ} = B - \rho_0$, exact in
rationals), the $\sigma^{\circ}$ dictionary, the dual-norm
identity $Q_m = t^{T}H^{-1}t$ and the tail-kernel compression
$T = \iint[K_N - K_8]\,d\sigma^{\circ}d\sigma^{\circ}$ (dps 160
through $N = 184$, rel $3.3\times10^{-160}$); the three exact
error formulas ($\delta h_n = (R_1)_{12}$; the central rest
identity; the error-kernel sandwich at $1.1\times10^{-81}$); the
contour formula $R_1 = \tfrac{1}{2\pi i}\oint(R(z) - I)\,dz$
($3.2\times10^{-14}$); the exact augmented tau quotient $D =
B^{\circ} - Q_7 - T = \tau^{\rm aug}/\tau$ at full depth (three
routes, $10^{-11}$ / mp $10^{-16}$) with the pole classification
(rank-1 border poles removable by residue closure; the $D$-layer
essentially singular, $\exp(c/(z-s))$ type); and the PSD base
theorem (positive basis $\Rightarrow$ Gram PSD $\Rightarrow$
border readout $\ge 0$; bordered PSD $=$ wall $+$ budget by exact
Schur complement; the frozen signed witness fires ---
positive-basis structure alone proves the wrong statement).
\emph{Typed honestly (measurements, not identities):} the corner
census (\textsc{corner-imported-only}, PSD $0/42$ for all three
sealed candidates) and the head-heavy budget profile
(\textsc{irregular-bulk}, Gini $0.91$); amplification extensive
($\mathfrak{A} \sim N^{1.01}$), annihilation neutral, the fiber
fully in the error; the gauge thesis refuted
(\textsc{no-global-gauge}, \textsc{model-orientation-blind} ---
\textsc{base-blocked-by-drifting-layer} on record); the diagonal
pairing thesis refuted (diagonal $19\%$, wrong sign on scramble;
\textsc{rel-structureless}, \textsc{pairing-extensive}).
\emph{The open contract (FULLSOURCE.BASEFIBER, the campaign;
the coupled-tau dictionary and telescope certified in
\veri{v959\_coupledtau\_terminal\_dictionary.py}; carried at the
wave-4 state since 2026-08-25 --- surface and orientation
certified in \veri{v960\_terminal\_surface\_closure.py} and
\veri{v961\_midpoint\_orientation\_dictionary.py}, block
below):} per the
round-256 causal decision the separate treatment of base and
fiber is \emph{artificial} --- the campaign object is the
indivisible full-source pair $(\tau, \tau^{\rm aug})$ with $D =
\tau^{\rm aug}/\tau > 0$ plus the no-pole statement, on the full
von Mangoldt comb; admissible target form stays an
\emph{integrated} statement over the compressed CD readout (no
uniform $Y_n$ parametrix; the v959 coupled recursion is the
one-dimensional budget cocycle of the v958 formulation in closed
form).  \emph{The precise localization after rounds 256--259:}
\textbf{the last fiber edge is a positive tau cross-ratio} ---
the entire fiber positivity is the one terminal inequality
$h_{N-1}/F_{N-1}^2 > 7/5$ (equivalently $q_N < 1$; the $5/7$
floor typed \textsc{floor-imported}, derivation open; the
measured driver is the rank-perfect $F$-$h$ covariance $+1.000$:
the ratio is the object) --- and \textbf{the last base edge is a
globally oriented prefix resummation} --- the determinant sign is
carried by coherent cancellation over the configuration ensemble
(a resummation gap, not a selection gap).  The two edges are
registered as the successor contracts
\texttt{PRIME.PORT.COUPLEDTAU.\allowbreak
TERMINAL\_CROSSRATIO.01} [O] and
\texttt{PRIME.PORT.FULLSOURCE.\allowbreak
PREFIX\_RESUMMATION.01} [O] (blocks below; wave-4 state: the
42-rung surface of the fiber edge is \emph{certified} in
\texttt{v960}, the orientation edge has its blind Maslov GO and
its named successor in \texttt{v961}).  \emph{The formalized
target form (wave 4):} both edges are corollaries of the one
master theorem \texttt{augmented\_prefix\_positive}
(\texttt{rh/lean/RH/Window.lean}) --- $A_{w,n} = \bigl[\begin{smallmatrix} H_n & u_n\\ u_n^{T} & B \end{smallmatrix}\bigr] \succ 0$
for every admissible MAIN window and prefix degree, with
\texttt{MainWindow} an opaque predicate (the three reviewer
counterexamples are permanent Lean guards); modulo this one
\texttt{sorry} the two ex-edges are Lean-proved, and per the
wave-4 measurements the predicate must encode \emph{metric}
exactness (sub-gap-exact comb placement), not combinatorial Euler
properties.  \emph{The north star reformulated (wave 5,
\veri{v962\_halffilling\_pinning\_theory.py}):} by the moment
counting theorem T1 (the free pivots are exactly
$h_0..h_{N_w-1}$), the crossing budget theorem T2 (the crossing
count is $S_-$, world-blind) and the main window reduction T4,
the campaign's open center is, in reinstform, \textbf{why is the
signed prime moment form quasi-definite up to the maximally free
order?} --- $\forall n < N_w : h_n > 0$, exactly equivalent to
$\min C \ge N_w$, and via T4 the base half of the master
\texttt{sorry} (Lean: \texttt{free\_window\_positivity},
\texttt{rh/lean/RH/Window.lean}; carried on the successor
contract \texttt{PRIME.PORT.RHP.FULLSOURCE.\allowbreak
QUASIDEFINITENESS.01} [O], block below).  \emph{The canonical
form (wave 6, \veri{v963\_lstar\_reduction\_dictionary.py}):}
by the r283 A2 chain the north star is exactly \textbf{the
missing lemma L*} --- for every real polynomial $p$ with $\deg
p < N_w$: $\int p^2\,d\nu < \int p^2\,d\mu$, equivalently
$\lambda_{\max}(E_{N_w}) < 1$ (the $\nu$-dressed $\mu$-CD kernel
is a strict contraction at half-filling depth) --- a two-measure
moment problem instead of a determinant cascade; registered as
\texttt{PRIME.LSTAR.\allowbreak SUBORDINATION.01} [O] (block
below) with the standalone problem document
\texttt{rh/problem/lstar\_problem.tex} and the Lean statement
\texttt{lstar\_subordination} (the direction L* $\Rightarrow$
free-window positivity is proved).  \emph{The measured
state after
rounds 250--259 (rounds 250/254/255 experiments-side, SPEC
\texttt{f9bf299d} / \texttt{1083f7a0} / \texttt{45875904}):}
\textsc{outer-model-fails} (rates carried
--- plateau $((b-a)/4)^2 = 0.2500$ exact within $0.040$, $h$-rate
slopes $\le 0.007$ dec/degree --- pointwise and fiber failed; the
naked electrostatic model beats the dressed one
$2.2$--$3.8\times$); \textsc{offdiag-extensive} ($D_{\rm part}
\approx 0.195\,N$; the atom diagonal carries $78.6\%$ of the
world gap; the round-254 compactness-inversion headline and the
Epstein fiber anomaly are \emph{downgraded on record} by the
round-256 firewall --- both are base contamination of the
indefinite continuation and may not be cited as border/fiber
discriminators); \textsc{cocycle-law} (round 257: the drain is
monotone on MAIN through the entire free window and its increment
law breaks exactly at the control flips $21/25/27$ --- base and
fiber are coupled in one scalar law; both augmented border
variants cross the wall: no bordered spectral no-pole shortcut);
and the round-255 one-bit reading is \emph{revised} (round 259:
the same short-range swap structure sits on the mains with fully
overlapping gap distributions --- the base flip stays a threshold
event of an extensively accumulated quantity, but the missing
information is not one-dimensional).  \emph{Kill (of any
mechanism candidate; unchanged plus the v959 no-gos):} a base
layer must produce the control flips on Epstein/smooth
(orientation, not just rate); a fiber main term must separate
scramble by $\ge 1$ decade before readout and survive the
$\times2$ share window; mass majorants, level crossing, one-swap
statistics and $k$-swap truncations are stop-listed (measured
dead in rounds 258/259).  No marker move beyond the [E]
registrations; no RH statement.\par}

\par\smallskip\noindent\textbf{The coupled-tau / terminal-budget
theorem set (\texttt{PRIME.PORT.RHP.COUPLEDTAU.\allowbreak
TERMINAL.01} [E]) and the two terminal-edge contracts
(\texttt{PRIME.PORT.COUPLEDTAU.\allowbreak
TERMINAL\_CROSSRATIO.01} [O],
\texttt{PRIME.PORT.FULLSOURCE.\allowbreak
PREFIX\_RESUMMATION.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-25 (rounds
256--259, notes DLXXXVII--DXC;
\veri{v959\_coupledtau\_terminal\_dictionary.py}, probes embedded
byte-exact, $21/21 + 26/26 + 27/27 + 19/19$ first pass, plus the
module-own exact section S0).  \emph{Exact (COUPLEDTAU.TERMINAL):}
the positive-prefix firewall (MAIN $p_{\max} = 184 = N$ with zero
negative modes, the only positive-prefix world; controls
\textsc{indefinite-continuation} at $21/25/27$; every prefix
kernel numerically PSD at $5.7\times10^{-16}$, re-proven as exact
algebra in-module: $H_p$ PD iff the whole $h$-prefix is positive,
$\det H_p = \prod h_i$, Schur-complement must-fail firing) with
the exact $2\times2$ causal census and its two contamination
downgrades on record (the r254 scramble compactness is base
contamination, $f_{\rm neg} = 0.9311$; the Epstein fiber anomaly
lives entirely in the indefinite continuation $k \ge 25$); the
coupled two-term recursion $\tau_{n+1} = a_n\tau_n$,
$\tau^{\rm aug}_{n+1} = a_n\tau^{\rm aug}_n + b_n\tau_n$ with
$a_n = c_n^2h_n$, $b_n = -(c_nF_n)^2$, equivalently $D_{n+1} =
D_n - F_n^2/h_n$ (anchor rel $4.8\times10^{-11}$, mp dps 220
through the scramble flip), with the bilinear tau form
$\tau^{\rm aug}_n\tau_{n+1} - \tau^{\rm aug}_{n+1}\tau_n =
(c_nF_n)^2\tau_n^2$ derived and gated in-module --- the base
alone carries the sign, the border only nonnegative magnitude,
and the cocycle breaks exactly at the control flips; the blind
coefficient-field pass of the frozen micro-falsifier (control
flips $3/3$ and forced tail $0/2/2/3/1$ on all five windows; the
parametrix half open); and the Schur budget telescope $D_N = B -
S_{N-1}$ (mp dps 220 at $2.4\times10^{-16}$; $B = S_{N-2} + 5/7$
by construction) with the consequence gate $\text{margin} > 0
\iff q_N < 1$ and the corner-ray $t^*$ normal form (slope exactly
$5/7$, root exactly $q_N$).  \emph{Typed \textsc{numerical}
(measurements, not laws):} the $q$ census ($42/42$, flat, min
margin $0.0139$); the $F$-$h$ covariance $+1.000$ (a measured
structural fact); the $5/7$ surface (\textsc{floor-imported},
derivation open); the r259 ensemble-coherence findings and the
parametrix orientation blindness.  \emph{Honest negatives
(load-bearing):} the level-crossing no-go (branches cross
$5$--$16$ degrees too early; MAIN crosses 45 times with zero true
flips), the one-swap no-go (r255's \textsc{orientation-lowdim(1)}
revised on record), the mass-majorant no-go (all three sealed
majorants $0/42$, median $\times490$, break at $0.97N$).
\emph{The open contract (TERMINAL\_CROSSRATIO, the fiber edge):}
prove $h_{N-1}/F_{N-1}^2 > 7/5$ on the cofinal ladder ---
equivalently derive the true terminal floor source-pure; the
bound must bind the \emph{ratio} directly (no separate
$F$-majorant against an $h$-minorant), hold against the flat
measured margin profile (min $0.0139$, no $N$-decay to spend) and
survive the near-terminal zone at $0.97N$; \emph{kill list
(stop-listed):} the mass-majorant class, compression, constants,
band locality, any one-sided bound on the covariant pair.
\emph{The open contract (PREFIX\_RESUMMATION, the base edge):}
the closed analytic form of the coherent ensemble resummation ---
the determinant as a globally oriented signed partition sum,
completing the coefficient-field pass to a parametrix-level pass
(predict the control flips and the forced tail blind from a
closed model, no minors/slogdets, AST-scanned); \emph{kill list
(stop-listed):} two-branch level crossing, one-swap/$k$-swap
statistics, saddle-point dominance, the r255 parity-rule and
hysteresis hybrids ($0/3 + 0/5$ blind).  \emph{Wave-4 progress
(2026-08-25, block below):} on TERMINAL\_CROSSRATIO the 42-rung
surface is now \emph{certified} ($41$ mechanism target-blind $+$
kz15 exact-finite; the cofinal front is the single hypothesis H5
/ lemma L2).  \emph{Wave-7 progress (2026-08-25,
\veri{v964\_lstar\_coherence\_census.py}):} the H5/L2 front has
its \emph{first named classical theorem candidate} --- the exact
van der Corput inequality at $H = \lceil\sqrt{m}\rceil$
(constant-free, arithmetic-free) delivers $\delta' = +0.309 >
0.21$ world-blind and certifies $6/7$ exceptions $+ 38/42$
rungs; only the kz15 razor blocks $7/7$ (exact-finite per r270);
registered as the successor contract
\texttt{PRIME.PORT.L2.\allowbreak VDC\_LEMMA.01} [O] (block
below) --- the remaining input is the chain origin of the
$P$-variance scaling.  On PREFIX\_RESUMMATION round 261 closed the
finite-combinatorial half honestly negative and the named path is
the oriented midpoint theorem (Maslov census GO; successor
contract \texttt{PRIME.PORT.RHP.MIDPOINT.\allowbreak
ORIENTED\_THEOREM.01} [O], executed as rounds 279--281 and
consumed in wave 5, block below).  Mincut base 4 /
refined 5 unchanged; no RH statement.\par}

\par\smallskip\noindent\textbf{The terminal-surface closure and
the midpoint-orientation theorem sets
(\texttt{PRIME.PORT.COUPLEDTAU.\allowbreak SURFACE\_CLOSURE.01}
[E], \texttt{PRIME.PORT.RHP.MIDPOINT.\allowbreak
ORIENTATION\_DICTIONARY.01} [E]) and the two wave-4 contracts
(\texttt{PRIME.PORT.RHP.MIDPOINT.\allowbreak
ORIENTED\_THEOREM.01} [O],
\texttt{PRIME.PORT.WALL.\allowbreak METRIC\_FIREWALL.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-25 (rounds
260--278, notes DXCII--DCXI;
\veri{v960\_terminal\_surface\_closure.py} with $31/31 + 26/26 +
26/26 + 28/28 + 29/29 + 29/29 + 28/28 + 28/28 + 24/24 + 24/24$
first-pass gates and
\veri{v961\_midpoint\_orientation\_dictionary.py} with $32/32 +
25/25 + 29/29 + 31/31$; probes embedded byte-exact and executed
in their sealed \texttt{--smoke} stage, SPEC SHAs pinned, full
records re-verified by \texttt{rh/verification/run\_rh.py} ---
since this wave a gate of \texttt{bash build.sh audit}).
\emph{Exact (SURFACE\_CLOSURE):} \textbf{the 42-rung surface
census $q_N < 1$ is certified on all 42 rungs as a finite fact}
--- the two-branch theorem ($U =$ the sealed C1b triangle form,
$M = \sqrt{5/7}$, $Z$-identity at $1.8\times10^{-14}$; cheap
branch $g_w \ge 0$ exactly $35/42$ including both mains
\emph{without cancellation}; exception set exactly $\{kz15, kz20,
kz22, kz36, kz38, kz39, kz52\}$), the sealed phase bounds (edge
truncation: kz22, the first source-pure exception certification
ever; block alternation: $+0.26$ to $+0.41$ decades on all 42,
six exceptions certified, no detector fires), the kz15 interval
certificate (outward-rounded intervals, dps 640, end width
$1.5\times10^{-92}$, margin $+0.02680$, dps-halving ward ---
typed an exact finite enclosure, no mechanism), and the universal
pair theorem (H1--H4 structural, Lean-proved in
\texttt{rh/lean/RH/PairBound.lean}; H5 $=$ the margin
$|Z_{\rm local}| + \varepsilon < \sqrt{5/7}$ the single
window-dependent hypothesis).  \emph{Exact
(ORIENTATION\_DICTIONARY):} the base Casoratian is the pivot
chain itself ($\hat\pi_{n+1}q_n - \hat\pi_nq_{n+1} = h_n$, $c' =
1$; right solution triple-constructed, the node polynomial is the
right Dirichlet boundary), the $h$-free midpoint form \emph{is}
the node polynomial (the r231 sign-blindness as an identity), the
augmented telescope $D_{n+1} = B - W^{\rm aug}_n/W^{\rm base}_n$
exact against the independent r263 route, and the Maslov census
GO (rule R2 $=$ Jacobi interlacing/reality: blind $42/42$, mains
safe at full depth, controls fire exactly at flip$+$1, one-way,
not $h$-equivalent).  \emph{Typed \textsc{numerical}
(measurements, not laws):} the L2 scaling anatomy
(\textsc{bound-coarseness}: the truth margin rises with $N$; flip
condition $\delta' > 0.21$ of available $0.45$, address c3), the
Euler surgery ladder (\textsc{perturbation-insensitive} $+$ the
firewall map: the wall, not the cancellation rate, is what is
arithmetically special), the metric-firewall continuum
($D \sim \theta^b$, support exactness first), the exact position
gradients (Hellmann--Feynman class, identity fine type) with the
predictive small-primes-loaded u-profile, and the
perturbative-only stability law.  \emph{Honest negatives
(load-bearing):} the eventual-triangle no-go (kz52 itself an
exception), three identity routes dead with break loci, the
retracted w13 Gram provenance, the KYP memory no-go from
structure (exact double obstruction o1/o2 on 46/46 worlds; the
admissible Riccati memory target-inverse), and the raw atom-Sturm
census refuted as winding quantity
(\textsc{sturm-chain-verified} honestly not awarded).  \emph{The
open contract (ORIENTED\_THEOREM, the orientation edge):} the
contraposition form --- an independent index obstruction that
forbids the R2 interlacing break before half-filling on MAIN; the
detection direction (break at flip$+$1, after the flip) forces
exactly this form, and the \texttt{MainWindow} arithmetic must
enter here as \emph{metric} exactness; \emph{wave-5 state}:
rounds 279--281 are executed and consumed
(\veri{v962\_halffilling\_pinning\_theory.py}, block below) ---
the theorem share is promoted, the candidate generic two-sided
obstruction is refuted exactly, and the center is precisified to
the budget localization (free-window survival).  \emph{The open contract (METRIC\_FIREWALL, the
firewall lane):} the global form of the wall's metric stability
--- a wall lemma quantitatively stable under family/assignment
permutations at small dose while consuming the metric data
exactly; the certified state is graded-continuous with exact
gradients but perturbative-only ($\theta^*$ $25$--$170\times$
below the smallest measured dose, $L_D$ window-specific, no
uniform lemma candidate); the MAIN positivity itself (H5 / the
master theorem) is not part of this demand and stays the open
center.  \emph{The surface fence (binding):} the surface closure
is a census-plus-certificate result, \emph{not} a cofinal theorem
--- the window quantifier stays.  Mincut base 4 / refined 5
unchanged; no RH statement.\par}

\par\smallskip\noindent\textbf{The half-filling pinning theory
(\texttt{PRIME.WALL.\allowbreak
HALFFILLING\_PINNING\_THEORY.01} [E]) and the two wave-5
contracts (\texttt{PRIME.PORT.\allowbreak
REPRESENTATION.\allowbreak CONTEST.01} [O],
\texttt{PRIME.PORT.RHP.\allowbreak FULLSOURCE.\allowbreak
QUASIDEFINITENESS.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-25 (rounds
279--281, notes DCXII/DCXIV/DCXV;
\veri{v962\_halffilling\_pinning\_theory.py} with $32/32 + 29/29
+ 28/28$ first-pass gates; the three probes embedded byte-exact
in their sealed \texttt{--smoke} stage, SPEC SHAs pinned, full
records re-verified by \texttt{rh/verification/run\_rh.py}; plus
the module-own exact section S0 in pure Fractions).  \emph{Exact
(the four theorems as named gates):} (T1) the \textbf{moment
counting theorem} --- the pivot $h_n$ consumes $m_0..m_{2n}$, the
$S$ free moments are $m_0..m_{S-1}$, hence the free pivots are
exactly $h_0..h_{N_w-1}$ and $h_{N_w}$ is the first forced one:
half-filling is the end of the free moment space, arithmetic-exact
for all $S = 2..2000$ with the exact-rational freedom
demonstration --- ``why half-filling'' is answered by counting;
(T2) the \textbf{crossing budget theorem} --- $\#\{n < S : h_n <
0\} = S_-$ exactly (Jacobi/Sylvester), world-blind; (T3) the
\textbf{two-sided parity theorem} --- node-sign pattern, gap
parity and the census bilanz, $h$-blind at every degree, with the
$87376$-case exhaustion; (T4) the \textbf{main window reduction}
--- given T1 and T2 the entire open statement of the wall is
$\min C \ge N_w$, exactly equivalent to $\forall n < N_w : h_n >
0$: one placement question, the north star in reinstform.
\emph{The refutations (named negative gates, load-bearing):}
\textsc{no-universal-o1-pinning} (exact; the one-negative toy has
offset $N_w - 2$, unbounded in $S$; only provable ceiling: the
pigeonhole $\min C \le S - S_-$; $C \le 5$ is a 42-rung
measurement), \textsc{no-extremality} (measured $+$ exact
witness: three mp-confirmed directions push the w9 crossing $184
\to 185$; a 4-atom one-negative measure has $\min C = N_w + 1$
exactly), \textsc{no-generic-maslov-obstruction} (exact; three
rational counterexamples --- the two-sided machinery carries no
arithmetic), \textsc{no-simple-offset-law} (measured; six sealed
source-pure candidates, max $|\mathrm{sp}| = 0.273 \ll 0.75$, all
world-blind).  \emph{Lean state:} T1 and T4 \emph{proved}
(\texttt{moment\_counting\_free\_pivots},
\texttt{main\_window\_reduction}); T2 stated
(\texttt{crossing\_budget}, \texttt{sorry} with v962 reference);
the fog-free central hole is \texttt{free\_window\_positivity}
(\texttt{rh/lean/RH/Window.lean}, via T4 the base half of the
master \texttt{sorry}); the exact one-negative instance is the
permanent guard \texttt{upper\_pinning\_not\_universal}.
\emph{The open contract (REPRESENTATION.CONTEST):} the sealed
contest of representations for the localization statement ---
which coordinate system (dual chain, moment/Weyl,
gradient/criticality, Jacobi interlacing, tau/cocycle) carries a
MAIN-separating, non-restatement handle on free-window survival;
\emph{forbidden (binding):} no derived $5/7$, no bound mechanism
claim, no asymptotic law, no localization claim from any census,
no undeclared $h$-reading candidates, no world-blind carrier; the
v962 refutation set binds the candidate space.  \emph{The open
contract (FULLSOURCE.QUASIDEFINITENESS, the open center):}
$\forall n < N_w : h_n > 0$ on every admissible MAIN window ---
why is the signed prime moment form quasi-definite up to the
maximally free order?  \emph{Forbidden (binding, the measured
kill lists):} no bookkeeping-only proof (the Lean counterexample
guards are permanent), no world-blind mechanism, no Weyl
$\mu/\nu$ perturbation route (range $1.1\%$), no dual
restatement, no variational local-maximum route, no simple offset
law, no universal $O(1)$ upper pinning.  Both rounds
(\texttt{representation\_contest\_probe.py},
\texttt{fullsource\_quasidefiniteness\_probe.py}) were in flight
at the wave-5 cut and are \emph{executed and consumed in wave 6}
(\veri{v963\_lstar\_reduction\_dictionary.py}, block below):
REPRESENTATION.CONTEST adjudicated \textsc{contest-all-dead}
(the four classical languages closed with exact reasons),
FULLSOURCE.QUASIDEFINITENESS adjudicated
\textsc{mechanism-partial}(A2) --- L* established as the
canonical reduction, the open center carried forward on
\texttt{PRIME.LSTAR.\allowbreak SUBORDINATION.01} [O].  Mincut
base 4 / refined 5 unchanged; no RH statement.\par}

\par\smallskip\noindent\textbf{The L* reduction dictionary
(\texttt{PRIME.LSTAR.\allowbreak REDUCTION\_DICTIONARY.01} [E])
and the L* subordination contract
(\texttt{PRIME.LSTAR.\allowbreak SUBORDINATION.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-25 (rounds
282--285, notes DCXVI/DCXVII/DCXIX--DCXXI, plus the standalone
problem document of note DCXX;
\veri{v963\_lstar\_reduction\_dictionary.py} with $30/30 + 26/26
+ 30/30 + 33/33$ first-pass gates; the four probes embedded
byte-exact in their sealed \texttt{--smoke} stage, SPEC SHAs
pinned, full records re-verified by
\texttt{rh/verification/run\_rh.py}; plus the module-own exact
section S0 in pure Fractions).  \emph{Exact (the reduction as
named gates):} the $\mu$-frame congruence
$\mathrm{minor}_k(D_\mu - G) = D_k(\tilde\mu)$ at every $k \le
S_+$ (truncation must-fail loud); the frame contraction
$h_0..h_{m-1} > 0 \iff \lambda_{\max}(E_m) < 1$ (both truth
values); the crossing dictionary $\lambda_{\max}$ crosses $1$
exactly at $\min C + 1$ (\textsc{capacity-ceiling} typed where
$\min C = S_+$); the pigeonhole ceiling $\min C \le S_+$
re-proved in the frame; capacity-as-counting refuted by the
rank-one pair (metric values $1.25\times10^{-13}$ vs
$1.25\times10^{2}$ decide); the r285 bookkeeping
$\lambda_{\max} = \mathrm{maxdiag}\times(1 + \mathrm{assist})$
with the sign-exact budget equivalence; and the r282
four-language elimination as named negative gates
(\textsc{contest-all-dead} --- SOS iff empty negative register,
Kasteleyn orientation iff $S_- = 0$ by full $2^S$ exhaustion,
Hamiltonian-PSD $=$ $h > 0$ bookkeeping, dual-pair sync by
theorem; the common deep reason: every classical positivity
language forces positivity exactly for the positive measure
class, and MAIN is signed).  \emph{Typed \textsc{numerical}
(measurements, not laws):} the one-atom-vs-collective dichotomy
($n_{\rm DIAG} = 187$ vs crossing $185$; controls collective),
the sub-classical edge growth ($p = 0.38$, classical coverage
$0/42$ --- the (D) route must be a discrete bound), the ensemble
position (MAIN's wall assist the extreme \textsc{low-outlier},
pct $0.00$), and the first two positively MAIN-separating
detectors of the program (assist-at-crossing $0.0195$ vs dead
$1.69..2.99$; random-sign $z = -3.15$ destructive vs dead
$+4.95..+12.44$).  \emph{The open contract (SUBORDINATION, the
canonical open center):} for every admissible anchor $z$ and
every real polynomial $p \ne 0$ with $\deg p < N_w = (S+1)/2$:
$\int p^2\,d\nu_z < \int p^2\,d\mu_z$ --- equivalently
$\lambda_{\max}(E_{N_w}) < 1$; \emph{state (wave 7,
\veri{v964\_lstar\_coherence\_census.py}):} $57/57$ measured (the
42 plus the 15 extension anchors, $N_w$ up to $1218$), \textbf{the
margin decay resolved harmless} (r286: no counterexample, the
$O(1)$ census offset survives beyond the cap, flattening power
law $\alpha \approx 3.05$ with driver $c_w \to 1$, and the margin
falls \emph{slower} than the local loading speed --- no acute
falsification threat; the unbounded family stays open); the
coherence half has a \emph{named carrier class} (r288: antiphase
next-nearest ARCH--ARCH pairs, $z_v = -3.149$, total control
reversal, finest alignments below phase resolution); \textbf{the
diophantine route is excluded} (r289 \textsc{metric-only}: the
rational twin keeps the full signature identical while every
exact log-relation is destroyed; metric coherence threshold
$10^{-3}..3\times10^{-3}$ of the local gap; Baker $\sim$7900x too
weak \emph{and} unnecessary); \emph{state (wave 8,
\veri{v965\_lstar\_curvature\_arc.py}):} the profile-functional
question is \textbf{functionally negative-sealed} --- after five
sealed class families (local scalars, non-local/low-rank, the
curvature two-form in the mixed metric, the metric-reconciled
two-form, the hardened partial-free $|\mathrm{sp}|$ form) no closed
predictive profile functional of the working set exists over the
honest bar (r294 \textsc{f10-fragile} $+$ r295
\textsc{f10-sp-majority}: F10 unpromoted), while the working-set
geometry is now \emph{measured} (anisotropic tube, one-degree
budget ridge, rank-1 DENS curvature valley, simple $h_{184}$ flip
zeros); the successor question is
\texttt{PRIME.LSTAR.\allowbreak CLOSED\_FUNCTIONAL.01} [O] (block
below); no round is in flight at the wave-8 cut; the
standalone document \texttt{rh/problem/} is the
external reference; \emph{kill lists (inherited, binding):} the
v962 refutation set, the r282 four-language closure, the r283
forbidden list (no triangle/Gershgorin bound --- dead at $n = 21
\ll 185$ --- no target-inverse quantity, no truncation, no
posthoc window), the r284 shielding/Nyquist refutations, the
r285 no-fixed-$\delta/\delta'$ trend, plus the wave-7 kills (no
diophantine/linear-forms route, no plain phase-dispersion
separator, no sinc/CD-only sign mechanism), no derived $5/7$, no
bound mechanism claim, no asymptotic law.  Lean:
\texttt{lstar\_subordination} stated
(\texttt{rh/lean/RH/Window.lean}, documented \texttt{sorry});
the direction L* $\Rightarrow$ free-window positivity is
\emph{proved} (\texttt{lstar\_implies\_free\_window}).
\emph{State (wave 11,
\veri{v968\_architecture\_adjudication.py}):} the formal frame is
\textbf{restructured} --- the master theorem is now \emph{proved
from} L* $+$ terminal
(\texttt{lstar\_terminal\_implies\_master}, r305, with
\texttt{augmented\_prefix\_positive} and
\texttt{free\_window\_positivity} as corollaries; sorries $9 \to
5$, the two TRUE holes standing alone:
\texttt{lstar\_subordination} and
\texttt{terminal\_positive\_main}), the window source is a real
Lean construction (r310/r310b: nodes $= \log p^k$, weights $=
\Lambda(p^k)$ derived; source exactness, mass conservation of any
folding, the strong stabilization form), and the surrounding
architecture is fully adjudicated (Lane A closed as cone language,
B not overtaking with the rank-one identities banked, C parked);
additional kill entries: no fixed-head route (r307), no
constructive PSD-G rule in the planned cone language (r312), no
transfer-operator contraction at chain level (r313), no two-regime
pointwise majorant under a mid-ladder freeze (r316).
\emph{State (wave 12, \veri{v969\_forks\_and\_redteam.py}):} the
indefinite fork is adjudicated (r318 \textsc{p2-main-specific}: P1
banked \emph{as language} --- the index statement is a total
restatement of L*, no lever; classical sign regularity dead on
MAIN) and the antiphase fingerprint is closed as
\textsc{algorithm-artifact} (r322: a least-norm-proximal
Dykstra-basin property, not a psd-solution-set property; the
selection geometry is the named residual); the Lean statement
class is red-team hardened (the r319 U-findings repaired by the
r320 retypes $+$ guards, census 5); \textbf{per the binding
reviewer adjudication the lane is internally paused} (allocation
0\,\%; the selection geometry is not pursued further; reopening
only under the six documented conditions of the r322 round notes;
the external memo candidate: the two-weights sampling formulation
$\lVert J_N\rVert < 1$), and the hole itself is typed
\textbf{window-local} (Level B of the wave-12 three-level proof
graph --- see the red-team block below).  Additional kill entries
(wave 12): no Pontryagin/Krein index lever, no
sign-regularity/variation-diminishing route, no antiphase sign-law
theorem without a canonical selection rule.
\emph{State (wave 13, \veri{v970\_extraction\_and\_composition.py}):}
the pause held --- the planned r323 fork was cleanly aborted at the
end of its design phase, before any write access (nothing counts as
a round); the Lean census is now $5 \to 8$ (three typed Level-C
extraction statements, r326 --- none touches this hole;
\texttt{lstar\_subordination} byte-identical).  Mincut
base 4 / refined 5 unchanged; no RH statement.\par}

\par\smallskip\noindent\textbf{The L* coherence census and the
vdC L2 lemma contract (\texttt{PRIME.LSTAR.\allowbreak
COHERENCE\_CENSUS.01} [E], \texttt{PRIME.PORT.L2.\allowbreak
VDC\_LEMMA.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-25 (rounds
286--289, notes DCXXIV/DCXXII/DCXXV/DCXXVI;
\veri{v964\_lstar\_coherence\_census.py} with $29/29 + 25/25 +
31/31 + 42/42$ record gates; the four probes embedded byte-exact
in their sealed \texttt{--smoke} stage, SPEC SHAs
\texttt{0a44ac4e}/\texttt{761d88fa}/\texttt{da46f7ee}/%
\texttt{91cdc2b1} pinned; plus the module-own exact section ---
the Fej\'er window-sum identity, the van der Corput inequality
with its Cauchy--Schwarz proof route, the Abel/Erd\H{o}s--Tur\'an
bound, the mutant must-fails, the 2-power resonance count
$\binom{8}{2} = 28$, in pure Fractions).  \emph{Exact (the vdC
theorem):} $|\sum_j P_j|^2 \le
\frac{m+H-1}{H}\sum_{|h|<H}(1-\frac{|h|}{H})A(h)$ ---
constant-free, arithmetic-free, proved module-own; at the frozen
window $H = \lceil\sqrt{m}\rceil$ it delivers $\delta' = +0.309 >
0.21$ world-blind (admissible: L2 is the generic half), certifies
$6/7$ exceptions $+ 38/42$ rungs, and only the kz15 razor blocks
$7/7$ (exact-finite per r270, reserve $0.0268$); F1
discrepancy/Erd\H{o}s--Tur\'an dies loud ($\delta' = -0.21$,
$0/7$); the autocorrelation map relocates the c3 address: on
$39/44$ worlds the block sequence \emph{reinforces} --- the
cancellation is already absorbed into the $P_j$ magnitudes, the
frames win through the $\sqrt{m}$ count economy.  \emph{Typed
\textsc{numerical} (measurements, not laws):} the r286 extension
census (15 new anchors $N_w = 942..1218$, all margins mp-sign-safe
positive, min $+1.806\times10^{-8}$; off census $O(1)$, max $+4$;
flattening power law $\alpha \approx 3.05$; \textsc{harmless}
quantified; $q_N$ reconciled; \textsc{extrap-calibrated} $15/15$),
the r288 carrier map (antiphase next-nearest ARCH--ARCH pairs,
$z_v = -3.149$, total control reversal, finest alignments below
phase resolution), the r289 twin table and jitter ladder (metric
coherence threshold $10^{-3}..3\times10^{-3}$ of the local gap).
\emph{Honest negatives (load-bearing):}
\textsc{sampling-blind} (the sinc/CD predictor partial,
$0.728/0.878 < 0.90$), \textsc{source-separator-not-found} (plain
phase dispersion world-blind, ensemble-generic),
\textsc{diophantine-excluded}(\textsc{metric-only}) --- the
rational twin (every tent center replaced by a rational $p/q
\times \Delta$ at position cost $\le 2.1\times10^{-9}$, every
exact log-linear-form relation destroyed) keeps the full
signature \emph{identical}: the effect of the diophantine
structure is null, Baker territory cleanly excluded ($\sim$7900x
too weak in the exponent \emph{and} unnecessary).  \emph{The open
contract (VDC\_LEMMA, the L2 chain statement):} prove the vdC
form $|R| \le \sqrt{\frac{m+H-1}{H}\sum_{|h|<H}
(1-\frac{|h|}{H})A(h)}$ at $H = \lceil\sqrt{m}\rceil$ as a
\emph{chain} statement --- derive the $P$-variance scaling
(measured small and large-lag structureless) from the
chain/source, source-pure and window-independent, so the bound
closes the L2 lemma (H5, address c3, $\delta' > 0.21$ of
available $0.45$) as a theorem; kz15 needs no new mechanism
(exact-finite per r270); \emph{kill list (inherited):} l1 pair
hierarchies, state-space/KYP, s-flows, mass splits, H-peek
(selection-by-answer, sealed away), fit primitives; world-blind
validity is \emph{admissible} here (the declared difference to
the L* lane); \emph{state (wave 9,
\veri{v966\_l2\_reduction\_chain.py}, block below):} the
remaining input stands \textbf{reduced to one inequality} ---
the four-round chain r297$\to$r300 froze the target level
$\sigma^* = -0.516$, proved the exact sign-preserving transfer
to the difference measure, settled the ratio half structurally
and left \textsc{neff-target} ($\mathrm{slope}(n_{\rm eff}) \ge
+0.908$, measured $+0.963$, margin $0.055$;
\texttt{PRIME.L2.\allowbreak NEFF\_TARGET.01} [O]); \emph{state
(wave 10, \veri{v967\_l2\_cascade\_closure.py}, block below):}
the reduction route is \textbf{documented stopped} --- the r303
regress audit retyped the cascade r297$\to$r302 as an exact
dictionary around one measured core $S = \sigma^* -
\mathrm{sl}_D = +0.0547$ (\textsc{regress-confirmed}) and the
r304 stop case fired (\textsc{law-longrange}: the global lag
profile is a stable period-4 comb, no $k_0$); the honest stop
state is L2 generic $\Leftrightarrow$ anti-concentration of the
explicit $dc$ block field; return later with new tools.  \emph{The successor front on SUBORDINATION:} the
firewall reads the tent-split fraction \emph{profile} metrically,
not its arithmetic --- the open question was the profile
functional (rounds 290--295 executed and consumed in wave 8,
\veri{v965\_lstar\_curvature\_arc.py}, block below: the question is
functionally negative-sealed over five class families and carried
forward as \texttt{PRIME.LSTAR.\allowbreak CLOSED\_FUNCTIONAL.01}
[O]).  Mincut base 4 /
refined 5 unchanged; no RH statement.\par}

\par\smallskip\noindent\textbf{The L* curvature arc and the
closed-functional contract (\texttt{PRIME.LSTAR.\allowbreak
CURVATURE\_ARC.01} [E], \texttt{PRIME.LSTAR.\allowbreak
CLOSED\_FUNCTIONAL.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-26 (rounds
290--295, notes DCXXVII/DCXXIX--DCXXXIII;
\veri{v965\_lstar\_curvature\_arc.py} with $32/32 + 30/30 + 36/36 +
43/43 + 42/42 + 44/44$ full record gates; the six probes embedded
byte-exact in their sealed \texttt{--smoke} stage, SPEC SHAs
\texttt{f953dd71}/\texttt{bb512c17}/\texttt{050821ff}/%
\texttt{33c44cc6}/\texttt{88c6fd1e}/\texttt{4d7d8095} pinned; plus
the module-own exact section --- the r292 polarization identity
$H(u,v) = [d^2(u{+}v) - d^2(u{-}v)]/4$ with its wrong-prefactor
must-fail, the r291 budget telescope with the dropped-atom
must-fail, the r293 $\alpha = 1$ simple-zero doubling law with the
double-zero mutant, the sealed r294/r295 decision bars as exact
logic with tipping must-fails, in pure Fractions).  \emph{Measured
(the geometry of the working set):} a soft-shouldered, strongly
anisotropic TUBE in the exact $\theta_{\rm eq}$ coordinate
(killfrac $0.38/0.62/1.00$ at $5\times10^{-4}/10^{-3}/
2\times10^{-3}$; world axes kill 5--50x earlier; the SMOOTH axis a
privileged yet gradient-orthogonal killer, $\cos =
-3\times10^{-5}$); a REAL one-degree ridge ($\min C = 185$ at
factors $1..8$) whose lift is a first-order budget phenomenon (one
threshold in $(1.280, 1.291]$ over all 18 matched-dose cases,
$k_{\min} = 9$, one TOP6 retraction; no fixed point, plateau $185$;
MAIN-specific --- EPSTEIN first-order flat); a RANK-1 DENS
curvature valley ($92.5\,\%$, $\lambda_{\rm top} = -0.418$; not
SMOOTH, not the ridge; EPSTEIN second-order structureless); and
SIMPLE $h_{184}$ flip zeros ($\alpha = 1.000 \pm 0.003$; the TOP6
retraction a second simple zero at $f_{\rm ret} = 7.107$;
\textsc{mstar-no-law} --- no fixed safety distance in any unit).
\emph{Honest negatives (load-bearing, the frozen content):}
\textsc{all-functionals-blind} over FIVE sealed class families
(local scalars best $|\mathrm{sp}|$ $0.263$; non-local/low-rank
best $0.471 < 0.881$; curvature two-form mixed $0.884 < 0.907$;
the metric-reconciled F10 --- home win $+0.024$ and partial
$+0.423$, but r294 \textsc{f10-fragile}: win $5/5$, partial $2/5$;
the hardened partial-free form --- r295 \textsc{f10-sp-majority}
$14/20$, margin median $+0.028$, min $-0.079$: the sealed
three-clause HARDENED bar fails on two clauses); \textsc{r293-luck}
(the r293 partial $+0.423$ and the r294 $5/5$ were both
top-of-distribution; the composition-matched subsample gains
$+0.067$ and misses the level clause by $0.004$);
\textsc{l2-not-dens} $+$ \textsc{l2-via-conservation}
(near-tautological, $\eta_0$ share $2.9\times10^{-30}$) --- F10 is
NOT promoted, the sealed rejections stand verbatim.  \emph{The open
contract (CLOSED\_FUNCTIONAL, the successor question):} exhibit a
closed, source-pure functional of the tent-split fraction profile
that predicts the survival depth of the working set at HARDENED
grade (the sealed r295 three-clause bar: $\ge 18/20$ wins AND
margin median $\ge +0.02$ AND no loss beyond $0.02$) with a stable
partial channel --- or prove non-existence in a named class;
\emph{kill list (sealed, binding):} the five class families above,
\textsc{mstar-no-law}, no diophantine/linear-forms route (wave-7
\textsc{metric-only} inherited), no plain phase-dispersion
separator, no sinc/CD-only sign mechanism, no re-training of the
hash-sealed $H_{\rm tr}$ (\texttt{3447ed198a56}), no posthoc bar
moves; \emph{open fronts (named):} the loss-corpus forensics
(K05/K07/K19, margins $-0.041/-0.057/-0.079$), the rank-2 DENS core
(top DENS eigenaxis alone $+0.855$, DENS share $0.989$,
mechanism-constant across w7/w9/w11: $0.795/0.989/0.834$), the
substantive ``why L2'' question, and larger window corpora for the
w7/w11 knife edge; \emph{measurement standard (sealed):}
\textsc{partial-std} $=$ the partial on the full sealed 147-point
fresh protocol (r295 pins: median $+0.279$, IQR $[+0.182,
+0.401]$).  Lean: no new statement --- the wave carries no new
formal target form, the hole stays \texttt{lstar\_subordination}.
\emph{State (wave 9, \veri{v966\_l2\_reduction\_chain.py}):} the
DENS-identity fork is \textbf{closed} --- r296 adjudicated
\textsc{dens-world-blind} under the sealed rules (the coupling
number $\cos(e_{\rm top}, \nabla\lambda_{\max}) = +0.394$ below the
sealed $0.40$ bar, noise max $0.331$, miss $0.006$ honest; the
$\lambda_3$ control couples \emph{harder}, $-0.574$: a
near-crossing eigenvalue-band property, not top-specific; every
arithmetic candidate $\le 0.38$; the moment subspace ($0.825$)
construction-adjacent, holding on both dead controls
$0.571/0.603$): front (2) --- the rank-2 DENS core --- is answered
negatively at the identity level (a mechanism hint, not an
arithmetic identity, not the L*-gradient direction), the lane
routed to L2 per the pre-adjudicated reviewer fork; the remaining
open fronts are (1), (3) and (4).
\emph{State (wave 11,
\veri{v968\_architecture\_adjudication.py}):} \textbf{Lane A (the
constructive PSD-G rule fixed by the r308 feasibility census) is
closed as a cone language} --- r311 \textsc{strict-source-cone}
(exact rational in-span Farkas certificates on MINI\#0 $+$ SM1\#0;
the r308 world separation demarcated as the budget sign / a
chordal restatement) and r312 \textsc{coefficient-sign-wall} with
the mechanism \emph{named} (block-psd membership without
rank-one-SDD membership, obstructed at the budget/border sign;
Farkas mass on the border/budget row, $t$-share $0.583/0.725$) ---
no GO per the reviewer tree, the resources moved to the fiber; the
PSD-G rule stays open as a \emph{question}, closed as a
\emph{lane}; the closed-functional demand itself is untouched (the
sealed r295 bar stands).
Mincut base 4 / refined 5 unchanged; no RH statement.\par}

\par\smallskip\noindent\textbf{The L2 reduction chain and the NEFF
target contract (\texttt{PRIME.L2.\allowbreak REDUCTION\_CHAIN.01}
[E], \texttt{PRIME.L2.\allowbreak NEFF\_TARGET.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-26 (rounds
296--300, notes DCXXXV--DCXXXIX;
\veri{v966\_l2\_reduction\_chain.py} with $39/39 + 27/27 + 28/28 +
32/32 + 31/31$ full record gates; the five probes embedded
byte-exact in their sealed \texttt{--smoke} stage, SPEC SHAs
\texttt{ffb413c8}/\texttt{e42a76eb}/\texttt{05e831be}/%
\texttt{f432e944}/\texttt{55218b5d} pinned; plus the module-own
exact section --- the $\sigma^*$-composition arithmetic
($\delta'(\sigma^*(\mathrm{pad})) = \mathrm{pad}$ exactly, the
pad-dropped mutant composes to $0$), the decomposition identity
$S_F = B(\omega,\omega) + B(\Delta,\omega+\beta)$ on the exact
positional Fej\'er block form (toy $3 = 1 + 2$; the $|\Delta|$ and
wrong-weight mutants break by exactly $4$ and $2/3$), the
participation identity $D \times n_{\rm eff} = L_1^2$ with ratio
multiplicativity and the machine-checked DIAG$\leftrightarrow$NEFF
equivalence (margin $0.055$ exact in both coordinates; signed-sum
mutant $8/3$), the kernel envelope $F_H(s) \le \min(H, 1/(Hs))$
exact-rational with equality witnesses on both branches, and the
five sealed verdict bars as exact decision logic with tipping
mutants, in pure Fractions).  \emph{Delivered (the chain):} the
r296 DENS fork closed honestly (\textsc{dens-world-blind}, above);
the r297 target inequality frozen ($\sigma \le \sigma^* = -0.516$,
measured $-0.714$, margin $0.198$ --- provenance missing, not
room; the chain exactly orthogonal on the WINDOW measure); the
r298 exact sign-preserving transfer (\textsc{transfer-dominant}:
the window main term empty, $S_F \approx B(P\Delta,P\Delta)$ ---
the vdC input IS the Fej\'er energy of the difference measure);
the r299 decay split (LOWPASS $0.93$; full-support overlap $42/42$
--- $\Delta$ a pure c-value difference; ET/Abel dead $+1.948$; the
live edge $\mathrm{sl}_D = -0.571 \le \sigma^*$ with falling ratio
$-0.168$); the r300 diagonal anatomy ($\mathrm{sl}_D =
2\,\mathrm{sl}_{L1} - \mathrm{sl}_{\rm neff}$ exact; both
magnitude routes closed honestly --- the $|dw|$ sum-rule attach
BREAKS, the fill decay $-0.225$ is invisible to $\max\times$mass
bounds; \textsc{ratio-bounded-structural}: $R_{\rm env}$ med
$1.61$ falls at $-0.122$).  \emph{Honest negatives (load-bearing,
the magnitude catalog):} r297 B1 pair-gap ($-0.535$ vs needed
$\le -1.178$) and B2 mass imbalance ($+0.244$ growth); the r299
ET/Abel wall ($+1.948$, \textsc{mass-target} missed by $2.46$);
the r300 chain-norm and $\max\times L_1$ closures; no pointwise
c-value convergence (cconv $0.86$, $+0.045$); the lobe-width
heuristic refuted ($\mathrm{sp} = -0.47$).  \emph{The open
contract (NEFF\_TARGET, the one remaining inequality):} prove
$\mathrm{slope}(n_{\rm eff}) \ge 2\,\mathrm{sl}_{L1} - \sigma^* =
+0.908$ for the participation number $n_{\rm eff} = L_1^2/D$ of
the aggregated difference profile (measured $+0.963$, margin
$0.055$, thin) --- exactly equivalent to \textsc{diag-target}
$\mathrm{sl}_D \le \sigma^*$, and through the settled chain it
closes $\delta' > 0.21$ on the generic half:
$[\textsc{neff-target}] \Rightarrow \mathrm{sl}_D \le \sigma^*
\Rightarrow \sigma \le \sigma^* \Rightarrow$ r297 target
$\Rightarrow$ v964-S0 vdC $\Rightarrow \delta' > 0.21$
(exceptions: 6 via r287 F2, kz15 exact-finite via r270);
\emph{the named bridge (correlational, not causal):} the carrier
count grows where the boundary discrepancy falls
($D_{\rm rank}(\Delta)$ slope $-0.117$,
$\mathrm{sp}(D_{\rm rank}, n_{\rm eff}) = -0.81$) --- a proof must
turn the correlation into a mechanism; \emph{kill list (measured
dead, binding):} every max/mean magnitude majorization (r297
B1/B2, r300 chain-norm CS on $|dw|$, r300 $\max\times L_1$), the
ET/Abel position-bound composition, pointwise c-value convergence,
the lobe-width heuristic, H-peek, fit primitives; world-blind
validity admissible (L2 is the generic half); $D_{\rm rank}$
statistics typed \textsc{measurement-only} (forbidden as proof
premises).  \emph{State:} round 301
(\textsc{neff-equidistribution}) was in flight on exactly this
question at the wave-9 cut; \emph{wave-10 resolution (rounds
301--304 executed and consumed,
\veri{v967\_l2\_cascade\_closure.py}, block below):} the demand
is \textbf{retyped to the documented stop state} --- the r303
regress audit shows NEFF/UNIF/ATOM are \emph{one} algebraic
slack $S = \sigma^* - \mathrm{sl}_D = +0.0547$ (invariance $\le
9\times10^{-16}$; the $\tfrac12$-conversion refuted), so the
rest is no longer a ladder of targets but the stop state: L2
generic $\Leftrightarrow$ anti-concentration of the explicit
$dc$ block field (core slack $S = +0.0547$ measured); the
global-profile mixing route is closed (period-4 comb,
\textsc{law-longrange}); the mechanism is two-scale; return
later with new tools --- \emph{the hard reviewer rule binds: a
round counts as progress only if it adds new information, not
if it re-expresses the same slack in a new coordinate}.
Lean: no new statement --- \textsc{neff-target} is a measured
ladder-slope aggregate, not an exact finite identity, and a
universal ladder form would be refutable (the r273 guard pattern);
the hole stays \texttt{lstar\_subordination}.  Mincut base 4 /
refined 5 unchanged; no RH statement.\par}

\par\smallskip\noindent\textbf{The cascade closure and the
documented lane stop (\texttt{PRIME.L2.\allowbreak
CASCADE\_CLOSURE.01} [E]).}
{\sloppy
\emph{Registered and delivered} 2026-08-26 (rounds 301--304, notes
DCXL/DCXLII--DCXLIV; \veri{v967\_l2\_cascade\_closure.py} with
$32/32 + 30/30 + 26/26 + 37/37$ full record gates; the four probes
embedded byte-exact in their sealed \texttt{--smoke} stage, SPEC
SHAs \texttt{6f8cc404}/\texttt{36df9424}/\texttt{375e9f2b}/%
\texttt{2cc5d23f} pinned; plus the module-own exact section --- the
r303 margin-invariance algebra (all four target margins $m_D =
m_{\rm NEFF} = m_{\rm UNIF} = m_{\rm ATOM} = 3/50$ exact on the
rational slope set; the halved-need mutant breaks by exactly
$\mathrm{sl}_{L1} = 1/5$; the $\sigma$ decomposition $0.198 = 0.055
+ 0.143$ exact; the $\tfrac12$-conjecture refutation $0.099 \ne
0.055$), the r302 coherence identity $(1 + CV^2)\,\mathrm{surv}^2\,
n_{\rm eff}^{\rm atom} = n_{\rm act}\,\chi$ on two rational toys
(mutant breaks $5/7$ and $48/35$ exact), the r304 $\chi$-lag
decomposition $\chi = 1 + 2\sum_k T_k/Q = D/Q$ on both toys ($5/7$
and $5/19$ exact; factor-1 mutant $1/7$), the r304 zero-sum
tautology as a gate (mutants $1/2$ and $3/4$ exact; $NC(1) = 0.556$
exact, $NC(16) = 0.712 < 1$; the period-4 comb certificate with no
$k_0$), the $-/-/+/+$ sign-pattern certificate with the kz18/kz23
Fractions pins, the four sealed verdict bars as exact decision
logic with tipping mutants, and the lane-stop composition gate, in
pure Fractions).  \emph{Delivered (the rejections and retypings ARE
the content):} r301 \textsc{neff-split} ($n_{\rm eff} = n_{\rm
act}/(1 + CV^2)$ exact with the perfect count link $n_{\rm act} =
m$ on $42/42$; the R\'enyi family at one exponent --- echt
anti-concentration; jackknife-stable; rest relocated onto
\textsc{unif-target}); r302 \textsc{unif-derived}(B1{+}A2) --- the
\emph{first} \textsc{derived} of the lane, all hypotheses typed
\textsc{measured} (\textsc{profile-stationary}, KS $0.043$; exact
$1/N$ transient onto $m_2^\infty = 1.973$; $\chi = 0.63$
destructive, falling; rest relocated onto \textsc{atom-target});
r303 \textsc{regress-confirmed} $+$ \textsc{mixing-insufficient}
(the retyping: all four margins ONE number $S = +0.0547$,
invariance $\le 9\times10^{-16}$; the cascade an exact dictionary,
not six proof steps; the first causal coordinate --- the $\rho_1$
ladder is monotone and the flip kills the inequality at $-0.044$
--- but the $\chi$ level misses by $0.134$; $n_{\rm eff}^{\rm
atom}$ a pure marginal functional, $10^{-15}$ on all builds); r304
\textsc{longrange-structure} $+$ \textsc{law-longrange} (the
reviewer stop case: a stable, world-specific period-4 comb, no
$k_0 \le 8$; the reviewer condition splits --- $NC(16) = 0.712 <
1$ holds, summability fails at $1.563$; the $\chi$-relevant
structure is exactly short-range, the r303 gap at $k \le 3$,
lag-2-dominated; lag-8 matching hits the level, breaks the slopes:
the mechanism is \textbf{two-scale}; the graduated ladder is
monotone in $\chi$, not in the margin).  \emph{The consequence
(sealed map (iii)):} \textbf{the global-profile mixing route of
the L2 lane is documented closed} --- L2 generic $\Leftrightarrow$
anti-concentration of an explicit block field with long-range
period-4 structure; return later with new tools.  \emph{What
stands honestly:} the exact identity dictionary (count, coherence,
$\chi$-lag), the two-scale split, $NC < 1$, the $-/-/+/+$ sign
pattern (Fractions-exact kz18/kz23), the marginal-functional
invariance ($10^{-15}$ on all $2100 + 1008$ builds).  \emph{The
hard reviewer rule (binding):} a round counts as progress only if
it adds new information, not if it re-expresses the same slack in
a new coordinate --- no further coordinate change counts.  Lean:
no new statement --- the stop carries no new exact target form
(measured ladder aggregates; the r273 guard pattern); the hole
stays \texttt{lstar\_subordination}.  Mincut base 4 / refined 5
unchanged; no RH statement.\par}

\par\smallskip\noindent\textbf{The architecture adjudication and
the wave-11 claim split (\texttt{PRIME.LSTAR.\allowbreak
ARCHITECTURE\_ADJUDICATION.01} [E], \texttt{PRIME.SOURCE.\allowbreak
RANKONE.\allowbreak UPDATE.\allowbreak IDENTITY.01} [E],
\texttt{PRIME.SOURCE.\allowbreak TENSOR.\allowbreak MECHANISM.01}
[O], \texttt{PRIME.L2.\allowbreak RENYI3.\allowbreak
PROVENANCE.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-26 (rounds
305--316 incl.\ 310b, notes DCXLV--DCLVIII;
\veri{v968\_architecture\_adjudication.py} with $27/27 + 28/28 +
30/30 + 33/33 + 34/34 + 32/32 + 32/32 + 29/29 + 29/29 + 35/35$
full record gates; the TEN probes embedded byte-exact in their
sealed \texttt{--smoke} stage, SPEC SHAs
\texttt{3bb365e1}/\allowbreak\texttt{ec2bb008}/\allowbreak
\texttt{d5147850}/\allowbreak\texttt{f8d99877}/\allowbreak
\texttt{fac7a8df}/\allowbreak\texttt{6c32f749}/\allowbreak
\texttt{6505dd10}/\allowbreak\texttt{841b3196}/\allowbreak
\texttt{92d35a3a}/\allowbreak\texttt{5c28b12b} pinned; plus the
module-own exact section --- the
r306 Hill/Lagrange chain with both witnesses and the R\'enyi bridge
with all constants, the r314 opening-flux lemma $F_1 \equiv 0$ with
the exact flux telescope, the collision counting identity
$3p_1p_2 - 2p_3 = 8(n + 3n(n{-}1))$ with the exact factor-8 mutant
break, an r311-class exact rational Farkas mini-certificate (a PD
target in the span with a forced negative generator weight,
strictly outside the cone), the TEN sealed verdict bars as exact
decision logic with tipping mutants, and the four-level
composition gate, in pure Fractions).  \emph{Delivered (the
reviewer's binding four-level structure):} \textbf{Level 1} ---
real formal theorems (Lean, already green; no Lean change ships:
\texttt{lstar\_terminal\_implies\_master}, the four closed Inertia
theorems, the real PrimeWindow construction, source exactness $+$
mass conservation of any folding $+$ the strong stabilization
form, the r309 rank-one update identities, the Hill monotonicity
$+$ R\'enyi-3${}\Rightarrow N_2$ bridge; sorries $9 \to 5$, the
two TRUE holes standing alone).  \textbf{Level 2} --- certified
finite statements (R\'enyi-3 on 57 rungs at $C = 1.069$, $A = 2$;
the fixed-head census $77/77$; the block-Green identity $+$ the
strict cone with exact rational Farkas certificates; paired-cone
soundness on 20714 steps; fold multiplicity exactly 2; 93\,\%
far-triple cancellation; the signed cubic identity with vanishing
boundary, Fractions-exact on the real data; the 21 small-$m$
certificates).  \textbf{Level 3} --- negative architecture
decisions verbatim (\textsc{fixed-head-dead};
\textsc{paired-cone-no-induction} $+$ \textsc{world-blind} --- B
does not overtake A; the r308 discriminator $=$ the budget sign /
a chordal restatement; \textsc{coefficient-sign-wall} --- Lane A
closed as cone language with the mechanism named: block-psd
membership without rank-one SDD, obstructed at the budget/border
sign; \textsc{floquet-expanding};
\textsc{triple-type-majorants-wrong}; \textsc{phi3-all-blind};
\textsc{two-regime-dead} with the anatomy: the obstruction family
cuts across the FCIX stratum, the first-5 protocol load-bearing,
kz55/kz67 near-one-block).  \textbf{Level 4} --- open mechanisms
typed (the constructive PSD-G rule as a question, the signed cubic
flux theorem, the quantitative source functional,
\texttt{crossing\_budget}, the definitional source bridge, L*, the
terminal).  \emph{The claim split (binding --- not one
\textsc{transfer} title):} \texttt{RANKONE.UPDATE.IDENTITY} [E]
carries \emph{only} the exact update identities (determinant
dictionary, Sherman--Morrison chain, $\pm(\eta\pm e_t)$ border
split, local Schur reserve, signed source updates);
\texttt{TENSOR.MECHANISM} [O] carries the open question whether a
source-pure cofinal control arises from them, registered
\emph{after} r313/r316 with the documented negatives.  \emph{The
provenance contract (RENYI3.PROVENANCE):} the pointwise GO stands
as a certified finite statement ($\sum q^3 \le
1.069(\log m)^2/m^2$ on 57 rungs, growing reserve; fiber target
$n_{\rm eff} \ge m/(1.034\log m)$); the provenance is open ---
trilogy state: the identity exact (r314), the functional blind
(r315), the two-regime form dead (r316); the R317 fork verbatim
(reviewer adjudication pending): a near-critical family coordinate
(TOP1/QMAX source-pure in advance; kz53 needs a second coordinate)
\emph{or} a certified exception family (the r287-F2 pattern).
\emph{Framing:} the universality-class insight (base fine-metric
--- \textsc{metric-only}, twin invariance; fiber recursive ---
\textsc{epstein} holds, \textsc{scramble} breaks; shared algebra,
not necessarily shared mechanism); the reviewer maturity raise
$4 \to 5$ (formal architecture $9.5$) documented as a
\emph{reviewer assessment}, not a machine-checked fact.  Mincut
base 4 / refined 5 unchanged; no round in flight at this cut ---
R317 waits on the reviewer fork; no RH statement.\par}

\par\smallskip\noindent\textbf{The extraction-chain red team, the
sliding bound, and the wave-12 adjudication
(\texttt{PRIME.REDTEAM.\allowbreak EXTRACTION\_AUDIT.01} [E],
\texttt{PRIME.L2.\allowbreak RENYI3.\allowbreak SLIDING\_BOUND.01}
[O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-27 (rounds
317--322, notes DCLXI--DCLXVI;
\veri{v969\_forks\_and\_redteam.py} with $38/38 + 25/25 + 39/39 +
25/25$ full record gates; the FOUR probes embedded byte-exact in
their sealed \texttt{--smoke} stage, SPEC SHAs
\texttt{04fbe5c0}/\allowbreak\texttt{f2d98683}/\allowbreak
\texttt{e68883ad}/\allowbreak\texttt{761b51d4} pinned; plus the
module-own exact section S0 --- the two-sided concentration
bracket $q_{\max}\Phi_{H1} \le \rho_2 \le \Phi_{H1}$ re-proved
from scratch with both mutant directions caught, the U1--U3
red-team witnesses in exact arithmetic (the $B = -1$ window
against the terminal type; the mesh-level-0 total node collision
reduced to three rational facts about $e$ on the certified bracket
$685/252 \le e \le 685/252 + 1/35280$, kill $0 < 0$ exact; the
adversary $(|F|+1)^2 < 5/7$ false for every $F$), the r320
separation/canonical-split certificates (witness mesh level 4 via
$2^7 < 3^5 < 2^8$, reduction $1/8 < 2/5$ exact; the level-0
failure), six sealed verdict bars with tipping mutants and the
wave-12 composition gate, in pure Fractions; no Lean call --- the
r319/r320 Lean rounds are consumed as \emph{reports}, their
artifacts in \texttt{rh/lean/} re-verified by \texttt{lake build}
and the RH suite).  \emph{Delivered (the audit and the repair):}
the r319 reconstruction found three kernel-checked \emph{type
inconsistencies} three levels above the boxes (U1: the r310b
bridge bound neither $u$ nor $B$ --- a $B = -1$ window represents
the empty spec against \texttt{terminal\_positive\_main}; U2: the
mesh-level-0 tolerance admits the total node collision and
$p = X - 1$ forces $0 < 0$ against the L* type; U3: the pre-r320
\texttt{pair\_margin\_main} forces its source predicate empty)
plus the mesh-vs-anchor cofinality seam --- \textbf{the false
cofinality direction is documented, not solved}; r320 repaired the
interface completely (bridge retype with $u$/$B$ fidelity $+$
separation discipline $+$ \texttt{budget\_pos};
\texttt{SourceExact} for the additional arch/border finding; the
canonical extraction with \texttt{canonical\_split} \emph{proved};
three permanent sorry-free guards; the sorry-free witness; census
$5 \to 5$ with two typed retypes).  \emph{The honest chain reading
(binding):} ``two lemmata $\Rightarrow$ RH'' does not survive the
literal reading --- the two lemmata give \emph{only window-local
master positivity} for the opaque, currently witness-less
\texttt{MainWindow}.  \emph{The three-level proof graph (reviewer
adjudication, binding; the two holes are named
\textbf{window-local} everywhere from wave 12 on):} Level A ---
local window mathematics, \emph{proved} ($L^*(w) + T(w)
\Rightarrow \mathrm{MasterPositive}(w)$); Level B --- the
universal window family, the two arithmetic window-local holes;
Level C --- extraction to Weil/RH, \emph{open} (source-exact
realization $+$ admissible transport $+$
convergence/stabilization in the correct directed order $+$
density/continuity).  Two binding caveats: the sorry census does
\emph{not} measure the Level-C distance (the missing extraction
architecture appears in no \texttt{sorry}), and
\texttt{SourceExact} must not stand as a free assumption --- the
named open Lean target is \texttt{CanonicalPrimeWindow} $+$
exactly one construction theorem
\texttt{sourceExact\_buildPrimeWindow} (load-bearing; the
extraction-repair fork R325 in flight, not consumed).  \emph{The
fiber fork (SLIDING\_BOUND):} fork (b) sealed
\textsc{whac-a-mole}(kz53, kz83) by contract (the spike family
$B = \{kz55, kz67\}$ is real, $C_B = 1.0536$; the $F_A$ top-3 are
exactly the near-critical family, but as a \emph{continuum});
fork (a) fired \textsc{sliding-bound-go}($G_{SQ}$): $\sum_j q_j^3
\le 1.3056\,F_A(w)^2\,(\log m)^2/m^2$ for $m \ge 73$, $0/39$ test
violations, all four named violators inside at reserves
$7.0$--$9.6$ (honest: $C_{\rm impl} = 7.97$ is $7.5\times$ looser
--- form, not sharpness; the demand is the two-part $F_A$/qmax
provenance split; the terminal sharpened to the
QMAX${}\times M_2$ shape route --- R324 in flight, not consumed).
\emph{The typed reviewer assessment (supersedes the wave-11
$4 \to 5$ scalar):} finite dictionaries $9.5/10$, formal
window-local architecture $9/10$, terminal mechanism $6/10$, L*
mechanism $2/10$, source/extraction architecture $2$--$3/10$,
complete RH proof path $\sim 3/10$, audit quality $9/10$ ---
``stronger as a window-local research program; as a complete RH
proof chain currently less far than previously assumed --- the
removal of a shortcut that did not exist.''  Mincut base 4 /
refined 5 unchanged; no RH statement.  \emph{State (wave 13,
\veri{v970\_extraction\_and\_composition.py}):} both in-flight
rounds are executed and consumed --- the extraction-repair fork
R325 fired \textsc{elementwise-stabilization-go} and the named
open Lean target is \emph{implemented and proved} in r326
(\texttt{sourceExact\_buildPrimeWindow}; census $5 \to 8$ with
three named typed Level-C statements --- the census caveat
partially discharged; see the wave-13 block below); the terminal
QMAX${}\times M_2$ route composed MEASURED in R324
(\textsc{pileup-grows-subcritical} $+0.172$ --- subcritical also
at the r328B chain-honest bar $0.188$; $m_0^*$ record $10^{59.6}$,
chain-honest $10^{238}$;
\texttt{PRIME.L2.\allowbreak RENYI3.\allowbreak
MEASURED\_COMPOSITION.01} [O]).\par}

\par\smallskip\noindent\textbf{The elementwise extraction repair
and the measured terminal composition --- the wave-13 block
(\texttt{PRIME.EXTRACTION.\allowbreak ELEMENTWISE.01} [E],
\texttt{PRIME.L2.\allowbreak RENYI3.\allowbreak
MEASURED\_COMPOSITION.01} [O]).}
{\sloppy
\emph{Registered and delivered / registered} 2026-08-27 (rounds
323--327 incl.\ the r326 Lean round, notes DCLXVII--DCLXXII;
\veri{v970\_extraction\_and\_composition.py} with $36/36 + 36/36 +
18/18 + 34/34$ full record gates; the FOUR probes embedded
byte-exact in their sealed \texttt{--smoke} stage, SPEC SHAs
\texttt{9a6696f8}/\allowbreak\texttt{dc36cacb}/\allowbreak
\texttt{31277f91}/\allowbreak\texttt{11e4fd40} pinned (r324
imports the embedded r324-pre, r327 the embedded r324-pre and
r324); plus the module-own exact section S0 --- the r324
interpolation $M_3 \le q_{\max} M_2$ with slack exactly $1/36$ and
double equality at the flat profile, the $F_A$ identity
$F_A B \log m = m\,q_{\max}$ as exact constructional algebra, the
r327 group ledger with the two-ancestor bound (both mutants break
by exactly 1), the r325 onset formula $\alpha^* = (n_g+1)D_0/2$ as
an exact toy stabilization, the $m_0^*$ log-space solver verbatim
(landing $10^{59.6}$; the disclosed pre-freeze linear-space bug
rebuilt and caught), the r328B chain-audit gate S0-T5d, six sealed
verdict bars with tipping mutants
and the wave-13 composition gate, in pure Fractions; no Lean call
--- the r326 Lean round is consumed as a \emph{report}, the r323
abort as a \emph{note}: the planned L* fork was cleanly aborted
before any write access, nothing counts as a round).
\emph{Delivered (ELEMENTWISE, the extraction repair):} r325
adjudicated the three reviewer variants for the R319 cofinality
seam --- \textsc{elementwise-stabilization-go}: on the native v749
class all three channels (comb/arch/pole) of the canonical tower
windows stabilize \emph{exactly} at the predicted finite anchor
onset and are constant under dyadic mesh refinement --- no
mesh-cofinal ladder, no transport, \textbf{$H_{\rm cof}$ is not
needed and is replaced as the target route} (variant B $=$ the
construction substrate, variant C $=$ the quantified density step
inside the rigorous interpolation envelope, no wall margin
anywhere); r326 implemented the architecture
(\texttt{rh/lean/RH/Elementwise.lean}, merged and green):
\texttt{sourceExact\_buildPrimeWindow} \emph{proved}
(\texttt{SourceExact} eliminated as a free assumption from the
extraction route), the comb stabilization elementwise
\emph{proved} (onset predefined from the element's support alone),
\texttt{weil\_nonneg\_of\_windowlocal} \emph{proved} (the
extraction without the ladder: one finite instantiation per
element); sorry census $5 \to 8$ with three named typed Level-C
statements (arch/pole transcription classical, source completion
definitional).  \emph{The honest formulation of the new connection
(binding, verbatim):} ``two window-local lemmata (over the
...
(definitional).''  \emph{Registered (MEASURED\_COMPOSITION, the
terminal end state):} r324 --- after the binding mid-round course
correction, with the pre-work banked as r324-pre
(\textsc{fa-bounded-distributional} $+$ \textsc{qmax-share-open}
$+$ \textsc{not-composed}; $F_A$ de-black-boxed by the exact
identity $F_A B \log m = m\,q_{\max}$) --- fired
\textsc{pileup-grows-subcritical} ($e_{\rm tot} = +0.172 < 0.224$,
decided): the $O(\log m)$ scale count certifies pointwise
($C_{\rm NSC} = 2.0258$, $0/39$), the per-scale mass is refuted
--- the near-critical windows are \emph{one heavy source scale};
the composition typed MEASURED: $\sum_j q_j^3 \le
8.941\,(\log m)\,m^{+0.172}/m^2 \Rightarrow N_2 \ge m^{0.888}$ for
all $m \ge m_0^* = 10^{59.6}$, the gap $(274, 10^{59.6})$ the
disclosed extrapolation hypothesis --- no cofinal claim (the
independent chain audit r328B, carried as a chain annotation with
the record sealed: the bar $0.224$ used the atom-level need
$0.888$, the block-level need is $908/1002 \approx 0.9062$ --- the
chain-honest bar is $0.188$, the verdict survives $0.172 < 0.188$
exactly, the chain-honest $m_0^*$ is $10^{238}$, the going-forward
bar is $0.188$; exact v970 gate S0-T5d); r327
located the spike structurally (\textsc{cap-partial}, anatomy
SOURCE): on all 65 rungs the heaviest group of the argmax block is
exactly \emph{one} $\beta/\omega$ fold pair (kz53 $=$ one
bulk/window coincidence at $88.8\,\%$); the $\Lambda$-pair cap
refuted, \textbf{the group count certified} ($C_{NG} = 2.6351$,
$0/39$, named $4/4$ --- the third certified $O(\log m)$ count);
\emph{the demand (the coincidence geometry):} where can the single
heavy bulk/window coincidence \emph{sit} --- a source-geometry
question, not group count (certified) or multiplicity (proven);
alternatively the MEASURED composition is the honest end state of
the route.  \emph{Honest boundary:} the window-local positivity
premise is proved nowhere; the two true window-local Lean holes
are unchanged; L* stays paused; nothing bounds anything beyond the
measured rungs.  Mincut base 4 / refined 5 unchanged; no RH
statement.\par}

\par\smallskip\noindent\textbf{The wave-14 consolidation --- terminal
martingale, cover/growth/K2, the closed L* margin chain, the Borodin
duality and the second arithmetic
(\texttt{PRIME.TERMINAL.\allowbreak DENSITY\_MARTINGALE.01} [E],
\texttt{PRIME.L2.\allowbreak COVER\_GROWTH\_K2.01} [E],
\texttt{PRIME.LSTAR.\allowbreak MARGIN\_CHAIN.01} [E],
\texttt{PRIME.LSTAR.\allowbreak DUAL\_HOLE.01} [E],
\texttt{PRIME.WORLD.\allowbreak DIRICHLET\_MATCHED\_FRAME.01} [E]).}
{\sloppy
\emph{Registered and delivered} 2026-08-27 (rounds 334--357, notes
DCLXXXI--DCCIX; reviewer-authorized after the double substance
condition r355 $+$ r357; five modules
\veri{v978\_terminal\_density\_martingale.py} /
\veri{v979\_cover\_growth\_k2.py} /
\veri{v980\_lstar\_margin\_chain.py} /
\veri{v981\_lstar\_borodin\_duality.py} /
\veri{v982\_dirichlet\_matched\_frame.py}; each re-derives its exact
layer from scratch --- sympy symbolic $+$ pure Fractions, no probe
imports --- and re-runs the sealed round verdicts as exact decision
logic on the frozen record aggregates with tipping mutants; the
eighteen discovery probes stay sealed in
\texttt{experiments/tfpt-discovery/}, pinned and re-verified by the
RH suite).  \emph{Delivered (theorem-grade exact layers):} the
density martingale from mass conservation alone with the moment
dictionary $E[X_\infty^3] = m^2 M_3$, the tilted tower, the exact
hand-off and the pair ceiling $\Gamma \le 4$ (v978); the FAB
identity, the data-free K1 formula $R_{\rm ALG} = 4^{1/3} \to 8/5$
and the mesh identity $h - \nu u \in (0, 3/2]$ (v979); the one-line
identity, the two-level dressed-scalar theorem, the pinning theorem
($m_2' = $ margin identically under orthonormal top-2 geometry),
the rate-equality theorem and the $\rho_K$ identities (v980); the
Borodin complementation at half filling with rational conjugator,
the equivalence $L^* \Leftrightarrow R > \tfrac12 I$, and the
reciprocal dual weight $u^\vee \propto c_j(1-x_j)/|f|$ that makes
the r354 anti-correlation \emph{duality algebra by design} (v981);
the derived $\chi$-archimedean side with bitwise trivial reduction,
exact conductor shift and parity kernel (v982).  \emph{Registered
(census, honest):} the first certifying three-arm cover
($m_0^* = 10^{22.6}$), the P02 predictor, the growth ceiling
$C_{\rm FAB} = 14.93$ and every $m_0^*$ of the lane are
\emph{Frame-A census} (binding r353: the second construction family
breaks the ceiling and kills the reserve floor at $m \sim 760$ ---
the sliding coverage is finite); K2 ($\le 11.87$) is the sole
cross-family survivor, NU-free over three aspects and $0/126$ on
the second arithmetic; the L* margin chain is complete and typed
(weights dictionary / deficits candidate law / pinning theorem /
$\rho_r = \varphi$-difference computable), the duality verdict is
\textsc{duality-reparam-only}, and the lane is \emph{frozen} as a
specialist problem at \texttt{rh/problem/lstar\_problem.tex} with
the one-object question halved (the $\sim500\times$ smallness of
by-design-coupled blocks, localized in local dual two-point data);
\textsc{second-arithmetic-lives}: the r330 death was a frame
artifact, the wall holds on $n = 3$ arithmetics on 126 windows and
the matched scramble breaks --- the wall needs the arithmetic; the
zeta-graded signatures (exact pinning, saturated reserve, $\varphi$
suppression, counting constants) type the L* margin legislation as
a statement about how close zeta's world sits to the wall.
\emph{Honest boundary:} no cofinal claim anywhere (the r324
MEASURED composition stays the terminal end state); the equivalence
is a reparametrization of the open condition;
\texttt{PRIME.LSTAR.\allowbreak SUBORDINATION.01} and
\texttt{PRIME.L2.\allowbreak RENYI3.\allowbreak SLIDING\_BOUND.01}
stay [O] with wave-14 notes; everything census-side is finite,
kein Satz.  Mincut base 4 / refined 5 unchanged; no RH statement,
no GRH statement.\par}

\par\smallskip\noindent\textbf{The gapcode disenchantment
(\texttt{PRIME.DIVISOR210.\allowbreak GAPCODE.01}).}
{\sloppy
\emph{Registered} 2026-08-09, an honest negative/limit row
(\veri{v886\_register\_gapcode\_null.py}, $24/24$,
\textsc{gapcode-measured}; five honest SPEC amendments on
record, none loosening a criterion).  \emph{Measured:} the
error-code reading of the $210$-register is literally true on
the prime side --- the register bits $\{2,3,5,7\}$ are the first
four parity checks of the sieve (rate $48/210$; $16$ syndrome
labels; $0$ bits of message capacity); the hard floor $g_k \ge
w(p_k \bmod 210)$ holds with zero violations in
$5.76\times10^{6}$ consecutive pairs;
$I(\text{state};\text{gap}) = 1.14$ bits --- and the code is the
sieve: a zero-parameter wheel--Cram\'er null explains $R^2 =
0.99839$ of the state profile, the arithmetic excess is $4.02\%$
and shrinking ($21.81 \to 4.02\%$ over $X = 10^5$--$10^8$), no
knee at four checks, $210$ on the foreign-wheel trend ($z =
-0.61\sigma$), all eight anchor Walsh seats below $2.5\sigma$
exactly as CRT predicts.  \emph{The bound:} everything the
register knows about prime gaps, every $4$-prime wheel knows;
nothing moves with $c_3$ or $\varphi_0$; the register's
uniqueness remains the arithmetic forcing $\prod(p-1) = 48$
(\texttt{v868}).  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The rounds-42--49 consolidation
(the complete certified ladder, the finite Pfaffian/$K_6$
signature, the four route decisions, the surviving positivity
architecture, and the M\"obius kills).}
{\sloppy
\emph{Recorded} 2026-08-09 (a campaign note, round CVIII;
fourteen frozen sandbox probes, every spec amendment fail-first
on record; the promoted verification modules
\veri{v887\_certified\_ladder\_complete.py},
\veri{v888\_finite\_pfaffian\_signature.py},
\veri{v889\_route\_decisions.py},
\veri{v890\_total\_positivity\_flow.py} and
\veri{v891\_moebius\_firewall\_kills.py} embed their discovery
probes byte-exact).  The campaign completes the certificate
rollout ($\sigma_h > 0$ proven on all $42$ reachable rungs),
closes the finite Pfaffian/$K_6$ signature of the compiler (the
canonical sign character; the self-hosting counting theorem;
the flavor hexagon as the automorphism orbit), decides the four
routes of the sum-rule program (the wall dies off-diagonally;
PNT is insufficient; the diagonal input is pair-correlation
class --- the sum-rule contract retyped conditional), measures
the surviving positivity architecture (flag positivity -- all leading principal minors, Sylvester; typing corrected per the round-51 review, classical total positivity is not claimed; the
wall margin as the pole distance of the $s$-flow; multi-factor
inheritance), and honestly kills the
M\"obius/carrier-invariance route with mechanisms.  The
registrations and updates follow as the blocks below (plus the
dated retypes on the sum-rule and tail contracts above).  Full
narrative in the prime-front paper and Paper~1; audit cards in
Paper~3.  No marker moves; no RH statement.\par}
...
{\sloppy
\emph{Registered and delivered} 2026-08-09
(\veri{v887\_certified\_ladder\_complete.py}, $7/7$,
\textsc{certified-ladder-complete}).  \emph{Proven:} $\sigma_h >
0$ on \emph{all} $42$ reachable rungs of the deployed window
ladder --- $12$ rungs ($h \le 300$) by exact-rational integer
Bareiss/LDL certificates, $30$ rungs by validated-precision
\texttt{mpmath} Cholesky at dps $120/200$ (precision doubling;
every pivot $> 10^{6}\times$ the accumulated per-pivot rounding
bound at both precisions); $0$ refusals, $0$ out-of-reach; the
module closes the two deepest rungs $h = 859/878$ with floors
$\ge 8.352/9.187\times10^{-6}$ and pivot-to-bound ratios
${\sim}10^{115}/10^{195}$; the Epstein control refused at pivot
index $10$ by the identical machinery; the $40$-rung parent
rollout stays sandbox evidence, runtime-excluded.  \emph{Typed
honestly:} the validated-precision tier is never labeled
exact-rational; proven modulo the declared conservative
lag-evaluation error model (validated-numerics standard); the
ball-arithmetic closure and the Lean composition through
\texttt{krein\_cofinal\_weil} remain the named steps of
\texttt{PRIME.PORT.TAIL.01}; the asymptotic tail is untouched.
\emph{Interval note (2026-08-09, round CXV):} the error model is
upgraded --- \veri{v897\_certified\_interval\_ladder.py}
(\texttt{PRIME.PORT.BALLLADDER.01}, below) re-proves all $42$
rungs with \emph{rigorous} interval-arithmetic shifts ($15$
exact-rational $+$ $27$ validated-precision; Epstein refused at
pivot $10$ by the identical interval machinery); the informal
conservative $\varepsilon_c$ model is retired from the ladder,
and only the Lean composition remains named.
\emph{Kill:} a certified pivot failure on any rung under the
frozen error model.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The $Gr(2,5)$ Pl\"ucker reading,
the Specht channels, the Hadamard anchor and the vacuum
selection (\texttt{DOILY.PLUCKER.\allowbreak GR25.01},
closed).}
{\sloppy
\emph{Registered and closed} 2026-08-09
(\veri{v888\_finite\_pfaffian\_signature.py}, $27/27$,
\textsc{s6plucker-closed}; exact integer/sympy).  \emph{Exact:}
the $15$ doily lines are the $15$ monomials of the five
Pl\"ucker quadrics of $Gr(2,5)$ --- bijection $15/15$ with
perfect sign match (every quadric reads $(+,-,+)$, the
\texttt{v880} Hodge table); the $S_6$ duad representation
decomposes $15 = 1 + 5 + 9$ with the doily operator acting by
the exact scalars $(1, 0, 2/3)$, $2/3 = e_3/(e_2 - e_3)$ at the
anchor $(4, 5, 2)$; the $q^{*}$ zero set is a $(16, 6, 2)$
Hadamard difference set with anchor signature $(2^{e_1}, p_2,
e_3, \binom{e_2}{2}) = (16, 6, 2, 10)$; and the
vacuum-selection theorem: $\mathrm{Stab}(q^{*})$ of order $120$
acts faithfully and surjectively as $S_5$ on the remaining five
Arf-$1$ refinements, orbits $[1, 5, 10] = (\Lambda^0,
\Lambda^4, \Lambda^2)$.  \emph{Controls:} Arf-$0$ census
$(16,10,6)$; a secant breaks the bijection; the wrong scalar
breaks the channel law.  The physical vacuum-selection
mechanism stays [O].  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The flavor-hexagon automorphism
orbit (\texttt{CFIN.AUT.\allowbreak FLAVOR.C6.01}, closed).}
{\sloppy
\emph{Registered and closed} 2026-08-09
(\veri{v888\_finite\_pfaffian\_signature.py}, $19/19$,
\textsc{c6flavor-measured} $+$ \textsc{intertwined}).
\emph{Exact:} $\mathrm{Aut}(C_{\rm fin}) \cong C_6$ by
exhaustive enumeration; the pinned generator satisfies $g^2 =
\sigma$ (the deployed family $3$-cycle) exactly and $g^3 =$ the
$W$-edge sheet swap; the action on $\Lambda^2 E$ has cycle type
$1 + 3 + 6$ with orbits equal to the charge shells; the
$g$-orbit of $\{f_1, w_1\}$ is the deployed hexagon transport
edge for edge ($D_6$-equivalent); and $(|Z_2|/\Nfam)^{|
\mathrm{Aut}(C_{\rm fin})|} = (2/3)^6 = 64/729$ equals the
frozen recovery value as an exact Fraction.  \emph{Typed
honestly:} the exponent reading is a consistent \emph{new
reading}, not the unique derivation (the corpus route via $p_2
= |R_+(A_3)|$ stands).  \emph{Control:} $0$ of $600$ symplectic
non-automorphisms factor; scrambled transport fails the $D_6$
match.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The fermionic Pfaffian signature
(\texttt{DOILY.PFAFFIAN.\allowbreak WICK.01}, closed --- the
$K_6$ motif, the sign theorem, the self-hosting counting
theorem, and the mod-30 clock).}
{\sloppy
\emph{Registered and closed} 2026-08-09
(\veri{v888\_finite\_pfaffian\_signature.py}, $42/42$,
\textsc{k6pfaffian-closed}; exact integer/sympy, first frozen
run).  \emph{Exact:} the $K_6$ motif theorem --- the $35$ lines
of $PG(3,2)$ are exactly $15$ perfect matchings (the doily) $+$
$10$ triangles through the vacuum vertex (the secants) $+$ $10$
avoiding it (the externals), with $q^{*}(e) = 0$ iff $e$
touches the vacuum; the sign theorem --- the Pfaffian signs
match the Hodge table under the canonical character $\chi(i) =
(-1)^{i+1}$, triple-identified (vacuum-row Laplace,
$\Lambda^4/\Lambda^5$ volume form, $C_6$ crossing character; a
global gauge, not a line-dependent fudge --- the flipped-$\chi$
control produces exactly the $3$ expected mismatches); the
self-hosting counting theorem --- $\binom{g+1}{2} = g!!$ has $g
= 5$ as its only nontrivial solution, $(g-2)!! = 3 = \Nfam$;
and the mod-$30$ anchor clock, period exactly $4 = |\mu_4|$,
\textsc{clock-distinguished} (period-$4$ moduli exactly $\{5,
10, 15, 30\}$).  \emph{Typed honestly:} the counting theorem is
[E]; the Wick-compiler premise stays [O] --- P2 narrowed to one
premise, not eliminated; the operative-Coxeter-$30$ reading is
separately dead (sandbox); no canonical
vertex--one-factorization bijection exists (outer-automorphism
obstruction, report only).  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The diagonal separation
measurement (\texttt{PRIME.PORT.\allowbreak DIAGSEP.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v889\_route\_decisions.py}, $8/8$,
\textsc{diagsep-measured (diag-intermediate)}).
\emph{Measured:} under the exact off-critical injection family
the wall dies $100\%$ through off-diagonal coherence --- at the
kill point the diagonal testing profile is statistically frozen
(max $T_m = 0.94$--$0.99$; the coherence share equals the
entire baseline diagonal margin); the separation ratio is
$2.0$--$2.7$ on the head rungs and $> 10$ deep (no diagonal
crossing up to $10\times$ the wall's kill amplitude).
\emph{The typed reading:} the $O(1)$ diagonal margin does not
track the exponentially collapsing wall margin --- $T_h \le 1$
is a genuinely weaker finite statement; the RH-scale content
stays in the coherence share.  \emph{Kill:} a rung on which the
diagonal crosses before the wall.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The zone split
(\texttt{PRIME.CASE.\allowbreak ZONESPLIT.01}, frozen).}
{\sloppy
\emph{Registered and frozen} 2026-08-09
(\veri{v889\_route\_decisions.py}, $12/12$ under SPEC v2,
\textsc{zonesplit-measured (zone-frozen($\theta^{*} = 0.700$))}).
\emph{Measured:} outside the critical zone $a > h^{2\theta^*}$
the testing margin $1 - T$ is uniformly positive on all $42$
rungs (deep-half infimum $+0.0214 \ge$ the $0.02$ bar;
cut-stable; $\theta = 2/3$ fails at $+0.0191$); inside, the
margin law is ${\sim}0.87/\log^{2} h$ --- the bar the
arithmetic input must beat.  \emph{Typed honestly:}
\textsc{outliers-dangerous} (on kz 13 the margin minimizer
coincides with a top-3 oscillation node --- the density defects
are not provably measure-zero for the margin).  \emph{Control:}
scramble drives the outside margin to $-1.4\times10^{4}$.
\emph{Kill:} a rung with negative outside margin at the frozen
$\theta^{*}$.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The PNT decider
(\texttt{PRIME.CASE.\allowbreak PNTGAMMA.01}, decided).}
{\sloppy
\emph{Registered and decided} 2026-08-09
(\veri{v889\_route\_decisions.py}, $10/10$ under SPEC v2,
\textsc{pntgamma-measured (pnt-insufficient)}).
\emph{Measured:} the four matched worlds (mass $\times$ mask in
$\{$actual, PNT-smooth$\}$, identical pipeline, exact
tent-integral construction self-tested at $6\times10^{-12}$)
show the truth's port correction $\gamma > 0$ at every alias on
every rung, \emph{but} the PNT-smooth reference itself has
$\min\gamma < 0$ on every rung ($-0.96$ to
$-7.8\times10^{-3}$) --- the smooth reference violates the very
inequality the diagonal theorem needs; the dominant rescue
piece is the mask--mass \emph{interaction} (positive on $4/4$
deep rungs); the measured arithmetic deviation $\varepsilon_A =
0.23$--$6.2$ is $6$--$970\times$ larger than every gap.
\emph{The decision:} the diagonal theorem's arithmetic input is
genuinely stronger than PNT on this family --- no classical
zero-free-region-strength input closes it by smooth comparison.
\emph{Control:} scramble flips $6/6$ port gammas.  No marker
move; no RH statement.\par}

\par\smallskip\noindent\textbf{The signed-homotopy
classification
(\texttt{PRIME.CASE.\allowbreak SIGNEDHOMOTOPY.01}, decided).}
{\sloppy
\emph{Registered and decided} 2026-08-09
(\veri{v889\_route\_decisions.py}, $12/12$ under SPEC v2,
\textsc{homotopy-measured (homotopy-indefinite-arithmetic)};
ward metrology only --- run 1 ended honestly
\textsc{pipeline-broken} on an over-sharp deep ward).
\emph{Measured:} the signed homotopy identity along the exact
PNT-to-truth interpolation is exact calculus
($4.3\times10^{-14}$ with exact rational mask-crossing times);
$66.3\%$ of the $14047$ samples have $J < 0$ (pointwise
positivity dead); the fixed-chamber quadratic kernel is
concavity-locked negative-semidefinite on both frozen rungs;
the polynomial response drains the gain ($-17$ to $-231\%$,
worsening with $h$).  \emph{The classification:} the truth
endpoint wins only through the signed \emph{first-order}
alignment of the arithmetic residual with the
extremal-polynomial weights --- the diagonal route's required
input is \emph{pair-correlation class}, not any PSD kernel
positivity; \texttt{PRIME.CASE.SUMRULE.01} is retyped from
unconditional candidate to conditional reduction theorem (the
dated update above).  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{Flag positivity (Sylvester) and the
pole-free $s$-corridor
(\texttt{PRIME.PORT.\allowbreak HIROTA.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN --- the finite architecture
measured (\veri{v890\_total\_positivity\_flow.py}, $10/10 +
7/7$, \textsc{hirota-measured (minors-positive)} $+$
\textsc{sflow-measured (corridor-clear)}).  \emph{Measured:}
every nested leading principal minor $\tau^{(k)}_h$ of $I -
C_h$ ($k = 1, \dots, 12$) is strictly positive on all $37$
full-window rungs --- the section family is \emph{flag-positive}
(Sylvester: all leading principal minors positive; classical
total positivity of all minors is not claimed), a strictly
stronger finite statement than the wall
scalar; the Desnanot--Jacobi identity is warded exact; the
$s$-corridor $s \in [0, 1]$ is pole-free on every rung (the
controls pull their pole inside), and by Cauchy interlacing
$s^{*}_h = 1/\lambda_{\max}(C_h)$, so $s^{*}_h - 1 =
\tau_h/(1 - \tau_h)$ --- the wall margin \emph{is} the pole
distance of the integrable $s$-flow (ladder slope $-2.90$).
\emph{Typed honestly:} no naive rung-to-rung bilinear closure
in raw $h$ (median residual $0.655$); the Toda form holds
pointwise ($0.001$) but with drifting coefficients ---
\textsc{toda-varying}.  \emph{The demand:} the multi-factor
inheritance question as a total-positivity propagation
statement, derived from the IIKS generators.  \emph{Kill:} a
full-window rung with a non-positive section minor.
\emph{Skeleton update} (2026-08-09, round CXI): the
propagation question is answered in \emph{bounded-margin} form
by \veri{v892\_closure\_architecture.py} and
\veri{v893\_relative\_margins.py} (the hermitian congruence
margin $\eta$ and the flag chain; the soft flag is the $\tau$
scale after the kz-21 coordinate artifact) --- the demand
narrows to the $O(1)$ ratio bound
(\texttt{PRIME.PORT.ETASOURCE.01}).  \emph{Dictionary update}
(2026-08-24, round 226): the \textsc{toda-varying} finding is
resolved into an exact dictionary --- the bilinear Toda form
holds with the source-pure coefficient $\hat\gamma_n/\gamma_n$
from a scaled \emph{signed} Stieltjes recursion, and its
positivity is \textsc{wall-equivalent} (quasi-definiteness of
the signed defect measure $\mu - \nu$;
\veri{v955\_tau\_iiks\_toda\_dictionary.py},
\texttt{PRIME.PORT.TAU.FINITE.IIKS.01} [E]); no source-pure
positive cone was found (\textsc{hirota-cone-go} not reached
--- measured and closed via the Cauchy--Schwarz route).  No marker
move; no RH statement.\par}

\par\smallskip\noindent\textbf{The multi-factor determinant
anatomy (\texttt{PRIME.PORT.TAU.\allowbreak MOEBIUS.01},
decided).}
{\sloppy
\emph{Registered and decided} 2026-08-09
(\veri{v890\_total\_positivity\_flow.py}, $14/14$ under SPEC
v2, \textsc{taumoebius-measured (bridge-diffuse)}).
\emph{Measured (the reviewer's key run):} the exact
factorization $Q_h = \det(W_{h+1})/\det(W_h) = \prod_i (1 +
\mu_i)$ is warded at $9.2\times10^{-12}$; the truth keeps every
factor off $-1$ on all $31$ full-window steps over $24$ orders
of magnitude, while the frame-surviving smooth-mass world
crosses factors on $22$ of its $28$ steps (first at $h\,210 \to
218$) and flips $Q$ thirteen times --- the arithmetic detector
fires.  \emph{The decision:} the one-scalar M\"obius bridge is
closed --- \textsc{bridge-diffuse} (bulk factor margin $0.005$
vs the frozen $0.5$ bar; the fitted M\"obius datum $|\alpha|
\sim 10^{-3}$ is determinant-inert); positivity inheritance, if
it exists, is a multi-factor/total-positivity statement, not a
scalar cocycle inequality.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The M\"obius kill battery
(\texttt{PRIME.PORT.MOEBIUS.\allowbreak KILLS.01}, decided).}
{\sloppy
\emph{Registered and decided} 2026-08-09
(\veri{v891\_moebius\_firewall\_kills.py}, $10/10 + 13/13 +
7/7$, \textsc{crfirewall-measured (cr-dead)} $+$
\textsc{gaugefw-measured (gauge-made)} $+$
\textsc{moebiuscensus-measured (undecided)} --- the verdicts
are kills and are recorded as such).  \emph{Measured:} the
earlier carrier-invariance finding was a normalization artifact
--- fit-free gauge-free cross-ratios over $8126$
well-conditioned quadruples: pooled median defect $0.128$
(bars $10^{-3}/10^{-2}$), anchor triples disagree, the steps do
not compose; none of the three source-frozen gauges is
raw-invariant (two contrast-degenerate --- invariance vacuous;
the determinant-phase frame shows real motion); the scalar
cocycle does not track the wall ($\operatorname{corr} =
0.13$); the Jacobi-transfer identification is dead ($d_P =
0.997$); and the six-world census shows the arithmetic
separating \emph{at the frame} --- every control world
including the smooth-mass ones loses window subcriticality
(truth $41/41$ alive, all controls $0$), and scramble beats
truth on the bare M\"obius bars ($0.0060/0.0011$ vs
$0.0218/0.0446$): rank-2 window kinematics, not arithmetic
dynamics.  \emph{The surviving remnant (the named object):} the
deep-core $8$-node cross-ratio coherence ($4.3\times10^{-4}$ on
truth vs $2.7\times10^{-2}$ smooth --- prime-mass-driven);
related sandbox kills recorded (the static Cauchy
identification dead; $\mathrm{dev}/\tau$ grows at slope
$+1.46$ --- the Cauchy route can never certify the wall).  No
marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The rounds-50--53 campaign (the
closure architecture of the wall, the bounded-margin skeleton,
the diagonal refinement, the collectivity, and the Wick
functor arc).}
{\sloppy
\emph{Recorded} 2026-08-09 (a campaign note, round CXI;
fourteen frozen sandbox probes, every spec amendment
fail-first on record; the promoted verification modules
\veri{v892\_closure\_architecture.py},
\veri{v893\_relative\_margins.py},
\veri{v894\_diagonal\_refinement.py},
\veri{v895\_collective\_comb.py} and
\veri{v896\_wick\_block\_functor.py} embed their discovery
probes byte-exact).  The campaign gives the wall its closure
architecture (the exact hermitian congruence with a
non-decaying inheritance margin; the flag/pivot chain positive
by pure algebra; the honest two-sided Rouch\'e kill; the wall
as one fixed $8\times8$ Schur core), sources both bounded
margins and prints the theorem skeleton whose \emph{one}
unconditionally open statement is the $\tau$-sign inheritance
in bounded-margin form, makes the conditional diagonal
contract concrete (the exact band-limited prime kernel, frozen
in-probe), measures the collectivity of the arithmetic (the
one-sided law; the deep core as the entire multiplicative von
Mangoldt comb), and closes the Wick functor arc of the finite
compiler at the structure level (the proved scalar
obstruction; the block resolution; the seam-diagonal demand).
The registrations follow as the blocks below (plus the dated
skeleton update on \texttt{PRIME.PORT.HIROTA.01} above).  Full
narrative in the prime-front paper and Paper~1; audit cards in
Paper~3.  No marker moves; no RH statement.\par}

\par\smallskip\noindent\textbf{The hermitian congruence
inheritance form
(\texttt{PRIME.PORT.\allowbreak RELCONG.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v892\_closure\_architecture.py}, $16/16$,
\textsc{relcong-measured}).  \emph{Measured:} $A_{h+1} =
A_h^{1/2}(I + H_h)A_h^{1/2}$ exact ($6.5\times10^{-14}$; $H_h
= A_h^{-1/2}\Delta_h A_h^{-1/2}$ exactly hermitian); the
margin $\eta_h = 1 + \lambda_{\min}(H_h)$ is
\emph{non-decaying} (min $0.0050$, median $0.29$, power slope
$+0.108$ while $\tau$ collapses at $-2.74$) --- the
inheritance inequality is bounded-margin-shaped, not
collapsing; $\rho(W^{-1}\Delta C)$ was a misleading statistic
(strongly non-normal: $\|M\|/\rho$ up to $26$, $\kappa(V)$ up
to $134$); the smooth-mass world is \textsc{cone-exited} on
all $28$ pairs.  \emph{Mini-theorem (verified):} base PD $+$
$\eta_h > 0$ for all $h$ $\Rightarrow$ the whole ladder
(Sylvester congruence).  \emph{Source:} the $\eta$ scale is
explained by \texttt{PRIME.PORT.ETASOURCE.01}; the $O(1)$
ratio is the open piece.  \emph{Kill:} a truth step with
$\eta \le 0$.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The flag-chain induction
(\texttt{PRIME.PORT.\allowbreak PIVOTFACTOR.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v892\_closure\_architecture.py}, $15/15$ under SPEC v2,
\textsc{pivotfactor-measured}).  \emph{Measured:} the $12$
LDL$^{\mathsf T}$ pivots of $A_h = I - C_h$ factor
rung-to-rung as $d_{k,h+1} = d_{k,h}(1 + \mu_{k,h})$ with
\emph{every} factor positive --- all $12$ flag quotients $Q_k
> 0$ on all $31$ truth steps (pivot $=$ minor quotient warded
$8.3\times10^{-14}$); the ladder minimum sits at the soft flag
$k = 12$ at $2.06\times10^{-14}$, sign-decidable at
$6.4\times10^{9}\times$ the eigenvalue noise floor
(Cauchy-interlacing guard); the smooth world violates at the
\emph{bulk} flags first ($k = 1$ on $10$ steps, $k = 3$ on
$9$; zero steps violate the soft flag alone); per-prime
non-decompositional (leverage to $987\times$).  \emph{The
shape:} the shortest induction of the program --- base
(\texttt{v884}/\texttt{v887}) $+$ $\min_k Q_{k,h} > 0$
$\Rightarrow$ the whole ladder by pure algebra; the soft-flag
scale is $\tau$ itself (\texttt{PRIME.PORT.RELFLAG.01}).
\emph{Kill:} a truth step with a non-positive flag quotient.
No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The Rouch\'e route
(\texttt{PRIME.PORT.EULER.\allowbreak ROUCHE.01}, dead).}
{\sloppy
\emph{Registered and closed} 2026-08-09
(\veri{v892\_closure\_architecture.py}, $14/14$ under SPEC v2,
\textsc{rouche-measured} --- an honest kill with the mechanism
documented).  \emph{Measured:} on complex contours enclosing
the corridor $s \in [0, 1]$, $\sup_\Gamma
\|A_h(s)^{-1}\Delta_h(s)\| \ge 1$ on $22/31$ truth steps (max
$63.4$) \emph{including} off-cap --- not merely
contour-limited: a two-sided contour norm bound is the wrong
theorem shape for inheritance on this ladder (consistent with
the one-sided avoidance law of
\texttt{PRIME.PORT.FACTORAVOID.01}); the argument-principle
machinery itself verifies exactly (integer zero counts at
$5\times10^{-15}$); the smooth world is corridor-broken on
$28/28$ but the mechanism is non-discriminating since truth
fails the bound too.  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The $8\times8$ Schur-core
reduction (\texttt{PRIME.PORT.DEEPCORE.\allowbreak SCHUR.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v892\_closure\_architecture.py}, $16/16$ under SPEC v2,
\textsc{deepcoreschur-measured}).  \emph{Measured:} on the
full wall operator, the block split at the fixed deep-core
aliases $\{2, \dots, 16\}$ gives Haynsworth-exact inertia and
the wall \emph{is} the fixed $8\times8$ Schur core:
$\lambda_{\min}(S_h)\,w_{\rm core}/\tau_h = 1 \pm
8.4\times10^{-8}$ with core mass $w_{\rm core} \ge 0.999937$
on all $41$ full-core rungs --- the RH-critical object is a
fixed $8\times8$ family, not a growing operator; the absolute
exterior margin shrinks ($\lambda_{\min}(R) \sim h^{-2.865}$)
but $\lambda_{\min}(R)/\tau$ is trendless at $210$--$2200$ ---
the reduction is conditional on a \emph{$\tau$-relative}
exterior bound only ($c_R = 210$ measured,
\texttt{PRIME.PORT.RELFLAG.01}); core inheritance holds ($0$
crossings, $\eta_{\rm core}$ min $0.0315$); the per-prime
grain is dead at fixed dimension too.  \emph{Kill:} a
full-core rung breaking the seven-digit identity.  No marker
move; no RH statement.\par}

\par\smallskip\noindent\textbf{The $\eta$-margin source
(\texttt{PRIME.PORT.\allowbreak ETASOURCE.01}).}
{\sloppy
\emph{Registered} 2026-08-09, explained
(\veri{v893\_relative\_margins.py}, $20/20$ under SPEC v2,
\textsc{etasource-measured (scale-matched)}).
\emph{Measured:} in the dangerous direction the increment
$d_h = |v^{\mathsf T}\Delta v|$ and the operator $a_h =
v^{\mathsf T}Av$ collapse \emph{together} (slopes $-3.716$ vs
$-3.456$, residual correlation $0.974$, $94\%$ shared
variance): the non-decay of the inheritance margin $\eta$ is
\emph{explained} --- $\eta$ is scale-free because numerator
and denominator are sections of one arithmetic object.
\emph{Open:} only the $O(1)$ scatter of the ratio (the fitted
$\tau$-laws predict none of it); seat anatomy: the minimizer
sits in the core while the binding restricted block is the
$4$-alias exterior ($0.0056$ vs core $0.0319$).  The round-53
sandbox continues with the mechanism decomposition.  No marker
move; no RH statement.\par}

\par\smallskip\noindent\textbf{The relative margin laws and
the theorem skeleton
(\texttt{PRIME.PORT.\allowbreak RELFLAG.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v893\_relative\_margins.py}, $18/18$ under SPEC v2,
\textsc{relflag-measured}).  \emph{Measured:} $d_{12} =
1/(A^{-1})_{12,12}$ exact with the factorization $r_{12} =
(\tau'/\tau)(c'/c)$ at $2.1\times10^{-16}$; the $c =
d_{12}/\tau$ ladder is trendless with one outlier (kz 21)
diagnosed in the round-53 sandbox as a \emph{coordinate
artifact} (the kz-19 mechanism repeats; corrected spread $124
\to 15.0$ --- the soft flag is $\tau$-tied);
$\lambda_{\min}(R)/\tau$ trendless with ladder min $c_R =
210$; \textsc{compressions-distinct} (honest: the port bulk
margin is an \emph{absolute} floor while the core exterior is
$\tau$-tied --- different statements at two compressions).
\emph{The printed skeleton:} (i) base certified
(\texttt{v884}/\texttt{v887}; interval-rigorous since
\texttt{v897}); (ii) $\lambda_{\min}(R) \ge
210\,\tau$ trendless; (iii) core $\leftrightarrow$ $\tau$ at
seven digits; (iv) $\eta > 0$ non-decaying --- the \emph{one}
unconditionally open statement is the $\tau$-sign inheritance,
reorganized from collapsing-margin form into
\emph{bounded-margin} form ($c$, $c'/c$, $c_R$, $\eta$ --- all
$O(1)$; round-55 sharpening: $c'/c$ sensitivity range
$[0.223, 5.258]$ and the exact Christoffel identification of
$c$, \veri{v899\_christoffel\_normsquare.py}).  No marker move;
no RH statement.\par}

\par\smallskip\noindent\textbf{The extracted conditional
contract
(\texttt{PRIME.CASE.PAIRCORR.\allowbreak CONTRACT.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN --- the conditional
reduction extracted with every object machine-defined
(\veri{v894\_diagonal\_refinement.py}, $11/11 + 13/13 +
12/12$, \textsc{paircontract-extracted (kernel-bandlimited)}
$+$ \textsc{endpoint-measured (endpoint-partial)} $+$
\textsc{innerlinear-measured (inner-empty)}; the route stays
conditional per \texttt{v889}).  \emph{Measured:} the
first-order homotopy gain regroups exactly
($4.5\times10^{-15}$) into the prime-side form with the
explicit node kernel $K_{h,m}$; the kernel is band-limited
(compact spectral support --- the contract hypothesis is
one-sided band-limited explicit-formula positivity at the port
frequencies, pair-correlation class); the full LaTeX contract
statement is frozen in-probe; all $79$ (rung, alias) finite
margins positive; the endpoint refinement: the global linear
collapse fails ($142/227$) but deep small-$a$ cells pass
monotonically; the inner-zone accounting localizes the
irreducibly-hard part to a \emph{non-shrinking} band
(${\sim}20\%$ of aliases, $38$--$56\%$ of the mass).
\emph{Demand:} state-and-prove the band's weighted pair
inequality, or accept conditionality.  \emph{Norm-square note
(2026-08-09, round CXV):} the kernel's frequency weights are
$W_{f,m} \ge 0$ \emph{exactly} and the single top-edge tent
defect is an extension-convention artifact killed by the
periodic full-weight fold (negative index $0$; $79/79$ modified
margins positive; the boundary term $\beta$ carried closed-form,
max $\beta/\mathrm{margin}$ $0.21$) --- the contract is now a
\emph{norm square} in the frequency weights
(\veri{v899\_christoffel\_normsquare.py},
\texttt{PRIME.CASE.KERNEL.SOS.01} $+$
\texttt{PRIME.CASE.EDGEDEFECT.01} below); the arithmetic
hypothesis itself stays conditional.  \emph{Kill:} a negative
finite margin on the frozen census.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The one-sided collective law
(\texttt{PRIME.PORT.\allowbreak FACTORAVOID.01}, decided).}
{\sloppy
\emph{Registered and decided} 2026-08-09
(\veri{v895\_collective\_comb.py}, $18/18$ under SPEC v2/v3,
\textsc{factoravoid-measured (avoidance-structural)}).
\emph{Measured:} the norm-bound theorem shape does not exist
--- $\rho(W^{-1}\Delta C) \ge 1$ on $17/31$ truth steps (max
$57.3$) but always via the harmless $\mu > +1$ side; the true
law is \emph{one-sided}: $\max(-\min\mu) = 0.9950 < 1$ on all
$31$ steps, which \emph{is} PD inheritance itself; the
geometry/atom split is exactly degenerate (the atom cutoff is
slaved to the tent-support edge; every leaving step is
atom-nil bitwise) --- no separable atom channel: the increment
is the collective window re-test of the full comb; per-prime
leave-one-out is non-decompositional (a leverage census, not a
decomposition).  No marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The deep core is the port core
(\texttt{PRIME.PORT.\allowbreak DEEPCORE.01}).}
{\sloppy
\emph{Registered} 2026-08-09, measured
(\veri{v895\_collective\_comb.py}, $12/12$,
\textsc{deepcore-measured}).  \emph{Measured:} the $8$ deepest
common nodes of the carrier ladder are the fixed even aliases
$\{2, \dots, 16\}$ $=$ the Bessel coordinates $a_m = \pi^2
m^2$ ($0.1\%$ at depth; census $39/41$ steps; coherence knee
$k^{*} = 10$); the certificate-level cross-ratio coherence
(truth $4.3\times10^{-4}$ vs smooth $2.7\times10^{-2}$) is
killed by \emph{all four} surgical modifications --- edge-only
smoothing, interior-only smoothing, prime-power echoes
removed, and the wrong-$\Lambda$ world: \textbf{the signal is
the entire multiplicative von Mangoldt comb} --- positions,
powers, and exact $\log p$ values all load-bearing; no smaller
frozen object exists --- the global rigidity of \texttt{v883}
at the finest coordinate.  \emph{Kill:} a surgery that
preserves the coherence.  No marker move; no RH
statement.\par}

\par\smallskip\noindent\textbf{The Wick functor arc
(\texttt{SEAM.CFIN.\allowbreak WICKFUNCTOR.01} [O]).}
{\sloppy
\emph{Registered} 2026-08-09, OPEN --- structure exact,
obstruction proved, block candidate constructed
(\veri{v896\_wick\_block\_functor.py}, $27/27 + 33/33 +
26/26$, \textsc{wickfunctor-partial} $+$
\textsc{wickmix-measured (wickmix-obstructed)} $+$
\textsc{wickblock-measured (blockfunctor-constructed /
seam-diagonal)}).  \emph{Exact:} the six compiler roles exist
in the deployed seam ($D_5$ slot pairs $+$ the $A_3$ boundary
block; $\mathfrak{so}(16)$ sector law, $q^{*}$ edge grading,
$C_6$ intertwining all exact) but the deployed vacuum kernel
is channel-diagonal; \emph{the proved scalar obstruction:} the
pinned $C_6$ generator's $2$-cycle $\{4, 5\}$ forces a duad
zero --- every $C_6$-covariant antisymmetric scalar covariance
reaches at most $14/15$ duads (a theorem of the deployed
action); \emph{the block resolution:} the $C_6$-covariant
cross-block space has dimension $33$; the canonical block
covariance is exactly covariant, strictly CAR-positive,
carries $15/15$ blocks and $15/15$ block Wick monomials with
the canonical $\chi$ sign law; \emph{the decisive fact:} the
deployed seam covariance has \emph{zero} cross-blocks
everywhere (\textsc{seam-diagonal}).  \emph{Demand:} a
physical channel-mixing mechanism (a
boundary/transport-twisted state, not the vacuum) realizing
the block functor's value level --- then the $K_6$/Pfaffian
grammar of \texttt{v888} would be the measured correlation
grammar of the seam.  \emph{Candidate note (2026-08-09, round
CXV):} the demanded state now has a \emph{constructed
candidate} (\texttt{SEAM.CFIN.KMSMIX.01} below,
\veri{v898\_kms\_schur\_mixing.py}); whether the \emph{actual}
seam realizes it is untouched.  The [O] premise is unmoved.  No
marker move; no RH statement.\par}

\par\smallskip\noindent The rounds 54--56 block (2026-08-09;
promoted in \veri{v897\_certified\_interval\_ladder.py},
\veri{v898\_kms\_schur\_mixing.py},
\veri{v899\_christoffel\_normsquare.py} and
\veri{v900\_core\_update\_anatomy.py}) delivers the reviewer's
``immediate'' list: the interval-rigorous certificate base, the
channel-mixing candidate state, the Christoffel/norm-square
anatomy of the diagonal contract, and the exact core-update
coordinates with their honest negatives.

\par\smallskip\noindent\textbf{The certified interval ladder
(\texttt{PRIME.PORT.\allowbreak BALLLADDER.01}, delivered).}
{\sloppy
\emph{Registered and delivered} 2026-08-09
(\veri{v897\_certified\_interval\_ladder.py}; probe verdicts
\textsc{ballhead-proven} and \textsc{ballladder-complete},
$10/10$, $3960$~s).  \emph{Proven:} $\sigma_h > 0$ with
\emph{rigorous} \texttt{mpmath.iv} interval-arithmetic shifts on
all $42$ reachable rungs $h = 142$--$878$ --- $15$
exact-rational integer Bareiss/Sylvester certificates ($h \le
300$, floors $4.5\times10^{-5}$--$2.1\times10^{-4}$) $+$ $27$
validated-precision Cholesky certificates (dps $120/200$, floors
$5.2\times10^{-6}$--$8.4\times10^{-5}$, pivot-to-bound ratios
${\sim}10^{115}$); the node-enclosure lemma, native
outward-rounded interval transcendentals, exact midpoint/radius
extraction and the rigorous shift $2h\,\mathrm{rad}_{\max}$;
\textbf{the informal $\varepsilon_c$ lag-evaluation error model
of \texttt{v884}/\texttt{v887} is retired from the ladder}; the
Epstein control refused at pivot index $10$ by the identical
interval machinery.  Suite economy declared: the promoted module
re-certifies all $15$ tier-1 $+$ $3$ tier-2 rungs verbatim and
freezes the full census with both probe SHAs.  \emph{Remaining
named step:} the Lean composition through
\texttt{krein\_cofinal\_weil}; the asymptotic tail stays with
\texttt{PRIME.PORT.TAIL.01} / \texttt{PRIME.PORT.LEADING.%
SIGN.01}.  \emph{Kill:} a refused pivot on any rung under the
interval shifts.  No marker move; no RH statement.\par}
...
{\sloppy
\emph{Registered and constructed} 2026-08-09
(\veri{v898\_kms\_schur\_mixing.py}, $30/30$,
\textsc{kmsmix-measured [kmsmix-found(u=1, t=1/8, beta=1),
schurmix-generates(10/10), rp-theta-open]}).  \emph{Constructed:}
the $C_6$-covariant channel-mixing KMS state exists --- the
first point of the frozen grid passes all block-functor gates
(strict CAR, exact covariance with the forced zeros, all $15$
blocks at the floor, grading $5/5 + 10/10$, all $15$ block Wick
monomials with the canonical Pf$_4$ signs; robust at $\beta =
2$); \emph{exact (sympy):} Schur/Feshbach elimination of the
boundary gives $A_{\rm eff} = \kappa A_{CC} +
\tfrac{t^2 m}{1 - m^2} V J_3 V^{\mathsf T}$ and generates all
$10$ carrier duads from the bare diagonal state --- on both
fronts (RH port, Wick functor) the real correlation lives in
the \emph{eliminated boundary}; the frozen $15$-signature table
is the registered prediction.  \emph{Named open check:}
\textsc{rp-theta-open} (no spatial reflection is deployed on the
$16$-dim space).  \emph{Demand:} the physical realization ---
the [O] premise of \texttt{SEAM.CFIN.WICKFUNCTOR.01} is
unmoved.  \emph{Kill:} a physical seam state that violates the
frozen signature pattern.  No marker move; no RH statement.
\emph{Dated note (2026-08-10, rounds 57--59):}
\textsc{rp-theta-open} is now \emph{measured} at family level and
the answer is an exclusion --- strict reflection positivity and
the mixing floor are mutually exclusive on this family (see the
seam RP/modular exclusion block below,
\veri{v903\_seam\_rp\_exclusion.py}); the [O] premise stays
unmoved.\par}
...
{\sloppy
\emph{Registered and measured} 2026-08-09
(\veri{v899\_christoffel\_normsquare.py}, $19/19 + 15/15 +
19/19$, \textsc{christratio-measured (winner-christoffel /
extsource-decoupled)} $+$ \textsc{kernelsos-measured
(index-large; rmin=1; not-absorbed)} $+$
\textsc{edgedefect-measured (normsquare-achieved;
kill=periodic; chain=carried)}).  \emph{Exact:} $1/d_{12} = 1 +
v_{*} K_\sigma(y_{*})$ --- the soft pivot is a
deflated-Christoffel evaluation (independent route at
$2.0\times10^{-11}$), winning the boundedness exposure by two
orders of magnitude; \emph{the honesty fence, warded exactly:}
$\lambda_{\min}(I - G) = \tau$ --- the positivity premise is
the wall's own PD premise in Christoffel coordinates, a
coordinate change, not an independent source; $c'/c$
sensitivity range $[0.223, 5.258]$, $r_{12} > 0$ on $31/31$,
exterior reserve decoupled (corr $0.454$).  \emph{The
norm-square form:} the pair kernel's frequency weights are
$W_{f,m} \ge 0$ exactly; the entire negative content is one
top-edge tent (an extension-convention artifact), killed
exactly by the periodic full-weight fold (negative index $0$;
$79/79$ modified margins positive; the boundary term carried
closed-form, max $0.21$) --- the conditional contract of
\texttt{PRIME.CASE.PAIRCORR.CONTRACT.01} is a norm square in
the frequency weights; \textbf{the arithmetic hypothesis itself
stays conditional} (pair-correlation class per \texttt{v889}).
\emph{Kill:} a negative modified margin on the frozen census.
No marker move; no RH statement.\par}
...
{\sloppy
\emph{Registered and measured} 2026-08-09
(\veri{v900\_core\_update\_anatomy.py}, $20/20 + 23/23$,
\textsc{normcore-measured (family-bounded / update-exact /
region-open)} $+$ \textsc{graphregion-measured (coupling-free /
graph-region-open / shadow-lawless)}).  \emph{Exact:} the
normalized deep core $X_h = S_h/\tau_h$ obeys $X' = c\,(X + U)$
at $8.3\times10^{-16}$ with $c = \tau/\tau'$; the family is
bounded (diameter $2533$) and the input effectively
two-dimensional ($c$ $+$ one matrix mode at $97.5\%$ energy).
\emph{The registered honest negatives (boundary conditions for
any invariant-region theorem):} no linear coupling law $u \sim
F(X)$ (both $R^{2}$ below the $200$-shuffle null $q_{95}$); the
rank-1 truncation fails --- the positivity is carried by the
$2.5\%$-energy off-mode; the naive product and graph-tube
invariant sets are refused.  \emph{The registered direction:}
keep the exact $X + U$ with its sign-dependent off-mode
structure --- no mode may be truncated.  \emph{Kill:} an update
defect above the ward, or a coupling law beating the null.  No
marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The seam RP/modular exclusion
(\texttt{SEAM.STATE.\allowbreak DERIVATION.01} $+$
\texttt{SEAM.CFIN.TWISTED.\allowbreak RP.01} $+$
\texttt{SEAM.CFIN.RP.\allowbreak DILATION.01} $+$
\texttt{SEAM.CFIN.GAP.\allowbreak PENCIL.01}).}
{\sloppy
\emph{Registered and measured} 2026-08-10, rounds 57--59
(\veri{v903\_seam\_rp\_exclusion.py}, $25/25 + 20/20 + 23/23 +
18/18$ on these four claims, \textsc{seamstate-measured} $+$
\textsc{twistedrp-measured} $+$ \textsc{dilation-split} $+$
\textsc{gappencil-measured}; six frozen probes embedded
byte-exact, all fail-first spec amendments on record).
\emph{The exclusion:} the odd-sector Gram eigenvalues under both
deployable spatial reflections are \emph{exactly} $\pm|a_J|$ of
the $\{4,5\}$ duad (identity $2.2\times10^{-16}$), so OS
positivity $\iff a_J = 0 \iff$ the \texttt{v898} mixing floor
falls: \textbf{reflection positivity and the block-functor mixing
floor are mutually exclusive on this family}; strict RP forces
$t = 0$; $u \ge t$ is forced (the first non-decorative
normalization demand; $u = 0$ is RP-excluded, correcting the
round-57 reading); the $2\pi$-KMS locus of the boundary reduction
is exactly the point $(t{=}0, \beta{=}2\pi)$ --- the geometric
temperature kills the mixing instead of pinning it.  \emph{The
complete twisted census:} $48$ candidates $\to$ $16$ involutive
$\to$ $6$ admissible $\to$ $0/6$ admit strict RP at any $t > 0$
(\textsc{twisted-rp-exclusionary}; a measurement on the complete
finite candidate set, not a universal no-go).  \emph{The dilation
split:} the \texttt{v898} M2 parent is a $\theta_{abT}$-marginal
witness \emph{on} the RP cone boundary whose Schur compression
carries the full Pfaffian mixing with the exact rational identity
$t^2 \cdot 3m/(1-m^2) = 1/200$; the strict collar route is
family-wide obstructed by an exact linear law ($30$ defect
entries, all of magnitude $2t$).  \emph{The gap pencil (exact):}
$\operatorname{Pf}(A_{\rm dep} + t A_{\rm int}) =
-(t-1)(3t^2-1)\,q(t)$ with $q = 9t^3 + 21t^2 - t - 1$
irreducible; $t_{\rm gap} = 0.2309488708\ldots$ is the unique
root in $(0, 1/4]$ (exact Sturm), a generalized eigenvalue of the
fixed integer pencil with kernel dimension exactly $2$; the three
critical surfaces ($1p$-gap closure, modular rank-2 ground-state
flip $\|\Delta\|_F = 2\sqrt2$, hermitized RP boundary
$t_c(\beta) \to t_{\rm gap}$) are projections of one variety; the
coupling is load-bearing for the root; \emph{no root is} $1/8$.
\emph{Demand (sharpened):} a state construction that carries the
mixing \emph{and} a positivity structure must leave this family
--- the named parent route is non-equilibrium
(\textsc{ness-parent}, named, not built).  \emph{Dated note
(2026-08-10/11, round 60; promoted rounds 60--63):} the
\textsc{ness-parent} route is now \emph{built and closed as
not-needed} --- at finite size stationarity forces zero entropy
production exactly, drive buys no reflection positivity, and
\emph{equilibrium} witnesses on the $\{J, Z\}$ coupling kernel
carry the full $1/200$ mixing (see the seam equilibrium wiring
block below, \veri{v908\_seam\_equilibrium\_wiring.py}); the [O]
premise stays unmoved.  \emph{Dated note (2026-08-27,
\texttt{SEAM.STATE.RPMIXING.01},
\veri{v972\_seam\_interaction\_front.py}):} the exclusivity is
now \emph{typed} as ray- and reflection-relative, not
slice-wide --- on the full $33$-dimensional $C_6$-covariant
mixing slice $\theta_{abT}$ kills exactly \emph{one} coordinate
($a_J$; a $32$-dimensional marginal-RP hyperplane remains) and
$\theta_S$ leaves a $17$-dimensional first-order-neutral
subspace with strict finite-$s$ witnesses: reflection
positivity plus covariance do \emph{not} force the
channel-diagonal point.  \emph{Kill:} a strict
RP point with $a_J \ne 0$ inside the family.  The [O] premise of
\texttt{SEAM.CFIN.WICKFUNCTOR.01} is unmoved; no marker move; no
RH statement.\par}

\par\smallskip\noindent\textbf{The two dead readings
(\texttt{SEAM.CFIN.MIXING.\allowbreak NORMALIZATION.01} $+$
\texttt{SEAM.CFIN.MINIMAL.\allowbreak MEDIATOR.01}) ---
registered negatives.}
{\sloppy
\emph{Registered as first-class negatives} 2026-08-10, rounds
57--58 (\veri{v903\_seam\_rp\_exclusion.py}, $23/23 + 20/20$ on
these two claims, \textsc{seamnorm-measured} $+$
\textsc{mediator-measured}).  \emph{Dead reading 1:} `$t = 1/8 =
2\pi c_3/|Z_2|$ is a compiler value' is a \emph{decoration} ---
the three factors are individually deployed but the combination
is free: all $55$ scanned $t$ pass all frozen \texttt{v898} gates
(the allowed set is mathematically $(0, \infty)$; $u \in \{0,
\ldots, 2\}$ and $\beta \in \{1/2, \ldots, 4\}$ pass too), no
frozen functional distinguishes $1/8$ (\textsc{pin-absent}), and
the anti-numerology census finds four equally cheap
deployed-constant readings; the kept structure fact: the channel
mixing is \emph{thermal} (it dies to the ground state, $0/15$
cross-blocks at $\beta = 100$).  \emph{Dead reading 2:}
`$N_{\rm fam} = 3$ is the minimal boundary-mediation rank' is
\emph{refuted} at theorem grade for this finite size --- one
boundary pair suffices ($\det B_x = \gamma_{ab}\gamma_{cde}$
symbolically for every symplectic-rank-1 mediator; the explicit
integer witness passes the full requirement $10/10$; seeded
census $2000/2000$), the deployed mediator has rank $2$
\emph{exactly} ($S = 1_5 \otimes 3J$), and what $3$ really pins
is boundary \emph{purity} = dimension bookkeeping
($\dim(A_3) = 6 = |Z_2| N_{\rm fam}$), not a mediation theorem.
Negative results carry their own ledger rows, not footnotes.
\emph{Kill (for the deadness itself):} a frozen v898-gate
functional that pins $t = 1/8$, or a physical demand that
excludes the rank-1 witness.  No marker move; no RH
statement.\par}

\par\smallskip\noindent The rounds 60--63 block (2026-08-10/11;
promoted in \veri{v905\_bfloor\_ideal\_certificate.py},
\veri{v906\_tail\_cartography.py},
\veri{v907\_halfgap\_registered\_target.py} and
\veri{v908\_seam\_equilibrium\_wiring.py}) closes the round-60
seam questions on the compiler side and delivers, on the RH
side, the certified $B$-half surface floor, the tail mechanism
map and the registered half-gap target with its first blind
holdout (the three RH-side items live in the prime-front
document; the seam wiring is registered here).  The rounds
64--71 block (2026-08-11/12; promoted in
\veri{v909\_finite\_wall\_closure.py},
\veri{v910\_finite\_zero\_transfer.py} and
\veri{v911\_wiring\_freedom.py}) closes the \emph{finite}
composed wall census and its transfer law on the prime front
(\texttt{PRIME.WALL.FINITE\_CLOSURE.01},
\texttt{PRIME.WALL.FINITE\_ZERO\_TRANSFER.01}; both in the
prime-front document) and, on the seam side, answers the wiring
selector below.
...
not-needed).}
{\sloppy
\emph{Registered and measured} 2026-08-10, round 60 (promoted
rounds 60--63; \veri{v908\_seam\_equilibrium\_wiring.py},
$22/22 + 25/25$, \textsc{margstab-measured (interior-empty /
stratum-not-isolated / not-a-closure-point /
escape-noncovariant / floor-exchange)} $+$
\textsc{nessparent-measured (price-zero / two-seat-law /
finite-ness-nogo / drive-rp-neutral / ness-not-forced)}; both
probes exact-arithmetic, embedded byte-exact).
\emph{The 2-cycle side:} the closure-vs-isolated dilemma of the
\texttt{v903} marginal witness is refuted \emph{as a dilemma}
--- covariance reduces the $\theta_{abT}$ seat law exactly to
$M_1 = \operatorname{diag}(a_J, -a_J)$ with
$\det M_2 = -a_J^2$: strict 2-cycle reflection positivity is
\textbf{impossible on the whole $C_6$-covariant class}
(interior empty); the witness is neither a limit of covariant
strict equilibria (there are none) nor isolated (the whole
$s = 0$ family is a marginal stratum;
$\sup(\text{RP-margin} \times \text{mixing}) = 0$ exactly); the
only strict-enabling escape is the covariance-\emph{breaking}
$X$ coordinate at exact linear price $2\varepsilon$; and the
sharpest exact object, the floor exchange:
$\operatorname{Pf}_4(\varepsilon) = (\varepsilon -
\tfrac1{200})(\varepsilon + \tfrac1{200})$ --- the canonical
sign gate \emph{is} the negative effective RP margin, with the
crossover exactly at the $1/200$ floor.  \emph{The collar side,
no NESS needed:} strict $\theta_S$ hermiticity is a
\emph{two-seat linear law} of the coupling (exact rank $12$,
kernel $12$ $=$ the $\{J, Z\}$ coordinates of the $24$-dim
covariant coupling space); the deployed seam wiring is
\textsc{pure-i} (every $2\times2$ subblock exactly $-I_2$) $=$
a \emph{maximally obstructed} covariant direction, explaining
the \texttt{v903} strict-collar law (`$30$ entries, magnitude
$2t$') exactly --- rounds 69--71 add the frame qualifier: the
obstruction is maximal \emph{in the deployed $\theta_S$ collar
frame}, and frame-relative as such (the wiring selector below);
the kernel contains \emph{equilibrium}
witnesses --- $V_J$ (and $V_Z$) pass strict collar RP in exact
arithmetic while their Schur compressions carry the full
canonical Pfaffian mixing with the $J$ coordinate exactly
$t^2 \cdot 3m/(1-m^2) = 1/200$; at finite size every exactly
stationary quasi-free state has $\sigma \equiv 0$ exactly
(`NESS with positive entropy production' is
category-inapplicable) and drive buys \emph{no} reflection
positivity --- the minimal entropy production at which the
mixing gates open is exactly zero:
\texttt{SEAM.CFIN.NESS.PARENT.01} \textbf{closes as
not-needed}.  \emph{Demand (the registered successor):} the
wiring selector below --- \textbf{delivered and closed in
rounds 69--71} (\veri{v911\_wiring\_freedom.py}).
\emph{Kill:} a covariant strict
2-cycle RP point, or an equilibrium collar witness whose
compression breaks the canonical census.  The witnesses live on
a different covariant coupling direction than the deployed
$A_{\rm int}$ wiring --- whether the actual seam allows them is
untouched; the [O] premise of
\texttt{SEAM.CFIN.WICKFUNCTOR.01} / \texttt{v898} is unmoved;
no marker move; no RH statement.\par}

\par\smallskip\noindent\textbf{The wiring selector
(\texttt{SEAM.STATE.WIRING.\allowbreak SELECTOR.01}, closed:
deployment choice).}
{\sloppy
\emph{Registered} 2026-08-11 (rounds 60--63) as the question:
is the deployed \textsc{pure-i} seam wiring
\emph{compiler-forced}?  \emph{Closed} 2026-08-12, rounds
69--71 (\veri{v911\_wiring\_freedom.py}, $30/30 + 26/26 +
21/21$, verdicts \textsc{wiring-degenerate} $+$
\textsc{theta-conventional} $+$ \textsc{rp-frame-covariant};
three frozen probes, exact integer/rational arithmetic,
embedded byte-exact), and the answer is a \emph{freedom
theorem}: the exact Gr\"obner census of the constraint classes
(unit law, CAR/KMS, Pfaffian pencil, orientation propagation,
covariance $+$ seat stack) decomposes the real variety
certified and leaves exactly \textbf{one admissible
three-dimensional wiring component modulo the rule gauge} (the
rotation cell $C_{\rm rot}$); the $Z$/$X$ wirings fall at
exactly one class --- orientation propagation ($\det u < 0$
identically) --- and this exclusion is genuinely
rule-gauge-orbit-invariant; \textsc{pure-i} is an
\emph{interior} point of $C_{\rm rot}$, not a vertex, and the
integer gauge element $(\oplus_5 J_2) \oplus I_6$ (preserving
the vacuum, the $C_6$ action, the rule subspace and the
constraint ideal) connects it to the pure-$J$ ray:
\textbf{\textsc{pure-i} is a deployment representative, not a
compiler theorem}.  The companion probes close the frame side:
no compiler-side demand distinguishes the $\theta_S$ sheet-swap
frame within the $9$-dim rule-gauge algebra (five demand
classes --- $\mu_4$/$C_6$, CAR/state, pencil/orientation, doily
combinatorics, the round-59 census property --- all
compatible-not-forcing or silent on the angle;
\textsc{theta-conventional}), and the strict-collar RP Grams
transform \emph{exactly covariantly} under the rule gauge
(symbolic kernel-covariance law on the full torus $+$ exact
Gram witnesses): reflection positivity of a (frame, wiring)
pair depends only on their \emph{relative angle} on the
$\delta$-circle --- the same deployed \textsc{pure-i} parent
that fails strict collar RP at $\theta_S$ (defect $2t$)
\emph{passes it exactly} in the integer-conjugated frame, so
the \texttt{v903}/\texttt{v908} obstruction is a
\emph{frame statement}, not a wiring no-go
(\textsc{rp-frame-covariant}).  The frozen canonical sentence:
the compiler determines the wiring \emph{orbit}; strict
Hermiticity in the $\theta_S$ frame would select pure-$J$, the
same demand in the integer-conjugated frame selects
\textsc{pure-i}; a canonical wiring selection requires an
additionally \emph{derived} physical frame.  (Forbidden from
now on: ``\textsc{pure-i} is compiler-forced'' and equivalents
without the frame qualifier.)  \emph{Honest scope:} the census
covers the five enumerated demand classes --- a compiler demand
outside the list is not excluded; whether the actual seam
physics derives a specific collar frame remains outside the
formalized rules; the [O] premise of
\texttt{v898}/\texttt{v903} is unmoved.  \emph{Kill (of the
closure):} a compiler-side demand outside the censused classes
that cuts the $\delta$-circle, or a derived physical collar
frame (which would then \emph{select} a wiring via the exact
selection map).  No marker move; no RH statement.\par}
...
\emph{The law derived, and the counterweight given its first OS-viable cut geometry
(2026-08-27, \texttt{SEAM.INT.ODDSPLIT.01} $+$ \texttt{SEAM.INT.OSAVOID.01},
\veri{v972\_seam\_interaction\_front.py}; toy-level census, the v529 fence inherited, no
status change):} the straddle law refines to the \emph{odd-split law} --- the
Jaffe--Pedrocchi sufficient condition for Majorana reflection positivity
(\texttt{arXiv:1406.1384}, \texttt{arXiv:1601.01994}), made decidable, \emph{predicts} RP
survival on the centre-straddled ($2{+}2$ mirror-split) cuts and measurement confirms it,
with the predicted-vs-measured RP confusion matrix perfect over all $35$ (member, cut)
cells; the $\mu_4$-covariant quartic space (dimension $464$) contains \emph{no}
JP-certifiable direction on all cuts. And the threat gains a positive counterweight: the
quartet-avoiding cut family of the clock-invariant $m{=}4$ member generates the \emph{full}
even algebra (rank $15$, dimension $32768$) and carries a \emph{positive} OS/GNS structure
at $g{=}1$ (minimum eigenvalue $+4.43\times10^{-5}$) while the straddled control fails
($-6.94\times10^{-2}$), and the $\mu_4$ clock maps the avoiding pair onto itself --- the
kill-test-2 counterweight is \emph{narrowed} (an OS-viable cut geometry exists), not
removed.
...
% =====================================================================
\section*{Research Contract --- \texttt{CCC.SEAM.CROSSOVER.01} (the conformal-cyclic crossover on the seam geometry)}
\addcontentsline{toc}{section}{Research Contract: CCC.SEAM.CROSSOVER.01}
% =====================================================================
\noindent\emph{Typing.} A research contract \mO\ (ledger row \texttt{CCC.SEAM.CROSSOVER.01}:
Open, research contract), \emph{not} a claim.  It engages Penrose's Conformal Cyclic Cosmology
(Tod, \texttt{arXiv:1309.7248}; Penrose--Meissner, \texttt{arXiv:2503.24263}) as a \emph{named
reference frame} for the shape of the target --- never a confirmation in either direction.  The
cyclic reading of \texttt{origin\_theory} stays \mC\ regardless of this contract's fate; nothing
below asserts that CCC follows from TFPT.

\par\smallskip\noindent\textbf{The certified kinematic base \mE\ (\veri{v957\_ccc\_crossover\_kinematics.py},
\texttt{CCC.SEAM.KINEMATICS.01}; six discovery probes embedded byte-exact, $28{+}29{+}18{+}21{+}14{+}7$
gates).}  The corpus objects already carry the complete CCC crossover \emph{kinematics}:
(i)~Penrose's reciprocal gauge $\hat\Omega\check\Omega=-1$ is the element $\tau=\rho^2\sigma:
z\mapsto-1/z$ of the proven $\mu_4$ M\"obius stabiliser $D_4$ --- $\tau$ reverses the clock
($=$ the $\Z_2$ sheet parity), swaps the aeon poles $\{0,\infty\}$, and the crossover locus
$|\hat\Omega|=|\check\Omega|$ is the unit circle carrying all four marks; (ii)~conformal-factor
rigidity: on the 2D seam the clock alone leaves exactly \emph{one} constant-curvature modulus and
the reciprocal $\Z_2$ kills it; on the 3D crossover the deck alone kills \emph{all} conformal
moduli (the $so(4,1)$ deck-commutant is $4$-dimensional, all Killing, boost content $0$; globally
unique on the explicit Obata family) --- TFPT's candidate answer to CCC's open unique-conformal-factor
problem, with $v_{\mathrm{geo}}$ staying pure unit calibration (\texttt{VGEO.TORSOR.01});
(iii)~the entropy reset of the cyclic reading is a \emph{reduced channel}: the v221 transport is
CPTP with an explicit Stinespring isometry, $\|T^n-P_\star\|_2=(2/3)^{6n}$ exactly, the global
overlap invariant, the microstate record in the complement --- ``unitarity $+$ rank-$1$ attractor''
is isometry $+$ partial trace, not a contradiction (with the executed sharpening that this deployed
Kraus dilation is \emph{entanglement breaking}: it stores the record entirely in the environment and
carries no coherent transport in the visible three-state system, quantum capacity $0$,
\veri{v976\_seam\_lift\_birkhoff.py}); (iv)~the kinematic $2\mathrm D\to3\mathrm D\to
4\mathrm D$ uplift: the seam sphere \emph{is} the Hopf base of $S^3$ (stereographic $=z_1/z_2$
exactly), the crossover $X=S^3/\Z_4$ is the lens boundary of the corpus $A_3$ ALE (deck free,
clock and reciprocal descend, marks $=$ four linked fibre circles in the two sheet orbits), the
4D bridge is the cone $\mathbb C^2/\Z_4$ with the inversion as conformal aeon swap fixing the unit
lens pointwise, and the Lorentzian shadow $(1/\cos^2T)(-dT^2+g_{S^3})$ is Einstein $R_{ab}=3g_{ab}$
with a spacelike round-$S^3$ crossover and $\check\Omega=-\cos T$ vanishing to first order;
(v)~the KMS/modular closure at the free level: $T=e^{-H_{\mathrm{mod}}}$ with
$\operatorname{spec}(H_{\mathrm{mod}})=\{0,\,6\ln\tfrac32,\,6\ln3\}$ exactly, detailed balance and
OS positivity exact, and the \vref{v526} normalisation $\beta_{\mathrm{angle}}=2\pi=1/(4\cthree)$
$=$ the Gibbons--Hawking normalisation on the crossover fibre --- one discrete clock tick $=$ one
unit of modular time; (vi)~the holonomy dictionary: pole strings carry the full $\mu_4$ monodromy,
every seam string carries exactly the $2$-torsion $-1$ $=$ the \vref{v522} GSO/fermion-parity wrap
(typed \mC\ correspondence); (vii)~the selector necessity: the admissible OS reflections of the
mark family $M(\delta)$ close to $D_4$ \emph{iff} $\delta=\pi/2$, and at Majorana operator level
(fermionic signs kept) the clock-axis cuts are $\Theta$-real only at the $\mu_4$ configuration ---
the \emph{uniqueness} half of the \vref{v534} straddle selector is thereby \emph{derived} as a
$\Theta$-reality necessity (the survival half and the coupling sign stay v534's nonperturbative toy
facts, cited); (viii)~the frozen Gate-6 kernel: a sharp causal top-hat disc of radius
$\theta_{\max}=\eta_{\mathrm{rec}}/(\eta_0-\eta_{\mathrm{rec}})\approx1.16^\circ$, decay rates
$\ln(729/64)$ and $\ln729$ (ratio $\ln3/\ln\tfrac32=2.709511$), $\kappa$ derived from the
\vref{v526} normalisation up to a declared convention band (relic contrast $\le3.3\times10^{-4}$
across all of it) --- frozen \emph{before} any data contact and feeding the preregistered search
\texttt{experiments/ccc-crossover-disc} (hypotheses, kill criteria K1--K6, null battery and
decision rule frozen there; verdict slot \texttt{data\_limited}, no data pass).

\par\smallskip\noindent\textbf{Demands (the dynamical crossover, in order).}
\textbf{(D1)}~the interacting seam algebra $\mathcal A_{\mathrm{hol}}$ realised \emph{on} the lens
crossover with reflection positivity protected on the mid-gap cut class while the aeon transfer
uses the through-mark class (the two $D_4$ reflection classes whose product is the clock; the
\vref{v529} straddle law is exactly this separation at toy level) --- this \emph{is}
\texttt{WOIT.OS.TWISTOR.01} stage $\gamma$ in the crossover geometry, and closing it closes D1;
\textbf{(D2)}~the aeon-transfer channel as a Stinespring isometry at \emph{field} level (the v957
finite-dimensional dilation lifted to the reconstructed net); \textbf{(D3)}~the cosmology fork
\emph{decided}: exactly one active story --- Starobinsky ($r=12/N_\star^2\in[0.0033,0.0048]$,
within reach of $\sigma(r)\sim10^{-3}$ experiments), a crossover/gravitational-wave epoch, or a
computed hybrid; \textbf{(D4)}~the preregistered disc search executed per its frozen hypotheses ---
\textbf{EXECUTED 2026-08-24 with a robust null} (first data contact, Planck PR3 SMICA map,
integrity SHA-256 pinned in \texttt{results.json}, 2\,013\,312\,960 bytes; freeze intact $3/3$
hashes, frozen-kernel guards $5/5$ \emph{before} data contact; seed 20260824, reruns
bit-identical): $\max|\mathrm{SNR}| = 5.119$ on data vs null mean $5.20$ over 100 Gaussian
$\Lambda$CDM sims (matched mask and spectrum), $p_{\rm global} = 0.673$, $p_{\rm counts} = 0.446$,
BH-$q_{\min} = 0.673$ --- the $q < 0.01$ threshold clearly missed; 171 candidates $|\mathrm{SNR}|
\ge 4$ with $83$ pos / $88$ neg (no sign excess); injection recovery $100\%$ ($20 \times
200\,\mu$K discs at $1.16^\circ$ --- the search is well-powered, the null is a real null); kill
criteria K1--K6 all NA (they fire only on a resolved relic; none exists); the replication legs
(half-mission, Commander/NILC) are not required for a null per the frozen decision rule and stay
available for future candidates; three amendments disclosed (seed pinning of the unseeded
synfast preflight, reporting fields, the documented estimator $\ell$-weighting) --- statistics,
thresholds and kill criteria unchanged.  \textbf{No kill criterion fired; the contract stays
open via D1--D3; no type change.}

\par\smallskip\noindent\textbf{Kill criteria (frozen).}  A free fitted conformal factor anywhere;
failure of the mid-gap RP protection on the interacting algebra (kill test~(2) of the WOIT
$\gamma$ stage); the interacting level breaking the \vref{v522} wrap $=(-1)^F$ identification
(which would reopen the deck fork the kinematics closed conditionally); a measured $r$
incompatible with the surviving cosmology branch; a resolved crossover relic violating the frozen
template class (K1--K6, including the K6 contrast bound that would kill the derived-$\kappa$
chain); and --- typed numerological, forbidden --- any identification of the four $\mu_4$ marks
with Hawking-point \emph{counts}.

\par\smallskip\noindent\textbf{Honest scope.}  Deck fork: topology searches cannot decide it
(lens geodesic $\ge28\,D_H$ vs matched-circle reach $6.3\,D_H$; $32\sigma$ from curvature
detectability) --- it is decided \emph{conditionally} by the v522 wrap identification (spacetime
branch) and stays typed.  The executed D4 null leaves the compiler core untouched (the frozen
kernel was \emph{output} of the certified kinematics, not input) and asserts nothing about CCC
in either direction; the cyclic reading stays \mC.  The contract now closes only through
D1--D3; the kinematic base \mE\ stands on its own either way.

% =====================================================================
% The 2026-08-27 external-review block: dimension / dynamics / gravity
% firewalls and the 4D construction ladder.  Eight new contract IDs.
% =====================================================================
\section*{Firewall Contract --- \texttt{DIMENSION.UPLIFT.FIREWALL.01} (no $1{+}1$D theorem closes a $3{+}1$D claim)}
\addcontentsline{toc}{section}{Firewall Contract: DIMENSION.UPLIFT.FIREWALL.01}
% =====================================================================
\noindent\emph{Typing.} A \emph{standing discipline rule}, in force immediately (ledger row
\texttt{DIMENSION.UPLIFT.FIREWALL.01}); the uplift theorems it names are separate \mO\ contracts.
It formalizes the core diagnosis of the 2026-08-27 external deep review: \emph{a chiral $1{+}1$D
seam net does not imply a local, interacting $3{+}1$D quantum world}.  The corpus's strongest
finite results are boundary/kinematic building blocks --- the review found several surfaces where
boundary theory, continuous relaxation, modular time, Lorentz geometry and physical 4D dynamics
were in danger of being silently equated.  This firewall separates them by rule.

\par\smallskip\noindent\textbf{The rule (two clauses).}
\textbf{(F1)} No theorem whose hypotheses live in $1{+}1$D (a chiral net, a VOA, a lattice
collar, the seam circle) may be cited as \emph{closing} a claim about $3{+}1$D physics.
\textbf{(F2)} Every contract closure must state the spacetime dimension of its \emph{hypotheses}
and of its \emph{conclusion} (dimension typing), with the same discipline as the
\mE/\mC/\mO/\mX\ markers.

\par\smallskip\noindent\textbf{Instances audited at registration (honesty census).}
(i)~\texttt{SEAM.EQUIV.01} and both its routes are $1{+}1$D in \emph{and} out: the cited
scaling-limit framework (Osborne--Stottmeister, \texttt{arXiv:2107.13834}) treats $1{+}1$D free
lattice fermions, the OS-reconstruction selector (Adamo--Moriwaki--Tanimoto,
\texttt{arXiv:2407.18222}) treats 2D full VOAs, and lemma L2 of the externalization package
below claims convergence to a \emph{chiral $c{=}8$ CFT} --- nothing more.  \textbf{Even a fully
proved \texttt{SEAM.EQUIV.01} is a rigorous boundary building block, not the 4D theory.}
(ii)~The \texttt{WOIT.OS.TWISTOR.01} target theorem (Poincar\'e representation, local causality,
one chiral generation, the internal gauge action) \emph{is} the uplift demand; it stays \mO\ with
all seven kill tests live.
(iii)~The kinematic $2\mathrm D\to3\mathrm D\to4\mathrm D$ uplift (\vref{v957}; the discovery
probe \texttt{experiments/tfpt-discovery/ccc\_uplift\_4d\_probe.py} correctly self-labels as
kinematic) supplies a canonical Hopf fibre / lens crossover / 4D cone with an Einstein Lorentzian
shadow --- \emph{none of which} closes this firewall: a metric uplift is not an interacting local
quantum theory.
(iv)~Anomaly inflow: the non-gappable $c_-{=}8$ edge is naturally the boundary of a
higher-dimensional \emph{invertible/topological} bulk (Witten--Yonekura,
\texttt{arXiv:1909.08775}; Hsieh--Tachikawa--Yonekura, \texttt{arXiv:2003.11550}); the inflow
bulk and the \emph{physical} $3{+}1$D spacetime bulk are two different identifications that must
never be silently equated.

\par\smallskip\noindent\textbf{The realistic route ordering (registered as the programme
skeleton).}
\textbf{(1)}~keep the seam as an internal/topological boundary module (all current \mE\ results
survive unchanged as such);
\textbf{(2)}~build a genuine 4D Euclidean lattice theory --- TFPT fixes the representations,
charges, coupling relations and boundary conditions; the 4D lattice carries spacetime, gauge
fields, Higgs and fermions;
\textbf{(3)}~prove the 4D OS package on it (\texttt{QFT4D.OS.RECON.01} below);
\textbf{(4)}~prove chirality and mirror decoupling \emph{in the same 4D model}
(\texttt{CHIRAL4D.NOMIRROR.01} below);
\textbf{(5)}~only then apply Bisognano--Wichmann/CHM and entanglement gravity (the direction
discipline of \texttt{GRAV.NONCIRCULAR.01} below);
\textbf{(6)}~then prove bulk--seam equivalence, identifying the seam as the topological/anomalous
boundary condition of the \emph{constructed} 4D bulk (\texttt{SEAM.BULK4D.RECON.01} below).
The twistor/celestial alternative (reconstruct the bulk from boundary correlators) is riskier:
boundary correlators do not guarantee bulk locality (cf.\ \texttt{arXiv:2602.04223}); it stays a
named parallel route (\texttt{WOIT.OS.TWISTOR.01}/\texttt{CELEST.SEAM.01}), not the trunk.

\par\smallskip\noindent\textbf{Status.} Firewall in force (F1/F2 bind every future closure);
the uplift theorems live in \texttt{SEAM.BULK4D.RECON.01}, \texttt{QFT4D.OS.RECON.01} and
\texttt{CHIRAL4D.NOMIRROR.01}, all \mO.

% =====================================================================
\section*{Research Contract --- \texttt{SEAM.BULK4D.RECON.01} (from seam data to a 4D local quantum theory)}
\addcontentsline{toc}{section}{Research Contract: SEAM.BULK4D.RECON.01}
% =====================================================================
\noindent\emph{Typing.} A research contract \mO\ (ledger row \texttt{SEAM.BULK4D.RECON.01}:
Open, research contract), \emph{not} a claim.  It makes explicit the theorem that was previously
only implicit between the \texttt{SEAM.EQUIV.01} conclusion (a chiral $c{=}8$ boundary object)
and the \texttt{WOIT.OS.TWISTOR.01} goal statement (a Poincar\'e-covariant, causal, chiral 4D
theory): \textbf{the missing seam$\to$bulk reconstruction is a theorem of its own, and no
existing corpus result closes any part of it.}

\par\smallskip\noindent\textbf{Input.} The certified seam data: the gapped $\mu_4$-equivariant
collar (\vref{v302}), the index-4 Q-system (\vref{v125}/\vref{v154}), the $(E_8)_1$ character
and modular data (\vref{v377}/\vref{v378}), reflection positivity (\vref{v379}), the clock and
mark structure (\vref{v519}--\vref{v524}), and the kinematic uplift dictionary (\vref{v957}).

\par\smallskip\noindent\textbf{Target theorem (all seven demands; none is currently closed).}
From seam data alone, construct a 4D family of local algebras
$\{\mathcal A(O)\}_{O\subset\mathbb R^{1,3}}$ (equivalently a family of Schwinger functions) and
prove:
\textbf{(B1)}~4D locality/microlocality (spacelike commutativity; the correct wavefront sets);
\textbf{(B2)}~Poincar\'e covariance with a positive energy spectrum (the spectrum condition);
\textbf{(B3)}~spin--statistics and CPT;
\textbf{(B4)}~\emph{both} helicities (the anti-self-dual completion --- the known hard wall of
the twistor route);
\textbf{(B5)}~cluster decomposition;
\textbf{(B6)}~a nontrivial S-matrix (the theory is interacting, not generalized-free);
\textbf{(B7)}~a \emph{unique} seam$\leftrightarrow$bulk operator dictionary (which boundary
operator is which bulk field, uniquely --- not only at character level).

\par\smallskip\noindent\textbf{Kill tests (pre-registered).}
\textbf{(1)}~every 4D extension of the seam state is a generalized free field (B6 fails
structurally); \textbf{(2)}~the inflow bulk of the $c_-{=}8$ edge is \emph{provably} only
invertible/topological, with no local propagating completion carrying both helicities (B1/B4
fail); \textbf{(3)}~the dictionary is non-unique at operator level (character-level agreement
with OPE mismatch --- the same failure mode as kill test (5) of \texttt{WOIT.OS.TWISTOR.01}).

\par\smallskip\noindent\textbf{Honest relations.} \texttt{WOIT.OS.TWISTOR.01} (twistor OS
quotient) and the route ordering of \texttt{DIMENSION.UPLIFT.FIREWALL.01} (constructive 4D
lattice first, seam as its boundary condition) are the two candidate proofs; this contract is
route-agnostic and states only the target.  A Hopf-fiber uplift, a 4D cone, or an Einstein
metric on the bridge (\vref{v957}) does \emph{not} close it.  Status:
\texttt{SEAM.BULK4D.RECON.01} \mO.

% =====================================================================
\section*{Research Contracts --- \texttt{DYN.MARKOV.EMBED.01} / \texttt{DYN.UNITARY.DILATION.01} (the three meanings of ``dynamics'')}
\addcontentsline{toc}{section}{Research Contracts: DYN.MARKOV.EMBED.01 / DYN.UNITARY.DILATION.01}
% =====================================================================
\noindent\emph{Typing.} Two contracts that split what the corpus has loosely called ``dynamics''
into its three inequivalent meanings: \textbf{(a)}~the positive relaxation $T^n$ to a fixed
point; \textbf{(b)}~a continuous dissipative semigroup $e^{t\log T}$; \textbf{(c)}~reversible
Lorentzian/unitary evolution $e^{-itH}$.  Step (a)$\to$(b) is now a machine-checked theorem for
the concrete seam kernel (\mE\ at the finite level); step (b)$\to$(c) is genuinely open and gets
its own contract.  Nothing here moves \texttt{CONTRACT.F.01} or the four observables.

\par\smallskip\noindent\textbf{\texttt{DYN.MARKOV.EMBED.01} --- the finite kernel half is
executed \mE.}  For the v221 seam transfer
kernel $T=J+(2/3)^6\,u_2u_2^{\mathsf T}+(1/3)^6\,u_3u_3^{\mathsf T}$ (spectrum exactly
$\{1,64/729,1/729\}$), the principal logarithm $Q=\log T$ is an \emph{exact classical Markov
generator} \veri{v971\_markov\_embedding\_generator.py}: sympy-exact spectral calculus gives
$e^Q=T$; every row of $Q$ sums to zero
\emph{symbolically}; the three independent off-diagonal entries are exactly
$\{\ln(9/8),\,2\ln 3,\,2\ln 3\}=\{0.117783,\,2.197225,\,2.197225\}$, all positive (the reviewer's
independently verified numbers), with diagonals $-\ln(81/8),-\ln(81/8),-4\ln3$; the
uniformization certificate $Q+(4\ln3)I\ge0$ entrywise (diagonals $\ln8,\ln8,0$, exact) proves
$e^{tQ}=e^{-4t\ln3}\,e^{t(Q+4\ln3\,I)}$ entrywise nonnegative and doubly stochastic for
\emph{all} $t\ge0$ --- so $T$ is embeddable and every root $T^{1/n}=e^{Q/n}$ is again a
stochastic matrix.  The generator spectrum is
$\operatorname{spec}(Q)=\{0,-6\ln(3/2),-6\ln3\}=-\operatorname{spec}(H_{\mathrm{mod}})$
(\vref{v957}: $T=e^{-H_{\mathrm{mod}}}$), the gap is the seam gap $\Delta=6\ln(3/2)$, and $Q$ is
symmetric (detailed balance w.r.t.\ the uniform state --- the KMS/OS-positive free level).
\emph{Negative control (embeddability is not generic):} the same construction with second
eigenvalue $-1/10$ is still doubly stochastic but has $\det=-1/200<0$, while every embeddable
stochastic matrix has $\det e^Q=e^{\operatorname{tr}Q}>0$ --- provably \emph{not} embeddable.
The RP/OS spectral positivity of the seam kernel (\texttt{CONTRACT.F.01} axiom 3) is exactly
what makes (a)$\to$(b) real.  \emph{What stays \mO\ in this contract:} the same embedding
statement at \emph{field} level (on the reconstructed net / quasi-local algebra, uniformly in
system size), not just on the 3-state kernel.

\par\smallskip\noindent\textbf{\texttt{DYN.UNITARY.DILATION.01} --- the (b)$\to$(c) step,
\mO.}  The semigroup $e^{tQ}$ is \emph{irreversible statistical} dynamics --- an H-theorem, not
a Schr\"odinger equation.  Fundamental quantum dynamics requires the chain
$T=e^{-aH}\;\xrightarrow{\ \mathrm{OS}\ }\;U(t)=e^{-itH}$ with $H$ positive self-adjoint, and
the dilation must be \emph{physical}: \textbf{(D1)}~a Stinespring/Hamiltonian dilation of the
semigroup whose generator is (quasi-)local; \textbf{(D2)}~size consistency (the dilation is
compatible with the thermodynamic limit --- no environment that grows pathologically with the
system); \textbf{(D3)}~a Lieb--Robinson bound (finite propagation speed for the dilated
dynamics); \textbf{(D4)}~the OS analytic continuation to a unitary one-parameter group on the
reconstructed Hilbert space.  \emph{Kill tests:} every dilation of the seam semigroup is
provably nonlocal; the Lieb--Robinson velocity diverges in the scaling limit; the OS
continuation fails on the interacting algebra.  \emph{Relation:} demand D2 of
\texttt{CCC.SEAM.CROSSOVER.01} (the Stinespring isometry at field level) already presupposes
this contract; it is hereby named as the prerequisite.

\par\smallskip\noindent\textbf{Executed sharpening --- the lift structure of the finite kernel
(\mE\ at matrix level; the contract itself stays \mO).}  Two modules now pin down \emph{which}
dilations are even candidates.  \textbf{(i)}~\emph{The no-go}: the Kraus dilation the corpus
actually deploys, $K_{ij}=\sqrt{T_{ij}}\,|i\rangle\langle j|$, is measure-and-prepare --- one
step kills every coherence (fully symbolic), its Choi matrix is diagonal in the product basis,
hence the channel is \emph{entanglement breaking} with quantum capacity $0$, and $T>0$
entrywise gives zero-error capacity $C_0(T)=0$ exactly
(\veri{v976\_seam\_lift\_birkhoff.py}); any dilation satisfying D1--D4 must therefore be a
genuinely different object.  \textbf{(ii)}~\emph{The hidden fibre}: the complete set of
random-permutation (Birkhoff) quantizations of the single step $B$ is a one-parameter line
$t\in[0,1/18]$ whose coordinate is the sign-character Fourier coefficient
($\widehat w_t(\mathrm{sgn})=6t$); populations are exactly $t$-free, the one-step coherence
spectrum is $\{1,\,6t,\,(2/3)^{\times2},\,(1/3)^{\times2}\}$, and the triangle loop current is
an exact eigen-observable $\Psi_t(A)=6t\,A$ --- one step and one current readout measure $t$
without an ancilla.  On the contrast qubit the Choi partial transpose has spectrum
$\{-3t/2,\,1/6+3t/2,\,1/3+3t/2,\,1/2-3t/2\}$: the deployed sixth-root spectrum sits
\emph{exactly} on the entanglement-breaking boundary because $2/3+1/3=1$ (mutated spectra miss
by $\mp1/12$), while at the full three-level the family stays NPT at every tested iteration,
$\lambda_{\min}=-(1/3)(2/3)^n$, $t$-independent (\veri{v976\_seam\_lift\_birkhoff.py}).
\textbf{(iii)}~\emph{The coherent end}: $B$ is \emph{unistochastic} --- an exact $U\in SU(3)$
with $|U_{ij}|^2=B_{ij}$ entrywise, unique up to rephasing and complex conjugation, with
Jarlskog invariant $J=\pm1/27$ exactly; no \emph{fixed} microstep unitary generates the
relaxing sequence ($B^2\neq|U^2|^2$; a Dirichlet recurrence witness returns $U^{n^*}$
arbitrarily close to the identity while $B^{n^*}$ is uniform), so repeated steps
\emph{require} environment contact between steps --- precisely the D1--D4 demand --- while the
six-step macro-gate exists as its own reversible $U_6\neq U_1^6$
(\veri{v977\_transfer\_unistochastic\_wilson.py}).  The classical kernel thus admits the full
lift trichotomy \{EB dilation $\mid$ Birkhoff fibre with hidden circulation $t$ $\mid$
reversible macro-gate with hidden orientation $\operatorname{sgn}J$\}: the same classical
data, three inequivalent quantum dynamics.  Ledger rows:
\texttt{TRANSFER.HIDDEN.CIRCULATION.01} and \texttt{TRANSFER.COHERENT.WILSON.01} (executed
halves \mE; the fibre-point/branch selection, the defect--Birkhoff forcing functor --- named
\texttt{TRANSFER.DEFECT.BIRKHOFF.01} --- and the center--transport intertwiner stay \mO).
These results fix the contract's boundary conditions; they do not close it.

\par\smallskip\noindent\textbf{Precision (binding for all ``universal dynamics'' prose).}  The
shared relaxation shape of the four \texttt{F\_transfer} sectors
(\texttt{FR.DYNAMICS.01}/\texttt{DYN.TRANSFER.UNIVERSAL.01}) is a \emph{universal contraction
class} --- shared gap, shared multiplier algebra --- \emph{not} one physical clock: both
thermal-time routes are machine-killed (\vref{v723}/\vref{v724}) and the frozen external-clock
contract fired its K1 kill exactly (\vref{v777}: no common continuous clock exists for the four
jets).  Any corpus phrase of the form ``one native flow'' is to be read in this contraction-class
sense; the clocks stay external and sector-specific.

% =====================================================================
\section*{Research Contract --- \texttt{QFT4D.OS.RECON.01} (the genuine 4D Euclidean lattice package)}
\addcontentsline{toc}{section}{Research Contract: QFT4D.OS.RECON.01}
% =====================================================================
\noindent\emph{Typing.} A research contract \mO\ (ledger row \texttt{QFT4D.OS.RECON.01}: Open,
research contract), \emph{not} a claim.  This is step (3) of the
\texttt{DIMENSION.UPLIFT.FIREWALL.01} route ordering: the \emph{nonperturbative} 4D existence
statement that no perturbative or seam-level result can substitute for.

\par\smallskip\noindent\textbf{Input.}  TFPT fixes the discrete data: the representation content
and charge lattice (carrier $+$ glue), the coupling relations at the matching scale, and the
boundary conditions (the seam as internal/topological boundary module).  The 4D Euclidean
lattice carries everything else: spacetime, gauge fields, Higgs, fermions.

\par\smallskip\noindent\textbf{Target theorem (the OS package).}  For the so-specified 4D
lattice theory prove: \textbf{(O1)}~reflection positivity of the lattice measure;
\textbf{(O2)}~Euclidean covariance restoration (hypercubic $\to$ full Euclidean group in the
limit); \textbf{(O3)}~clustering; \textbf{(O4)}~uniform bounds (moment/regularity estimates
uniform in lattice spacing and volume); \textbf{(O5)}~existence of the continuum limit;
\textbf{(O6)}~Osterwalder--Schrader reconstruction of the Wightman/Haag--Kastler theory.

\par\smallskip\noindent\textbf{Firewall against the perturbative leg (binding phrasing rule).}
The existing all-orders result (\texttt{QFT4D.EG.ALLORDER.01}, \vref{v381}) is a \emph{local
perturbative construction under the stated Epstein--Glaser/BRST hypotheses} --- that and no
more.  The weak adiabatic limit for massless particles exists as a cited theorem (Duch,
\texttt{arXiv:1801.10147}), but its hypotheses must be \emph{verified for the concrete TFPT
model}; a seam-transfer gap does not substitute for the infrared structure of massless
photons/gluons.  Even a fully successful EG/BRST proof yields \emph{none} of: confinement or the
hadron spectrum, the Yang--Mills mass gap, asymptotic charged states in QED (the infraparticle
problem), the nonperturbative Higgs vacuum, or a controlled full S-matrix.  Every corpus phrase
of the form ``the perturbative 4D leg closes to all orders'' is bound to this reading.
Status: \texttt{QFT4D.OS.RECON.01} \mO.

% =====================================================================
\section*{Research Contract --- \texttt{CHIRAL4D.NOMIRROR.01} (the chiral 4D lattice matter theorem)}
\addcontentsline{toc}{section}{Research Contract: CHIRAL4D.NOMIRROR.01}
% =====================================================================
\noindent\emph{Typing.} A research contract \mO\ (ledger row \texttt{CHIRAL4D.NOMIRROR.01}:
Open, research contract), \emph{not} a claim.  Step (4) of the firewall route ordering: chirality
and mirror decoupling must be proven \emph{in the same 4D model} that
\texttt{QFT4D.OS.RECON.01} constructs --- the seam's $c_-{=}8$ and Chern/inflow structure are a
boundary-\emph{anomaly} signal, not yet 4D matter.

\par\smallskip\noindent\textbf{Target theorem.}  An explicit chiral lattice operator
(Ginsparg--Wilson/overlap type) for the TFPT representation content with proofs of:
\textbf{(C1)}~a local, gauge-invariant fermion measure; \textbf{(C2)}~perturbative \emph{and}
global anomaly freedom; \textbf{(C3)}~no Witten $SU(2)$ anomaly; \textbf{(C4)}~no mirror
generation in the physical spectrum (the mirror decouples with a mass that survives the
continuum limit); \textbf{(C5)}~the correct index and charges (the TFPT charge lattice,
generation count $\Nfam$); \textbf{(C6)}~a stable continuum limit compatible with the
\texttt{QFT4D.OS.RECON.01} package.

\par\smallskip\noindent\textbf{Calibration (state of the art, honestly).}  The strongest
comparable theorem is Mastropietro's construction (\texttt{arXiv:2303.02790}): a \emph{weakly
coupled} 4D $U(1)$ chiral gauge theory with a \emph{massive} gauge field.  The TFPT target ---
chiral $SU(3){\times}SU(2){\times}U(1)$ with three generations, massless nonabelian gauge
fields, EWSB and confinement downstream --- is far stronger; the gap between the two is the
honest measure of this contract's difficulty.  \emph{Kill tests:} the mirror fails to decouple
(kill test (3) of \texttt{WOIT.OS.TWISTOR.01} at lattice level); a Witten anomaly obstructs the
carrier $SU(2)$; every gauge-invariant measure for the TFPT content is provably nonlocal.
\emph{Consistency note (ledger wins):} the celestial-branch scope fence above honestly lists
interacting RP, mirror decoupling, the internal $SU(2)$, both helicities, local matter fields,
EWSB and confinement as \emph{not} closed; the contradictory ``zero open internal mechanisms''
phrase in \texttt{docs/OPEN\_PROBLEMS.md} is corrected in the same change that registers this
contract (that phrase was a certification statement about a \emph{different}, narrower residual
matrix and had drifted into an overclaim).  Status: \texttt{CHIRAL4D.NOMIRROR.01} \mO.

% =====================================================================
\section*{Firewall Contract --- \texttt{GRAV.NONCIRCULAR.01} (the Bisognano--Wichmann direction discipline)}
\addcontentsline{toc}{section}{Firewall Contract: GRAV.NONCIRCULAR.01}
% =====================================================================
\noindent\emph{Typing.} A \emph{standing discipline rule}, in force immediately (ledger row
\texttt{GRAV.NONCIRCULAR.01}), with an audit census at registration.  Bisognano--Wichmann (BW)
is a theorem \emph{within} an already-local, Poincar\'e-covariant QFT with a suitable vacuum:
the modular flow of a wedge algebra acts as the boost.  It is \emph{not} a device that generates
Lorentz spacetime from an arbitrary modular flow --- there exist Poincar\'e-covariant nets
violating BW (Mund, \texttt{arXiv:hep-th/0101227}, characterizes when BW holds; Morinelli,
\texttt{arXiv:1703.06831}, exhibits the failure landscape).  The only correct proof order is:
4D net $+$ locality $+$ vacuum $+$ spectrum condition $\Rightarrow$ BW $\Rightarrow$ geometric
modular time.  \emph{Never}: KMS/modular spectrum $\Rightarrow$ Lorentz space $\Rightarrow$ BW.

\par\smallskip\noindent\textbf{The rule.}  Every use of BW or of the CHM ball modular
Hamiltonian in the corpus must \emph{declare} whether 4D Poincar\'e covariance is an
\textsc{assumption} (imported; the result is then conditional on
\texttt{QFT4D.OS.RECON.01}-class existence) or a \textsc{result} (derived intrinsically).  The
cycle risk this fences: BW/CHM entering the Einstein derivation as input while the 4D
locality/Lorentz structure is still supposed to \emph{come out} of the seam.

\par\smallskip\noindent\textbf{Audit census at registration (all current BW/CHM usages,
annotated).}
\begin{itemize}
\item \textsc{assumption} --- the entanglement-equilibrium Einstein derivation
  (\vref{v323}/\vref{v358}/\vref{v359}; \texttt{tfpt\_4\_frontier} and the $(G_{\mathrm{metric}})$
  section above): the CHM ball modular Hamiltonian and the BW boost are \emph{inputs}; the
  derivation is conditional on a 4D Poincar\'e-covariant QFT with the right vacuum existing
  (\texttt{QFT4D.OS.RECON.01}) and must never be cited as evidence \emph{for} that existence.
\item \textsc{assumption (declared)} --- \texttt{QG.AMB.01} as a \mC\ redundancy (\vref{v369})
  is everywhere stated ``conditional on \texttt{SEAM.EQUIV.01}$+$Bisognano--Wichmann''; correct
  as declared.
\item \textsc{assumption (dimension-typed)} --- the seam KMS normalisation chain
  (\vref{v259}/\vref{v526}; $\beta_{\mathrm{angle}}=2\pi$): uses only the universal modular/KMS
  angle of Tomita--Takesaki theory on the \emph{seam} ($1{+}1$D/2D objects), not 4D BW; the
  $2\pi$ is a normalisation, not a spacetime derivation.
\item \textsc{result-direction (open)} --- the intrinsic-BW programme
  (\vref{v438}/\vref{v440}/\vref{v446} and the half-sided modular inclusion route in the
  $(G_{\mathrm{metric}})$ section): the honest attempt to \emph{derive} geometric modular action
  intrinsically; explicitly not closed, and the corpus already records the circularity warning
  (``BW would presuppose the very covariance it should produce'', \vref{v215}).
\item \textsc{evidence, not derivation} --- the compressed-state flow to the CHM/BW geometric
  form (\vref{v478}): a numerical convergence statement in the result direction; it does not
  establish 4D covariance.
\item \textsc{conditional (declared)} --- \texttt{origin\_theory}'s ``BW downstream of the seam
  being a chiral net'': stated as conditional on net identification; correct as declared.
\end{itemize}
Status: firewall in force; no marker moves --- the census confirms the declarations exist but
binds all \emph{future} uses to carry them explicitly.

% =====================================================================
\section*{Research Contract --- \texttt{OBS.TRANSDUCTION.01} (the transduction principle: eigenvalue, projection, coupling)}
\addcontentsline{toc}{section}{Research Contract: OBS.TRANSDUCTION.01}
% =====================================================================
\noindent\emph{Typing.} A research contract (ledger row \texttt{OBS.TRANSDUCTION.01}); the
principle is machine-checked on the minimal model at exploration level, the per-bundle
transduction proofs are \mO.  It elevates the S15 typing rule to a named principle:
\[
  \text{observable signature}
  \;=\;
  \underbrace{\text{internal eigenvalue}}_{\text{compiler \mE}}
  \;+\;
  \underbrace{B\,P_r\neq0}_{\text{transduction}}
  \;+\;
  \underbrace{\text{physical coupling operator}}_{\text{physics}} .
\]
\textbf{Without the middle arrow, neither a non-observation falsifies nor an observation
confirms.}  The minimal model is machine-checked in
\texttt{experiments/\allowbreak theory-contracts/\allowbreak fo01\_transduction\_invisibility.py}: a uniform measurement
functional sees the seam kernel \emph{exactly not at all} ($\mathbf1^{\mathsf T}(T^np-\pi)=0$ to
$10^{-14}$; burst-train statistics KS-indistinguishable from a scrambled control), while a
character-resolved channel reconstructs the eigenvalue $64/729$ to $10^{-10}$ --- invisibility
is a property of the readout $B$, not of the channel.

\par\smallskip\noindent\textbf{Demands.}  For each empirical bundle (FRB/GW/pulsar nulls; the
axion/birefringence family; any future search), \emph{prove or refute} $B\,P_r\neq0$ for the
declared readout before the null or the hit is scored.  \emph{Binding application (the
birefringence caution):} the ACT$+$Planck isotropic birefringence being close to
$\beta_{\mathrm{TFPT}}=\varphi_0/(4\pi)$ (\texttt{arXiv:2608.06480}) is an interesting
coincidence, \emph{not} proof, until the shared normalized axion action and the transduction
chain are proven --- the axion bundle ($\beta$, $f_a$, $m_a$, $g_{a\gamma\gamma}$, $\Omega_a$,
EDM) must follow from \emph{one} action with \emph{one} proven coupling.  Status:
\texttt{OBS.TRANSDUCTION.01} --- principle registered, minimal model checked; every per-bundle
middle arrow \mO.

% =====================================================================
\section*{Research Contract --- \texttt{PRED.JOINTLIKELIHOOD.01} (from hit-counting to overdetermined relations)}
\addcontentsline{toc}{section}{Research Contract: PRED.JOINTLIKELIHOOD.01}
% =====================================================================
\noindent\emph{Typing.} A research contract \mO\ (ledger row \texttt{PRED.JOINTLIKELIHOOD.01}:
Open, statistics-construction contract), \emph{not} a claim.  The $27{+}$ individually matched
values all descend from the same few atoms ($\varphi_0$, $c_3$, $g_{\mathrm{car}}$, $\mu_4$) and
are therefore statistically \emph{correlated}: counting hits overstates independent evidence.
The evidence is to be reorganized into \emph{overdetermined relation bundles}, each carried by
one proof object:
\textbf{(i)}~the flavor invariant sum rule --- one spurion/operator proof must yield
$\theta_{12}$, $\theta_{13}$, $\delta_{\mathrm{PMNS}}$, $J_{\mathrm{PMNS}}$, the mass splittings
and the CKM texture \emph{together};
\textbf{(ii)}~the inflation consistency relation --- $r=3(1-n_s)^2$ and $\alpha_s=-r/6$
(eliminating $N_\star$ as a fitted middleman);
\textbf{(iii)}~the axion/birefringence bundle --- $\beta$, $f_a$, $m_a$, $g_{a\gamma\gamma}$,
$\Omega_a$ and the EDM bound from the \emph{same} normalized axion action, with the transduction
chain of \texttt{OBS.TRANSDUCTION.01};
\textbf{(iv)}~the character-resolved transfer signature (\texttt{OBS.TRANSDUCTION.01} as the
readout-side principle).

\par\smallskip\noindent\textbf{The statistical demand.}  A joint likelihood over the frozen
registry with: the full covariance matrix induced by the shared atoms; the \emph{effective}
number of degrees of freedom (not the raw hit count); a prespecified null space and
formula-selection rule; and a genuine holdout dataset (future measurements only).  \emph{Honest
reading of the existing null test:} the ``13/13, $<10^{-30}$'' Monte-Carlo statement (\vref{v100},
\texttt{docs/FALSIFICATION.md}) quantifies the look-elsewhere burden \emph{under its declared
null model} and is only interpretable as evidence to the extent that the null space, the formula
grammar, and all development data were prespecified --- which for historically known observables
they were not and cannot retroactively be.  The 2026-06-09 freeze (\vref{v84}) genuinely
protects \emph{future} measurements; it does not turn historically known data into blind
predictions.  The wording of \texttt{docs/FALSIFICATION.md} is adjusted accordingly in the same
change (honest, not weakened: the freeze and the kill tests stand).  Status:
\texttt{PRED.JOINTLIKELIHOOD.01} \mO.

% =====================================================================
\section*{Research Programme --- the TFPT4D master route (one 4D transfer object instead of twenty formulas)}
\addcontentsline{toc}{section}{Research Programme: the TFPT4D master route}
% =====================================================================
\noindent\emph{Typing.} A programme section (registered 2026-08-27 from the external bird's-eye
review), \emph{not} a claim: it organizes the existing 4D contracts and the four new ones below
into one route and moves \emph{no} marker.  The organizing observation: TFPT is probably not
missing twenty independent formulas; it is missing \emph{one} mathematical growth principle that
unfolds the discrete structure into a local, unitary, reflection-positive 4D quantum world.  The
discrete theory is then not spacetime itself but its \emph{genetic code}; the missing bridge
must produce spatial locality, reversible quantum dynamics, chiral matter, gauge fields, a
controlled continuum limit and concrete observables \emph{simultaneously}.

\par\smallskip\noindent\textbf{The master formulation (working hypothesis, unproven).}
\emph{The physical world is the unique local, reflection-positive and approximately
quantum-Markovian 4D completion of the TFPT seam algebra that is anomaly-free and has exactly
one relevant dimensionful direction.}  ``Exactly one dimensionful direction'' is the No-Unit
theorem already in force: the dimensionless compiler cannot produce an absolute scale, and
$v_{\rm geo}$ is the declared calibration torsor (\texttt{ANCHOR.VGEO.02}, \vref{v153},
\vref{v725}) --- the master route inherits this, it does not re-open it.

\par\smallskip\noindent\textbf{The seven conditions, mapped onto contracts.}
\textbf{(1)}~seam compatibility --- the boundary restriction of the 4D theory reproduces the
TFPT seam data \emph{exactly}, not only dimensions or group labels
(\texttt{TFPT4D.LATTICE.ACTION.01} gate T3 below);
\textbf{(2)}~reflection positivity / positive transfer operator (gate T2;
\texttt{QFT4D.OS.RECON.01} demand O1);
\textbf{(3)}~a local unitary dilation of the observed Markov dynamics,
$\Phi(\rho)=\mathrm{Tr}_E[V(\rho\otimes\sigma_E)V^\dagger]$, with size-uniform locality and a
non-divergent Lieb--Robinson velocity (\texttt{DYN.UNITARY.DILATION.01});
\textbf{(4)}~chiral anomaly freedom with no mirror sector and the correct index
(\texttt{CHIRAL4D.NOMIRROR.01});
\textbf{(5)}~only one relevant scale direction --- no hidden tunable dimensionless parameter
(kill test in \texttt{TFPT4D.LATTICE.ACTION.01}; $v_{\rm geo}$ calibrates the unit only);
\textbf{(6)}~approximate quantum-Markov recovery,
$I(A{:}C\mid B)\le C_0\,e^{-\mathrm{dist}(A,C)/\xi}$, so the bulk is quasi-locally
reconstructable from overlapping boundary data --- the small-CMI/recovery-map link is
Fawzi--Renner (arXiv:1410.0664), and local Markov properties of Gibbs states are established in
Chen--Rouz\'e (arXiv:2504.02208) (\texttt{SEAM.BULK4D.RECON.01} route input);
\textbf{(7)}~nontriviality --- at least one connected four-point function and ultimately a
nontrivial S-matrix survive the limit (gate T7; \texttt{SEAM.BULK4D.RECON.01} demand B6).

\par\smallskip\noindent\textbf{The dynamics correction (discrete $\to$ dynamics).}  The finite
transport object already embeds exactly into a continuous Markov semigroup
(\texttt{DYN.MARKOV.EMBED.01} \mE\ at kernel level, \vref{v971}) --- but a Markov process is
irreversible and cannot be the fundamental dynamics.  The fundamental order must be
\emph{local unitary amplitudes $\to$ decoherence/partial trace $\to$ Markov matrix $T$}.  This
matches the now machine-checked finding that the classical unistochastic matrix $B_{ij}=|U_{ij}|^2$
keeps probabilities but loses the orientation sign of the Jarlskog invariant $J=\pm1/27$
(\veri{v977\_transfer\_unistochastic\_wilson.py}; the Birkhoff-fibre circulation parameter $t$ is a
second, independent classically hidden datum, \veri{v976\_seam\_lift\_birkhoff.py}): the fundamental
variable must carry a complex
holonomy/amplitude, which makes Wilson loops $W_\gamma=\mathrm{Tr}\prod_{e\in\gamma}U_e$ more
natural than bare transition probabilities.  Discrete Weyl dynamics on unitary cellular automata
show such a continuum transition is possible in principle (Bia\l ynicki-Birula,
arXiv:hep-th/9304070); for TFPT it remains to be constructed
(\texttt{DYN.UNITARY.DILATION.01}).

\par\smallskip\noindent\textbf{Programme ordering (the efficient sequence).}
\textbf{(P1)}~the conditional 4D dimension selector --- \emph{done at conditional level}
(\texttt{DIMENSION.SELECTOR.4D.01} \mC, \veri{v975\_dimension\_selector\_4d.py}, next section);
\textbf{(P2)}~the single simple-current/spin-field generator
(\texttt{SEAM.SIMPLECURRENT.GENERATOR.01} \mO);
\textbf{(P3)}~the explicit finite 4D lattice action with its seven finite gates
(\texttt{TFPT4D.LATTICE.ACTION.01} \mO);
\textbf{(P4)}~the local unitary dilation with size-uniform Lieb--Robinson bounds
(\texttt{DYN.UNITARY.DILATION.01} \mO);
\textbf{(P5)}~the continuum/OS package (\texttt{QFT4D.OS.RECON.01} \mO);
\textbf{(P6)}~all four transfer bridges from one generating functional
(\texttt{FTRANSFER.GENERATING.01} \mO);
\textbf{(P7)}~gravitation strictly downstream --- modular Hamiltonians $\to$ entanglement
geometry $\to$ effective Einstein dynamics, in the assumption-declared discipline of
\texttt{GRAV.NONCIRCULAR.01} (Jacobson's entanglement equilibrium, arXiv:1505.04753,
\emph{presupposes} a local QFT and can never be cited as producing one);
\textbf{(P8)}~the prespecified joint likelihood with holdout
(\texttt{PRED.JOINTLIKELIHOOD.01} \mO).
The Yang--Mills mass gap is \emph{not} defined away: it remains the open Clay problem; at best
the route reduces it to a concrete, structured lattice theory.

\par\smallskip\noindent\textbf{Kill criteria (the route is allowed to die early).}
No local reflection-positive finite 4D model with the correct seam boundary theory --- master
route dead.  The simple-current generator is nonlocal or fails to converge --- seam route dead.
The chiral measure term stays nonlocal or a global anomaly survives --- chiral route dead.
Mirror fermions do not decouple --- Standard-Model route dead.  The Lieb--Robinson velocity or
correlation length diverges uncontrolled --- no local continuum limit.  All connected four-point
functions vanish --- only a (generalized-)free theory.  Extra freely tunable dimensionless
parameters are needed --- the strong parameter-freeness claim is refuted.  The uniform twistor
flat direction (the sector-0 Okubo admixture of \veri{v973\_seam\_route\_narrowing.py}) shifts
\emph{normalized} observables rather than only the vacuum normalization of $Z$ --- a genuine
undetermined modulus.

\par\smallskip\noindent\emph{Separation (binding).}  RH and the \texttt{PRIME.*} programmes stay
separate mathematical programmes: a green RH probe would be neither necessary nor sufficient for
the 4D physics, and this architecture yields no RH statement.  Status: programme section; every
constructive contract in it stays \mO; the only verified piece is the conditional selector
below.

% =====================================================================
\section*{Verified Selector --- \texttt{DIMENSION.SELECTOR.4D.01} (why $d=4$, conditionally)}
\addcontentsline{toc}{section}{Verified Selector: DIMENSION.SELECTOR.4D.01}
% =====================================================================
\noindent\emph{Typing.} A \emph{conditional} identity \mC\ (ledger row
\texttt{DIMENSION.SELECTOR.4D.01}), machine-checked in
\veri{v975\_dimension\_selector\_4d.py} ($16/16$, sympy-exact).  The $\mu_4$ structure of the
compiler must \emph{not} be read as a proof of four spacetime dimensions
(\texttt{DIMENSION.UPLIFT.FIREWALL.01}); what \emph{is} checkable is a selector theorem under a
named axiom set.  \textbf{Statement.}  Under
\textbf{A1}~$d>2$ (propagating local gauge fields, $d-2\ge1$ transverse polarizations),
\textbf{A2}~a dimensionless Yang--Mills coupling,
\textbf{A3}~real self-dual/anti-self-dual 2-form sectors (the Euclidean Hodge star an involutive
endomorphism of 2-forms), and
\textbf{A4}~chiral Weyl representations (even $d$),
the dimension $d=4$ is \emph{unique and minimal} on the exact scan window $d=2..12$ --- and the
selector is \emph{overdetermined}: A2 alone pins $\{4\}$ ($[g_{\rm YM}]=(4-d)/2$ forced
symbolically from dimensionless-action power counting) and A3 alone pins $\{4\}$
($*:\Omega^2\to\Omega^{d-2}$ is a self-map iff $d=4$; the explicit $6\times6$ star on
$\Lambda^2(\mathbb R^4)$ satisfies $*^2=I$ with eigenvalues $\pm1$ at multiplicities $3/3$ ---
the real $\mathfrak{su}(2)_+\times\mathfrak{su}(2)_-$ split), while A1\,\&\,A4 alone give
$\{4,6,8,10,12\}$: chirality does \emph{not} select $4$ by itself (honest ablation ledger).
Weyl chirality is certified by exact Clifford algebras $d=2..7$ (even $d$: $\gamma_*$
anticommutes with every $\gamma_\mu$, $\gamma_*^2=I$, $\mathrm{tr}\,\gamma_*=0$; odd $d$: the
full product is central and scalar --- no grading).  Signature is typed honestly: in Lorentzian
signature $*^2=-1$ on 2-forms in $d=4$, so the \emph{real} split is a Euclidean/OS-side
statement.  Mutants are caught (the wrong power counting $[g]=(6-d)/2$ selects $\{6\}$; the
``A4 alone selects 4'' overclaim is refused).  \textbf{Honest boundary.}  $\mu_4$ and
$g_{\rm car}$ enter nowhere --- deliberately, because ``$\mu_4\Rightarrow$ 4D'' is exactly the
forbidden reading.  This is a selector, \emph{not} a construction and \emph{not} a compiler
derivation: \emph{axiom provenance is the open half} --- if A1--A4 cannot themselves be forced
from $\{c_3, g_{\rm car}\}$, the selector remains a consistency statement, not a prediction
(the registered kill criterion).  Status: \texttt{DIMENSION.SELECTOR.4D.01} \mC; every 4D
construction contract stays \mO.

% =====================================================================
\section*{Research Contract --- \texttt{SEAM.SIMPLECURRENT.GENERATOR.01} (one generator instead of 128 currents)}
\addcontentsline{toc}{section}{Research Contract: SEAM.SIMPLECURRENT.GENERATOR.01}
% =====================================================================
\noindent\emph{Typing.} A research contract \mO\ (ledger row
\texttt{SEAM.SIMPLECURRENT.GENERATOR.01}), \emph{not} a claim.  \textbf{Context.}  The seam
coverage carve (\veri{v973\_seam\_route\_narrowing.py}) shows the $\mathfrak{so}(16)_1$
fermion-bilinear half of the seam target is covered by the cited scaling-limit theorem; the
named residual is N1 (the $\mu_4$/GSO extension $SO(16)_1\to(E_8)_1$ as a \emph{net} statement
--- the 128 spinor currents are not bilinears), N2 (the twist/spin-field sector), N3 (the
one-sided chiral edge) and N4 (the holomorphy selector).  \textbf{Demand (the compression).}
Construct \emph{one} normalized simple-current/spin-field intertwiner --- the generator of the
index-4 Q-system extension (\vref{v125}/\vref{v154}) --- and prove
\textbf{(G1)}~convergence on the finite-particle core with energy bounds,
\textbf{(G2)}~locality relative to the bilinear net,
\textbf{(G3)}~the correct braiding/monodromy relation, and
\textbf{(G4)}~Q-system closure,
so that symmetry and fusion generate the \emph{entire} 128-dimensional extension sector from the
single generator --- collapsing N1\,+\,N2 (and, via uniqueness of the extension, the N4
selector) onto \emph{one} convergence theorem instead of 128 separate current controls.
\textbf{Kill tests.}  The generator is provably nonlocal relative to the bilinear net; the
energy bounds fail on the core; the braiding closes on the wrong extension ($SO(16)_1$ stays
unextended); the convergence rate is incompatible with the measured bilinear collar rate
($p=3.48$ inside the preregistered band, \veri{v973\_seam\_route\_narrowing.py}).
\emph{Dimension typing (binding):} hypotheses and conclusion live in $1{+}1$D --- closing this
contract does \emph{not} close \texttt{SEAM.BULK4D.RECON.01}.  Status:
\texttt{SEAM.SIMPLECURRENT.GENERATOR.01} \mO.

\par\smallskip\noindent\textbf{Externalization package (hand-off for outside specialists in
conformal nets / subfactors / VOA).}  In the same six-part format as the \texttt{SEAM.EQUIV.01}
externalization contract below:
\textbf{(1)~minimal theorem statement.}  Let $\mathcal A$ be the $\mathfrak{so}(16)_1$
fermion-bilinear net on the seam circle (the half covered by the cited scaling-limit theorem,
\veri{v973\_seam\_route\_narrowing.py}), and let $\sigma$ be the simple-current sector of the
index-4 $\mu_4$ Q-system (finite side explicit and machine-verified, \vref{v125}/\vref{v154};
net-level locality integer $h_s = 16/16 = 1$, \vref{v469}).  \emph{Claim:} there exists one
normalized intertwiner $\Psi_\sigma$ (a charged/spin field for $\sigma$) satisfying
G1~convergence on the finite-particle core with polynomial energy bounds, G2~relative locality
to $\mathcal A$, G3~the $\mu_4$ braiding/monodromy relation, and G4~Q-system closure; then the
extension generated by $\{\mathcal A, \Psi_\sigma\}$ is the $(E_8)_1$ net and the v973
residual N1\,+\,N2 (and N4 via uniqueness) closes.
\textbf{(2)~typed hypothesis list.}  \textsc{proven-finite}: the index-4 Q-system
(\vref{v125}/\vref{v154}), the crossed-product/locality package (\vref{v469}), the coverage
carve with the named N1--N4 residual and the measured collar rate $p = 3.48$
(\veri{v973\_seam\_route\_narrowing.py}), the gapped collar model (\vref{v367}/\vref{v368}).
\textsc{cited-classical}: the Osborne--Stottmeister scaling limit (arXiv:2107.13834);
Longo--Rehren / B\"ockenhauer--Evans simple-current extension theory; KLM $\mu$-index
classification.  \textsc{open}: G1--G4 themselves (exactly one convergence theorem).
\textbf{(3)~runnable finite models:} \texttt{verification/v125}, \texttt{v154}, \texttt{v469},
\texttt{v973} (embedded probes with pinned SPEC hashes).
\textbf{(4)~honest Lean surface:} none for G1--G4; the finite Q-system arithmetic only.
\textbf{(5)~must-fail controls:} the same construction attempted on $SO(16)_1$ \emph{without}
the order-4 clock must \emph{not} produce a holomorphic net ($\det K = 4$ rival); a
non-order-4 clock must break the marks; the trivial $M{=}3$ collar must give no chiral limit.
\textbf{(6)~acceptance tests:} a solution counts iff it (a)~proves G1--G4 for one explicit
$\Psi_\sigma$, or (b)~proves that no local normalized generator exists (the registered kill ---
then the seam route is dead and the ledger records it as exactly that).

% =====================================================================
\section*{Research Contract --- \texttt{TFPT4D.LATTICE.ACTION.01} (the explicit finite 4D lattice action and its seven finite gates)}
\addcontentsline{toc}{section}{Research Contract: TFPT4D.LATTICE.ACTION.01}
% =====================================================================
\noindent\emph{Typing.} A research contract \mO\ (ledger row \texttt{TFPT4D.LATTICE.ACTION.01});
route step (2) of the \texttt{DIMENSION.UPLIFT.FIREWALL.01} ordering made concrete --- the
single object on which nearly all open 4D questions become simultaneously testable.
\textbf{Demand.}  Exhibit one fully explicit finite family of 4D Euclidean lattice theories
\[
\mathcal Z_a[J]=\int\mathcal DU\,\mathcal D\psi\,\mathcal D\Phi\;
e^{-S_a[U,\psi,\Phi]+J\mathcal O},
\qquad
S_a=S_{\rm Wilson}[U]+\bar\psi\,D_{\rm GW}[U,\Phi]\,\psi+S_\Phi[U,\Phi]+S_{\rm seam/top},
\]
where $U$ are the fundamental holonomies (TFPT fixes representation content, charge lattice,
coupling relations and boundary conditions), $D_{\rm GW}$ is a Ginsparg--Wilson/overlap operator
for the TFPT chiral representation content (the \texttt{CHIRAL4D.NOMIRROR.01} operator), $\Phi$
is the \emph{only} allowed relevant scalar/link-mixing sector (EWSB downstream), and
$S_{\rm seam/top}$ is the boundary/topological module transporting the seam data into the bulk
--- first at finite lattice spacing $a$, deliberately \emph{without} continuum claims.
\textbf{Finite gates} (all machine-checkable at finite size; the promotion criteria for any
candidate): \textbf{(T1)}~exact gauge invariance; \textbf{(T2)}~a positive transfer matrix (RP
at finite $a$); \textbf{(T3)}~the seam boundary restriction reproduces the TFPT seam data
\emph{exactly}; \textbf{(T4)}~anomaly bookkeeping including the global $SU(2)$ anomaly;
\textbf{(T5)}~the index/generation count; \textbf{(T6)}~the mirror spectrum separated;
\textbf{(T7)}~at least one \emph{nonvanishing} connected four-point function.
\textbf{Kill tests.}  No local reflection-positive action with the correct seam restriction
exists (master route dead); every candidate is generalized-free (T7 unreachable); a hidden
freely tunable dimensionless parameter is required (the strong parameter-freeness claim is
refuted); the uniform twistor flat direction shifts \emph{normalized} observables rather than
only the vacuum normalization of $\mathcal Z$ (a genuine undetermined modulus).
\textbf{Relation.}  The continuum limit is the separate contract \texttt{QFT4D.OS.RECON.01};
the unitary dilation is \texttt{DYN.UNITARY.DILATION.01}; $d=4$ is typed by
\texttt{DIMENSION.SELECTOR.4D.01} \mC; the generating-functional collapse is
\texttt{FTRANSFER.GENERATING.01}.  Chiral lattice state of the art is calibrated in
\texttt{CHIRAL4D.NOMIRROR.01} (L\"uscher's exactly gauge-invariant abelian construction,
arXiv:hep-lat/9811032; the nonabelian anomaly analysis, arXiv:hep-lat/9904009; reflection
positivity proven for \emph{free} overlap fermions, Kikukawa--Usui arXiv:1005.3751 --- none of
this is the interacting chiral Standard Model).  Nothing in the corpus closes any gate.
Status: \texttt{TFPT4D.LATTICE.ACTION.01} \mO.

% =====================================================================
\section*{Research Contract --- \texttt{SEAM.DETLINE.UNIFICATION.01} (the determinant-line unification hypothesis)}
\addcontentsline{toc}{section}{Research Contract: SEAM.DETLINE.UNIFICATION.01}
% =====================================================================
\noindent\emph{Typing.} A research \emph{hypothesis} contract \mO\ (ledger row
\texttt{SEAM.DETLINE.UNIFICATION.01}) --- a precise conjecture, \emph{not} a result.
\textbf{Observation.}  Three separately open structures may be projections of \emph{one}
geometric object: \textbf{(1)}~the seam extension is described by a Q-system/simple current
(\texttt{SEAM.SIMPLECURRENT.GENERATOR.01}); \textbf{(2)}~the chiral fermion measure lives on a
determinant line (\texttt{CHIRAL4D.NOMIRROR.01} demand C1); \textbf{(3)}~anomalies and CP
phases appear as holonomies of that determinant line --- the \emph{finite} Quillen identity is
already two-sided (\veri{v974\_alpha\_faces\_computed.py}: Berry integral $=$
Poincar\'e--Lelong divisor integral $=+1$), and the continuum Bismut--Freed identification is
the named missing step of \texttt{ALPHA.QUILLEN.EXACT.01}.  \textbf{Demand (the compatibility
theorem).}
\[
\operatorname{Res}_{\rm seam}\det D_{4D}\;\cong\;\det D_{\rm seam}
\]
as line bundles \emph{with connection}, and the holonomy of the bulk determinant line equals
the discrete TFPT orientation phase (the classically invisible $\operatorname{sgn}J$
orientation bit, now machine-checked at matrix level with the Wilson-plaquette interference
readout as its minimal observable, \veri{v977\_transfer\_unistochastic\_wilson.py}; the
bulk-determinant identification demanded here stays open).  If such a theorem exists, seam extension, chiral
anomaly freedom, the generation index and the discrete CP orientation become different
projections of the same object.  \textbf{Kill tests.}  The two determinant lines differ by a
nontrivial local factor no counterterm removes; the bulk holonomy disagrees with the discrete
orientation phase on a computable family.  \emph{Honest boundary:} the continuum
Quillen/Bismut--Freed identification is exactly the critical missing step; nothing in the
corpus proves any part of the 4D side.  Status: \texttt{SEAM.DETLINE.UNIFICATION.01} \mO.

% =====================================================================
\section*{Research Contract --- \texttt{FTRANSFER.GENERATING.01} (four transfer bridges from one generating functional)}
\addcontentsline{toc}{section}{Research Contract: FTRANSFER.GENERATING.01}
% =====================================================================
\noindent\emph{Typing.} A research contract \mO\ (ledger row \texttt{FTRANSFER.GENERATING.01});
the \emph{constructive successor} named by the \texttt{FR.TRANSFER.01} typing guard.  The four
continuous-transfer interfaces ($F_{\rm pole}$: source masses $\to$ pole, \vref{v183};
$F_{\rm Boltzmann}$: CP source $\to\eta_B$, \vref{v169}/\vref{v184}; $F_{\rm relic}$:
$(f_a,m_a,\theta_i,N_{\rm DW})\to\Omega_a$, \vref{v185}; $F_{\rm QCD}$:
$(\alpha_s,b_3,\Lambda_{\rm QCD},\text{lattice})\to m_p/m_e$, \vref{v164}) are currently four
\emph{separate} bridges.  \textbf{Demand.}  If the 4D theory $\mathcal Z_a[J]$ of
\texttt{TFPT4D.LATTICE.ACTION.01} exists with a controlled continuum limit
(\texttt{QFT4D.OS.RECON.01}), the four bridges must arise from \emph{one} generating functional
$W[J]=\log Z[J]$: lepton and quark masses as poles of two-point functions (zeros of
$\Gamma^{(2)}$); $m_p/m_e$ from the baryon and electron correlator poles of the \emph{same}
theory; $\eta_B$ from the real-time Schwinger--Keldysh dynamics of the \emph{same} theory (CP
violation, $B$ violation, nonequilibrium); the axion relic from the cosmological solution of
the \emph{same} normalized action.  $F_{\rm transfer}$ is then not another $3\times3$ matrix;
it is the full $W[J]$.  \textbf{Binding.}  The \texttt{FR.TRANSFER.01} guard stays in force:
the four frontier observables remain \mC/\mO-typed until $W[J]$ exists and each bridge is
\emph{derived} from it --- this contract permits no marker upgrade; it names the single object
whose existence would collapse the four transfer problems into computations within one theory.
\textbf{Kill test.}  The four bridges provably require mutually inconsistent normalizations of
the same $W[J]$.  Status: \texttt{FTRANSFER.GENERATING.01} \mO.
...
\item \textbf{(L1) Uniform energy / nuclearity bounds.} The rescaled collar family satisfies
  ...
  (\vref{v302}) and exact quasi-freeness (Wick/Pfaffian closure, \vref{v113}/\vref{v155}), but
  \emph{no} uniform nuclearity estimate exists in the corpus; this is genuinely open analysis.
...
\begin{itemize}
\item \textbf{(T1) Gap-stable continuum limit of the determinant line.} For the seam operator
  family $\{D_L\}$ (the collar Bogoliubov--de~Gennes/Dirac operators with twisted boundary
  conditions) converging in the norm-resolvent sense to the continuum seam operator
  $D_{\mathrm{seam}}$, with the Fermi gap uniformly bounded below over the whole twist torus
  (so the Dai--Freed section has no zero), the curvature class of the finite determinant line
  converges to the Quillen curvature of the $\zeta$-regularised determinant line:
  $c_1(\det D_L)\;\to\;c_1(\det\nolimits_\zeta D_{\mathrm{seam}})$. The finite side is computed:
  the FHS curvature of the occupied-frame determinant line integrates to $2\pi\cdot1$ exactly,
  size-independently (\vref{v472}).
...
% =====================================================================
\section*{Externalization Contract --- \texttt{PRIME.RESIDUE.EXTERNAL.01} (the prime-front terminal residue, packaged for external specialists)}
\addcontentsline{toc}{section}{Externalization Contract: PRIME.RESIDUE.EXTERNAL.01}
% =====================================================================
\noindent\emph{Typing.} The third hand-off document: the \emph{terminal open statements} of the
prime-front programme (diary and status record: \texttt{tfpt\_prime\_front}), packaged
self-containedly for outside specialists in analytic number theory --- large sieve, sampling and
frame theory, de~Branges spaces, explicit-formula methods.  It \emph{documents}, it does not
close: the contract identifier names this document, not a ledger row; every prime-front ledger
row behind it (\texttt{PRIME.*}) is an \mE-typed \textsc{not-rh-evidence} identity, finite
theorem or honest kill record, and none moves here.  \textbf{This contract offers open problems.
It asserts no progress toward the Riemann Hypothesis in either direction; nothing below is
evidence for or against it, and no submitted solution is solicited as such evidence.}
Throughout, ``$\lambda$-uniform'' is house shorthand for \emph{uniform over the whole rung
ladder} (the infinite quantifier), as opposed to per-rung, finite, or classical-per-census legs;
``census'' always means the finite root set defined below, never a zero set of $\zeta$.

\par\smallskip\noindent\textbf{(1) The finite objects, self-contained.}  Fix an integer rung
$h \ge 4$ and set $a = \tfrac12\log h$, $K = \lceil 1.25\,h\log h\rceil$, modes
$\omega_k = k\pi/a$ ($k = 0..K{-}1$), poles $b_k = \omega_k^2$, and the block height
$T_z = 2\pi h$.  The \emph{source data} are the von~Mangoldt atoms
$(u_q, w_q) = (\log q,\ \Lambda(q)/\sqrt q)$ over the prime powers $q \le h$ --- the only
arithmetic input anywhere.  The \emph{wall matrix} $M_h = M_{\mathrm{pole}} + M_{\mathrm{arch}}
- M_{\mathrm{prime}}$ is the $K\times K$ matrix of the Weil-explicit-formula quadratic form in
the normalised even trigonometric mode basis $\{\cos(\omega_k u)\}$ on the window $[0, 2a]$:
$M_{\mathrm{pole}}$ is the rank-one pole block, $M_{\mathrm{arch}}$ the archimedean
($\Gamma$-term) block, and $M_{\mathrm{prime}}$ the \emph{only} atom-dependent block ---
\emph{linear} in exactly two atom transforms per mode, $P_k = \sum_q w_q\sin(\omega_k u_q)$
off-diagonal and $\tilde P_k = \sum_q w_q\,[(a - u_q/2)\cos(\omega_k u_q) - \sin(\omega_k u_q)/
(2\omega_k)]$ on the diagonal (the \vref{v935} per-mode linearity; the exact assembly is frozen
in the builder cited in (6) and rebuildable from the explicit formula for this window class).
Let $\tau_h = \lambda_{\min}(M_h)$ and let $d$ (the \emph{wall argmin ray}) be its unit
eigenvector; the \emph{source coefficients} $c_k$ are the de-normalised components of $d$.
From these: $A_0 = \sum_k(-1)^k c_k$, $A_2 = \sum_{k\ge1}(-1)^k c_k b_k$,
$y_t = |A_2/A_0|$, $B_1 = \sum_{k\ge1} b_k = (\pi/a)^2(K{-}1)K(2K{-}1)/6$,
$\kappa = B_1/y_t$; the rational function
\[
  F(y) \;=\; c_0 + \sum_{k\ge1}(-1)^k c_k\,\frac{y}{y - b_k},
\]
whose numerator $N(y)$ (degree $K{-}1$, leading coefficient $A_0$) is the \emph{census
polynomial}; its roots $y_j$ are \emph{the census}.  The moment--Laurent normal form is
$(y/y_t)\,F(y)/A_0 = \Phi(z) = z - 1 + \sum_{m\ge1} J_{m+1} z^{-m}$ at $z = y/y_t$
(\vref{v924}).  The \emph{jet mass} at rung $h$ is
\[
  \delta_h \;=\; \frac{\sum_\gamma \sin^2(a\gamma)\,F(\gamma^2)^2/\gamma^2}
                      {A_0^2 \sum_\gamma \sin^2(a\gamma)/\gamma^2},
\]
the sum over verified ordinates $\gamma \in (T_z + 6,\ \gamma_{7000}]$ (caches in (6)); the
step-A identity $\delta_h = |J_h|^2_{G_h}$ with the jet vector $J = d/A_0$ and $G_h$ the Gram
of $\psi_k(\gamma) = \gamma^2/(\gamma^2 - b_k)$ under the weight $\sin^2(a\gamma)/\gamma^2$ is
\emph{exact}, remainder $0$ (\vref{v939}).  The three per-rung hypotheses:
\begin{itemize}
\item \textbf{H1$(h; c^*)$:} no census root with $\operatorname{Re} y \ge c^* y_t$, with
  $c^* \in [1.10, 1.15]$ (grid values, flat over all built rungs).  H1 is \emph{certified
  source-pure} per rung ($h \le 26$): one exact evaluation of the monotone envelope,
  $\mathrm{ENVJ}(c^* y_t) < |A_0|$ --- no cache, no zeros (\vref{v931}).
\item \textbf{H2$(h)$:} the census is complete-real and nonnegative (all $K{-}1$ roots real,
  $\ge 0$).  Landed at all 25 built rungs including the degree-116 census at $h = 28$; finite
  classical per rung, hard $h \le 13$ (\vref{v931}).
\item \textbf{H3$(h)$:} $y_t \le 0.155\,T_z^4$.  Source-pure certificate, certified $26/26$
  with margin $\ge 1.99$, \emph{refutable} (an explicit inflation witness breaks it) and not
  norm-continuous (\vref{v932}).
\end{itemize}

\par\smallskip\noindent\textbf{(2) The three minimal open statements.}  These are the
programme's terminal residue in the canonical form (note DII of the discovery record), stated
as clean problems about the finite family above.
\begin{itemize}
\item \textbf{(R1) The triple.}  \emph{Prove $\{$H1 $\wedge$ H2 $\wedge$ H3$\}$-cofinal: for
  every sufficiently large $k$ there is a rung $h$ in the dyadic block $(2^k, 2^{k+1}]$ at
  which H1, H2 and H3 hold simultaneously --- one rung per block, all three at the same $h$.}
  Only this cofinal form is asked; the limsup form is available only modulo the measured
  quantifier defect $D = 0.0042$ (block-average $+$ pigeonhole is proven exact yet
  subsidy-empty, \vref{v933}).  Everything downstream of the triple is finite algebra or
  classical-per-census: the product floor and its assembly are theorems conditional
  \emph{exactly} on it (\vref{v931}).
\item \textbf{(R2) The loop characterization.}  \emph{The census-forall-$k$ statement} ---
  the same census schedule quantified over \emph{all} blocks at once --- \emph{is identified
  as RH-equivalent-in-currency by four independent machine-detected proof-route cycles}:
  census universalization $==$ the RH loop (\vref{v928}); the pinning-supply route $==$ the
  same loop's costume (\vref{v929}); BANDPOS on the unbounded ladder $\iff$ RH (\vref{v930});
  and the de~Bruijn--Newman backward-persistence leg $==$ the $[\Lambda \le 0]$ class
  (\vref{v938}).  Stated honestly: within the corpus vocabulary this statement and RH price
  each other; no proof of equivalence in the literature sense is claimed.  The external value
  is either a genuinely \emph{new independent sign source} for it, or a \emph{proof that the
  loop is essential} (that census-forall-$k$ is RH-equivalent), which would close the
  programme's hardness question honestly.
\item \textbf{(R3) The H-pin.}  \emph{The one $\lambda$-uniform edge of the counting pair
  $\{$L1, WPD$\}$} (\vref{v937}): L1 $=$ TAIL is proven and the H-pin is open; the weak
  positional dominance WPD$(a < \gamma_1^2)$ follows from the H-pin for every integer rung
  $x \ge 3$.  The pin splits as $\Omega$-a $\oplus$ $\Omega$-b $\oplus$ H123M counting:
  $\Omega$-a (EPS-LOCK) is the variational quotient $\bar\varepsilon \le
  \mathrm{poly}(x)\,|A_0|\sqrt G$ --- numerator trial-priceable, denominator floor $==$ the
  flagged $A_0$-triangle loop; $\Omega$-b (SPACING-REMAINDER) is the QSUBGAP kernel.  The
  exact place the floor escapes linearity is machine-exhibited at \emph{two} maps
  (\vref{v937}): the eigenvector map ($\lambda_{\min}$ is not additive ---
  $\lambda_{\min}(A{+}B) = -\sqrt2 \ne \lambda_{\min}(A) + \lambda_{\min}(B) = -2$ on an exact
  $2\times2$ instance) and the census-root map (not affine, $d^2y^*/dw_1^2 \ne 0$
  symbolically); the bridge atoms are $c$-free while the pin floor $m_j =
  4|A_0\sin(a\mu_j)|\,\mathrm{PR}_j$ is $c$-loaded --- the floor lives on the vector side of
  the mode-level bridge, and the conditioning wall of \vref{v935} applies verbatim.
\end{itemize}

\par\smallskip\noindent\textbf{(3) What is proven and citable (complete typed hypothesis
list).}  \textsc{proven-finite} $=$ machine-checked in the suite (module cited);
\textsc{cited-classical} $=$ a published theorem, used as stated, never re-proved;
\textsc{open} $=$ the targets themselves.
\begin{center}\footnotesize
\renewcommand{\arraystretch}{1.22}
\begin{tabularx}{\linewidth}{@{}l>{\hsize=1.45\hsize\raggedright\arraybackslash}X>{\hsize=0.55\hsize\raggedright\arraybackslash}X@{}}
\toprule
\textbf{Type} & \textbf{Hypothesis} & \textbf{Witness} \\
\midrule
\textsc{proven-finite} & Theorem PF: the product floor $F/A_0 \ge (1 - c^*/z)^{(1+\kappa)/c^*}$, exponent $=$ the trace, conditional \emph{exactly} on $\{$H1 $+$ H2 per rung$\}$; H1 source-pure (one envelope evaluation per rung); witness modus tollens & \vref{v931} \\
\textsc{proven-finite} & the rate dictionary $a = p/2 - 1$ proven at $p = 3, 4, 5$; TR-CAP proven \emph{and} typed circular-for-TOPROOT; the H3 certificate ($26/26$, margin $\ge 1.99$, refutable) & \vref{v932} \\
\textsc{proven-finite} & block-average $+$ pigeonhole exact yet subsidy-empty ($D = 0.004183$); the witness boundary a theorem boundary, DK-scoped & \vref{v933} \\
\textsc{proven-finite} & the unconditional Landau/Gonek pricing: Landau 1912 $+$ Gonek 1985/1993 identified unconditional; Gonek 1984 machine-checked \emph{not an ancestor} (hidden loop flagged, never consumed); envelope closed form sympy-exact, census-depth-insensitive & \vref{v934} \\
\textsc{proven-finite} & the mode-level Landau bridge: the first linear per-mode classical identity ($M_{\mathrm{prime}}$ linear in two atom transforms per mode); the rate dictionary a proven bijection (transports every $\theta_\infty$, pins none) & \vref{v935} \\
\textsc{proven-finite} & the tau-free DK exclusion theorem: $\sin\theta \le (r_0/\tau)/(\mathrm{FG} - a)$ with $\tau$ cancelling, proven $h \le 13$ at the $10^{-25}$ bar $+$ the exact required-bar schedule $b^*(h)$; the BA3 scale-gauge theorem & \vref{v936} \\
\textsc{proven-finite} & WPD's $\lambda$-uniform content $==$ the H-pin edge (AND-fire at the declared edges); the restatement licenses; the two linearity-escape exhibits & \vref{v937} \\
\textsc{proven-finite} & the exact census semigroup: the de~Bruijn--Newman flow on the derived census polynomial is $e^{-tT}$, $T = 4y\,\partial^2 + 2\,\partial$, closed form, trace law at measured deviation $0.0$; the factor-4 normalisation lemma; the dictionary threefold-inequivalent & \vref{v938} \\
\textsc{proven-finite} & step A exact: $\delta_h = |J_h|^2_{G_h}$ with remainder $0$, generic; the cluster normal form (basis-change determinant $=$ the spacing product); Montgomery--Vaughan carried at $3.15\pi$ with exact margin $1/21$ & \vref{v939} \\
\textsc{proven-finite} & the sub-dof composition chain typed exactly (four exact legs; the floor \emph{value} is the measured leg); the complete unconditional lower-class pricing table & \vref{v940} \\
\textsc{proven-finite} & the Gram is source-free (proven three ways); the ray-to-law map projective-linear and $A_0$-free ($s_i A_0$ linear in $d$); the one shared nonlinear link is the wall eigenmap & \vref{v941} \\
\textsc{cited-classical} & Landau 1912 (the classical zero-counting/sum form over all nontrivial zeros, unconditional) & \vref{v934} \\
\textsc{cited-classical} & Gonek 1985/1993 (unconditional exponential-sum bounds; the RH-conditional Gonek 1984 family is \emph{flagged}, never consumed) & \vref{v934} \\
\textsc{cited-classical} & Montgomery--Vaughan 1974 (the generalized Hilbert inequality; killed at linkage $1$, carrying at $3.15\pi$) & \vref{v939} \\
\textsc{cited-classical} & Ortega-Cerd\`a--Seip 2002 (Fourier frames, Ann.\ of Math.\ \textbf{155}, 789--806: the Carleson box condition is equivalent to the \emph{upper} inequality alone --- wrong direction for a floor) & \vref{v940} \\
\textsc{cited-classical} & Nazarov--Tur\'an 1993 (explicit unconditional lower bound, exponential in the term count --- the classical mirror of the measured occupancy law) & \vref{v940} \\
\textsc{cited-classical} & Rodgers--Tao 2020 ($\Lambda \ge 0$), consumed as a \emph{cited ceiling} only --- its proof consumes pair correlation, which is exactly the flagged loop class & \vref{v938} \\
\textsc{cited-classical} & Polymath 15 2019 ($\Lambda \le 0.22$), consumed as a \emph{cited ceiling} only & \vref{v938} \\
\textsc{open} & \textbf{R1--R3 above} --- the cofinal triple, the loop characterization, the H-pin & this contract \\
\bottomrule
\end{tabularx}
\end{center}

\par\smallskip\noindent\textbf{(4) What is measured (honest).}  Measured legs are carried as
measurements, never as theorems: the unscoped sub-block floor holds with margins $\ge 1.405$ at
all built rungs and the source-only Bertrand-cofinal selector ($\hat h_k = 7/13/31/\ldots$)
clears the SF2 demand with margins $2.14/3.05$ (\vref{v939}/\vref{v940}); the alignment law is
a frozen measured \emph{triple} --- the half-power envelope ($\alpha$ inside the
\emph{descriptive} band $(0.38, 0.54)$ at all 14 rungs, never a prediction), the quarter-depth
wall lock ($\mu_m = 0.2372\,\log_{10} g_{\min}$, $R^2\,0.9916$) and the deep excess
($5.64$--$7.58$ dex) (\vref{v941}); the classical-evaluation face $\theta_\infty$ sits in the
measured band $[0.0766, 0.0977]$ and is typed \textsc{open-nonperturbative-variational} ---
deeper census improves the data ${\sim}1/T$ but provably never the limit
(\vref{v933}/\vref{v935}); and the full-space refuter ladder is \emph{always cited paired}: the
measured first-order minimum sits $30$--$33$ orders above the $10^{-25}$ bar, while the
\emph{proven} schedule is $b^*(h) = 23.8/21.7/15.7/4.4$ orders at $h = 4/5/8/13$ and \emph{none}
at $h \ge 16$ (\vref{v936}).  The quartic growth law behind H3 stays \textsc{measured}
(\vref{v926}); the quantifier defect $D = 0.004183$ is a measurement.

\par\smallskip\noindent\textbf{(5) The kill atlas (so external solvers do not rebuild
corpses).}  The campaign's obstruction record is machine-checked: an 11-node implication DAG
with exactly one open edge and a candidate map complete over the corpus vocabulary --- 24
candidate sign sources, $0$ pass (\texttt{tfpt\_prime\_front}, kill-atlas section; sandbox,
typed editorial).  The named dead route classes, each with its one-line death mechanism:
\begin{center}\footnotesize
\renewcommand{\arraystretch}{1.22}
\begin{tabularx}{\linewidth}{@{}>{\hsize=0.62\hsize\raggedright\arraybackslash}X>{\hsize=1.08\hsize\raggedright\arraybackslash}X>{\hsize=0.30\hsize\raggedright\arraybackslash}X@{}}
\toprule
\textbf{Route class} & \textbf{Death mechanism} & \textbf{Round / witness} \\
\midrule
...
$\ell^1$ / absolute-value majorants & the target is a five-order cancellation of three ${\sim}200$-sized pieces; the $\ell^1$ majorant overshoots by $10^{3}$--$10^{6}$ & diary, bilinear arc (T170) \\
phase-discarding additive source forms & the source-only additive (Weyl-split) route is vacuous by $10.9$--$54.6$ dex: the cancellation \emph{is} the arithmetic & r166, \vref{v927} \\
Vieta pinch / power sums & $1/A_0$ appears in every symmetric function --- the resummation never decouples from the Connes scale & r172, \vref{v932} \\
moment / trace caps & TR-CAP proven yet circular-for-TOPROOT; finite-$P$ moment models undershoot by $1.4$--$4.5$ dex (rate-blind trace-cap vacuity) & r162/172, \vref{v926}/\vref{v932} \\
Riccati / half-gap induction, manifold transport & transport relabels, never reduces: frame legs exact, source legs non-transporting, the level is cheaper than the increment --- the infinite quantifier does not fall by induction & r177, \vref{v936} \\
de~Bruijn--Newman pinch & theta-flow-inert ($\ge 4021\times$ the unconditional window); wall fragility $8$--$51$ orders below the window; the only closing leg is the flagged $[\Lambda \le 0]$ class & r179, \vref{v938} \\
global CBJ frame inequality & the dof kill ($n = 474$ coefficients vs Landau time-bandwidth rank $128$), seam coupling $0.994$, occupancy collapse & r180, \vref{v939} \\
sub-dof scoping & the jet mass leaves the well-conditioned cell at $+0.999$ dex per rung; the scoped margin dies by ${\sim}125\times$ at $\hat h(B3) = 13$ & r181, \vref{v940} \\
Carleson embedding as a floor & wrong direction: the box condition is equivalent to the \emph{upper} inequality alone; every unconditional lower class misses for a named reason & r181, \vref{v940} \\
naive floor-power composition & overshoots the honest composed floor by $3.41/10.14$ dex; the level needs a per-mode lower bound on the known variational-quotient wall & r182, \vref{v941} \\
\bottomrule
\end{tabularx}
\end{center}

\par\smallskip\noindent\textbf{(6) Available finite models (runnable).}  All models live in
the repository and run standalone (Python 3, \texttt{mpmath}/\texttt{sympy}/\texttt{numpy};
\texttt{python verification/run\_all.py} runs the whole suite):
\begin{itemize}
\item \veri{v931\_jetmass\_floor\_theorems.py} --- theorem PF, H1/H2, the assembled floor
\item \veri{v932\_toproot\_theta\_statement.py} --- TOPROOT, the rate dictionary, H3
\item \veri{v933\_h3\_cofinal\_adjudication.py} --- the cofinal adjudication, $D$, $\theta_\infty$
\item \veri{v934\_gonek\_pricing\_unconditional.py} --- the unconditional classical pricing
\item \veri{v935\_thetainf\_landau\_bridge.py} --- the mode-level Landau bridge
\item \veri{v936\_manifold\_invariance\_exclusion.py} --- the DK exclusion, the refuter ladder
\item \veri{v937\_l1wpd\_closure\_reduction.py} --- the H-pin split and linearity exhibits
\item \veri{v938\_dbn\_heatflow\_census.py} --- the exact census semigroup
\item \veri{v939\_cbj\_frame\_adjudication.py} --- step A, the Gram, the selector
\item \veri{v940\_cbj\_subdof\_blockfloor.py} --- the sub-dof kill, the lower-class table
\item \veri{v941\_alignment\_law\_factorization.py} --- the alignment-law factorization
\end{itemize}
The substrate modules \vref{v922}--\vref{v930} carry the spacing/jet sum rules, the razor
enclosure, the moment--Laurent ladder and the block-average/transfer/sigma-floor algebra.  The
frozen discovery probes (deterministic, SPEC-hashed, in
\texttt{experiments/tfpt-discovery/}):
\begin{itemize}
\item \texttt{jetmass\_floor\_probe.py} (r171), \texttt{toproot\_theta\_probe.py} (r172)
\item \texttt{h3\_cofinal\_probe.py} (r173), \texttt{gonek\_pricing\_probe.py} (r174)
\item \texttt{thetainf\_pin\_probe.py} (r175), \texttt{manifold\_invariance\_probe.py} (r177)
\item \texttt{l1wpd\_closure\_probe.py} (r178), \texttt{dbn\_heatflow\_probe.py} (r179)
\item \texttt{cbj\_frame\_probe.py} (r180), \texttt{cbj\_subdof\_probe.py} (r181)
\item \texttt{alignment\_law\_probe.py} (r182)
\end{itemize}
In the same directory: the frozen instrument
builder \texttt{radius4\_an\_probe.py} (\texttt{build\_cell}; worlds
\textsc{main}/\allowbreak\textsc{smooth}/\allowbreak\textsc{scrarith}/\allowbreak
\textsc{epstein}) and the verified ordinate caches \texttt{verified\_zeros\_big.npy} and
\texttt{verified\_zeros\_n7000.npy} (with \texttt{\_meta.json} sidecars; the big cache holds
$2\times10^{7}$ verified ordinates and tops out at $T(2\times10^{7}) = 9\,499\,220.48$).
The verified height horizon is cited as PT21
($T_{\rm PT} = 3\,000\,175\,332\,800$) with HSW22 Cor.~1.2 as the envelope form.

\par\smallskip\noindent\textbf{(7) Formalization surface (honest).}  The only prime-front
Lean~4 content is exact finite algebra: the spacing product for generic finite $K$ over an
arbitrary field, the jet sum rules, and the cofinal-wall composition/predefinition audits
(\texttt{TfptCarrier/SpacingProduct.lean}, \texttt{JetSumRules.lean},
\texttt{CofinalWeil.lean}, \texttt{CofinalEnvelope.lean}, \texttt{CofinalPredefinition.lean},
\texttt{WallCofinalComposition.lean}, \texttt{WallCertifiedHead.lean}; \texttt{lake build}
clean, no \texttt{sorry}).  \textbf{Nothing of R1--R3 is formalised}: H1/H2/H3, the census
objects, $\delta_h$, the product floor and the pin have \emph{zero} Lean content --- no
analysis, no zeros, no operator statements.  An external solver should treat the Lean layer as
contributing only the finite spacing/jet algebra audit.

\par\smallskip\noindent\textbf{(8) Must-fail controls.}  A submitted solution is accepted only
if it respects the following (each already realised as a machine-checked negative control in
the probes above):
\begin{itemize}
\item \textbf{MF-P1 (Epstein).}  Any proposed sign source or positivity argument must fail, or
  demonstrably lose its constant, on the \textsc{epstein} world (the $x^2 + 5y^2$ lattice
  atoms: an $L$-function class with off-line zeros).  An argument that goes through unchanged
  there proves too much.
\item \textbf{MF-P2 (Scramble).}  It must fail on \textsc{scrarith} (the golden-ratio
  permutation of the atom weights on the same support): the law must consume the joint
  sign-and-magnitude structure of the source, not its histogram.
\item \textbf{MF-P3 (Smooth).}  It must fail on the atom-free \textsc{smooth} world: the
  conclusion must be arithmetic, not archimedean bookkeeping.
\item \textbf{MF-P4 (no-zero test).}  The proof must consume \emph{no} zero tables: machine
  ancestry of the delivered chain must be zero-free (the G44-class no-zero gate).  The caches
  of (6) are for \emph{verification} of finite claims, never inputs to a submitted proof.
\item \textbf{MF-P5 (predefinition test).}  Clusters, weights and selectors must be predefined
  and sealed \emph{before} any sign evaluation (the sealed-pre-ward discipline of r180); no
  sign-mined selection.
\item \textbf{MF-P6 (loop firewall).}  The proof must not consume the four machine-flagged
  loop classes: census-forall-$k$ (R2 itself); the $A_0$-triangle; zero-verification-as-hypothesis
  (including the RH-conditional Gonek 1984 family); and the RH-conditional second moments
  (Montgomery pair correlation / Goldston--Montgomery).  Consuming any of them re-imports the
  hypothesis.
\end{itemize}

\par\smallskip\noindent\textbf{(9) Acceptance tests.}  An external result counts against this
contract exactly when it delivers one of:
\begin{itemize}
\item \textbf{(a) an explicit all-$h$, all-$a$ bound} --- a $\lambda$-uniform inequality (over
  the whole rung ladder, uniform in the window parameter) that implies R1 or the H-pin edge of
  R3 under the hypothesis list of (3), with any additional hypothesis flagged explicitly and
  the must-fail controls of (8) verifiably respected;
\item \textbf{(b) a genuinely new sign source} --- one that \emph{separates the worlds and
  independently orients} (the frozen gate the 24 corpus candidates all fail): it must break on
  Epstein/Scramble/Smooth while carrying an unconditional orientation on the true source; or
\item \textbf{(c) a proof that one of R1--R3 is RH-equivalent} --- which would close the
  programme's hardness question honestly and is recorded as exactly that, never as progress
  toward RH.
\end{itemize}
Partial results that close a single leg (e.g.\ the $\Omega$-b kernel alone, or one dyadic
block class) narrow the target and are recorded as such.  Any marker move happens in
\texttt{status\_ledger.csv} by the house process, never in this document.  \textbf{No
statement about the truth, provability or falsity of the Riemann Hypothesis is made here, and
none is implied by the acceptance of a submission.}
...
\emph{[2026-08-27 precision (binding; \texttt{DYN.MARKOV.EMBED.01},
\veri{v971\_markov\_embedding\_generator.py}): the ``one native flow'' is a universal
\emph{contraction class} --- shared gap and multiplier algebra --- not one physical clock; the
clocks are external and sector-specific (\vref{v723}/\vref{v724}/\vref{v777}), and the
continuous semigroup $e^{t\log T}$ is irreversible statistical dynamics, not unitary evolution
--- the (b)-vs-(c) firewall of the
\texttt{DYN.MARKOV.EMBED.01}/\texttt{DYN.UNITARY.DILATION.01} contract pair above.]}
