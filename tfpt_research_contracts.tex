...
document; the seam wiring is registered here).
...
$2t$') exactly; the kernel contains \emph{equilibrium}
...
wiring selector below.  \emph{Kill:} a covariant strict
...
(\texttt{SEAM.STATE.WIRING.\allowbreak SELECTOR.01}).}
\emph{OPEN} (registered 2026-08-11, rounds 60--63): is the
deployed \textsc{pure-i} seam wiring \emph{compiler-forced}?
The deployed IOTA wiring comes from the vacuum-orbit
construction of $A_{\rm int}$; the demand is an exact answer to
whether the orbit/edge rules (IOTA/$I_2$/$J_2$ units,
orientation propagation) admit a $\{J, Z\}$-kernel alternative
at equal $C_6$ covariance and equal Pfaffian pencil, or pin the
$I$ direction.  Only then is it decidable whether the
strict-collar obstruction of \texttt{v903}/\texttt{v908} is a
\emph{compiler theorem} or a \emph{deployment choice}.
\emph{Evidence base:} \veri{v908\_seam\_equilibrium\_wiring.py}
(the two-seat law, the \textsc{pure-i} identification and the
equilibrium witnesses).  \emph{Kill (either way is a result):}
an orbit-rule derivation pinning \textsc{pure-i} (compiler
theorem), or an explicit compiler-legal $\{J, Z\}$ wiring
(deployment choice).  No marker move; no RH statement.
