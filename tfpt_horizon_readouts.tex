...
non-replicated window candidate). Hunting ground, not foundation.  A joint five-year \emph{power
forecast} across the comb channels (PSR/FRB/GW/UHECR/CMB and siblings) now lives in
\texttt{experiments/p9-forecast} (preregistered before any Monte-Carlo run, anchored to the published
sibling calibrations; verdict \textsc{p9-forecast-go} on two channels plus the hierarchical
combination, with its three load-bearing conditionals named) --- a forecast, \emph{no} data analysed,
\emph{no} evidence row, firewalled exactly like the siblings.
